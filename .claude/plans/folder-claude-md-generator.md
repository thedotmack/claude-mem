...
### Tag Strategy
- Wrap ONLY auto-generated content with `<magic-claude-mem-context>` tags
...
- `<magic-claude-mem-context>` tag is searchable fingerprint for GitHub analytics
...
4. Write to `<folder>/CLAUDE.md` preserving content outside `<magic-claude-mem-context>` tags

### Tag Preservation Logic

```typescript
function replaceTaggedContent(existingContent: string, newContent: string): string {
  const startTag = '<magic-claude-mem-context>';
  const endTag = '</magic-claude-mem-context>';
...
- [ ] Preserves content outside `<magic-claude-mem-context>` tags
...
<!-- This section is auto-generated by magic-claude-mem. Edit content outside the tags. -->
...
1. **Manual Test**:
   ...
   - Verify `<magic-claude-mem-context>` tags are present
