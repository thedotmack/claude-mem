...
MCP tools use a **schema reference pattern** to reduce token usage by ~90%. Tools show minimal schemas; call `get_schema(tool_name)` for full details on demand. Implementation in `src/servers/mcp-server.ts`.

## Pro Features Architecture

Claude-mem separates open-source core from optional Pro features:
