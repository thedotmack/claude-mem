/**
 * changes command — list file changes tracked in memory.
 * cmem changes [options]
 */
...
export function registerChangesCommand(program: Command): void {
  program
    .command('changes')
    .description('List file change observations stored in memory')
    .option('-l, --limit <n>', 'max results', '20')
    .option('-p, --project <name>', 'filter by project')
    .option('--json', 'output as JSON for agent use')
    .action(async (opts: ChangesOpts) => {
      ...
      try {
        ...
        const response = await client.changes({ limit, project: opts.project });
