/**
 * how command — list "how it works" observations from memory.
 * cmem how [options]
 */
...
export function registerHowCommand(program: Command): void {
  program
    .command('how')
    .description('List "how it works" observations stored in memory')
    .option('-l, --limit <n>', 'max results', '20')
    .option('-p, --project <name>', 'filter by project')
    .option('--json', 'output as JSON for agent use')
    .action(async (opts: HowOpts) => {
      ...
      try {
        ...
        const response = await client.howItWorks({ limit, project: opts.project });
