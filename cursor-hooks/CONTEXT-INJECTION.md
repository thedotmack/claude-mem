...
1. **Install**: `magic-claude-mem cursor install` creates initial context file
2. **Stop hook**: `session-summary.sh` updates context after each session ends
3. **Cursor**: Automatically includes `.cursor/rules/magic-claude-mem-context.mdc` in all chats
...
3. Fetches existing context from magic-claude-mem and writes to `.cursor/rules/magic-claude-mem-context.mdc`
...
cat > .cursor/rules/magic-claude-mem-context.mdc << EOF
---
alwaysApply: true
---
# Memory Context
...
Located at: `.cursor/rules/magic-claude-mem-context.mdc`
...
[Your context from magic-claude-mem appears here]
...
When you run `magic-claude-mem cursor install`, the project is registered in `~/.magic-claude-mem/cursor-projects.json`. This allows the worker to automatically update your context file whenever a new summary is generated - even if it happens from Claude Code or another IDE working on the same project.

To see registered projects:
```bash
cat ~/.magic-claude-mem/cursor-projects.json
```
...
| Initial setup | Automatic | `magic-claude-mem cursor install` creates initial context |
| MCP tool access | ✅ Full support | ✅ Full support |
| Web viewer | ✅ Available | ✅ Available |

## First Session Behavior

When you run `magic-claude-mem cursor install`:
...
Configure magic-claude-mem's MCP server in Cursor for search tools:
...
Ask the agent: "Check magic-claude-mem for any previous work on authentication"

## File Location

The context file is created at:
```
<workspace>/.cursor/rules/magic-claude-mem-context.mdc
```
