...
Expecting one of '${F.join("', '")}'`);if(this._lifeCycleHooks[D])this._lifeCycleHooks[D].push(u);else this._lifeCycleHooks[D]=[u];return this}exitOverride(D){if(D)this._exitCallback=D;else this._exitCallback=(u)=>{if(u.code!=="commander.executeSubCommandAsync")throw u};return this}_exit(D,u,F){if(this._exitCallback)this._exitCallback(new NF(D,u,F));y.exit(D)}action(D){let u=(F)=>{let C=this.registeredArguments.length,B=F.slice(0,C);if(this._storeOptionsAsProperties)B[C]=this;else B[C]=this.opts();return B.push(this),D.apply(this,B)};return this._actionHandler=u,this}createOption(D,u){return new z3(D,u)}_callParseArg(D,u,F,C){try{return D.parseArg(u,F)}catch(B){if(B.code==="commander.invalidArgument"){let $=`${C} ${B.message}`;this.error($,{exitCode:B.exitCode,code:B.code})}throw B}}_registerOption(D){let u=D.short&&this._findOption(D.short)||D.long&&this._findOption(D.long);if(u){let F=D.long&&this._findOption(D.long)?D.long:D.short;throw new Error(`Cannot add option '${D.flags}'${this._name&&` to command '${this._name}'`} due to conflicting flag '${F}'
...
- either make a new Command for each call to parse, or stop storing options as properties`);this._name=this._savedState._name,this._scriptPath=null,this.rawArgs=[],this._optionValues={...this._savedState._optionValues},this._optionValueSources={...this._savedState._optionValueSources},this.args=[],this.processedArgs=[]}_checkForMissingExecutable(D,u,F){if(Mu.existsSync(D))return;let C=u?`searched for local subcommand relative to directory '${u}'`:"no directory for search for local subcommand, use .executableDir() to supply a custom directory",B=`'${D}' does not exist
...
`,B=u.pad||" ",$=F!=="right"?g9:m9,E=!1;if(!Array.isArray(D))E=!0,D=String(D).split(C);let Q,X=0;return D=D.map(function(Z){return Z=String(Z),Q=h9(Z),X=Math.max(Q,X),{str:Z,width:Q}}).map(function(Z){return new Array($(X,Z.width)+1).join(B)+Z.str}),E?D.join(C):D}H0.left=function D(u){return H0(u,{align:"left"})};H0.center=function D(u){return H0(u,{align:"center"})};H0.right=function D(u){return H0(u,{align:"right"})};n8.exports=H0;function g9(D,u){return Math.floor((D-u)/2)}function m9(D,u){return D-u}});import{join as s,dirname as cu,sep as $B}from"path";import{homedir as G2}from"os";import{existsSync as r0,statSync as QB}from"fs";import{execSync as X6}from"child_process";import{fileURLToPath as XB}from"url";class k{static instance=null;_dataDirectory=null;_packageRoot=null;_claudeConfigDirectory=null;static getInstance(){if(!k.instance)k.instance=new k;return k.instance}getDataDirectory(){if(this._dataDirectory)return this._dataDirectory;return this._dataDirectory=process.env.CLAUDE_MEM_DATA_DIR||s(G2(),".claude-mem"),this._dataDirectory}getArchivesDirectory(){return s(this.getDataDirectory(),"archives")}getHooksDirectory(){return s(this.getDataDirectory(),"hooks")}getLogsDirectory(){return s(this.getDataDirectory(),"logs")}getIndexDirectory(){return this.getDataDirectory()}getIndexPath(){return s(this.getIndexDirectory(),"claude-mem-index.jsonl")}getTrashDirectory(){return s(this.getDataDirectory(),"trash")}getBackupsDirectory(){return s(this.getDataDirectory(),"backups")}getChromaDirectory(){return s(this.getDataDirectory(),"chroma")}getProjectArchiveDirectory(D){return s(this.getArchivesDirectory(),D)}getUserSettingsPath(){return s(this.getDataDirectory(),"settings.json")}getClaudeConfigDirectory(){if(this._claudeConfigDirectory)return this._claudeConfigDirectory;return this._claudeConfigDirectory=process.env.CLAUDE_CONFIG_DIR||s(G2(),".claude"),this._claudeConfigDirectory}getClaudeSettingsPath(){return s(this.getClaudeConfigDirectory(),"settings.json")}getClaudeCommandsDirectory(){return s(this.getClaudeConfigDirectory(),"commands")}getClaudeMdPath(){return s(this.getClaudeConfigDirectory(),"CLAUDE.md")}getMcpConfigPath(){return s(G2(),".claude.json")}getProjectMcpConfigPath(){return s(process.cwd(),".mcp.json")}getPackageRoot(){if(this._packageRoot)return this._packageRoot;try{let F=cD.resolve("claude-mem/package.json");return this._packageRoot=cu(F),this._packageRoot}catch{}let D=XB(import.meta.url),u=cu(D);for(let F=0;F<10;F++){let C=s(u,"package.json");if(r0(C)){if(cD(C).name==="claude-mem")return this._packageRoot=u,this._packageRoot}let B=cu(u);if(B===u)break;u=B}try{let F=X6("npm list -g claude-mem --json 2>/dev/null || npm list claude-mem --json 2>/dev/null",{encoding:"utf8"}),C=JSON.parse(F);if(C.dependencies?.["claude-mem"]?.resolved)return this._packageRoot=cu(C.dependencies["claude-mem"].resolved),this._packageRoot}catch{}throw new Error("Cannot locate claude-mem package root. Ensure claude-mem is properly installed.")}findPackageHookTemplatesDirectory(){let D=this.getPackageRoot(),u=s(D,"hook-templates"),F=["session-start.js","stop.js","user-prompt-submit.js","post-tool-use.js"];for(let C of F)if(!r0(s(u,C)))throw new Error(`Package hook-templates directory missing required template file: ${C}`);return u}findPackageCommandsDirectory(){let D=this.getPackageRoot(),u=s(D,"commands"),F=["save.md"];for(let C of F)if(!r0(s(u,C)))throw new Error(`Package commands directory missing required file: ${C}`);return u}ensureDirectory(D){if(!r0(D))cD("fs").mkdirSync(D,{recursive:!0})}ensureDirectories(D){D.forEach((u)=>this.ensureDirectory(u))}ensureAllDataDirectories(){this.ensureDirectories([this.getDataDirectory(),this.getArchivesDirectory(),this.getHooksDirectory(),this.getLogsDirectory(),this.getTrashDirectory(),this.getBackupsDirectory(),this.getChromaDirectory()])}ensureAllClaudeDirectories(){this.ensureDirectories([this.getClaudeConfigDirectory(),this.getClaudeCommandsDirectory()])}static extractProjectName(D){let u=D.split($B),F=["src","lib","app","project","workspace"];for(let C=u.length-1;C>=0;C--)if(F.includes(u[C])&&C>0)return u[C-1];if(u.length>1)return u[u.length-2];return"unknown-project"}static getCurrentProjectName(){try{let D=X6("git rev-parse --show-toplevel",{cwd:process.cwd(),encoding:"utf8",stdio:["pipe","pipe","ignore"]}).trim();return cD("path").basename(D)}catch{return cD("path").basename(process.cwd())}}static createBackupFilename(D){let u=new Date().toISOString().replace(/[:.]/g,"-").replace("T","_").slice(0,19);return`${D}.backup.${u}`}static isPathAccessible(D){return r0(D)&&QB(D).isDirectory()}}var qD=()=>{};var G6={};C0(G6,{initializeDatabase:()=>L2,getDatabase:()=>_D,DatabaseManager:()=>jD,Database:()=>H6});import{Database as H6}from"bun:sqlite";import wB from"path";import kB from"fs";class jD{static instance;db=null;migrations=[];static getInstance(){if(!jD.instance)jD.instance=new jD;return jD.instance}registerMigration(D){this.migrations.push(D),this.migrations.sort((u,F)=>u.version-F.version)}async initialize(){if(this.db)return this.db;let D=k.getInstance().getDataDirectory();kB.mkdirSync(D,{recursive:!0});let u=wB.join(D,"claude-mem.db");return this.db=new H6(u,{create:!0,readwrite:!0}),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.db.run("PRAGMA temp_store = memory"),this.db.run("PRAGMA mmap_size = 268435456"),this.db.run("PRAGMA cache_size = 10000"),this.initializeSchemaVersions(),await this.runMigrations(),nu=this.db,this.db}getConnection(){if(!this.db)throw new Error("Database not initialized. Call initialize() first.");return this.db}withTransaction(D){let u=this.getConnection();return u.transaction(D)(u)}close(){if(this.db)this.db.close(),this.db=null,nu=null}initializeSchemaVersions(){if(!this.db)return;this.db.run(`
    ...
    `).run(u.content||F.content,C,B,u.project||F.project,u.origin||F.origin,D),this.getById(D)}deleteById(D){return this.db.query("DELETE FROM overviews WHERE id = ?").run(D).changes>0}deleteBySessionId(D){return this.db.query("DELETE FROM overviews WHERE session_id = ?").run(D).changes>0}getUniqueProjects(){return this.db.query("SELECT DISTINCT project FROM overviews ORDER BY project").all().map((F)=>F.project)}getByProject(D){return this.db.query(`
      ...
      ORDER BY created_at_epoch DESC
      LIMIT 1
    `).get(D)||null}upsertByProject(D){let u=this.getByProject(D.project);if(u)return this.update(u.id,D);return this.create(D)}deleteByProject(D){return this.db.query("DELETE FROM overviews WHERE project = ?").run(D).changes>0}}var T2=HD(()=>{aD()});var O6={};C0(O6,{DiagnosticsStore:()=>w2});class w2{db;constructor(D){this.db=D||_D()}create(D){let{isoString:u,epoch:F}=FD(D.created_at),B=this.db.query(`
    ...
    `).run(u.message||F.message,u.severity||F.severity,C,B,u.project||F.project,u.origin||F.origin,D),this.getById(D)}deleteById(D){return this.db.query("DELETE FROM diagnostics WHERE id = ?").run(D).changes>0}deleteBySessionId(D){return this.db.query("DELETE FROM diagnostics WHERE session_id = ?").run(D).changes}getUniqueProjects(){return this.db.query("SELECT DISTINCT project FROM diagnostics ORDER BY project").all().map((F)=>F.project)}getStats(){return this.db.query(`
      ...
      FROM diagnostics
    `).get()}}var k2=HD(()=>{aD()});var j6={};C0(j6,{TranscriptEventStore:()=>P2});class P2{db;constructor(D){this.db=D||_D()}upsert(D){let{isoString:u,epoch:F}=FD(D.captured_at);return this.db.query(`
    ...
    `).get(D)?.max_event_index??-1}listBySession(D,u=200,F=0){return this.db.query(`
      ...
      LIMIT ? OFFSET ?
    `).all(D,u,F)}}var S2=HD(()=>{aD()});var T6={};C0(T6,{StreamingSessionStore:()=>O0});class O0{db;constructor(D){this.db=D||_D()}create(D){let{isoString:u,epoch:F}=FD(D.started_at),B=this.db.query(`
      INSERT INTO streaming_sessions (
        claude_session_id, project, user_prompt, started_at, started_at_epoch, status
      ) VALUES (?, ?, ?, ?, ?, 'active')
    `).run(D.claude_session_id,D.project,D.user_prompt||null,u,F);return this.getById(B.lastInsertRowid)}update(D,u){let{isoString:F,epoch:C}=FD(new Date);if(!this.getById(D))throw new Error(`Streaming session with id ${D} not found`);let $=[],E=[];if(u.sdk_session_id!==void 0)$.push("sdk_session_id = ?"),E.push(u.sdk_session_id);if(u.title!==void 0)$.push("title = ?"),E.push(u.title);if(u.subtitle!==void 0)$.push("subtitle = ?"),E.push(u.subtitle);if(u.status!==void 0)$.push("status = ?"),E.push(u.status);if(u.completed_at!==void 0){let{isoString:X,epoch:Z}=FD(u.completed_at);$.push("completed_at = ?","completed_at_epoch = ?"),E.push(X,Z)}return $.push("updated_at = ?","updated_at_epoch = ?"),E.push(F,C),E.push(D),this.db.query(`
    ...
    `).run(u.toISOString(),F,D)}markSessionFailed(D){let u=new Date,F=u.getTime();this.db.query(`
      ...
      WHERE id = ?
    `).run(u.toISOString(),F,D)}close(){this.db.close()}}var j0=HD(()=>{qD()});var x6={};C0(x6,{migrations:()=>uu,migration004:()=>S6,migration003:()=>P6,migration002:()=>k6,migration001:()=>w6});var w6,k6,P6,S6,uu;var tu=HD(()=>{w6={version:1,up:(D)=>{D.run(`
    ...
    `),console.log("\u2705 Added hierarchical memory fields to memories table")},down:(D)=>{console.log("\u26A0\uFE0F  Warning: SQLite ALTER TABLE DROP COLUMN not fully supported"),console.log("\u26A0\uFE0F  To rollback, manually recreate the memories table")}},P6={version:3,up:(D)=>{D.run(`
...
`)),this._prevFrame=D;return}this.output.write(JD.erase.down())}this.output.write(D),this.state==="initial"&&(this.state="active"),this._prevFrame=D}}}class gF extends wu{get cursor(){return this.value?0:1}get _value(){return this.cursor===0}constructor(D){super(D,!1),this.value=!!D.initialValue,this.on("value",()=>{this.value=this._value}),this.on("confirm",(u)=>{this.output.write(JD.cursor.move(0,-1)),this.value=u,this.state="submit",this.close()}),this.on("cursor",()=>{this.value=!this.value})}}var y7;y7=new WeakMap;var h7=Object.defineProperty,g7=(D,u,F)=>(u in D)?h7(D,u,{enumerable:!0,configurable:!0,writable:!0,value:F}):D[u]=F,a3=(D,u,F)=>(g7(D,typeof u!="symbol"?u+"":u,F),F);class mF extends wu{constructor(D){super(D,!1),a3(this,"options"),a3(this,"cursor",0),this.options=D.options,this.cursor=this.options.findIndex(({value:u})=>u===D.initialValue),this.cursor===-1&&(this.cursor=0),this.changeValue(),this.on("cursor",(u)=>{switch(u){case"left":case"up":this.cursor=this.cursor===0?this.options.length-1:this.cursor-1;break;case"down":case"right":this.cursor=this.cursor===this.options.length-1?0:this.cursor+1;break}this.changeValue()})}get _value(){return this.options[this.cursor]}changeValue(){this.value=this._value.value}}class dF extends wu{get valueWithCursor(){if(this.state==="submit")return this.value;if(this.cursor>=this.value.length)return`${this.value}\u2588`;let D=this.value.slice(0,this.cursor),[u,...F]=this.value.slice(this.cursor);return`${D}${i3.default.inverse(u)}${F.join("")}`}get cursor(){return this._cursor}constructor(D){super(D),this.on("finalize",()=>{this.value||(this.value=D.defaultValue)})}}var O=F0(SF(),1),ku=F0(kF(),1);import lD from"process";function m7(){return lD.platform!=="win32"?lD.env.TERM!=="linux":!!lD.env.CI||!!lD.env.WT_SESSION||!!lD.env.TERMINUS_SUBLIME||lD.env.ConEmuTask==="{cmd::Cmder}"||lD.env.TERM_PROGRAM==="Terminus-Sublime"||lD.env.TERM_PROGRAM==="vscode"||lD.env.TERM==="xterm-256color"||lD.env.TERM==="alacritty"||lD.env.TERMINAL_EMULATOR==="JetBrains-JediTerm"}var cF=m7(),o=(D,u)=>cF?D:u,d7=o("\u25C6","*"),F8=o("\u25A0","x"),C8=o("\u25B2","x"),aF=o("\u25C7","o"),r5=o("\u250C","T"),a=o("\u2502","|"),m0=o("\u2514","\u2014"),lF=o("\u25CF",">"),pF=o("\u25CB"," "),i5=o("\u25FB","[\u2022]"),o5=o("\u25FC","[+]"),s5=o("\u25FB","[ ]"),t5=o("\u25AA","\u2022"),e5=o("\u2500","-"),D$=o("\u256E","+"),u$=o("\u251C","+"),F$=o("\u256F","+"),c7=o("\u25CF","\u2022"),l7=o("\u25C6","*"),p7=o("\u25B2","!"),a7=o("\u25A0","x"),nF=(D)=>{switch(D){case"initial":case"active":return O.default.cyan(d7);case"cancel":return O.default.red(F8);case"error":return O.default.yellow(C8);case"submit":return O.default.green(aF)}},n7=(D)=>{let{cursor:u,options:F,style:C}=D,B=D.maxItems??Number.POSITIVE_INFINITY,$=Math.max(process.stdout.rows-4,0),E=Math.min($,Math.max(B,5)),Q=0;u>=Q+E-3?Q=Math.max(Math.min(u-E+3,F.length-E),0):u<Q+2&&(Q=Math.max(u-2,0));let X=E<F.length&&Q>0,Z=E<F.length&&Q+E<F.length;return F.slice(Q,Q+E).map((Y,J,A)=>{let H=J===0&&X,G=J===A.length-1&&Z;return H||G?O.default.dim("..."):C(Y,J+Q===u)})},B8=(D)=>new dF({validate:D.validate,placeholder:D.placeholder,defaultValue:D.defaultValue,initialValue:D.initialValue,render(){let u=`${O.default.gray(a)}
${nF(this.state)}  ${D.message}
...
${O.default.yellow(a)}  ${C}
${O.default.yellow(m0)}  ${O.default.yellow(this.error)}
`;case"submit":return`${u}${O.default.gray(a)}  ${O.default.dim(this.value||D.placeholder)}`;case"cancel":return`${u}${O.default.gray(a)}  ${O.default.strikethrough(O.default.dim(this.value??""))}${this.value?.trim()?`
${O.default.gray(a)}`:""}`;default:return`${u}${O.default.cyan(a)}  ${C}
${O.default.cyan(m0)}
`}}}).prompt();var R0=(D)=>{let u=D.active??"Yes",F=D.inactive??"No";return new gF({active:u,inactive:F,initialValue:D.initialValue??!0,render(){let C=`${O.default.gray(a)}
${nF(this.state)}  ${D.message}
`,B=this.value?u:F;switch(this.state){case"submit":return`${C}${O.default.gray(a)}  ${O.default.dim(B)}`;case"cancel":return`${C}${O.default.gray(a)}  ${O.default.strikethrough(O.default.dim(B))}
${O.default.gray(a)}`;default:return`${C}${O.default.cyan(a)}  ${this.value?`${O.default.green(lF)} ${u}`:`${O.default.dim(pF)} ${O.default.dim(u)}`} ${O.default.dim("/")} ${this.value?`${O.default.dim(pF)} ${O.default.dim(F)}`:`${O.default.green(lF)} ${F}`}
${O.default.cyan(m0)}
`}}}).prompt()},Pu=(D)=>{let u=(F,C)=>{let B=F.label??String(F.value);switch(C){case"selected":return`${O.default.dim(B)}`;case"active":return`${O.default.green(lF)} ${B} ${F.hint?O.default.dim(`(${F.hint})`):""}`;case"cancelled":return`${O.default.strikethrough(O.default.dim(B))}`;default:return`${O.default.dim(pF)} ${O.default.dim(B)}`}};return new mF({options:D.options,initialValue:D.initialValue,render(){let F=`${O.default.gray(a)}
${nF(this.state)}  ${D.message}
`;switch(this.state){case"submit":return`${F}${O.default.gray(a)}  ${u(this.options[this.cursor],"selected")}`;case"cancel":return`${F}${O.default.gray(a)}  ${u(this.options[this.cursor],"cancelled")}
${O.default.gray(a)}`;default:return`${F}${O.default.cyan(a)}  ${n7({cursor:this.cursor,options:this.options,maxItems:D.maxItems,style:(C,B)=>u(C,B?"active":"inactive")}).join(`
${O.default.cyan(a)}  `)}
${O.default.cyan(m0)}
`}}}).prompt()};var M0=(D="")=>{process.stdout.write(`${O.default.gray(m0)}  ${O.default.red(D)}

`)};var GD={message:(D="",{symbol:u=O.default.gray(a)}={})=>{let F=[`${O.default.gray(a)}`];if(D){let[C,...B]=D.split(`
`);F.push(`${u}  ${C}`,...B.map(($)=>`${O.default.gray(a)}  ${$}`))}process.stdout.write(`${F.join(`
`)}
...
`),console.log("\uD83D\uDCA1 Or use --historical 1 to regenerate this version's changelog"),process.exit(1)}}let Z=[];if(D.start&&D.end){let W=D.generate||F;Z.push({version:W,startTime:D.start,endTime:D.end}),console.log(`\uD83D\uDCC5 Using custom time range for ${W}:`),console.log(`   Start: ${new Date(D.start).toLocaleString()}`),console.log(`   End: ${new Date(D.end).toLocaleString()}`)}else try{let W=i1(`npm view ${C} time --json`,{encoding:"utf-8",timeout:5000}),T=JSON.parse(W),b="";if($>1){let w=B[B.length-1].split("."),g=parseInt(w[0]),m=parseInt(w[1]),gD=parseInt(w[2]);if(gD>0)b=`${g}.${m}.${gD-1}`;else if(m>0){let Q0=`${g}.${m-1}.`,r=Object.keys(T).filter((ZD)=>ZD.startsWith(Q0)).sort((ZD,mD)=>{let u0=parseInt(ZD.split(".")[2]||"0");return parseInt(mD.split(".")[2]||"0")-u0});if(r.length>0)b=r[0]}else if(g>0){let Q0=`${g-1}.`,r=Object.keys(T).filter((ZD)=>ZD.startsWith(Q0)).sort((ZD,mD)=>{let[,u0,$D]=ZD.split(".").map(Number),[,YD,f0]=mD.split(".").map(Number);if(u0!==YD)return YD-u0;return f0-$D});if(r.length>0)b=r[0]}if(D.verbose&&b&&T[b])console.log(`\uD83D\uDCCD Using ${b} as start boundary for time ranges`)}for(let v=0;v<B.length;v++){let w=B[v],g="2000-01-01T00:00:00Z";if(v===0){let gD=w.split("."),Q0=parseInt(gD[0]),r=parseInt(gD[1]),ZD=parseInt(gD[2]),mD="";if(ZD>0)mD=`${Q0}.${r}.${ZD-1}`;else if(r>0){let u0=`${Q0}.${r-1}.`,$D=Object.keys(T).filter((YD)=>YD.startsWith(u0)).sort((YD,f0)=>{let UF=parseInt(YD.split(".")[2]||"0");return parseInt(f0.split(".")[2]||"0")-UF});if($D.length>0)mD=$D[0]}if(T[mD])g=T[mD]}else if(v<B.length-1){let gD=B[v+1];if(T[gD])g=T[gD]}else if(b&&T[b])g=T[b];let m=T[w]||new Date().toISOString();if(Z.push({version:w,startTime:g,endTime:m}),D.verbose)console.log(`\uD83D\uDCC5 Version ${w}: ${new Date(g).toLocaleString()} - ${new Date(m).toLocaleString()}`)}if($===1){let v=Z[0];if(v)console.log(`\uD83D\uDCE6 Using npm time range for ${v.version}: ${new Date(v.startTime).toLocaleString()} - ${new Date(v.endTime).toLocaleString()}`)}}catch(W){console.log("\u274C Could not fetch npm publish times. Cannot proceed without time ranges."),process.exit(1)}console.log(`\uD83D\uDD0D Searching memories for versions: ${B.join(", ")}`),console.log(`\uD83D\uDCE6 Project: ${C}
...
`).filter((T)=>T.trim());for(let T of W)try{M.push(JSON.parse(T))}catch(b){}}let P=new Map;for(let W of M){if(!P.has(W.version))P.set(W.version,[]);P.get(W.version).push(W)}let x=`# Changelog

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/).

`,AD=Array.from(P.keys()).sort((W,T)=>{let b=W.split(".").map(Number),v=T.split(".").map(Number);for(let w=0;w<3;w++)if(b[w]!==v[w])return v[w]-b[w];return 0});for(let W of AD){let T=P.get(W),b=T[0].date||z;x+=`
## [${W}] - ${b}
...
`;x+=`
`}}}let wD=RD.join(process.cwd(),"CHANGELOG.md");if(n.writeFileSync(wD,x),console.log(`\u2705 Generated CHANGELOG.md with ${M.length} total entries across ${P.size} versions!`),D.preview){if(console.log(`
\uD83D\uDCC4 Preview:
`),console.log(x.split(`
`).slice(0,30).join(`
`)),x.split(`
`).length>30)console.log(`
... (truncated for preview)`)}}catch(u){if(console.error("\u274C Error generating changelog:",u instanceof Error?u.message:u),u instanceof Error&&u.stack)console.error("Stack:",u.stack);process.exit(1)}}async function o1(D){try{let u=RD.join(process.cwd(),"package.json"),F="unknown";if(n.existsSync(u))try{F=JSON.parse(n.readFileSync(u,"utf-8")).name||RD.basename(process.cwd())}catch(J){F=RD.basename(process.cwd())}let C=RD.join(process.env.HOME||process.env.USERPROFILE||"",".claude-mem","projects"),B=RD.join(C,`${F}-changelog.jsonl`);if(!n.existsSync(B))console.log("\u274C No changelog entries found. Generate some first with: claude-mem changelog"),process.exit(1);console.log("\uD83D\uDCDD Updating CHANGELOG.md from JSONL entries...");let $=[],E=n.readFileSync(B,"utf-8").split(`
`).filter((J)=>J.trim());for(let J of E)try{$.push(JSON.parse(J))}catch(A){}if($.length===0)console.log("\u274C No valid entries found in JSONL file"),process.exit(1);let Q=new Map;for(let J of $){if(!Q.has(J.version))Q.set(J.version,[]);Q.get(J.version).push(J)}let X=`# Changelog

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/).

`,Z=Array.from(Q.keys()).sort((J,A)=>{let H=J.split(".").map(Number),G=A.split(".").map(Number);for(let z=0;z<3;z++)if(H[z]!==G[z])return G[z]-H[z];return 0});for(let J of Z){let A=Q.get(J),H=A[0].date;X+=`
## [${J}] - ${H}

`;let G=["Added","Changed","Fixed","Removed","Deprecated","Security"];for(let z of G){let U=A.filter((I)=>I.type===z);if(U.length>0){X+=`### ${z}
`;for(let I of U)X+=`- ${I.description}
`;X+=`
`}}}let Y=RD.join(process.cwd(),"CHANGELOG.md");if(n.writeFileSync(Y,X),console.log(`\u2705 Updated CHANGELOG.md with ${$.length} entries across ${Q.size} versions!`),D.preview){if(console.log(`
\uD83D\uDCC4 Preview:
`),console.log(X.split(`
`).slice(0,30).join(`
`)),X.split(`
