...
Expecting one of '${F.join("', '")}'`);if(this._lifeCycleHooks[D])this._lifeCycleHooks[D].push(u);else this._lifeCycleHooks[D]=[u];return this}exitOverride(D){if(D)this._exitCallback=D;else this._exitCallback=(u)=>{if(u.code!=="commander.executeSubCommandAsync")throw u};return this}_exit(D,u,F){if(this._exitCallback)this._exitCallback(new i0(D,u,F));P.exit(D)}action(D){let u=(F)=>{let C=this.registeredArguments.length,B=F.slice(0,C);if(this._storeOptionsAsProperties)B[C]=this;else B[C]=this.opts();return B.push(this),D.apply(this,B)};return this._actionHandler=u,this}createOption(D,u){return new M2(D,u)}_callParseArg(D,u,F,C){try{return D.parseArg(u,F)}catch(B){if(B.code==="commander.invalidArgument"){let $=`${C} ${B.message}`;this.error($,{exitCode:B.exitCode,code:B.code})}throw B}}_registerOption(D){let u=D.short&&this._findOption(D.short)||D.long&&this._findOption(D.long);if(u){let F=D.long&&this._findOption(D.long)?D.long:D.short;throw new Error(`Cannot add option '${D.flags}'${this._name&&` to command '${this._name}'`} due to conflicting flag '${F}'
...
${M.default.cyan(Uu)}
`}}}).prompt();var tD=(D)=>{let u=D.active??"Yes",F=D.inactive??"No";return new QF({active:u,inactive:F,initialValue:D.initialValue??!0,render(){let C=`${M.default.gray(g)}
${VF(this.state)}  ${D.message}
...
${M.default.gray(g)}`;default:return`${C}${M.default.cyan(g)}  ${this.value?`${M.default.green(qF)} ${u}`:`${M.default.dim(YF)} ${M.default.dim(u)}`} ${M.default.dim("/")} ${this.value?`${M.default.dim(YF)} ${M.default.dim(F)}`:`${M.default.green(qF)} ${F}`}
${M.default.cyan(Uu)}
`}}}).prompt()},C0=(D)=>{let u=(F,C)=>{let B=F.label??String(F.value);switch(C){case"selected":return`${M.default.dim(B)}`;case"active":return`${M.default.green(qF)} ${B} ${F.hint?M.default.dim(`(${F.hint})`):""}`;case"cancelled":return`${M.default.strikethrough(M.default.dim(B))}`;default:return`${M.default.dim(YF)} ${M.default.dim(B)}`}};return new XF({options:D.options,initialValue:D.initialValue,render(){let F=`${M.default.gray(g)}
${VF(this.state)}  ${D.message}
...
${M.default.gray(g)}`;default:return`${F}${M.default.cyan(g)}  ${OC({cursor:this.cursor,options:this.options,maxItems:D.maxItems,style:(C,B)=>u(C,B?"active":"inactive")}).join(`
${M.default.cyan(g)}  `)}
${M.default.cyan(Uu)}
`}}}).prompt()};var eD=(D="")=>{process.stdout.write(`${M.default.gray(Uu)}  ${M.default.red(D)}
...
\uD83D\uDE80 Never lose your conversation history again`),{padding:2,margin:1,borderStyle:"double",borderColor:"magenta",textAlignment:"center"})),r7();let u=D.user||D.project||D.local||D.force,F;if(u)F={scope:D.local?"local":D.project?"project":"user",customPath:D.path,hookTimeout:D.timeout?parseInt(D.timeout):180,forceReinstall:!!D.force,enableSmartTrash:!1,saveMemoriesOnClear:!1};else{let X=o7(),Z=await s7(X);if(!Z)process.exit(0);F=Z}console.log(gF(`
\uD83D\uDE80 Beginning Installation Process
`));let C=[{name:"Creating directory structure",fn:()=>t7()},{name:"Installing Chroma MCP server",fn:()=>D9(F.forceReinstall)},{name:"Adding CLAUDE.md instructions",fn:()=>e7()},{name:"Installing Claude commands",fn:()=>E9()},{name:"Configuring Claude settings",fn:()=>u9(F9(F))},{name:"Configuring user settings",fn:()=>C9(F)}];if(F.enableSmartTrash)C.push({name:"Configuring Smart Trash alias",fn:()=>B9()});for(let X=0;X<C.length;X++){let Z=C[X],J=`[${X+1}/${C.length}]`,V=a7(`${N.gray(J)} ${Z.name}...`);V.start(),Z.fn(),V.stop(`${N.gray(J)} ${Z.name} ${gF("completed! \u2728")}`)}let B=Lu("\uD83C\uDF89 INSTALLATION COMPLETE! \uD83C\uDF89"),$=`
...
`))}HD();import{readFileSync as z8,writeFileSync as G0,existsSync as G8}from"fs";import{join as Nu}from"path";import{homedir as $9}from"os";async function Q9(){let D=$9(),u=[Nu(D,".bashrc"),Nu(D,".zshrc"),Nu(D,".bash_profile")],F='alias rm="claude-mem trash"',C=["# claude-mem smart trash alias","# claude-mem trash bin alias"],B=!1;for(let $ of u){if(!G8($))continue;let E=z8($,"utf8");if(!E.includes('alias rm="claude-mem trash"'))continue;let Q=E.split(`
...
To restore files: claude-mem restore`),console.log("To empty trash:   claude-mem trash empty")}catch(u){if(u.code==="ENOENT")DD.info("\uD83D\uDDD1\uFE0F  Trash is empty");else DD.error("Failed to read trash directory"),console.error(u)}}import{rmSync as F4,readdirSync as C4,existsSync as B4,statSync as E4}from"fs";import{join as L6}from"path";HD();async function I6(D={}){let u=O.getInstance().getTrashDirectory();if(!B4(u)){DD.info("\uD83D\uDDD1\uFE0F  Trash is already empty");return}try{let F=C4(u);if(F.length===0){DD.info("\uD83D\uDDD1\uFE0F  Trash is already empty");return}let C=0,B=0;for(let E of F){let Q=L6(u,E);if(E4(Q).isDirectory())C++;else B++}if(!D.force){let E=await tD({message:`Permanently delete ${C} folders and ${B} files from trash?`,initialValue:!1});if(VD(E)||!E){DD.info("Cancelled - trash not emptied");return}}let $=J3();$.start("Emptying trash...");for(let E of F){let Q=L6(u,E);F4(Q,{recursive:!0,force:!0})}$.stop(`\uD83D\uDDD1\uFE0F  Trash emptied - permanently deleted ${C} folders and ${B} files`)}catch(F){DD.error("Failed to empty trash"),console.error(F),process.exit(1)}}import{readdirSync as $4,renameSync as Q4}from"fs";import{join as N6}from"path";HD();async function T6(){let D=O.getInstance().getTrashDirectory(),u=$4(D);if(u.length===0){console.log("Trash is empty");return}let F=await C0({message:"Select file to restore:",options:u.map((C)=>({value:C,label:C}))});if(VD(F))return;Q4(N6(D,F),N6(process.cwd(),F)),console.log(`Restored ${F}`)}HD();import qu from"fs";import z4 from"path";HD();import{Database as X4}from"bun:sqlite";import Z4 from"path";import J4 from"fs";var y0=null;class pD{static instance;db=null;migrations=[];static getInstance(){if(!pD.instance)pD.instance=new pD;return pD.instance}registerMigration(D){this.migrations.push(D),this.migrations.sort((u,F)=>u.version-F.version)}async initialize(){if(this.db)return this.db;let D=O.getInstance().getDataDirectory();J4.mkdirSync(D,{recursive:!0});let u=Z4.join(D,"claude-mem.db");return this.db=new X4(u,{create:!0,readwrite:!0}),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.db.run("PRAGMA temp_store = memory"),this.db.run("PRAGMA mmap_size = 268435456"),this.db.run("PRAGMA cache_size = 10000"),this.initializeSchemaVersions(),await this.runMigrations(),y0=this.db,this.db}getConnection(){if(!this.db)throw new Error("Database not initialized. Call initialize() first.");return this.db}withTransaction(D){let u=this.getConnection();return u.transaction(D)(u)}close(){if(this.db)this.db.close(),this.db=null,y0=null}initializeSchemaVersions(){if(!this.db)return;this.db.run(`
    ...
    `),console.log("\u2705 Added hierarchical memory fields to memories table")},down:(D)=>{console.log("\u26A0\uFE0F  Warning: SQLite ALTER TABLE DROP COLUMN not fully supported"),console.log("\u26A0\uFE0F  To rollback, manually recreate the memories table")}},H4={version:3,up:(D)=>{D.run(`
