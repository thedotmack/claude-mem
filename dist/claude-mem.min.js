...
Expecting one of '${F.join("', '")}'`);if(this._lifeCycleHooks[D])this._lifeCycleHooks[D].push(u);else this._lifeCycleHooks[D]=[u];return this}exitOverride(D){if(D)this._exitCallback=D;else this._exitCallback=(u)=>{if(u.code!=="commander.executeSubCommandAsync")throw u};return this}_exit(D,u,F){if(this._exitCallback)this._exitCallback(new NF(D,u,F));y.exit(D)}action(D){let u=(F)=>{let C=this.registeredArguments.length,B=F.slice(0,C);if(this._storeOptionsAsProperties)B[C]=this;else B[C]=this.opts();return B.push(this),D.apply(this,B)};return this._actionHandler=u,this}createOption(D,u){return new _3(D,u)}_callParseArg(D,u,F,C){try{return D.parseArg(u,F)}catch(B){if(B.code==="commander.invalidArgument"){let $=`${C} ${B.message}`;this.error($,{exitCode:B.exitCode,code:B.code})}throw B}}_registerOption(D){let u=D.short&&this._findOption(D.short)||D.long&&this._findOption(D.long);if(u){let F=D.long&&this._findOption(D.long)?D.long:D.short;throw new Error(`Cannot add option '${D.flags}'${this._name&&` to command '${this._name}'`} due to conflicting flag '${F}'
...
`,B=u.pad||" ",$=F!=="right"?d9:c9,E=!1;if(!Array.isArray(D))E=!0,D=String(D).split(C);let Q,X=0;return D=D.map(function(Z){return Z=String(Z),Q=m9(Z),X=Math.max(Q,X),{str:Z,width:Q}}).map(function(Z){return new Array($(X,Z.width)+1).join(B)+Z.str}),E?D.join(C):D}H0.left=function D(u){return H0(u,{align:"left"})};H0.center=function D(u){return H0(u,{align:"center"})};H0.right=function D(u){return H0(u,{align:"right"})};r8.exports=H0;function d9(D,u){return Math.floor((D-u)/2)}function c9(D,u){return D-u}});import{join as s,dirname as cu,sep as XB}from"path";import{homedir as G2}from"os";import{existsSync as r0,statSync as ZB}from"fs";import{execSync as Z6}from"child_process";import{fileURLToPath as YB}from"url";class k{static instance=null;_dataDirectory=null;_packageRoot=null;_claudeConfigDirectory=null;static getInstance(){if(!k.instance)k.instance=new k;return k.instance}getDataDirectory(){if(this._dataDirectory)return this._dataDirectory;return this._dataDirectory=process.env.CLAUDE_MEM_DATA_DIR||s(G2(),".claude-mem"),this._dataDirectory}getArchivesDirectory(){return s(this.getDataDirectory(),"archives")}getHooksDirectory(){return s(this.getDataDirectory(),"hooks")}getLogsDirectory(){return s(this.getDataDirectory(),"logs")}getIndexDirectory(){return this.getDataDirectory()}getIndexPath(){return s(this.getIndexDirectory(),"claude-mem-index.jsonl")}getTrashDirectory(){return s(this.getDataDirectory(),"trash")}getBackupsDirectory(){return s(this.getDataDirectory(),"backups")}getChromaDirectory(){return s(this.getDataDirectory(),"chroma")}getProjectArchiveDirectory(D){return s(this.getArchivesDirectory(),D)}getUserSettingsPath(){return s(this.getDataDirectory(),"settings.json")}getClaudeConfigDirectory(){if(this._claudeConfigDirectory)return this._claudeConfigDirectory;return this._claudeConfigDirectory=process.env.CLAUDE_CONFIG_DIR||s(G2(),".claude"),this._claudeConfigDirectory}getClaudeSettingsPath(){return s(this.getClaudeConfigDirectory(),"settings.json")}getClaudeCommandsDirectory(){return s(this.getClaudeConfigDirectory(),"commands")}getClaudeMdPath(){return s(this.getClaudeConfigDirectory(),"CLAUDE.md")}getMcpConfigPath(){return s(G2(),".claude.json")}getProjectMcpConfigPath(){return s(process.cwd(),".mcp.json")}getPackageRoot(){if(this._packageRoot)return this._packageRoot;try{let F=cD.resolve("claude-mem/package.json");return this._packageRoot=cu(F),this._packageRoot}catch{}let D=YB(import.meta.url),u=cu(D);for(let F=0;F<10;F++){let C=s(u,"package.json");if(r0(C)){if(cD(C).name==="claude-mem")return this._packageRoot=u,this._packageRoot}let B=cu(u);if(B===u)break;u=B}try{let F=Z6("npm list -g claude-mem --json 2>/dev/null || npm list claude-mem --json 2>/dev/null",{encoding:"utf8"}),C=JSON.parse(F);if(C.dependencies?.["claude-mem"]?.resolved)return this._packageRoot=cu(C.dependencies["claude-mem"].resolved),this._packageRoot}catch{}throw new Error("Cannot locate claude-mem package root. Ensure claude-mem is properly installed.")}findPackageHookTemplatesDirectory(){let D=this.getPackageRoot(),u=s(D,"hook-templates"),F=["session-start.js","stop.js","user-prompt-submit.js","post-tool-use.js"];for(let C of F)if(!r0(s(u,C)))throw new Error(`Package hook-templates directory missing required template file: ${C}`);return u}findPackageCommandsDirectory(){let D=this.getPackageRoot(),u=s(D,"commands"),F=["save.md"];for(let C of F)if(!r0(s(u,C)))throw new Error(`Package commands directory missing required file: ${C}`);return u}ensureDirectory(D){if(!r0(D))cD("fs").mkdirSync(D,{recursive:!0})}ensureDirectories(D){D.forEach((u)=>this.ensureDirectory(u))}ensureAllDataDirectories(){this.ensureDirectories([this.getDataDirectory(),this.getArchivesDirectory(),this.getHooksDirectory(),this.getLogsDirectory(),this.getTrashDirectory(),this.getBackupsDirectory(),this.getChromaDirectory()])}ensureAllClaudeDirectories(){this.ensureDirectories([this.getClaudeConfigDirectory(),this.getClaudeCommandsDirectory()])}static extractProjectName(D){let u=D.split(XB),F=["src","lib","app","project","workspace"];for(let C=u.length-1;C>=0;C--)if(F.includes(u[C])&&C>0)return u[C-1];if(u.length>1)return u[u.length-2];return"unknown-project"}static getCurrentProjectName(){try{let D=Z6("git rev-parse --show-toplevel",{cwd:process.cwd(),encoding:"utf8",stdio:["pipe","pipe","ignore"]}).trim();return cD("path").basename(D)}catch{return cD("path").basename(process.cwd())}}static createBackupFilename(D){let u=new Date().toISOString().replace(/[:.]/g,"-").replace("T","_").slice(0,19);return`${D}.backup.${u}`}static isPathAccessible(D){return r0(D)&&ZB(D).isDirectory()}}var qD=()=>{};var z6={};C0(z6,{initializeDatabase:()=>L2,getDatabase:()=>_D,DatabaseManager:()=>jD,Database:()=>G6});import{Database as G6}from"bun:sqlite";import PB from"path";import SB from"fs";class jD{static instance;db=null;migrations=[];static getInstance(){if(!jD.instance)jD.instance=new jD;return jD.instance}registerMigration(D){this.migrations.push(D),this.migrations.sort((u,F)=>u.version-F.version)}async initialize(){if(this.db)return this.db;let D=k.getInstance().getDataDirectory();SB.mkdirSync(D,{recursive:!0});let u=PB.join(D,"claude-mem.db");return this.db=new G6(u,{create:!0,readwrite:!0}),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.db.run("PRAGMA temp_store = memory"),this.db.run("PRAGMA mmap_size = 268435456"),this.db.run("PRAGMA cache_size = 10000"),this.initializeSchemaVersions(),await this.runMigrations(),nu=this.db,this.db}getConnection(){if(!this.db)throw new Error("Database not initialized. Call initialize() first.");return this.db}withTransaction(D){let u=this.getConnection();return u.transaction(D)(u)}close(){if(this.db)this.db.close(),this.db=null,nu=null}initializeSchemaVersions(){if(!this.db)return;this.db.run(`
    ...
    `).run(u.content||F.content,C,B,u.project||F.project,u.origin||F.origin,D),this.getById(D)}deleteById(D){return this.db.query("DELETE FROM overviews WHERE id = ?").run(D).changes>0}deleteBySessionId(D){return this.db.query("DELETE FROM overviews WHERE session_id = ?").run(D).changes>0}getUniqueProjects(){return this.db.query("SELECT DISTINCT project FROM overviews ORDER BY project").all().map((F)=>F.project)}getByProject(D){return this.db.query(`
      ...
      ORDER BY created_at_epoch DESC
      LIMIT 1
    `).get(D)||null}upsertByProject(D){let u=this.getByProject(D.project);if(u)return this.update(u.id,D);return this.create(D)}deleteByProject(D){return this.db.query("DELETE FROM overviews WHERE project = ?").run(D).changes>0}}var T2=HD(()=>{aD()});var j6={};C0(j6,{DiagnosticsStore:()=>w2});class w2{db;constructor(D){this.db=D||_D()}create(D){let{isoString:u,epoch:F}=FD(D.created_at),B=this.db.query(`
    ...
    `).run(u.message||F.message,u.severity||F.severity,C,B,u.project||F.project,u.origin||F.origin,D),this.getById(D)}deleteById(D){return this.db.query("DELETE FROM diagnostics WHERE id = ?").run(D).changes>0}deleteBySessionId(D){return this.db.query("DELETE FROM diagnostics WHERE session_id = ?").run(D).changes}getUniqueProjects(){return this.db.query("SELECT DISTINCT project FROM diagnostics ORDER BY project").all().map((F)=>F.project)}getStats(){return this.db.query(`
      ...
      FROM diagnostics
    `).get()}}var k2=HD(()=>{aD()});var T6={};C0(T6,{TranscriptEventStore:()=>P2});class P2{db;constructor(D){this.db=D||_D()}upsert(D){let{isoString:u,epoch:F}=FD(D.captured_at);return this.db.query(`
    ...
    `).get(D)?.max_event_index??-1}listBySession(D,u=200,F=0){return this.db.query(`
      ...
      LIMIT ? OFFSET ?
    `).all(D,u,F)}}var S2=HD(()=>{aD()});var w6={};C0(w6,{StreamingSessionStore:()=>O0});class O0{db;constructor(D){this.db=D||_D()}create(D){let{isoString:u,epoch:F}=FD(D.started_at),B=this.db.query(`
    ...
    `).run(u.toISOString(),F,D)}markSessionFailed(D){let u=new Date,F=u.getTime();this.db.query(`
      ...
      WHERE id = ?
    `).run(u.toISOString(),F,D)}close(){this.db.close()}}var j0=HD(()=>{qD()});var b6={};C0(b6,{migrations:()=>uu,migration004:()=>x6,migration003:()=>S6,migration002:()=>P6,migration001:()=>k6});var k6,P6,S6,x6,uu;var tu=HD(()=>{k6={version:1,up:(D)=>{D.run(`
    ...
    `),console.log("\u2705 Added hierarchical memory fields to memories table")},down:(D)=>{console.log("\u26A0\uFE0F  Warning: SQLite ALTER TABLE DROP COLUMN not fully supported"),console.log("\u26A0\uFE0F  To rollback, manually recreate the memories table")}},S6={version:3,up:(D)=>{D.run(`
...
`)),this._prevFrame=D;return}this.output.write(JD.erase.down())}this.output.write(D),this.state==="initial"&&(this.state="active"),this._prevFrame=D}}}class gF extends wu{get cursor(){return this.value?0:1}get _value(){return this.cursor===0}constructor(D){super(D,!1),this.value=!!D.initialValue,this.on("value",()=>{this.value=this._value}),this.on("confirm",(u)=>{this.output.write(JD.cursor.move(0,-1)),this.value=u,this.state="submit",this.close()}),this.on("cursor",()=>{this.value=!this.value})}}var g7;g7=new WeakMap;var m7=Object.defineProperty,d7=(D,u,F)=>(u in D)?m7(D,u,{enumerable:!0,configurable:!0,writable:!0,value:F}):D[u]=F,n3=(D,u,F)=>(d7(D,typeof u!="symbol"?u+"":u,F),F);class mF extends wu{constructor(D){super(D,!1),n3(this,"options"),n3(this,"cursor",0),this.options=D.options,this.cursor=this.options.findIndex(({value:u})=>u===D.initialValue),this.cursor===-1&&(this.cursor=0),this.changeValue(),this.on("cursor",(u)=>{switch(u){case"left":case"up":this.cursor=this.cursor===0?this.options.length-1:this.cursor-1;break;case"down":case"right":this.cursor=this.cursor===this.options.length-1?0:this.cursor+1;break}this.changeValue()})}get _value(){return this.options[this.cursor]}changeValue(){this.value=this._value.value}}class dF extends wu{get valueWithCursor(){if(this.state==="submit")return this.value;if(this.cursor>=this.value.length)return`${this.value}\u2588`;let D=this.value.slice(0,this.cursor),[u,...F]=this.value.slice(this.cursor);return`${D}${o3.default.inverse(u)}${F.join("")}`}get cursor(){return this._cursor}constructor(D){super(D),this.on("finalize",()=>{this.value||(this.value=D.defaultValue)})}}var O=F0(SF(),1),ku=F0(kF(),1);import lD from"process";function c7(){return lD.platform!=="win32"?lD.env.TERM!=="linux":!!lD.env.CI||!!lD.env.WT_SESSION||!!lD.env.TERMINUS_SUBLIME||lD.env.ConEmuTask==="{cmd::Cmder}"||lD.env.TERM_PROGRAM==="Terminus-Sublime"||lD.env.TERM_PROGRAM==="vscode"||lD.env.TERM==="xterm-256color"||lD.env.TERM==="alacritty"||lD.env.TERMINAL_EMULATOR==="JetBrains-JediTerm"}var cF=c7(),o=(D,u)=>cF?D:u,l7=o("\u25C6","*"),C8=o("\u25A0","x"),B8=o("\u25B2","x"),aF=o("\u25C7","o"),o5=o("\u250C","T"),a=o("\u2502","|"),m0=o("\u2514","\u2014"),lF=o("\u25CF",">"),pF=o("\u25CB"," "),s5=o("\u25FB","[\u2022]"),t5=o("\u25FC","[+]"),e5=o("\u25FB","[ ]"),D$=o("\u25AA","\u2022"),u$=o("\u2500","-"),F$=o("\u256E","+"),C$=o("\u251C","+"),B$=o("\u256F","+"),p7=o("\u25CF","\u2022"),a7=o("\u25C6","*"),n7=o("\u25B2","!"),r7=o("\u25A0","x"),nF=(D)=>{switch(D){case"initial":case"active":return O.default.cyan(l7);case"cancel":return O.default.red(C8);case"error":return O.default.yellow(B8);case"submit":return O.default.green(aF)}},i7=(D)=>{let{cursor:u,options:F,style:C}=D,B=D.maxItems??Number.POSITIVE_INFINITY,$=Math.max(process.stdout.rows-4,0),E=Math.min($,Math.max(B,5)),Q=0;u>=Q+E-3?Q=Math.max(Math.min(u-E+3,F.length-E),0):u<Q+2&&(Q=Math.max(u-2,0));let X=E<F.length&&Q>0,Z=E<F.length&&Q+E<F.length;return F.slice(Q,Q+E).map((Y,J,A)=>{let H=J===0&&X,G=J===A.length-1&&Z;return H||G?O.default.dim("..."):C(Y,J+Q===u)})},E8=(D)=>new dF({validate:D.validate,placeholder:D.placeholder,defaultValue:D.defaultValue,initialValue:D.initialValue,render(){let u=`${O.default.gray(a)}
...
${O.default.gray(a)}`;default:return`${F}${O.default.cyan(a)}  ${i7({cursor:this.cursor,options:this.options,maxItems:D.maxItems,style:(C,B)=>u(C,B?"active":"inactive")}).join(`
${O.default.cyan(a)}  `)}
${O.default.cyan(m0)}
...
\uD83D\uDE80 Never lose your conversation history again`),{padding:2,margin:1,borderStyle:"double",borderColor:"magenta",textAlignment:"center"})),GB();let u=D.user||D.project||D.local||D.force,F;if(u)F={scope:D.local?"local":D.project?"project":"user",customPath:D.path,hookTimeout:D.timeout?parseInt(D.timeout):180,forceReinstall:!!D.force,enableSmartTrash:!1,saveMemoriesOnClear:!1};else{let X=_B(),Z=await UB(X);if(!Z)process.exit(0);F=Z}console.log(_2(`
\uD83D\uDE80 Beginning Installation Process
`));let C=[{name:"Creating directory structure",fn:()=>WB()},{name:"Installing Chroma MCP server",fn:()=>MB(F.forceReinstall)},{name:"Adding CLAUDE.md instructions",fn:()=>RB()},{name:"Installing Claude commands",fn:()=>TB()},{name:"Installing memory hooks",fn:()=>LB(F.hookTimeout)},{name:"Configuring Claude settings",fn:()=>IB(NB(F))},{name:"Configuring user settings",fn:()=>OB(F)}];if(F.enableSmartTrash)C.push({name:"Configuring Smart Trash alias",fn:()=>jB()});for(let X=0;X<C.length;X++){let Z=C[X],Y=`[${X+1}/${C.length}]`,J=HB(`${S.gray(Y)} ${Z.name}...`);J.start(),Z.fn(),J.stop(`${S.gray(Y)} ${Z.name} ${_2("completed! \u2728")}`)}let B=o0("\uD83C\uDF89 INSTALLATION COMPLETE! \uD83C\uDF89"),$=`
...
`))}qD();import{readFileSync as V6,writeFileSync as au,existsSync as A6}from"fs";import{join as K0}from"path";import{homedir as wB}from"os";async function kB(){let D=wB(),u=[K0(D,".bashrc"),K0(D,".zshrc"),K0(D,".bash_profile")],F='alias rm="claude-mem trash"',C=["# claude-mem smart trash alias","# claude-mem trash bin alias"],B=!1;for(let $ of u){if(!A6($))continue;let E=V6($,"utf8");if(!E.includes('alias rm="claude-mem trash"'))continue;let Q=E.split(`
...
`;n.appendFileSync(Q,T),console.log(`\u2705 Created placeholder changelog entry for v${B[0]}`),await s1(D);return}console.log(`\u2705 Found ${H.length} version-related memories
`);let z=o1('date "+%Y-%m-%d %H:%M:%S %Z"').toString().trim().split(" ")[0],U=`Analyze these memories and generate changelog entries.
...
Now generate the title and subtitle (two lines exactly):`;try{let $=await Z5({prompt:B,options:{allowedTools:[],pathToClaudeCodeExecutable:Fu()}}),E="";if($&&typeof $==="object"&&Symbol.asyncIterator in $){for await(let Y of $)if(Y?.type==="assistant"&&Y?.message?.content){let J=Y.message.content;if(typeof J==="string")E+=J;else if(Array.isArray(J)){for(let A of J)if(A.type==="text"&&A.text)E+=A.text}}}let Q=E.trim().split(`
