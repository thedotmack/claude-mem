...
Magic-Claude-Mem uses SQLite 3 with better-sqlite3 for persistent storage and FTS5 for full-text search.
...
**Primary Implementation**: better-sqlite3
- Used by: SessionStore and SessionSearch
- Format: Synchronous API with better performance
- **Note**: Database.ts (using better-sqlite3) is legacy code
...
**Priority Levels** (v9.9.0+):
- `critical` - Must-know information (architectural decisions, breaking changes, critical bugs)
- `important` - Significant work (features, major refactors, key discoveries)
- `informational` - Routine observations (default)

Context injection orders observations by priority (critical first), then by recency within each priority level.
...
- Migration 025: Add `priority` column to observations (default: `'informational'`)
