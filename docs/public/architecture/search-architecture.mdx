...
**Returns:** Compact index with IDs, titles, dates, types (~50-100 tokens per result), grouped under day headers (`### <date>`) so every served result carries its age. Day groups follow relevance order, not chronological order, so the highest-ranked match stays first even if a lower-ranked match is more recent.
