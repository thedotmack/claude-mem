...
`BM25SearchStrategy` queries the existing FTS5 virtual tables in SQLite. It searches observations and session summaries using the `unicode61` tokenizer with per-column BM25 weighting (title weighted highest, then narrative, facts, concepts).
