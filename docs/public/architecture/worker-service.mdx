...
#### 19. Start Agent (v9.9.0+)
```
POST /api/sessions/:sessionDbId/start-agent
```

**Purpose**: Start the SDK agent for a session. This is the preferred endpoint for session initialization, replacing the legacy `/sessions/:sessionDbId/init`.

**Request Body**:
```json
{
  "userPrompt": "Add login feature",
  "promptNumber": 1
}
```
...
#### 19b. Initialize Session (Deprecated)
```
POST /sessions/:sessionDbId/init
```

<Warning>
**Deprecated** as of v9.9.0. Use `POST /api/sessions/:sessionDbId/start-agent` instead. This endpoint logs a deprecation warning and will be removed in a future release.
</Warning>
