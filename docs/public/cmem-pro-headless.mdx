...
2. **Sign in** — skipped for `--provider claude`, and for non-interactive runs without `--provider` (fresh install defaults to claude; an existing config keeps its provider). A non-interactive install that skipped sign-in prints an optional sign-in link as its last line; agents should show that link to the user rather than open it. Local installs contact cmem.ai once, at signup, to create the sign-in link; nothing else is sent. Otherwise the CLI does **not** ask for an email:
