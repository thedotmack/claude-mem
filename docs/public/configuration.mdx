...
These apply to the Gemini and OpenRouter observer providers. When the observer's conversation history nears the model's context window, claude-mem replaces it with a continuation prompt and a recent-observations timeline to keep token usage bounded.
