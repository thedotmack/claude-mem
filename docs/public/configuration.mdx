...
### External Memory Mirror (pgvector + Valkey)

The worker runtime keeps SQLite as the authoritative local store. You can also enable an external mirror that writes stored observations and summaries to Postgres with the `pgvector` extension and caches recent item IDs in Valkey/Redis.

| Setting | Default | Description |
|---------|---------|-------------|
| `CLAUDE_MEM_EXTERNAL_MEMORY_ENABLED` | `false` | Enables the optional external mirror. SQLite remains authoritative. |
| `CLAUDE_MEM_PGVECTOR_URL` | — | Postgres connection string for the pgvector-backed table. Falls back to `CLAUDE_MEM_SERVER_DATABASE_URL`. |
| `CLAUDE_MEM_PGVECTOR_DIMENSIONS` | `1536` | Vector dimension used when bootstrapping the pgvector column. |
...
When enabled, the worker bootstraps `claude_mem_external_memory_items` with full-text and pgvector indexes, mirrors each successfully stored SQLite batch best-effort, and logs mirror failures without blocking local storage. The built-in mirror stores text rows immediately and leaves the nullable `embedding` column empty unless a caller supplies vectors through the external memory store API.

For local infrastructure, `docker-compose.external-memory.yml` starts pinned pgvector and Valkey services. With the example ports, set:

```sh
CLAUDE_MEM_EXTERNAL_MEMORY_ENABLED=true
CLAUDE_MEM_PGVECTOR_URL=postgres://claude_mem:<password>@127.0.0.1:15432/claude_mem
