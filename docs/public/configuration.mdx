...
| `CLAUDE_MEM_PROVIDER`         | `claude`                        | Runtime AI provider. Keep `claude` for Claude, LiteLLM, Gemini via LiteLLM, OpenRouter via LiteLLM, and Rapid-MLX via LiteLLM. `gemini` / `openrouter` are legacy Classic REST providers. |
...
### Gemini Classic Provider Settings
...
New installs should prefer Gemini via [LiteLLM Gateway](configuration/litellm-gateway), which keeps the Claude Agent SDK path. Use [Gemini Provider](usage/gemini-provider) only for the deprecated Classic REST path.

### OpenRouter Classic Provider Settings
...
New installs should prefer OpenRouter via [LiteLLM Gateway](configuration/litellm-gateway), which keeps the Claude Agent SDK path. Use [OpenRouter Provider](usage/openrouter-provider) only for the deprecated Classic REST path.
