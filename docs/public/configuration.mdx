...
| `CLAUDE_MEM_SEMANTIC_INJECT_MIN_SCORE` | `0.80` | Cosine-similarity floor for vector hits eligible for per-prompt semantic injection (`## Relevant Past Work`). Weak matches are how stale experience gets replayed into prompts, so below-floor hits are dropped. Set `0` to disable the floor. The high default is corpus/model-specific: e5-small's similarity band is compressed (measured on the live corpus: obvious nonsense scores 0.78–0.79, genuine queries 0.84+) |
