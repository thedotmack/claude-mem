...
| `CLAUDE_MEM_PROVIDER`         | `claude`                        | AI provider: `claude`, `gemini`, or `openai-compat` |
...
| Setting                                        | Default                     | Description                           |
|------------------------------------------------|-----------------------------|---------------------------------------|
| `CLAUDE_MEM_OPENAI_COMPAT_API_KEY`             | —                           | API key (Bearer token). For local servers that don't validate keys, use any non-empty string |
| `CLAUDE_MEM_OPENAI_COMPAT_MODEL`               | `xiaomi/mimo-v2-flash:free` | Model identifier (format varies by endpoint) |
| `CLAUDE_MEM_OPENAI_COMPAT_BASE_URL`            | OpenRouter URL              | Full URL to `/v1/chat/completions` endpoint. Set for Ollama, LM Studio, CLIProxyAPI, etc. |
| `CLAUDE_MEM_OPENAI_COMPAT_MAX_CONTEXT_MESSAGES`| `20`                        | Max messages in conversation history (1--100) |
| `CLAUDE_MEM_OPENAI_COMPAT_MAX_TOKENS`          | `100000`                    | Estimated token budget for context (1,000--1,000,000). Lower for small context models |
| `CLAUDE_MEM_OPENAI_COMPAT_SITE_URL`            | —                           | `HTTP-Referer` header for OpenRouter analytics (ignored by other endpoints) |
| `CLAUDE_MEM_OPENAI_COMPAT_APP_NAME`            | `claude-mem`                | `X-Title` header for OpenRouter analytics (ignored by other endpoints) |
