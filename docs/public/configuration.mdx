...
### Observation Trigger Notifications

Use the existing Telegram trigger filters (`CLAUDE_MEM_TELEGRAM_TRIGGER_TYPES` and `CLAUDE_MEM_TELEGRAM_TRIGGER_CONCEPTS`) to control both Telegram pushes and optional Grok Bot brainbeat webhooks.

| Setting | Default | Description |
|---------|---------|-------------|
| `CLAUDE_MEM_TELEGRAM_ENABLED` | `true` | Enables Telegram delivery when bot token + chat ID are configured |
| `CLAUDE_MEM_TELEGRAM_BOT_TOKEN` | — | Telegram Bot API token |
| `CLAUDE_MEM_TELEGRAM_CHAT_ID` | — | Telegram chat ID |
| `CLAUDE_MEM_TELEGRAM_TRIGGER_TYPES` | `security_alert,sensitive` | Comma-separated observation types that trigger notifications |
| `CLAUDE_MEM_TELEGRAM_TRIGGER_CONCEPTS` | — | Comma-separated concepts that trigger notifications |
| `CLAUDE_MEM_GROK_BOT_WEBHOOK_URL` | — | Optional URL for Grok Bot brainbeat webhook POSTs |
| `CLAUDE_MEM_GROK_BOT_WEBHOOK_SECRET` | — | Optional shared secret sent as `x-claude-mem-shared-secret` |

When an observation matches the trigger filters, claude-mem sends a fire-and-forget JSON POST to `CLAUDE_MEM_GROK_BOT_WEBHOOK_URL` (if set):
...
```json
{
  ...
  "subtitle": "Raw token printed in retry path",
  ...
  "timestamp": "2026-09-02T02:56:46.087Z"
}
```

Webhook failures are logged and never block observation ingest.
