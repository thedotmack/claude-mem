...
| `CLAUDE_MEM_USE_GIT_ROOT`     | `false`                         | When `true`, derive project names from the nearest enclosing Git repository root instead of `path.basename(cwd)` |
...
### Git Root Project Names

Set `CLAUDE_MEM_USE_GIT_ROOT=true` if you launch Claude Code from a parent folder that contains multiple independent Git repositories and then work inside those sub-repositories during the session. When enabled, claude-mem walks up from the hook `cwd` to the nearest `.git` file or directory and uses that directory basename as the project name.

This only affects newly stored sessions, observations, and summaries. Existing memories keep their current project labels.
