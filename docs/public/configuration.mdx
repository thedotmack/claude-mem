...
| `CLAUDE_MEM_PROVIDER`         | `claude`                        | AI provider: `claude`, `codex`, `gemini`, or `openrouter` |
...
Run `codex login` as the user running the worker, then install with `npx claude-mem install --provider codex`. Add `--model gpt-6-luna` to choose that model, or select Codex and enter a model in the viewer settings. Codex uses the local CLI subscription login; it does not require an OpenAI API key.

| Setting | Default | Description |
|---------|---------|-------------|
| `CLAUDE_MEM_CODEX_MODEL` | empty | Codex chooses its default model when empty; otherwise use a model available to your subscription. |
| `CLAUDE_MEM_CODEX_REASONING_EFFORT` | `low` | Reasoning effort. Advanced override in `settings.json` or the environment, subject to model support. |
| `CLAUDE_MEM_CODEX_PATH` | `codex` | CLI executable or path. File or environment only; the viewer settings API cannot change it. |
| `CLAUDE_MEM_CODEX_TIMEOUT_MS` | `120000` | Per-request timeout in milliseconds. |

The worker uses an isolated `codex app-server` session for observations and summaries. Codex CLI authentication must be a file-backed ChatGPT login; API-key login is not accepted.
