---
title: "Configuration"
description: "Environment variables and settings for Magic-Claude-Mem"
---
...
|-------------------------------|---------------------------------|---------------------------------------|
| `MAGIC_CLAUDE_MEM_MODEL`            | `haiku`                         | AI model for processing observations (when using Claude) |
| `MAGIC_CLAUDE_MEM_EFFORT`           | _(empty)_                       | SDK effort level: `low`, `medium`, `high`, `max`, or empty (SDK default) |
| `MAGIC_CLAUDE_MEM_PROVIDER`         | `claude`                        | AI provider: `claude`, `gemini`, or `openai-compat` |
| `MAGIC_CLAUDE_MEM_CLAUDE_AUTH_METHOD` | `cli`                         | Claude auth method: `cli` (subscription billing) or `api` (API key) |
| `MAGIC_CLAUDE_MEM_MODE`             | `code`                          | Active mode profile (e.g., `code--es`, `email-investigation`) |
| `MAGIC_CLAUDE_MEM_CONTEXT_OBSERVATIONS` | `50`                        | Number of observations to inject      |
| `MAGIC_CLAUDE_MEM_WORKER_PORT`      | `37777`                         | Worker service port                   |
| `MAGIC_CLAUDE_MEM_WORKER_HOST`      | `127.0.0.1`                     | Worker service host address           |
| `MAGIC_CLAUDE_MEM_SKIP_TOOLS`       | `ListMcpResourcesTool,SlashCommand,Skill,TodoWrite,AskUserQuestion` | Comma-separated tools to exclude from observations |

### Gemini Provider Settings

| Setting                       | Default                         | Description                           |
|-------------------------------|---------------------------------|---------------------------------------|
| `MAGIC_CLAUDE_MEM_GEMINI_API_KEY`   | —                               | Gemini API key ([get free key](https://aistudio.google.com/app/apikey)) |
| `MAGIC_CLAUDE_MEM_GEMINI_MODEL`     | `gemini-2.5-flash-lite`          | Gemini model: `gemini-2.5-flash-lite`, `gemini-2.5-flash`, `gemini-3-flash` |
| `MAGIC_CLAUDE_MEM_GEMINI_RATE_LIMITING_ENABLED` | `true`              | Enable rate limiting for Gemini free tier (10--30 RPM). Disable if you have billing set up |
...
| Setting                                        | Default                     | Description                           |
|------------------------------------------------|-----------------------------|---------------------------------------|
| `MAGIC_CLAUDE_MEM_OPENAI_COMPAT_API_KEY`             | —                           | API key (Bearer token). For local servers that don't validate keys, use any non-empty string |
| `MAGIC_CLAUDE_MEM_OPENAI_COMPAT_MODEL`               | `xiaomi/mimo-v2-flash:free` | Model identifier (format varies by endpoint) |
| `MAGIC_CLAUDE_MEM_OPENAI_COMPAT_BASE_URL`            | OpenRouter URL              | Full URL to `/v1/chat/completions` endpoint. Set for Ollama, LM Studio, CLIProxyAPI, etc. |
| `MAGIC_CLAUDE_MEM_OPENAI_COMPAT_MAX_CONTEXT_MESSAGES`| `20`                        | Max messages in conversation history (1--100) |
| `MAGIC_CLAUDE_MEM_OPENAI_COMPAT_MAX_TOKENS`          | `100000`                    | Estimated token budget for context (1,000--1,000,000). Lower for small context models |
| `MAGIC_CLAUDE_MEM_OPENAI_COMPAT_SITE_URL`            | —                           | `HTTP-Referer` header for OpenRouter analytics (ignored by other endpoints) |
| `MAGIC_CLAUDE_MEM_OPENAI_COMPAT_APP_NAME`            | `magic-claude-mem`                | `X-Title` header for OpenRouter analytics (ignored by other endpoints) |

See [OpenAI-Compatible Provider](usage/openai-compat-provider) for detailed settings reference, local model setup (Ollama, LM Studio, CLIProxyAPI), and usage guide.

### System Configuration

| Setting                       | Default                         | Description                           |
|-------------------------------|---------------------------------|---------------------------------------|
| `MAGIC_CLAUDE_MEM_DATA_DIR`         | `~/.magic-claude-mem`                 | Data directory location               |
| `MAGIC_CLAUDE_MEM_LOG_LEVEL`        | `INFO`                          | Log verbosity (DEBUG, INFO, WARN, ERROR, SILENT) |
...
Control the SDK effort level for observation processing:

```json
{
  "MAGIC_CLAUDE_MEM_EFFORT": "high"
}
```

- **Empty** (default) — Uses SDK default behavior
- `low` — Fastest processing, minimal analysis
- `medium` — Balanced processing
- `high` — Thorough analysis
- `max` — Maximum analysis depth

### Using the Interactive Script

```bash
./magic-claude-mem-settings.sh
```

This script manages settings in `~/.magic-claude-mem/settings.json`.
...
| Setting | Default | Description |
|---------|---------|-------------|
| `MAGIC_CLAUDE_MEM_MODE` | `code` | Defines behavior and language. See [Modes & Languages](modes). |
...
```json
{
  "MAGIC_CLAUDE_MEM_MODE": "email-investigation"
}
```
...
- **Production (installed plugin)**: `~/.magic-claude-mem/` (always, regardless of CLAUDE_PLUGIN_ROOT)
- **Development**: Can be overridden with `MAGIC_CLAUDE_MEM_DATA_DIR`

```
~/.magic-claude-mem/
├── magic-claude-mem.db           # SQLite database
...
Magic-Claude-Mem provides MCP search tools for querying your project history.
...
Worker service is managed by Node.js as a background process. The worker auto-starts on first session and runs continuously in the background.

## Context Injection Configuration

Magic-Claude-Mem injects past observations into each new session, giving Claude awareness of recent work. You can configure exactly what gets injected using the **Context Settings Modal**.
...
| **Savings %** | true | Show percentage of tokens saved |
...
| **Include last summary** | true | Add previous session's summary to context |
...
```json
{
  "MAGIC_CLAUDE_MEM_CONTEXT_OBSERVATIONS": "100",
  "MAGIC_CLAUDE_MEM_CONTEXT_SESSION_COUNT": "20",
  "MAGIC_CLAUDE_MEM_CONTEXT_OBSERVATION_TYPES": "bugfix,decision,discovery",
  "MAGIC_CLAUDE_MEM_CONTEXT_OBSERVATION_CONCEPTS": "how-it-works,gotcha",
  "MAGIC_CLAUDE_MEM_CONTEXT_FULL_COUNT": "10",
...
Settings can be customized in `~/.magic-claude-mem/settings.json`.
...
```json
{
  "MAGIC_CLAUDE_MEM_DATA_DIR": "/custom/path"
}
```
...
```json
{
  "MAGIC_CLAUDE_MEM_WORKER_PORT": "38000"
}
```
...
<Note>
**Automatic migration**: If you previously used `claude-sonnet-4-5` as the default, it will be automatically migrated to `claude-haiku-4-5` on next settings load. This migration only applies to the exact `claude-sonnet-4-5` value — custom model choices are preserved.
</Note>

### Custom Skip Tools

Control which tools are excluded from observations. Edit `~/.magic-claude-mem/settings.json`:
```json
{
  "MAGIC_CLAUDE_MEM_SKIP_TOOLS": "ListMcpResourcesTool,SlashCommand,Skill"
}
```
...
The worker service is managed by Node.js and will automatically restart if it encounters issues. Memory usage is typically low (~100-200MB).
...
2. Verify environment variables:
   ```bash
   echo $MAGIC_CLAUDE_MEM_MODEL
   echo $MAGIC_CLAUDE_MEM_WORKER_PORT
   ```
...
1. Set custom port:
   ```bash
   export MAGIC_CLAUDE_MEM_WORKER_PORT=38000
   ```
...
3. Verify new port:
   ```bash
   cat ~/.magic-claude-mem/worker.port
   ```
