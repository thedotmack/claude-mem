...
| `CLAUDE_MEM_PROVIDER`         | `claude`                        | AI provider: `claude`, `gemini`, `openrouter`, or `aimlapi` |
...
### aimlapi.com Provider Settings

| Setting                                      | Default                     | Description                           |
|----------------------------------------------|-----------------------------|---------------------------------------|
| `CLAUDE_MEM_AIMLAPI_API_KEY`                 | —                           | aimlapi.com API key ([get key](https://aimlapi.com/app/keys)) |
| `CLAUDE_MEM_AIMLAPI_MODEL`                   | `openai/gpt-5.6-terra`      | Model identifier (any aimlapi.com chat model) |
| `CLAUDE_MEM_AIMLAPI_BASE_URL`                | —                           | Optional: OpenAI-compatible base URL override; empty = `https://api.aimlapi.com/v1` |
| `CLAUDE_MEM_AIMLAPI_SITE_URL`                | —                           | Optional: URL sent as `HTTP-Referer` |
| `CLAUDE_MEM_AIMLAPI_APP_NAME`                | `Claude-Mem`                | Optional: App name sent as `X-Title` |

Select it with `npx claude-mem install --provider aimlapi` (or pick **aimlapi.com** in the interactive installer). The `AIMLAPI_API_KEY` environment variable is honored as a fallback for the key.
