...
| `CLAUDE_MEM_HEADROOM_ENABLED` | `false`                         | Opt-in: compress MCP search payloads via the [Headroom](headroom) proxy |
| `CLAUDE_MEM_HEADROOM_URL`     | `http://127.0.0.1:8787`         | Headroom proxy base URL               |
...
| **Token Budget** | 0 | 0-200000 | Hard cap on estimated tokens injected at session start (`0` = unlimited) |
...
- With a **Token Budget** set, observations are kept newest-first until the budget is reached and the remainder is trimmed; the `Stats:` line reports the trim. Passing `full=true` bypasses the budget.
...
Optional delivery-time compression of the heavy MCP search payloads (`search`, `timeline`, `get_observations`) through the [Headroom](headroom) proxy:

| Setting | Default | Description |
|---------|---------|-------------|
| **Enable Headroom** | false | Route search/timeline/get_observations payloads through the compression proxy |
| **Proxy URL** | `http://127.0.0.1:8787` | Base URL of the Headroom proxy (an empty value falls back to this default) |

Compression is opt-in and fail-safe: when the proxy is missing or unreachable, payloads pass through uncompressed. See [Headroom Compression](headroom) for setup and details.
...
```json
{
  ...
  "CLAUDE_MEM_CONTEXT_TOKEN_BUDGET": "4000",
  ...
  "CLAUDE_MEM_HEADROOM_ENABLED": "false",
