...
| `CLAUDE_MEM_WORKER_AUTOSTART` | `true`                          | When `false`, hooks do not lazy-spawn the worker daemon. Use for server-only or externally managed deployments; worker-only features are unavailable while off. |
...
| `CLAUDE_MEM_ALLOW_DISMISS`    | `false`                         | When `true`, enables observation dismiss/undismiss writes. Dismissed observations are hidden from proactive surfacing but remain fetchable by id. |
| `CLAUDE_MEM_SKIP_SUBAGENT_OBSERVATIONS` | `false`               | When `true`, skip every observation carrying an `agentId`. Main-session observations are unaffected. |
| `CLAUDE_MEM_SKIP_AGENT_TYPES` | _(empty)_                       | Comma-separated `agent_type` values to skip, e.g. `workflow-subagent,Explore`. |

### Subagent & Dynamic Workflow Filtering

Claude Code Dynamic Workflows can fan one prompt out through many subagents, commonly with `agent_type: workflow-subagent`. Each subagent tool call can emit a `PostToolUse` observation, which may create high-volume, low-signal provider work.

These settings filter before the hook sends a worker or server request, and the worker also applies the same filter for direct compatibility callers. Both defaults are off to preserve existing observation behavior.
...
Use this to skip every subagent observation identified by `agentId`.
...
Use this to skip only specific `agent_type` values. If both settings are set, an observation is skipped when either setting matches.
