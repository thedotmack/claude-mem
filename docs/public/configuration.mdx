...
| `CLAUDE_MEM_SKIP_BASH_PATTERNS` | —                             | Regular expression matched against each `Bash` command; on a match the observation is skipped. Empty captures every command. Use alternation for several patterns, e.g. `^(ls\|cat\|pwd\|git status)\b`. An invalid regex is ignored, so capture is never dropped |
