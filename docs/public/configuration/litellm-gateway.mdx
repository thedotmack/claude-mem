...
Use **OpenRouter Classic (deprecated)** or **Gemini Classic (deprecated)** only if you explicitly need claude-mem's old direct REST providers and do not need the Claude Agent SDK path.
...
For OpenAI-compatible local upstreams such as Rapid-MLX, Apple Foundation Models proxies, or LM Studio, start LiteLLM with Anthropic messages routed through chat completions:
...
Without that setting, current LiteLLM versions may translate Anthropic `/v1/messages` traffic to OpenAI `/v1/responses`. Many local OpenAI-compatible servers only expose `/v1/chat/completions`.
...
For OpenRouter, Gemini, and Rapid-MLX, the installer presents friendly backend names but still saves the same runtime shape:
...
The unqualified `Gemini` and `OpenRouter` installer choices mean "via LiteLLM gateway". The old direct REST implementations are still available as `Gemini Classic (deprecated)` and `OpenRouter Classic (deprecated)`.
...
Rapid-MLX exposes an OpenAI-compatible server on `http://localhost:8000/v1` with model `default`. Put LiteLLM in front of that endpoint and expose an Anthropic-facing alias for claude-mem:

```yaml
model_list:
  - model_name: claude-mem-rapidmlx
    litellm_params:
...
Then set `CLAUDE_MEM_MODEL=claude-mem-rapidmlx` and keep `CLAUDE_MEM_PROVIDER=claude`.
