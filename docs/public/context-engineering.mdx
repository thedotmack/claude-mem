...
**Priority-Based Ordering** (v9.9.0+): magic-claude-mem applies priority ordering to context injection. Critical observations (architectural decisions, breaking changes) are surfaced first, followed by important observations, then informational. This ensures the highest-signal tokens appear earliest in context, where model attention is strongest.
