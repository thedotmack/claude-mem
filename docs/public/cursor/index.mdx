...
<Info>
**No Claude Code subscription required.** Use Gemini (free tier) or an OpenAI-compatible endpoint as your AI provider.
</Info>
...
- [OpenAI-Compatible Setup](/cursor/openai-compat-setup) - 100+ models including free options
...
The plugin uses Claude's SDK by default but you can switch to Gemini or an OpenAI-compatible endpoint anytime.
...
After installation, these commands are available from the magic-claude-mem directory:
...
| OpenAI-Compatible | Pay-per-use + free models | Varies by model | Model variety, custom endpoints |
| Claude SDK | Included with Claude Code | Unlimited | Claude Code subscribers |

<Tip>
**Recommendation:** Start with Gemini's free tier. It handles typical individual usage well. Switch to an OpenAI-compatible endpoint or Claude SDK if you need higher limits.
</Tip>
...
- [OpenAI-Compatible Setup Guide](/cursor/openai-compat-setup) - Configure OpenAI-compatible endpoint
