---
title: "Development"
description: "Build from source, run tests, and contribute to Magic-Claude-Mem"
---
...
npm run test:agents       # SDK agent processing
npm run test:search       # Search functionality
npm run test:context      # Context injection
npm run test:infra        # Infrastructure utilities
npm run test:server       # HTTP server endpoints

# SDK integration tests (requires API key)
npm run test:sdk
```
...
│   ├── FilterBar.tsx      # Collapsible filter panel with chips
│   ├── FilterChip.tsx     # Reusable multi-select filter chip
│   ├── ActivityBar.tsx    # 90-day density timeline
│   ├── ThemeToggle.tsx    # Light/dark/system theme switcher
│   ├── AnalyticsBar.tsx   # Token analytics dashboard (Read/Work/Recalled/Saved)
...
│   ├── useActivityDensity.ts # 90-day activity data
│   └── useAnalytics.ts    # Token analytics API integration
├── utils/                 # Utilities
...
2. Build: `npm run build`
   ...
   ```bash
   npm run build-and-sync   # Build + sync to plugin + restart worker
   ```
...
Claude-mem uses **Vitest** for unit and integration testing alongside manual verification:

1. **Unit tests** - Core logic, utilities, settings, database operations (~1700 tests)
2. **Integration tests** - SDK agent behavior, search, context generation
3. **Manual verification** - Testing features in actual Claude Code sessions
4. **Database inspection** - Verifying data correctness via SQLite queries
5. **CLI tools** - Interactive tools for checking system state
...
3. **Check database state**:
   ```bash
   sqlite3 ~/.magic-claude-mem/magic-claude-mem.db "SELECT * FROM your_table;"
   ```
