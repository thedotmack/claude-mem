...
<Info>
Grok Bot is **not** Grok Build CLI. Plugin id: **`claude-mem-grok-bot`**. Cursor's listing is **`claude-mem-cursor`**. Neither requires the other IDE.
</Info>
...
```bash
# Grok Bot + Cursor on the same machine (not required)
...
Grok Bot's plugin store **is** the Cursor catalog. Install **`claude-mem-grok-bot`** from there (listing is submitted after this release at [cursor.com/marketplace/publish](https://cursor.com/marketplace/publish)). The listing does not install Cursor.
...
Install both IDEs if you want both; it is optional:

```bash
npx claude-mem install --ide grok-bot --ide cursor
```
