# How Magic-Claude-Mem Uses Hooks: A Lifecycle-Driven Architecture
...
Magic-Claude-Mem is fundamentally a **hook-driven system**. Every piece of functionality happens in response to lifecycle events:
...
**Key insight:** Magic-Claude-Mem doesn't interrupt or modify Claude Code's behavior. It observes from the outside and provides value through lifecycle hooks.
...
Magic-Claude-Mem had several architectural constraints:
...
Magic-Claude-Mem uses lifecycle hook scripts across 5 lifecycle events. SessionStart runs 3 hooks in sequence: smart-install, worker-service start, and context-hook.
...
4. Starts Node.js worker service
...
- ✅ Configurable observation count via `MAGIC_CLAUDE_MEM_CONTEXT_OBSERVATIONS`

**Output format:**
```markdown
# [magic-claude-mem] recent context
...
4. Starts Node.js worker service if not running
...
**Technology:** Node.js (JavaScript runtime and process manager)

**Why Node.js:**
- Auto-restart on failure
- Broad platform compatibility (macOS, Linux, Windows including WSL2)
- Native better-sqlite3 support without Bun-specific modules
...
- Worker crashed → Auto-restart via Node.js
...
<AccordionGroup>
  ...
  <Accordion title="Observations not captured">
    ...
    **Debugging:**
    1. Check database: `sqlite3 ~/.magic-claude-mem/magic-claude-mem.db "SELECT * FROM observation_queue"`
...
**Privacy guarantees:**
- All data stored locally in `~/.magic-claude-mem/magic-claude-mem.db`
...
*The hook-driven architecture enables Magic-Claude-Mem to be both powerful and invisible. Users never notice the memory system working - it just makes Claude smarter over time.*
