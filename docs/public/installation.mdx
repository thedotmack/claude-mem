...
- **Host observer (Grok Bot / Cursor)** — `--provider host` uses the already-logged-in agent over a local loopback. No API key. See [Grok Bot Integration](/grok-bot).
...
Grok Bot has no host hooks. Install it independently of Cursor or Claude Code:

```bash
npx claude-mem install --ide grok-bot --provider host
```

See the [Grok Bot Integration](/grok-bot) guide for observer XML, ports, MCP, and verification. This host ships in claude-mem **13.24** ([PR #3842](https://github.com/thedotmack/claude-mem/pull/3842)).
...
- [Cursor Integration](/cursor) - Persistent memory for Cursor IDE
