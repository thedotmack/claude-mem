---
title: "Installation"
description: "Install Magic-Claude-Mem plugin for persistent memory across sessions"
---
...
Install Magic-Claude-Mem directly from the plugin marketplace:
...
- **uv**: Python package manager (auto-installed if missing, required for Chroma vector search)
...
Data is stored in `~/.magic-claude-mem/`:
- Database: `~/.magic-claude-mem/magic-claude-mem.db`
- PID file: `~/.magic-claude-mem/.worker.pid`
- Port file: `~/.magic-claude-mem/.worker.port`
- Logs: `~/.magic-claude-mem/logs/worker-YYYY-MM-DD.log`
- Settings: `~/.magic-claude-mem/settings.json`

Override with environment variable:
```bash
export MAGIC_CLAUDE_MEM_DATA_DIR=/custom/path
```
...
**v9.1.0**: Viewer UI gains search, filtering, and activity timeline. Session completion lifecycle tracking added.

**v7.1.0**: PM2 replaced with native Node.js process management. Migration is automatic on first hook trigger.
...
See [CHANGELOG](https://github.com/doublefx/magic-claude-mem/blob/main/CHANGELOG.md) for complete version history.

## Next Steps

- [Getting Started Guide](usage/getting-started) - Learn how Magic-Claude-Mem works automatically
- [MCP Search Tools](usage/search-tools) - Query your project history
- [Configuration](configuration) - Customize Magic-Claude-Mem behavior
