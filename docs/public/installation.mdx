...
### Cursor and Grok Bot installs

- **Cursor only** — `npx claude-mem install --ide cursor`
- **Grok Bot only** — `npx claude-mem install --ide grok-bot`
- **Both** — select both in the interactive installer, or run the installer twice with each `--ide` flag
- **Local worker** — default; runs on your machine
- **Remote worker** — `npx claude-mem install --runtime server --server-url <url>`
...
- **Host observer (`--provider host`)** — uses a local OpenAI-compatible loopback shim backed by the already logged-in host agent, so no Claude CLI or xAI key is required. Cursor and Grok Bot can each point at their own host observer model.
- **Your OpenRouter key** — memory runs off-plan on your OpenRouter credit or another OpenAI-compatible endpoint such as `https://cmem.ai/api/inference/v1`.
...
- **Claude Code** or another supported IDE (Cursor, Grok Bot, Windsurf, OpenCode, Codex CLI, Antigravity CLI, OpenClaw)
...
After this PR lands, submit the repository to the Cursor marketplace at https://cursor.com/marketplace/publish. Grok Bot uses the same catalog, so the separate `claude-mem-cursor` and `claude-mem-grok-bot` listings can be published independently from the same repo.
