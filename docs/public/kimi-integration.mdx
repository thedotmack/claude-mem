...
<Steps>
  ...
  <Step title="User prompt submitted (UserPromptSubmit)">
    The hook sends the user prompt to `POST /api/sessions/init` so the worker can create or continue the session, then appends the injected observation timeline to Kimi's context via stdout. This is a blockable Kimi event; claude-mem always exits 0 so the prompt is never blocked.
  </Step>
