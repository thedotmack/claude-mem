...
Claude-mem needs an AI provider to compress observations. Kiro's own subscription cannot power this (its managed backend exposes no inference API), so choose one:

- **Claude Code installed on the same machine?** The default Claude provider works unchanged — nothing to configure.
- **Kiro-only machine:** use Gemini (free tier) or OpenRouter / any OpenAI-compatible endpoint:
...
See [Cursor's provider guides](/cursor/gemini-setup) — the provider setup is identical for Kiro.
