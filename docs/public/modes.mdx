...
description: "Configure Magic-Claude-Mem behavior and language with the Mode System"
---

# Modes & Languages

Magic-Claude-Mem uses a flexible **Mode System** to adapt its behavior, observation types, and output language. This allows you to switch between different workflows (like coding vs. email investigation) or languages without reinstalling the plugin.
...
Set the active mode using the `MAGIC_CLAUDE_MEM_MODE` setting in `~/.magic-claude-mem/settings.json`:
...
```bash
export MAGIC_CLAUDE_MEM_MODE="code--fr"
```
