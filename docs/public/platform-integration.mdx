...
Worker Service (port 37777)
    ↓
SQLite + Chroma Search
```

### Setup

<Steps>
  <Step title="Initialize transcript watching">
    ```bash
    ...
    ```
    Creates `~/.claude-mem/transcript-watch.json` with Codex schema
  </Step>
  <Step title="Start worker">
    ...
    ```
    Transcript watcher starts automatically if config exists
  </Step>
  <Step title="Verify watching">
    Check worker logs for transcript watcher startup:
    ```
    [TRANSCRIPT] Transcript watcher started
    ```
  </Step>
</Steps>
...
The transcript watcher monitors `~/.codex/sessions/**/*.jsonl` and captures:

<CardGroup cols={2}>
  <Card title="Session Metadata" icon="info-circle">
    Session ID, working directory, project context
  </Card>
  ...
  <Card title="Tool Results" icon="check-circle">
    Tool outputs and responses
  </Card>
  <Card title="Session End" icon="flag-checkered">
    Turn completion and session termination
  </Card>
</CardGroup>

### Configuration

The transcript watch config at `~/.claude-mem/transcript-watch.json`:
...
```json
{
  ...
  "schemas": {
    "codex": {
      "name": "codex",
      "version": "0.2",
      "description": "Schema for Codex session JSONL files",
      "events": [
        {
          "name": "session-meta",
        ...
        },
        {
          ...
          "match": {
            "path": "payload.type",
            "in": ["function_call", "custom_tool_call", "web_search_call"]
          },
          "action": "tool_use",
          "fields": {
            "toolId": "payload.call_id",
            "toolName": {
              "coalesce": ["payload.name", { "value": "web_search" }]
            },
            "toolInput": {
              "coalesce": ["payload.arguments", "payload.input", "payload.action"]
            }
          }
        }
      ]
...
<Tabs>
  <Tab title="Initialize">
    ```bash
    bun run src/services/worker-service.ts transcript init [--config <path>]
    ```
    Creates sample config at specified path (default: `~/.claude-mem/transcript-watch.json`)
  </Tab>
  <Tab title="Validate">
    ```bash
    bun run src/services/worker-service.ts transcript validate [--config <path>]
    ```
    Validates config structure and schema definitions
  </Tab>
  <Tab title="Watch (Standalone)">
    ```bash
    bun run src/services/worker-service.ts transcript watch [--config <path>]
    ```
    Runs watcher in foreground (for testing/debugging only)
  </Tab>
</Tabs>
...
The worker manages the transcript watcher lifecycle:

**Startup:**
1. Worker checks for `~/.claude-mem/transcript-watch.json`
2. If config exists, loads schema and watch targets
3. Starts file watcher for `~/.codex/sessions/**/*.jsonl`
4. Logs: `[TRANSCRIPT] Transcript watcher started`

**Shutdown:**
1. Worker receives SIGTERM/SIGINT
2. Stops transcript watcher gracefully
3. Saves watcher state to `~/.claude-mem/transcript-watch-state.json`
4. Logs: `[TRANSCRIPT] Transcript watcher stopped`

### Event Processing

Codex transcript events are mapped to claude-mem actions:

| Codex Event | Action | Description |
|-------------|--------|-------------|
| `session_meta` | `session_context` | Captures session ID and working directory |
| `turn_context` | `session_context` | Updates working directory for turn |
| `user_message` | `session_init` | Initializes session with user prompt |
| `agent_message` | `assistant_message` | Captures assistant response |
| `function_call` | `tool_use` | Records tool invocation |
| `function_call_output` | `tool_result` | Records tool output |
| `turn_aborted`, `task_complete` | `session_end` | Marks session completion |

### Field Extraction

The schema uses **coalesce** for flexible field extraction:

```typescript
// Tool name with fallback
toolName: {
  coalesce: [
    "payload.name",           // Try payload.name first
    { value: "web_search" }   // Fallback to "web_search"
  ]
}

// Tool input from multiple sources
toolInput: {
  coalesce: [
    "payload.arguments",  // Try arguments first
    "payload.input",      // Then input
    "payload.action"      // Finally action
  ]
}
```
...
<AccordionGroup>
  <Accordion title="Watcher not starting">
    **Check config exists:**
    ```bash
    ls -la ~/.claude-mem/transcript-watch.json
    ```
    ...
    **Check worker logs:**
    ```bash
    tail -f ~/.claude-mem/logs/worker.log | grep TRANSCRIPT
    ```
  </Accordion>

  <Accordion title="Events not being captured">
    **Verify Codex session files exist:**
    ```bash
    ...
    **Check watcher state:**
    ```bash
    cat ~/.claude-mem/transcript-watch-state.json
    ```

    **Test with standalone watcher:**
    ```bash
  ...
  <Accordion title="Schema mismatch errors">
    ...
    **Check Codex transcript format:**
    ```bash
    head -5 ~/.codex/sessions/latest.jsonl
    ```
  </Accordion>
</AccordionGroup>
...
sqlite3 ~/.claude-mem/claude-mem.db "SELECT COUNT(*) FROM observations WHERE project LIKE '%codex%';"
```
