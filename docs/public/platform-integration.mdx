...
// Start agent for a session (v9.9.0+, preferred over legacy init)
...
<AccordionGroup>
  ...
  <Accordion title="SessionRoutes" icon="timeline">
    - Session lifecycle (start-agent, observations, summarize, complete)
    - Start agent endpoint (`POST /api/sessions/:id/start-agent`) — preferred over legacy init
...
<Tabs>
  <Tab title="Initialize Session (Deprecated)">
    ```http
    POST /sessions/:sessionDbId/init
    Body: { userPrompt, promptNumber }
    ```
    Use `POST /api/sessions/:sessionDbId/start-agent` instead (v9.9.0+).
  </Tab>
...
<Tabs>
  ...
  <Tab title="Observation">
    ...
    interface Observation {
      ...
      priority: string;               // critical, important, informational (default)
      project: string;
      created_at: string;
...
- `contentSessionId` - String identifier from Claude platform (used by `/api/sessions/observations`, `/api/sessions/summarize`, `/api/sessions/complete`)
- `sessionDbId` is also used by `POST /api/sessions/:sessionDbId/start-agent` (v9.9.0+)
