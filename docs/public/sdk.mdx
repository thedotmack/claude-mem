...
- **Parse error.** If the generation provider returns text that
  `parseAgentXml` cannot understand, `client.generate(...)` rejects with
  a `"generation parse error"` message. The job row stays in
  `processing` so an operator can inspect the raw provider output.
