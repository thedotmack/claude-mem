...
| `chroma_available` | `true` | Whether the vector-search backend was reachable for a search — reachability only; zero-result fallbacks report `true` here, so count fallbacks via `fallback_reason` |
| `fallback_reason` | `none` | Why a search fell back from vector search: none / chroma_connection / chroma_error / chroma_not_initialized / chroma_zero_results — a closed enum, never an error message |
