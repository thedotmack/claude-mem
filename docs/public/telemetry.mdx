...
| `phase` | `login` | Installer OAuth pairing phase — a closed enum: login / enrollment / deferred |
| `provider_source` | `default` | How the installer decided the provider — a closed enum: flag (`--provider` given) / default (non-interactive fresh install fell back to claude) / persisted (non-interactive run reused the configured provider) / prompt (interactive choice) |
...
|---|---|---|
| `install_completed` | `npx claude-mem install` finishes | `ide`, `provider`, `provider_source`, `runtime_mode`, `is_update`, `outcome`, `duration_ms`, `interactive`, `install_method`, `bun_version`, `uv_version`, `claude_code_version` |
| `installer_oauth_started` | The installer opened the browser sign-in link | `version` — never the link, pairing id, secret, or device code |
| `installer_oauth_completed` | Browser sign-in finished | `version` |
| `installer_oauth_timeout` | Sign-in polling reached the pairing's expiry (the server's `expires_in`, capped at 30 minutes; 4 minutes when the server reports none) | `version`, `phase` (login / enrollment), `duration_ms` |
| `installer_oauth_start_failed` | `POST /api/installer/oauth/start` did not return a usable pairing | `version`, `outcome` (http_error / network / timeout / bad_body), `interactive`, `phase` (login / deferred) |
| `installer_oauth_deferred` | A non-interactive install that skipped sign-in printed the optional sign-in link at the end | `version`, `interactive` (always `false`) |
