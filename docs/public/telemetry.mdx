...
| `consecutive_invalid_outputs` | `0` | How many unusable observer outputs have occurred in a row without a successful parse; resets to zero on a valid parse or after the third consecutive drift |
| `respawn_triggered` | `false` | Legacy recovery flag for old invalid-output restarts |
| `abort_reason` | `idle` | Why a compression session was aborted: idle / shutdown / overflow / restart_guard / quota / drift / none — a closed enum |
