...
4. Check query syntax:
   ```bash
   # Phrase and punctuation-heavy queries now get tokenized before FTS5 matching,
   # but a simpler query is still the fastest way to rule out data issues.
   search(query="[test]")
   search(query="test")
   ```
...
**Solution**: Retry with a simpler query to isolate whether the problem is query parsing or missing data. If the simple form works and the original still fails, capture both examples when filing the bug.
