...
description: "Common issues and solutions for Magic-Claude-Mem"
---
...
1. Check browser localStorage is enabled:
   ```javascript
   // In browser console
   localStorage.getItem('magic-claude-mem-settings')
   ```
...
3. Clear localStorage and try again:
   ```javascript
   // In browser console
   localStorage.removeItem('magic-claude-mem-settings')
   ```
...
1. Clear install cache:
   ```bash
   rm ~/.claude/plugins/marketplaces/doublefx/.install-version
   ```
...
3. Check version marker:
   ```bash
   cat ~/.claude/plugins/marketplaces/doublefx/.install-version
   cat ~/.claude/plugins/marketplaces/doublefx/package.json | grep version
   ```
...
3. Check database for corruption:
   ```bash
   sqlite3 ~/.magic-claude-mem/magic-claude-mem.db "PRAGMA integrity_check;"
   ```
...
3. Verify database has observations:
   ```bash
   sqlite3 ~/.magic-claude-mem/magic-claude-mem.db "SELECT COUNT(*) FROM observations;"
   ```
...
npx tsx scripts/check-pending-queue.ts --process

# Process up to 5 sessions
npx tsx scripts/check-pending-queue.ts --process --limit 5
```
...
3. **View stuck messages directly**:
   ```bash
   sqlite3 ~/.magic-claude-mem/magic-claude-mem.db "
...
1. Check if summaries exist:
   ```bash
   sqlite3 ~/.magic-claude-mem/magic-claude-mem.db "SELECT COUNT(*) FROM session_summaries;"
   ```
...
3. Check database size (large database = slow queries):
   ```bash
   ls -lh ~/.magic-claude-mem/magic-claude-mem.db
   ```

4. Optimize database:
   ```bash
   sqlite3 ~/.magic-claude-mem/magic-claude-mem.db "VACUUM;"
   ```
...
3. Check package.json exists:
   ```bash
   ls -la ~/.claude/plugins/marketplaces/doublefx/package.json
   ```
...
2. Check for stale locks:
   ```bash
   lsof ~/.magic-claude-mem/magic-claude-mem.db
   ```
...
2. Backup database:
   ```bash
   cp ~/.magic-claude-mem/magic-claude-mem.db ~/.magic-claude-mem/magic-claude-mem.db.backup
   ```
...
4. Nuclear option - recreate database:
   ```bash
   rm ~/.magic-claude-mem/magic-claude-mem.db
...
1. Check FTS5 tables exist:
   ```bash
   sqlite3 ~/.magic-claude-mem/magic-claude-mem.db "SELECT name FROM sqlite_master WHERE type='table' AND name LIKE '%_fts';"
   ```
...
3. Check triggers exist:
   ```bash
   sqlite3 ~/.magic-claude-mem/magic-claude-mem.db "SELECT name FROM sqlite_master WHERE type='trigger';"
   ```
...
2. Verify FTS5 tables populated:
   ```bash
   sqlite3 ~/.magic-claude-mem/magic-claude-mem.db "SELECT COUNT(*) FROM observations_fts;"
   ```
...
**Symptoms**: `/plugin install magic-claude-mem` fails.
...
3. Verify installation:
   ```bash
   ls -la ~/.claude/plugins/marketplaces/doublefx/
   ```
...
**Solution**: Check `~/.magic-claude-mem/` exists and is writable.
...
2. **Create issue**: [GitHub Issues](https://github.com/doublefx/magic-claude-mem/issues)
...
- [Configuration](configuration) - Customize Magic-Claude-Mem
