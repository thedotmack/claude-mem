...
description: Use magic-claude-mem memory search in Claude Desktop with MCP tools
icon: desktop
---
...
Claude Desktop can access your magic-claude-mem memory database through **MCP tools**. This allows you to search past sessions, decisions, and observations directly from Claude Desktop conversations.

## Prerequisites

Before configuring MCP tools, ensure:

1. **magic-claude-mem is installed** and the worker service is running
    ...
    ```json
    {
      "mcpServers": {
        "mcp-search": {
          "command": "node",
          "args": [
            "C:\\Users\\YOUR_USERNAME\\.claude\\plugins\\marketplaces\\doublefx\\plugin\\scripts\\mcp-server.cjs"
          ]
        }
      }
    }
...
1. Ensure magic-claude-mem has recorded sessions (check http://localhost:37777)
2. Verify the database exists: `ls ~/.magic-claude-mem/magic-claude-mem.db`
