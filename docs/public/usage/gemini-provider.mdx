...
`CLAUDE_MEM_GEMINI_API_KEYS` accepts additional keys (newline- or comma-separated) alongside
`CLAUDE_MEM_GEMINI_API_KEY`. When a key hits a rate limit, spends its quota, or is rejected as
invalid, Gemini requests rotate to the next key instead of failing for the
