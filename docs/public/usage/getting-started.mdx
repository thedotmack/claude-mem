...
description: "Learn how Magic-Claude-Mem works automatically in the background"
---

# Getting Started with Magic-Claude-Mem

## Automatic Operation

Magic-Claude-Mem works automatically once installed. No manual intervention required!
...
Every time Claude uses a tool, magic-claude-mem captures it:
...
Context is stored in SQLite database at `~/.magic-claude-mem/magic-claude-mem.db`.
...
Magic-Claude-Mem supports sessions that span multiple user prompts:
...
Magic-Claude-Mem provides MCP tools for querying your project history. Simply ask naturally:
