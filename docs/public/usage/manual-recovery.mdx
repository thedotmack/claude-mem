...
### Session vs Message States

Note that sessions and messages have separate lifecycle states:

- **Session status** (`active` → `completed` → `active`): Tracks whether the session has received its summary. Sessions transition to `completed` when the summary is stored, and back to `active` if a new prompt arrives in the same Claude session.
- **Message status** (`pending` → `processing` → `processed`/`failed`): Tracks individual observation/summary processing in the queue.

Manual recovery deals with **message** states, not session states. A session can be `completed` while still having `pending` messages from a late-arriving observation.
...
2. **Reduce session limit**:
   ```bash
   npx tsx scripts/check-pending-queue.ts --process --limit 3
   ```
...
npx tsx scripts/check-pending-queue.ts --process --limit 20

echo "Waiting for processing to complete..."
sleep 10

echo "Stopping worker..."
magic-claude-mem stop
```
...
**Migration Steps**:
1. Upgrade to v5.x
2. Check for stuck messages: `npx tsx scripts/check-pending-queue.ts`
3. Process if needed: `npx tsx scripts/check-pending-queue.ts --process`
