...
The default API endpoint is `https://openrouter.ai/api/v1/chat/completions` (OpenRouter). Override this with `CLAUDE_MEM_OPENAI_COMPAT_BASE_URL` to use a different endpoint.
...
Any server that exposes an OpenAI-compatible `/v1/chat/completions` endpoint works. Set `CLAUDE_MEM_OPENAI_COMPAT_BASE_URL` to point at your local server.
...
{
  "CLAUDE_MEM_PROVIDER": "openai-compat",
  "CLAUDE_MEM_OPENAI_COMPAT_API_KEY": "ollama",
  "CLAUDE_MEM_OPENAI_COMPAT_BASE_URL": "http://localhost:11434/v1/chat/completions",
  "CLAUDE_MEM_OPENAI_COMPAT_MODEL": "llama3.1:8b"
}
```
...
{
  "CLAUDE_MEM_PROVIDER": "openai-compat",
  "CLAUDE_MEM_OPENAI_COMPAT_API_KEY": "lm-studio",
  "CLAUDE_MEM_OPENAI_COMPAT_BASE_URL": "http://localhost:1234/v1/chat/completions",
  "CLAUDE_MEM_OPENAI_COMPAT_MODEL": "your-loaded-model-name"
}
```

### CLIProxyAPI (Claude via subscription)

If you have a Claude Code Max subscription, you can use [CLIProxyAPI](https://gist.github.com/chandika/c4b64c5b8f5e29f6112021d46c159fdd) to route claude-mem through your subscription. CLIProxyAPI is an open-source proxy that converts API key requests into OAuth-authenticated calls to Anthropic's API, handling token refresh automatically.

```json
{
  "CLAUDE_MEM_PROVIDER": "openai-compat",
  "CLAUDE_MEM_OPENAI_COMPAT_API_KEY": "your-proxy-key",
  "CLAUDE_MEM_OPENAI_COMPAT_BASE_URL": "http://localhost:8317/v1/chat/completions",
  "CLAUDE_MEM_OPENAI_COMPAT_MODEL": "claude-sonnet-4-5-20250514"
}
```
...
|---------|---------|-------------|
| `CLAUDE_MEM_PROVIDER` | `claude` | Set to `openai-compat` to use this provider |
| `CLAUDE_MEM_OPENAI_COMPAT_API_KEY` | -- | API key for authentication |
| `CLAUDE_MEM_OPENAI_COMPAT_MODEL` | `xiaomi/mimo-v2-flash:free` | Model identifier to use |
| `CLAUDE_MEM_OPENAI_COMPAT_BASE_URL` | OpenRouter URL | API endpoint URL |
| `CLAUDE_MEM_OPENAI_COMPAT_MAX_CONTEXT_MESSAGES` | `20` | Conversation history limit |
| `CLAUDE_MEM_OPENAI_COMPAT_MAX_TOKENS` | `100000` | Token budget safety limit |
| `CLAUDE_MEM_OPENAI_COMPAT_SITE_URL` | -- | Analytics attribution URL |
| `CLAUDE_MEM_OPENAI_COMPAT_APP_NAME` | `claude-mem` | Analytics app name |
...
Can also be set via the `OPENAI_COMPAT_API_KEY` environment variable in `~/.claude-mem/.env`. The settings file value takes precedence.
...
Sent as the `X-Title` header. Used by OpenRouter to display your app name in their dashboard. Defaults to `claude-mem`. Ignored by most other endpoints.
...
{
  "CLAUDE_MEM_PROVIDER": "openai-compat",
  "CLAUDE_MEM_OPENAI_COMPAT_API_KEY": "sk-or-v1-your-key-here",
...
If you previously used `CLAUDE_MEM_OPENROUTER_*` settings, they are automatically migrated to `CLAUDE_MEM_OPENAI_COMPAT_*` on the next worker startup. No manual action required.
...
If the OpenAI-compatible API encounters errors, claude-mem automatically falls back to the Claude Agent SDK:
...
Either:
- Set `CLAUDE_MEM_OPENAI_COMPAT_API_KEY` in `~/.claude-mem/settings.json`, or
...
- Reduce `CLAUDE_MEM_OPENAI_COMPAT_MAX_CONTEXT_MESSAGES`
- Reduce `CLAUDE_MEM_OPENAI_COMPAT_MAX_TOKENS`
...
HTTP-Referer: https://github.com/doublefx/claude-mem
X-Title: claude-mem
