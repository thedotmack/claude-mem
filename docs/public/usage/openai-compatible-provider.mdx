...
**Repeated 30s timeouts.** The per-attempt deadline defaults to 30 seconds,
which some large models exceed. Raise it with `CLAUDE_MEM_LLM_TIMEOUT_MS` (in
milliseconds), or prefer a small, fast model for observation extraction —
