...
This keeps search token-efficient by narrowing candidates before fetching full details. A published top-10 recall baseline is still pending (#3442), so treat the workflow as guidance rather than a benchmarked savings claim.
