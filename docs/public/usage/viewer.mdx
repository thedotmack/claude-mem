...
| **Stored** | Estimated token size of all stored observations -- the compressed knowledge footprint |
| **Recalled** | Tokens actually delivered into sessions via context injection (session start, prompt submit, and MCP search) |
| **Saved** | Net tokens saved by reusing compressed context instead of re-doing the original work. Calculated as Work minus Stored minus Recalled. Shown as an absolute count and percentage |
