Brainstorming Report: CLAUDE.md Distribution Architecture
  ...
  Potential Claude Code feature request - support command substitution in context config:
  {
    "context": {
      "sources": [
        { "command": "magic-claude-mem live-context ${CWD}" }
      ]
    }
  }
  Or folder-level .claude-context files containing just:
  exec: magic-claude-mem live-context .
  ...
  - Timeline data in separate file: magic-claude-mem-timeline.csv
  - Generated fresh on SessionStart
  - Block Claude from reading timeline file via .claude/settings.local.json: "ignorePaths": ["magic-claude-mem-timeline.csv"]
  ...
  - Writes magic-claude-mem-timeline.csv and includes in push
  ...
  - Post-pull hook: magic-claude-mem sync --from-timeline
  ...
  - magic-claude-mem-timeline.csv = Machine-generated context sync (blocked from reading, already injected)
