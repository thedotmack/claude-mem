...
`+JSON.stringify(o,null,2):u=" "+this.formatData(o));let _="";if(n){let{sessionId:E,memorySessionId:T,correlationId:h,...m}=n;Object.keys(m).length>0&&(_=` {${Object.entries(m).map(([S,C])=>`${S}=${C}`).join(", ")}}`)}let g=`[${i}] [${a}] [${d}] ${c}${s}${_}${u}`;if(this.logFilePath)try{(0,I.appendFileSync)(this.logFilePath,g+`
...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),u=c?`${c[1].split("/").pop()}:${c[2]}`:"unknown",_={...s,location:u};return this.warn(e,`[HAPPY-PATH] ${t}`,_,n),o}},l=new W;var Ct={};function ht(){return typeof __dirname<"u"?__dirname:(0,b.dirname)((0,ce.fileURLToPath)(Ct.url))}var Ot=ht(),R=N.get("CLAUDE_MEM_DATA_DIR"),G=process.env.CLAUDE_CONFIG_DIR||(0,b.join)((0,de.homedir)(),".claude"),Wt=(0,b.join)(R,"archives"),Gt=(0,b.join)(R,"logs"),Yt=(0,b.join)(R,"trash"),Vt=(0,b.join)(R,"backups"),qt=(0,b.join)(R,"modes"),Kt=(0,b.join)(R,"settings.json"),le=(0,b.join)(R,"claude-mem.db"),Jt=(0,b.join)(R,"vector-db"),zt=(0,b.join)(R,"observer-sessions"),Qt=(0,b.join)(G,"settings.json"),Zt=(0,b.join)(G,"commands"),es=(0,b.join)(G,"CLAUDE.md");function _e(r){(0,pe.mkdirSync)(r,{recursive:!0})}function ue(){return(0,b.join)(Ot,"..")}var $=class{db;constructor(e=le){e!==":memory:"&&_e(R),this.db=new me.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn()}initializeSchema(){this.db.run(`
    ...
    `).all(e)}getObservationById(e){return this.db.prepare(`
      ...
      FROM observations
      WHERE id = ?
    `).get(e)||null}getObservationsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o,type:i,concepts:a,files:d}=t,c=s==="date_asc"?"ASC":"DESC",u=n?`LIMIT ${n}`:"",_=e.map(()=>"?").join(","),g=[...e],E=[];if(o&&(E.push("project = ?"),g.push(o)),i)if(Array.isArray(i)){let m=i.map(()=>"?").join(",");E.push(`type IN (${m})`),g.push(...i)}else E.push("type = ?"),g.push(i);if(a){let m=Array.isArray(a)?a:[a],f=m.map(()=>"EXISTS (SELECT 1 FROM json_each(concepts) WHERE value = ?)");g.push(...m),E.push(`(${f.join(" OR ")})`)}if(d){let m=Array.isArray(d)?d:[d],f=m.map(()=>"(EXISTS (SELECT 1 FROM json_each(files_read) WHERE value LIKE ?) OR EXISTS (SELECT 1 FROM json_each(files_modified) WHERE value LIKE ?))");m.forEach(S=>{g.push(`%${S}%`,`%${S}%`)}),E.push(`(${f.join(" OR ")})`)}let T=E.length>0?`WHERE id IN (${_}) AND ${E.join(" AND ")}`:`WHERE id IN (${_})`;return this.db.prepare(`
      SELECT *
      FROM observations
      ${T}
    ...
    `).get(e,t)?.prompt_text??null}storeObservation(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString(),u=this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.type,s.title,s.subtitle,JSON.stringify(s.facts),s.narrative,JSON.stringify(s.concepts),JSON.stringify(s.files_read),JSON.stringify(s.files_modified),n||null,o,d,a);return{id:Number(u.lastInsertRowid),createdAtEpoch:a}}storeSummary(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString(),c=this.db.prepare("SELECT id FROM session_summaries WHERE memory_session_id = ? LIMIT 1").get(e);if(c)return this.db.prepare(`
        UPDATE session_summaries
        SET project=?, request=?, investigated=?, learned=?, completed=?,
            next_steps=?, notes=?, prompt_number=?, discovery_tokens=?,
            created_at=?, created_at_epoch=?
        WHERE id = ?
      `).run(t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,d,a,c.id),{id:c.id,createdAtEpoch:a};let _=this.db.prepare(`
      INSERT INTO session_summaries
      (memory_session_id, project, request, investigated, learned, completed,
       next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,d,a);return{id:Number(_.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a){let d=a??Date.now(),c=new Date(d).toISOString();return this.db.transaction(()=>{let _=[],g=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let T of s){let h=g.run(e,t,T.type,T.title,T.subtitle,JSON.stringify(T.facts),T.narrative,JSON.stringify(T.concepts),JSON.stringify(T.files_read),JSON.stringify(T.files_modified),o||null,i,c,d);_.push(Number(h.lastInsertRowid))}let E=null;if(n){let T=this.db.prepare("SELECT id FROM session_summaries WHERE memory_session_id = ? LIMIT 1").get(e);if(T)this.db.prepare(`
            UPDATE session_summaries
            SET project=?, request=?, investigated=?, learned=?, completed=?,
                next_steps=?, notes=?, prompt_number=?, discovery_tokens=?,
                created_at=?, created_at_epoch=?
            WHERE id = ?
          `).run(t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,o||null,i,c,d,T.id),E=T.id;else{let m=this.db.prepare(`
            INSERT INTO session_summaries
            (memory_session_id, project, request, investigated, learned, completed,
             next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
          `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,o||null,i,c,d);E=Number(m.lastInsertRowid)}}return{observationIds:_,summaryId:E,createdAtEpoch:d}})()}storeObservationsAndMarkComplete(e,t,s,n,o,i,a,d=0,c){let u=c??Date.now(),_=new Date(u).toISOString();return this.db.transaction(()=>{let E=[],T=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let f of s){let S=T.run(e,t,f.type,f.title,f.subtitle,JSON.stringify(f.facts),f.narrative,JSON.stringify(f.concepts),JSON.stringify(f.files_read),JSON.stringify(f.files_modified),a||null,d,_,u);E.push(Number(S.lastInsertRowid))}let h;if(n){let f=this.db.prepare("SELECT id FROM session_summaries WHERE memory_session_id = ? LIMIT 1").get(e);if(f)this.db.prepare(`
            UPDATE session_summaries
            SET project=?, request=?, investigated=?, learned=?, completed=?,
                next_steps=?, notes=?, prompt_number=?, discovery_tokens=?,
                created_at=?, created_at_epoch=?
            WHERE id = ?
          `).run(t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,a||null,d,_,u,f.id),h=f.id;else{let C=this.db.prepare(`
            INSERT INTO session_summaries
            (memory_session_id, project, request, investigated, learned, completed,
             next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
          `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,a||null,d,_,u);h=Number(C.lastInsertRowid)}}return this.db.prepare(`
    ...
    `).all(...c)}getTimelineAroundTimestamp(e,t=10,s=10,n){return this.getTimelineAroundObservation(null,e,t,s,n)}getTimelineAroundObservation(e,t,s=10,n=10,o){let i=o?"AND project = ?":"",a=o?[o]:[],d,c;if(e!==null){let m=`
      ...
      `,f=`
        ...
        ORDER BY id ASC
        LIMIT ?
      `;try{let S=this.db.prepare(m).all(e,...a,s+1),C=this.db.prepare(f).all(e,...a,n+1);if(S.length===0&&C.length===0)return{observations:[],sessions:[],prompts:[]};d=S.length>0?S[S.length-1].created_at_epoch:t,c=C.length>0?C[C.length-1].created_at_epoch:t}catch(S){return l.error("DB","Error getting boundary observations",void 0,{error:S,project:o}),{observations:[],sessions:[],prompts:[]}}}else{let m=`
      ...
      `,f=`
        ...
        ORDER BY created_at_epoch ASC
        LIMIT ?
      `;try{let S=this.db.prepare(m).all(t,...a,s),C=this.db.prepare(f).all(t,...a,n+1);if(S.length===0&&C.length===0)return{observations:[],sessions:[],prompts:[]};d=S.length>0?S[S.length-1].created_at_epoch:t,c=C.length>0?C[C.length-1].created_at_epoch:t}catch(S){return l.error("DB","Error getting boundary timestamps",void 0,{error:S,project:o}),{observations:[],sessions:[],prompts:[]}}}let u=`
      SELECT *
      FROM observations
