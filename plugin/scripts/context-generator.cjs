...
`+JSON.stringify(o,null,2):l=" "+this.formatData(o));let u="";if(n){let{sessionId:E,memorySessionId:g,correlationId:h,...m}=n;Object.keys(m).length>0&&(u=` {${Object.entries(m).map(([f,N])=>`${f}=${N}`).join(", ")}}`)}let T=`[${i}] [${a}] [${d}] ${c}${s}${u}${l}`;if(this.logFilePath)try{(0,I.appendFileSync)(this.logFilePath,T+`
...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),l=c?`${c[1].split("/").pop()}:${c[2]}`:"unknown",u={...s,location:l};return this.warn(e,`[HAPPY-PATH] ${t}`,u,n),o}},_=new W;var Ot={};function bt(){return typeof __dirname<"u"?__dirname:(0,b.dirname)((0,ce.fileURLToPath)(Ot.url))}var ht=bt(),R=A.get("CLAUDE_MEM_DATA_DIR"),L=process.env.CLAUDE_CONFIG_DIR||(0,b.join)((0,de.homedir)(),".claude"),Ht=(0,b.join)(L,"plugins","marketplaces","thedotmack"),Bt=(0,b.join)(R,"archives"),Wt=(0,b.join)(R,"logs"),Yt=(0,b.join)(R,"trash"),Vt=(0,b.join)(R,"backups"),Kt=(0,b.join)(R,"modes"),qt=(0,b.join)(R,"settings.json"),_e=(0,b.join)(R,"claude-mem.db"),Jt=(0,b.join)(R,"vector-db"),Qt=(0,b.join)(R,"observer-sessions"),zt=(0,b.join)(L,"settings.json"),Zt=(0,b.join)(L,"commands"),es=(0,b.join)(L,"CLAUDE.md");function le(r){(0,pe.mkdirSync)(r,{recursive:!0})}function ue(){return(0,b.join)(ht,"..")}var w=class{db;constructor(e=_e){e!==":memory:"&&le(R),this.db=new me.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addSupersessionTrainingTables(),this.addHandoffObservationType()}initializeSchema(){this.db.run(`
    ...
    `),this.db.run("CREATE INDEX IF NOT EXISTS idx_supersession_training_created ON supersession_training(created_at_epoch DESC)"),this.db.run("CREATE INDEX IF NOT EXISTS idx_supersession_training_label ON supersession_training(label)"),this.db.run("CREATE INDEX IF NOT EXISTS idx_supersession_training_older ON supersession_training(older_observation_id)"),this.db.run("CREATE INDEX IF NOT EXISTS idx_supersession_training_newer ON supersession_training(newer_observation_id)"),this.db.run(`
    ...
    `),this.db.run("CREATE INDEX IF NOT EXISTS idx_learned_model_weights_trained ON learned_model_weights(trained_at_epoch DESC)"),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(21,new Date().toISOString()),_.debug("DB","Successfully added Supersession Training tables")}addHandoffObservationType(){if(this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(22))return;let t=this.db.query("PRAGMA table_info(observations)").all();if(this.db.query("SELECT sql FROM sqlite_master WHERE type='table' AND name='observations'").get()?.sql?.includes("'handoff'")){this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(22,new Date().toISOString());return}_.debug("DB","Adding handoff observation type for PreCompact continuity"),this.db.run("BEGIN TRANSACTION");try{let n=t.map(i=>i.name).filter(i=>i!=="id");this.db.run(`
        CREATE TABLE observations_new (
          id INTEGER PRIMARY KEY AUTOINCREMENT,
          memory_session_id TEXT NOT NULL,
          project TEXT NOT NULL,
      ...
      `),this.db.run(`
        CREATE TRIGGER observations_au AFTER UPDATE ON observations BEGIN
          ...
          VALUES (NEW.id, NEW.title, NEW.subtitle, NEW.narrative, NEW.text, NEW.facts, NEW.concepts);
        END
      `),this.db.run("COMMIT"),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(22,new Date().toISOString()),_.debug("DB","Successfully added handoff observation type")}catch(n){throw this.db.run("ROLLBACK"),n}}updateMemorySessionId(e,t){this.db.prepare(`
      UPDATE sdk_sessions
    ...
    `).run(e,t,s.type,s.title,s.subtitle,JSON.stringify(s.facts),s.narrative,JSON.stringify(s.concepts),JSON.stringify(s.files_read),JSON.stringify(s.files_modified),n||null,o,d,a);return{id:Number(l.lastInsertRowid),createdAtEpoch:a}}storeSummary(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString(),l=this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,d,a);return{id:Number(l.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a){let d=a??Date.now(),c=new Date(d).toISOString();return this.db.transaction(()=>{let u=[],T=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let g of s){let h=T.run(e,t,g.type,g.title,g.subtitle,JSON.stringify(g.facts),g.narrative,JSON.stringify(g.concepts),JSON.stringify(g.files_read),JSON.stringify(g.files_modified),o||null,i,c,d);u.push(Number(h.lastInsertRowid))}let E=null;if(n){let h=this.db.prepare(`
          ...
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,o||null,i,c,d);E=Number(h.lastInsertRowid)}return{observationIds:u,summaryId:E,createdAtEpoch:d}})()}storeObservationsAndMarkComplete(e,t,s,n,o,i,a,d=0,c){let l=c??Date.now(),u=new Date(l).toISOString();return this.db.transaction(()=>{let E=[],g=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let S of s){let f=g.run(e,t,S.type,S.title,S.subtitle,JSON.stringify(S.facts),S.narrative,JSON.stringify(S.concepts),JSON.stringify(S.files_read),JSON.stringify(S.files_modified),a||null,d,u,l);E.push(Number(f.lastInsertRowid))}let h;if(n){let f=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,a||null,d,u,l);h=Number(f.lastInsertRowid)}return this.db.prepare(`
      ...
      `).run(l,o),{observationIds:E,summaryId:h,createdAtEpoch:l}})()}getSessionSummariesByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o}=t,i=s==="date_asc"?"ASC":"DESC",a=n?`LIMIT ${n}`:"",d=e.map(()=>"?").join(","),c=[...e],l=o?`WHERE id IN (${d}) AND project = ?`:`WHERE id IN (${d})`;return o&&c.push(o),this.db.prepare(`
...
`}function Ze(r,e,t,s){let n=[];return s?n.push(...Xe(r)):n.push(...Ie(r)),s?n.push(...je()):n.push(...Ae()),s?n.push(...Ge()):n.push(...Le()),s?n.push(...He()):n.push(...Me()),P(t)&&(s?n.push(...Be(e,t)):n.push(...ye(e,t))),n}var ee=y(require("path"),1);function H(r){if(!r)return[];try{let e=JSON.parse(r);return Array.isArray(e)?e:[]}catch(e){return _.debug("PARSER","Failed to parse JSON array, using empty fallback",{preview:r?.substring(0,50)},e),[]}}function tt(r){return new Date(r).toLocaleString("en-US",{month:"short",day:"numeric",hour:"numeric",minute:"2-digit",hour12:!0})}function st(r){return new Date(r).toLocaleString("en-US",{hour:"numeric",minute:"2-digit",hour12:!0})}function rt(r){return new Date(r).toLocaleString("en-US",{month:"short",day:"numeric",year:"numeric"})}function et(r,e){return ee.default.isAbsolute(r)?ee.default.relative(e,r):r}function nt(r,e,t){let s=H(r);if(s.length>0)return et(s[0],e);if(t){let n=H(t);if(n.length>0)return et(n[0],e)}return"General"}function It(r){let e=new Map;for(let s of r){let n=s.type==="observation"?s.data.created_at:s.data.displayTime,o=rt(n);e.has(o)||e.set(o,[]),e.get(o).push(s)}let t=Array.from(e.entries()).sort((s,n)=>{let o=new Date(s[0]).getTime(),i=new Date(n[0]).getTime();return o-i});return new Map(t)}function At(r,e){return e.fullObservationField==="narrative"?r.narrative:r.facts?H(r.facts).join(`
`):null}function Lt(r,e,t,s,n,o){let i=[];o?i.push(...We(r)):i.push(...ve(r));let a=null,d="",c=!1;for(let l of e)if(l.type==="summary"){c&&(i.push(""),c=!1,a=null,d="");let u=l.data,T=tt(u.displayTime);o?i.push(...qe(u,T)):i.push(...ke(u,T))}else{let u=l.data,T=nt(u.files_modified,n,u.files_read),E=st(u.created_at),g=E!==d,h=g?E:"";d=E;let m=t.has(u.id);if(T!==a&&(c&&i.push(""),o?i.push(...Ye(T)):i.push(...De(T)),a=T,c=!0),m){let S=At(u,s);o?i.push(...Ke(u,E,g,S,s)):(c&&!o&&(i.push(""),c=!1),i.push(...xe(u,h,S,s)),a=null)}else o?i.push(Ve(u,E,g,s)):i.push(Ue(u,h,s))}return c&&i.push(""),i}function ot(r,e,t,s,n){let o=[],i=It(r);for(let[a,d]of i)o.push(...Lt(a,d,e,t,s,n));return o}function it(r,e,t){return!(!r.showLastSummary||!e||!!!(e.investigated||e.learned||e.completed||e.next_steps)||t&&e.created_at_epoch<=t.created_at_epoch)}function at(r,e){let t=[];return e?(t.push(...x("Investigated",r.investigated,p.blue)),t.push(...x("Learned",r.learned,p.yellow)),t.push(...x("Completed",r.completed,p.green)),t.push(...x("Next Steps",r.next_steps,p.magenta))):(t.push(...U("Investigated",r.investigated)),t.push(...U("Learned",r.learned)),t.push(...U("Completed",r.completed)),t.push(...U("Next Steps",r.next_steps))),t}function dt(r,e){return e?Je(r):$e(r)}function pt(r,e,t){return!P(e)||r.totalDiscoveryTokens<=0||r.savings<=0?[]:t?Qe(r.totalDiscoveryTokens,r.totalReadTokens):we(r.totalDiscoveryTokens,r.totalReadTokens)}var Mt=ct.default.join((0,_t.homedir)(),".claude","plugins","marketplaces","thedotmack","plugin",".install-version");function yt(){try{return new w}catch(r){if(r.code==="ERR_DLOPEN_FAILED"){try{(0,lt.unlinkSync)(Mt)}catch(e){_.debug("SYSTEM","Marker file cleanup failed (may not exist)",{},e)}return _.error("SYSTEM","Native module rebuild needed - restart Claude Code to auto-fix"),null}throw r}}function vt(r,e){return e?ze(r):Fe(r)}function Dt(r,e,t,s,n,o,i){let a=[],d=q(e);a.push(...Ze(r,d,s,i));let c=t.slice(0,s.sessionCount),l=Ne(c,t),u=Z(e,l),T=Re(e,s.fullObservationCount);a.push(...ot(u,T,s,n,i));let E=t[0],g=e[0];it(s,E,g)&&a.push(...at(E,i));let h=z(e,s,o,n);return a.push(...dt(h,i)),a.push(...pt(d,s,i)),a.join(`
`).trimEnd()}async function te(r,e=!1){let t=Y(),s=r?.cwd??process.cwd(),n=Te(s),o=r?.projects||[n],i=yt();if(!i)return"";try{let a=o.length>1?he(i,o,t):J(i,n,t),d=o.length>1?Oe(i,o,t):Q(i,n,t);return a.length===0&&d.length===0?vt(n,e):Dt(n,a,d,t,s,r?.session_id,e)}finally{i.close()}}0&&(module.exports={generateContext});
