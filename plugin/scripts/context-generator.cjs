...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),u=l?`${l[1].split("/").pop()}:${l[2]}`:"unknown",b={...t,location:u};return this.warn(e,`[HAPPY-PATH] ${s}`,b,n),o}},S=new ae;var U=class{static DEFAULTS={CLAUDE_MEM_MODEL:"claude-sonnet-4-5",CLAUDE_MEM_CONTEXT_OBSERVATIONS:"50",CLAUDE_MEM_WORKER_PORT:"37777",CLAUDE_MEM_WORKER_HOST:"127.0.0.1",CLAUDE_MEM_SKIP_TOOLS:"ListMcpResourcesTool,SlashCommand,Skill,TodoWrite,AskUserQuestion",CLAUDE_MEM_PROVIDER:"claude",CLAUDE_MEM_GEMINI_API_KEY:"",CLAUDE_MEM_GEMINI_MODEL:"gemini-2.5-flash-lite",CLAUDE_MEM_GEMINI_RATE_LIMITING_ENABLED:"true",CLAUDE_MEM_DATA_DIR:(0,be.join)((0,fe.homedir)(),".claude-mem"),CLAUDE_MEM_LOG_LEVEL:"INFO",CLAUDE_MEM_PYTHON_VERSION:"3.13",CLAUDE_CODE_PATH:"",CLAUDE_MEM_MODE:"code",CLAUDE_MEM_CONTEXT_SHOW_READ_TOKENS:"true",CLAUDE_MEM_CONTEXT_SHOW_WORK_TOKENS:"true",CLAUDE_MEM_CONTEXT_SHOW_SAVINGS_AMOUNT:"true",CLAUDE_MEM_CONTEXT_SHOW_SAVINGS_PERCENT:"true",CLAUDE_MEM_CONTEXT_OBSERVATION_TYPES:he,CLAUDE_MEM_CONTEXT_OBSERVATION_CONCEPTS:Se,CLAUDE_MEM_CONTEXT_FULL_COUNT:"5",CLAUDE_MEM_CONTEXT_FULL_FIELD:"narrative",CLAUDE_MEM_CONTEXT_SESSION_COUNT:"10",CLAUDE_MEM_CONTEXT_SHOW_LAST_SUMMARY:"true",CLAUDE_MEM_CONTEXT_SHOW_LAST_MESSAGE:"false",CLAUDE_MEM_SURPRISE_ENABLED:"true",CLAUDE_MEM_SURPRISE_THRESHOLD:"0.3",CLAUDE_MEM_SURPRISE_LOOKBACK_DAYS:"30",CLAUDE_MEM_MOMENTUM_ENABLED:"true",CLAUDE_MEM_MOMENTUM_DURATION_MINUTES:"5"};static getAllDefaults(){return{...this.DEFAULTS}}static get(e){return this.DEFAULTS[e]}static getInt(e){let s=this.get(e);return parseInt(s,10)}static getBool(e){return this.get(e)==="true"}static loadFromFile(e){try{if(!(0,G.existsSync)(e))return this.getAllDefaults();let s=(0,G.readFileSync)(e,"utf-8"),t=JSON.parse(s),n=t;if(t.env&&typeof t.env=="object"){n=t.env;try{(0,G.writeFileSync)(e,JSON.stringify(n,null,2),"utf-8"),S.info("SETTINGS","Migrated settings file from nested to flat schema",{settingsPath:e})}catch(a){S.warn("SETTINGS","Failed to auto-migrate settings file",{settingsPath:e},a)}}let o={...this.DEFAULTS};for(let a of Object.keys(this.DEFAULTS))n[a]!==void 0&&(o[a]=n[a]);return o}catch(s){return S.warn("SETTINGS","Failed to load settings, using defaults",{settingsPath:e},s),this.getAllDefaults()}}};var Je={};function Ke(){return typeof __dirname<"u"?__dirname:(0,f.dirname)((0,Ne.fileURLToPath)(Je.url))}var qe=Ke(),M=U.get("CLAUDE_MEM_DATA_DIR"),de=process.env.CLAUDE_CONFIG_DIR||(0,f.join)((0,Re.homedir)(),".claude"),gs=(0,f.join)(M,"archives"),hs=(0,f.join)(M,"logs"),Ss=(0,f.join)(M,"trash"),bs=(0,f.join)(M,"backups"),fs=(0,f.join)(M,"modes"),Rs=(0,f.join)(M,"settings.json"),Ae=(0,f.join)(M,"claude-mem.db"),Os=(0,f.join)(M,"vector-db"),Ns=(0,f.join)(de,"settings.json"),As=(0,f.join)(de,"commands"),Is=(0,f.join)(de,"CLAUDE.md");function Ie(d){(0,Oe.mkdirSync)(d,{recursive:!0})}function Le(){return(0,f.join)(qe,"..")}var Q=class{db;constructor(e=Ae){e!==":memory:"&&Ie(M),this.db=new Ce.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.createMemoryAccessTracking()}initializeSchema(){try{this.db.run(`
    ...
    `).all(e)}getObservationById(e){return this.db.prepare(`
      ...
      FROM observations
      WHERE id = ?
    `).get(e)||null}getObservationsByIds(e,s={}){if(e.length===0)return[];let{orderBy:t="date_desc",limit:n,project:o,type:a,concepts:p,files:c}=s,l=t==="date_asc"?"ASC":"DESC",u=n?`LIMIT ${n}`:"",b=e.map(()=>"?").join(","),R=[...e],m=[];if(o&&(m.push("project = ?"),R.push(o)),a)if(Array.isArray(a)){let r=a.map(()=>"?").join(",");m.push(`type IN (${r})`),R.push(...a)}else m.push("type = ?"),R.push(a);if(p){let r=Array.isArray(p)?p:[p],I=r.map(()=>"EXISTS (SELECT 1 FROM json_each(concepts) WHERE value = ?)");R.push(...r),m.push(`(${I.join(" OR ")})`)}if(c){let r=Array.isArray(c)?c:[c],I=r.map(()=>"(EXISTS (SELECT 1 FROM json_each(files_read) WHERE value LIKE ?) OR EXISTS (SELECT 1 FROM json_each(files_modified) WHERE value LIKE ?))");r.forEach(O=>{R.push(`%${O}%`,`%${O}%`)}),m.push(`(${I.join(" OR ")})`)}let A=m.length>0?`WHERE id IN (${b}) AND ${m.join(" AND ")}`:`WHERE id IN (${b})`;return this.db.prepare(`
      SELECT *
      FROM observations
      ${A}
    ...
    `).get(e)||null}getFilesForSession(e){let t=this.db.prepare(`
      ...
      FROM observations
      WHERE sdk_session_id = ?
    `).all(e),n=new Set,o=new Set;for(let a of t){if(a.files_read){let p=JSON.parse(a.files_read);Array.isArray(p)&&p.forEach(c=>n.add(c))}if(a.files_modified){let p=JSON.parse(a.files_modified);Array.isArray(p)&&p.forEach(c=>o.add(c))}}return{filesRead:Array.from(n),filesModified:Array.from(o)}}getSessionById(e){return this.db.prepare(`
    ...
    `).run(e,s,t,n.toISOString(),o).lastInsertRowid}getUserPrompt(e,s){return this.db.prepare(`
      ...
      LIMIT 1
    `).get(e,s)?.prompt_text??null}storeObservation(e,s,t,n,o=0,a){let p=a??Date.now(),c=new Date(p).toISOString(),u=this.db.prepare(`
      INSERT INTO observations
      (sdk_session_id, project, type, title, subtitle, facts, narrative, concepts,
       files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,s,t.type,t.title,t.subtitle,JSON.stringify(t.facts),t.narrative,JSON.stringify(t.concepts),JSON.stringify(t.files_read),JSON.stringify(t.files_modified),n||null,o,c,p);return{id:Number(u.lastInsertRowid),createdAtEpoch:p}}storeSummary(e,s,t,n,o=0,a){let p=a??Date.now(),c=new Date(p).toISOString(),u=this.db.prepare(`
      INSERT INTO session_summaries
      (sdk_session_id, project, request, investigated, learned, completed,
       next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,s,t.request,t.investigated,t.learned,t.completed,t.next_steps,t.notes,n||null,o,c,p);return{id:Number(u.lastInsertRowid),createdAtEpoch:p}}getSessionSummariesByIds(e,s={}){if(e.length===0)return[];let{orderBy:t="date_desc",limit:n,project:o}=s,a=t==="date_asc"?"ASC":"DESC",p=n?`LIMIT ${n}`:"",c=e.map(()=>"?").join(","),l=[...e],u=o?`WHERE id IN (${c}) AND project = ?`:`WHERE id IN (${c})`;return o&&l.push(o),this.db.prepare(`
      SELECT * FROM session_summaries
      ${u}
      ORDER BY created_at_epoch ${a}
      ${p}
    `).all(...l)}getUserPromptsByIds(e,s={}){if(e.length===0)return[];let{orderBy:t="date_desc",limit:n,project:o}=s,a=t==="date_asc"?"ASC":"DESC",p=n?`LIMIT ${n}`:"",c=e.map(()=>"?").join(","),l=[...e],u=o?"AND s.project = ?":"";return o&&l.push(o),this.db.prepare(`
      SELECT
        up.*,
        s.project,
        s.sdk_session_id
      ...
      ${p}
    `).all(...l)}getTimelineAroundTimestamp(e,s=10,t=10,n){return this.getTimelineAroundObservation(null,e,s,t,n)}getTimelineAroundObservation(e,s,t=10,n=10,o){let a=o?"AND project = ?":"",p=o?[o]:[],c,l;if(e!==null){let m=`
      ...
      `,A=`
        ...
        ORDER BY id ASC
        LIMIT ?
      `;try{let T=this.db.prepare(m).all(e,...p,t+1),r=this.db.prepare(A).all(e,...p,n+1);if(T.length===0&&r.length===0)return{observations:[],sessions:[],prompts:[]};c=T.length>0?T[T.length-1].created_at_epoch:s,l=r.length>0?r[r.length-1].created_at_epoch:s}catch(T){return console.error("[SessionStore] Error getting boundary observations:",T.message,o?`(project: ${o})`:"(all projects)"),{observations:[],sessions:[],prompts:[]}}}else{let m=`
    ...
    `;try{let m=this.db.prepare(u).all(c,l,...p),A=this.db.prepare(b).all(c,l,...p),T=this.db.prepare(R).all(c,l,...p);return{observations:m,sessions:A.map(r=>({id:r.id,sdk_session_id:r.sdk_session_id,project:r.project,request:r.request,completed:r.completed,next_steps:r.next_steps,created_at:r.created_at,created_at_epoch:r.created_at_epoch})),prompts:T.map(r=>({id:r.id,claude_session_id:r.claude_session_id,prompt_number:r.prompt_number,prompt_text:r.prompt_text,project:r.project,created_at:r.created_at,created_at_epoch:r.created_at_epoch}))}}catch(m){return console.error("[SessionStore] Error querying timeline records:",m.message,o?`(project: ${o})`:"(all projects)"),{observations:[],sessions:[],prompts:[]}}}getPromptById(e){return this.db.prepare(`
      SELECT
        p.id,
        p.claude_session_id,
        p.prompt_number,
    ...
    `).run(e.claude_session_id,e.prompt_number,e.prompt_text,e.created_at,e.created_at_epoch).lastInsertRowid}}};var ce=oe(require("path"),1);function pe(d){if(!d)return[];try{let e=JSON.parse(d);return Array.isArray(e)?e:[]}catch{return[]}}function Me(d){return new Date(d).toLocaleString("en-US",{month:"short",day:"numeric",hour:"numeric",minute:"2-digit",hour12:!0})}function ve(d){return new Date(d).toLocaleString("en-US",{hour:"numeric",minute:"2-digit",hour12:!0})}function De(d){return new Date(d).toLocaleString("en-US",{month:"short",day:"numeric",year:"numeric"})}function Qe(d,e){return ce.default.isAbsolute(d)?ce.default.relative(e,d):d}function ye(d,e){let s=pe(d);return s.length>0?Qe(s[0],e):"General"}var ke=oe(require("path"),1);function Ue(d){if(!d||d.trim()==="")return S.warn("PROJECT_NAME","Empty cwd provided, using fallback",{cwd:d}),"unknown-project";let e=ke.default.basename(d);if(e===""){if(process.platform==="win32"){let t=d.match(/^([A-Z]):\\/i);if(t){let o=`drive-${t[1].toUpperCase()}`;return S.info("PROJECT_NAME","Drive root detected",{cwd:d,projectName:o}),o}}return S.warn("PROJECT_NAME","Root directory detected, using fallback",{cwd:d}),"unknown-project"}return e}var H=require("fs"),z=require("path");var X=class d{static instance=null;activeMode=null;modesDir;constructor(){let e=Le(),s=[(0,z.join)(e,"modes"),(0,z.join)(e,"..","plugin","modes")],t=s.find(n=>(0,H.existsSync)(n));this.modesDir=t||s[0]}static getInstance(){return d.instance||(d.instance=new d),d.instance}parseInheritance(e){let s=e.split("--");if(s.length===1)return{hasParent:!1,parentId:"",overrideId:""};if(s.length>2)throw new Error(`Invalid mode inheritance: ${e}. Only one level of inheritance supported (parent--override)`);return{hasParent:!0,parentId:s[0],overrideId:e}}isPlainObject(e){return e!==null&&typeof e=="object"&&!Array.isArray(e)}deepMerge(e,s){let t={...e};for(let n in s){let o=s[n],a=e[n];this.isPlainObject(o)&&this.isPlainObject(a)?t[n]=this.deepMerge(a,o):t[n]=o}return t}loadModeFile(e){let s=(0,z.join)(this.modesDir,`${e}.json`);if(!(0,H.existsSync)(s))throw new Error(`Mode file not found: ${s}`);let t=(0,H.readFileSync)(s,"utf-8");return JSON.parse(t)}loadMode(e){let s=this.parseInheritance(e);if(!s.hasParent)try{let c=this.loadModeFile(e);return this.activeMode=c,S.debug("SYSTEM",`Loaded mode: ${c.name} (${e})`,void 0,{types:c.observation_types.map(l=>l.id),concepts:c.observation_concepts.map(l=>l.id)}),c}catch{if(S.warn("SYSTEM",`Mode file not found: ${e}, falling back to 'code'`),e==="code")throw new Error("Critical: code.json mode file missing");return this.loadMode("code")}let{parentId:t,overrideId:n}=s,o;try{o=this.loadMode(t)}catch{S.warn("SYSTEM",`Parent mode '${t}' not found for ${e}, falling back to 'code'`),o=this.loadMode("code")}let a;try{a=this.loadModeFile(n),S.debug("SYSTEM",`Loaded override file: ${n} for parent ${t}`)}catch{return S.warn("SYSTEM",`Override file '${n}' not found, using parent mode '${t}' only`),this.activeMode=o,o}if(!a)return S.warn("SYSTEM",`Invalid override file: ${n}, using parent mode '${t}' only`),this.activeMode=o,o;let p=this.deepMerge(o,a);return this.activeMode=p,S.debug("SYSTEM",`Loaded mode with inheritance: ${p.name} (${e} = ${t} + ${n})`,void 0,{parent:t,override:n,types:p.observation_types.map(c=>c.id),concepts:p.observation_concepts.map(c=>c.id)}),p}getActiveMode(){if(!this.activeMode)throw new Error("No mode loaded. Call loadMode() first.");return this.activeMode}getObservationTypes(){return this.getActiveMode().observation_types}getObservationConcepts(){return this.getActiveMode().observation_concepts}getTypeIcon(e){return this.getObservationTypes().find(t=>t.id===e)?.emoji||"\u{1F4DD}"}getWorkEmoji(e){return this.getObservationTypes().find(t=>t.id===e)?.work_emoji||"\u{1F4DD}"}validateType(e){return this.getObservationTypes().some(s=>s.id===e)}getTypeLabel(e){return this.getObservationTypes().find(t=>t.id===e)?.label||e}};var ze=ee.default.join((0,se.homedir)(),".claude","plugins","marketplaces","thedotmack","plugin",".install-version");function Ze(){let d=ee.default.join((0,se.homedir)(),".claude-mem","settings.json"),e=U.loadFromFile(d),s=e.CLAUDE_MEM_MODE,t=s==="code"||s.startsWith("code--"),n,o;if(t)n=new Set(e.CLAUDE_MEM_CONTEXT_OBSERVATION_TYPES.split(",").map(a=>a.trim()).filter(Boolean)),o=new Set(e.CLAUDE_MEM_CONTEXT_OBSERVATION_CONCEPTS.split(",").map(a=>a.trim()).filter(Boolean));else{let a=X.getInstance().getActiveMode();n=new Set(a.observation_types.map(p=>p.id)),o=new Set(a.observation_concepts.map(p=>p.id))}return{totalObservationCount:parseInt(e.CLAUDE_MEM_CONTEXT_OBSERVATIONS,10),fullObservationCount:parseInt(e.CLAUDE_MEM_CONTEXT_FULL_COUNT,10),sessionCount:parseInt(e.CLAUDE_MEM_CONTEXT_SESSION_COUNT,10),showReadTokens:e.CLAUDE_MEM_CONTEXT_SHOW_READ_TOKENS==="true",showWorkTokens:e.CLAUDE_MEM_CONTEXT_SHOW_WORK_TOKENS==="true",showSavingsAmount:e.CLAUDE_MEM_CONTEXT_SHOW_SAVINGS_AMOUNT==="true",showSavingsPercent:e.CLAUDE_MEM_CONTEXT_SHOW_SAVINGS_PERCENT==="true",observationTypes:n,observationConcepts:o,fullObservationField:e.CLAUDE_MEM_CONTEXT_FULL_FIELD,showLastSummary:e.CLAUDE_MEM_CONTEXT_SHOW_LAST_SUMMARY==="true",showLastMessage:e.CLAUDE_MEM_CONTEXT_SHOW_LAST_MESSAGE==="true"}}var $e=4,es=1,i={reset:"\x1B[0m",bright:"\x1B[1m",dim:"\x1B[2m",cyan:"\x1B[36m",green:"\x1B[32m",yellow:"\x1B[33m",blue:"\x1B[34m",magenta:"\x1B[35m",gray:"\x1B[90m",red:"\x1B[31m"};function Z(d,e,s,t){return e?t?[`${s}${d}:${i.reset} ${e}`,""]:[`**${d}**: ${e}`,""]:[]}function ss(d){return d.replace(/\//g,"-")}function ts(d){try{if(!(0,B.existsSync)(d))return{userMessage:"",assistantMessage:""};let e=(0,B.readFileSync)(d,"utf-8").trim();if(!e)return{userMessage:"",assistantMessage:""};let s=e.split(`
`).filter(n=>n.trim()),t="";for(let n=s.length-1;n>=0;n--)try{let o=s[n];if(!o.includes('"type":"assistant"'))continue;let a=JSON.parse(o);if(a.type==="assistant"&&a.message?.content&&Array.isArray(a.message.content)){let p="";for(let c of a.message.content)c.type==="text"&&(p+=c.text);if(p=p.replace(/<system-reminder>[\s\S]*?<\/system-reminder>/g,"").trim(),p){t=p;break}}}catch{continue}return{userMessage:"",assistantMessage:t}}catch(e){return S.failure("WORKER","Failed to extract prior messages from transcript",{transcriptPath:d},e),{userMessage:"",assistantMessage:""}}}async function rs(d,e=!1){let s=Ze(),t=d?.cwd??process.cwd(),n=Ue(t),o=null;try{o=new Q}catch(I){if(I.code==="ERR_DLOPEN_FAILED"){try{(0,B.unlinkSync)(ze)}catch{}return console.error("Native module rebuild needed - restart Claude Code to auto-fix"),""}throw I}let a=Array.from(s.observationTypes),p=a.map(()=>"?").join(","),c=Array.from(s.observationConcepts),l=c.map(()=>"?").join(","),u=o.db.prepare(`
    SELECT
    ...
    ORDER BY created_at_epoch DESC
    LIMIT ?
  `).all(n,...a,...c,s.totalObservationCount),b=o.db.prepare(`
    SELECT id, sdk_session_id, request, investigated, learned, completed, next_steps, created_at, created_at_epoch
    FROM session_summaries
    WHERE project = ?
    ORDER BY created_at_epoch DESC
    LIMIT ?
  `).all(n,s.sessionCount+es),R="",m="";if(s.showLastMessage&&u.length>0){let I=d?.session_id,O=u.find(D=>D.sdk_session_id!==I);if(O){let D=O.sdk_session_id,$=ss(t),N=ee.default.join((0,se.homedir)(),".claude","projects",$,`${D}.jsonl`),v=ts(N);R=v.userMessage,m=v.assistantMessage}}if(u.length===0&&b.length===0)return o?.close(),e?`
...
No previous sessions found for this project yet.`;let A=b.slice(0,s.sessionCount),T=u,r=[];if(e?(r.push(""),r.push(`${i.bright}${i.cyan}[${n}] recent context${i.reset}`),r.push(`${i.gray}${"\u2500".repeat(60)}${i.reset}`),r.push("")):(r.push(`# [${n}] recent context`),r.push("")),T.length>0){let O=X.getInstance().getActiveMode().observation_types.map(_=>`${_.emoji} ${_.id}`).join(" | ");e?r.push(`${i.dim}Legend: \u{1F3AF} session-request | ${O}${i.reset}`):r.push(`**Legend:** \u{1F3AF} session-request | ${O}`),r.push(""),e?(r.push(`${i.bright}\u{1F4A1} Column Key${i.reset}`),r.push(`${i.dim}  Read: Tokens to read this observation (cost to learn it now)${i.reset}`),r.push(`${i.dim}  Work: Tokens spent on work that produced this record (\u{1F50D} research, \u{1F6E0}\uFE0F building, \u2696\uFE0F  deciding)${i.reset}`)):(r.push("\u{1F4A1} **Column Key**:"),r.push("- **Read**: Tokens to read this observation (cost to learn it now)"),r.push("- **Work**: Tokens spent on work that produced this record (\u{1F50D} research, \u{1F6E0}\uFE0F building, \u2696\uFE0F  deciding)")),r.push(""),e?(r.push(`${i.dim}\u{1F4A1} Context Index: This semantic index (titles, types, files, tokens) is usually sufficient to understand past work.${i.reset}`),r.push(""),r.push(`${i.dim}When you need implementation details, rationale, or debugging context:${i.reset}`),r.push(`${i.dim}  - Use the mem-search skill to fetch full observations on-demand${i.reset}`),r.push(`${i.dim}  - Critical types (\u{1F534} bugfix, \u2696\uFE0F decision) often need detailed fetching${i.reset}`),r.push(`${i.dim}  - Trust this index over re-reading code for past decisions and learnings${i.reset}`)):(r.push("\u{1F4A1} **Context Index:** This semantic index (titles, types, files, tokens) is usually sufficient to understand past work."),r.push(""),r.push("When you need implementation details, rationale, or debugging context:"),r.push("- Use the mem-search skill to fetch full observations on-demand"),r.push("- Critical types (\u{1F534} bugfix, \u2696\uFE0F decision) often need detailed fetching"),r.push("- Trust this index over re-reading code for past decisions and learnings")),r.push("");let D=u.length,$=u.reduce((_,g)=>{let h=(g.title?.length||0)+(g.subtitle?.length||0)+(g.narrative?.length||0)+JSON.stringify(g.facts||[]).length;return _+Math.ceil(h/$e)},0),N=u.reduce((_,g)=>_+(g.discovery_tokens||0),0),v=N-$,W=N>0?Math.round(v/N*100):0,_e=s.showReadTokens||s.showWorkTokens||s.showSavingsAmount||s.showSavingsPercent;if(_e)if(e){if(r.push(`${i.bright}${i.cyan}\u{1F4CA} Context Economics${i.reset}`),r.push(`${i.dim}  Loading: ${D} observations (${$.toLocaleString()} tokens to read)${i.reset}`),r.push(`${i.dim}  Work investment: ${N.toLocaleString()} tokens spent on research, building, and decisions${i.reset}`),N>0&&(s.showSavingsAmount||s.showSavingsPercent)){let _="  Your savings: ";s.showSavingsAmount&&s.showSavingsPercent?_+=`${v.toLocaleString()} tokens (${W}% reduction from reuse)`:s.showSavingsAmount?_+=`${v.toLocaleString()} tokens`:_+=`${W}% reduction from reuse`,r.push(`${i.green}${_}${i.reset}`)}r.push("")}else{if(r.push("\u{1F4CA} **Context Economics**:"),r.push(`- Loading: ${D} observations (${$.toLocaleString()} tokens to read)`),r.push(`- Work investment: ${N.toLocaleString()} tokens spent on research, building, and decisions`),N>0&&(s.showSavingsAmount||s.showSavingsPercent)){let _="- Your savings: ";s.showSavingsAmount&&s.showSavingsPercent?_+=`${v.toLocaleString()} tokens (${W}% reduction from reuse)`:s.showSavingsAmount?_+=`${v.toLocaleString()} tokens`:_+=`${W}% reduction from reuse`,r.push(_)}r.push("")}let xe=b[0]?.id,we=A.map((_,g)=>{let h=g===0?null:b[g+1];return{..._,displayEpoch:h?h.created_at_epoch:_.created_at_epoch,displayTime:h?h.created_at:_.created_at,shouldShowLink:_.id!==xe}}),Fe=new Set(u.slice(0,s.fullObservationCount).map(_=>_.id)),le=[...T.map(_=>({type:"observation",data:_})),...we.map(_=>({type:"summary",data:_}))];le.sort((_,g)=>{let h=_.type==="observation"?_.data.created_at_epoch:_.data.displayEpoch,y=g.type==="observation"?g.data.created_at_epoch:g.data.displayEpoch;return h-y});let Y=new Map;for(let _ of le){let g=_.type==="observation"?_.data.created_at:_.data.displayTime,h=De(g);Y.has(h)||Y.set(h,[]),Y.get(h).push(_)}let Xe=Array.from(Y.entries()).sort((_,g)=>{let h=new Date(_[0]).getTime(),y=new Date(g[0]).getTime();return h-y});for(let[_,g]of Xe){e?(r.push(`${i.bright}${i.cyan}${_}${i.reset}`),r.push("")):(r.push(`### ${_}`),r.push(""));let h=null,y="",x=!1;for(let te of g)if(te.type==="summary"){x&&(r.push(""),x=!1,h=null,y="");let E=te.data,w=`${E.request||"Session started"} (${Me(E.displayTime)})`;e?r.push(`\u{1F3AF} ${i.yellow}#S${E.id}${i.reset} ${w}`):r.push(`**\u{1F3AF} #S${E.id}** ${w}`),r.push("")}else{let E=te.data,w=ye(E.files_modified,t);w!==h&&(x&&r.push(""),e?r.push(`${i.dim}${w}${i.reset}`):r.push(`**${w}**`),e||(r.push("| ID | Time | T | Title | Read | Work |"),r.push("|----|------|---|-------|------|------|")),h=w,x=!0,y="");let F=ve(E.created_at),V=E.title||"Untitled",K=X.getInstance().getTypeIcon(E.type),Pe=(E.title?.length||0)+(E.subtitle?.length||0)+(E.narrative?.length||0)+JSON.stringify(E.facts||[]).length,P=Math.ceil(Pe/$e),j=E.discovery_tokens||0,re=X.getInstance().getWorkEmoji(E.type),Ee=j>0?`${re} ${j.toLocaleString()}`:"-",ne=F!==y,me=ne?F:"";if(y=F,Fe.has(E.id)){let k=s.fullObservationField==="narrative"?E.narrative:E.facts?pe(E.facts).join(`
`):null;if(e){let C=ne?`${i.dim}${F}${i.reset}`:" ".repeat(F.length),q=s.showReadTokens&&P>0?`${i.dim}(~${P}t)${i.reset}`:"",Te=s.showWorkTokens&&j>0?`${i.dim}(${re} ${j.toLocaleString()}t)${i.reset}`:"";r.push(`  ${i.dim}#${E.id}${i.reset}  ${C}  ${K}  ${i.bright}${V}${i.reset}`),k&&r.push(`    ${i.dim}${k}${i.reset}`),(q||Te)&&r.push(`    ${q} ${Te}`),r.push("")}else{x&&(r.push(""),x=!1),r.push(`**#${E.id}** ${me||"\u2033"} ${K} **${V}**`),k&&(r.push(""),r.push(k),r.push(""));let C=[];s.showReadTokens&&C.push(`Read: ~${P}`),s.showWorkTokens&&C.push(`Work: ${Ee}`),C.length>0&&r.push(C.join(", ")),r.push(""),h=null}}else if(e){let k=ne?`${i.dim}${F}${i.reset}`:" ".repeat(F.length),C=s.showReadTokens&&P>0?`${i.dim}(~${P}t)${i.reset}`:"",q=s.showWorkTokens&&j>0?`${i.dim}(${re} ${j.toLocaleString()}t)${i.reset}`:"";r.push(`  ${i.dim}#${E.id}${i.reset}  ${k}  ${K}  ${V} ${C} ${q}`)}else{let k=s.showReadTokens?`~${P}`:"",C=s.showWorkTokens?Ee:"";r.push(`| #${E.id} | ${me||"\u2033"} | ${K} | ${V} | ${k} | ${C} |`)}}x&&r.push("")}let L=b[0],ue=u[0];if(s.showLastSummary&&L&&(L.investigated||L.learned||L.completed||L.next_steps)&&(!ue||L.created_at_epoch>ue.created_at_epoch)&&(r.push(...Z("Investigated",L.investigated,i.blue,e)),r.push(...Z("Learned",L.learned,i.yellow,e)),r.push(...Z("Completed",L.completed,i.green,e)),r.push(...Z("Next Steps",L.next_steps,i.magenta,e))),m&&(r.push(""),r.push("---"),r.push(""),e?(r.push(`${i.bright}${i.magenta}\u{1F4CB} Previously${i.reset}`),r.push(""),r.push(`${i.dim}A: ${m}${i.reset}`)):(r.push("**\u{1F4CB} Previously**"),r.push(""),r.push(`A: ${m}`)),r.push("")),_e&&N>0&&v>0){let _=Math.round(N/1e3);r.push(""),e?r.push(`${i.dim}\u{1F4B0} Access ${_}k tokens of past research & decisions for just ${$.toLocaleString()}t. Use the mem-search skill to access memories by ID instead of re-reading files.${i.reset}`):r.push(`\u{1F4B0} Access ${_}k tokens of past research & decisions for just ${$.toLocaleString()}t. Use the mem-search skill to access memories by ID instead of re-reading files.`)}}return o?.close(),r.join(`
