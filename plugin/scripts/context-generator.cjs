...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),u=l?`${l[1].split("/").pop()}:${l[2]}`:"unknown",f={...s,location:u};return this.warn(e,`[HAPPY-PATH] ${t}`,f,n),i}},S=new ce;var M=class{static DEFAULTS={CLAUDE_MEM_MODEL:"claude-sonnet-4-5",CLAUDE_MEM_CONTEXT_OBSERVATIONS:"50",CLAUDE_MEM_WORKER_PORT:"37777",CLAUDE_MEM_WORKER_HOST:"127.0.0.1",CLAUDE_MEM_SKIP_TOOLS:"ListMcpResourcesTool,SlashCommand,Skill,TodoWrite,AskUserQuestion",CLAUDE_MEM_PROVIDER:"claude",CLAUDE_MEM_GEMINI_API_KEY:"",CLAUDE_MEM_GEMINI_MODEL:"gemini-2.5-flash-lite",CLAUDE_MEM_GEMINI_RATE_LIMITING_ENABLED:"true",CLAUDE_MEM_OPENROUTER_API_KEY:"",CLAUDE_MEM_OPENROUTER_MODEL:"xiaomi/mimo-v2-flash:free",CLAUDE_MEM_OPENROUTER_SITE_URL:"",CLAUDE_MEM_OPENROUTER_APP_NAME:"claude-mem",CLAUDE_MEM_OPENROUTER_MAX_CONTEXT_MESSAGES:"20",CLAUDE_MEM_OPENROUTER_MAX_TOKENS:"100000",CLAUDE_MEM_DATA_DIR:(0,Oe.join)((0,Re.homedir)(),".claude-mem"),CLAUDE_MEM_LOG_LEVEL:"INFO",CLAUDE_MEM_PYTHON_VERSION:"3.13",CLAUDE_CODE_PATH:"",CLAUDE_MEM_MODE:"code",CLAUDE_MEM_CONTEXT_SHOW_READ_TOKENS:"true",CLAUDE_MEM_CONTEXT_SHOW_WORK_TOKENS:"true",CLAUDE_MEM_CONTEXT_SHOW_SAVINGS_AMOUNT:"true",CLAUDE_MEM_CONTEXT_SHOW_SAVINGS_PERCENT:"true",CLAUDE_MEM_CONTEXT_OBSERVATION_TYPES:fe,CLAUDE_MEM_CONTEXT_OBSERVATION_CONCEPTS:be,CLAUDE_MEM_CONTEXT_FULL_COUNT:"5",CLAUDE_MEM_CONTEXT_FULL_FIELD:"narrative",CLAUDE_MEM_CONTEXT_SESSION_COUNT:"10",CLAUDE_MEM_CONTEXT_SHOW_LAST_SUMMARY:"true",CLAUDE_MEM_CONTEXT_SHOW_LAST_MESSAGE:"false"};static getAllDefaults(){return{...this.DEFAULTS}}static get(e){return this.DEFAULTS[e]}static getInt(e){let t=this.get(e);return parseInt(t,10)}static getBool(e){return this.get(e)==="true"}static loadFromFile(e){try{if(!(0,B.existsSync)(e))return this.getAllDefaults();let t=(0,B.readFileSync)(e,"utf-8"),s=JSON.parse(t),n=s;if(s.env&&typeof s.env=="object"){n=s.env;try{(0,B.writeFileSync)(e,JSON.stringify(n,null,2),"utf-8"),S.info("SETTINGS","Migrated settings file from nested to flat schema",{settingsPath:e})}catch(a){S.warn("SETTINGS","Failed to auto-migrate settings file",{settingsPath:e},a)}}let i={...this.DEFAULTS};for(let a of Object.keys(this.DEFAULTS))n[a]!==void 0&&(i[a]=n[a]);return i}catch(t){return S.warn("SETTINGS","Failed to load settings, using defaults",{settingsPath:e},t),this.getAllDefaults()}}};var ze={};function Je(){return typeof __dirname<"u"?__dirname:(0,b.dirname)((0,Ie.fileURLToPath)(ze.url))}var Qe=Je(),v=M.get("CLAUDE_MEM_DATA_DIR"),pe=process.env.CLAUDE_CONFIG_DIR||(0,b.join)((0,Ne.homedir)(),".claude"),St=(0,b.join)(v,"archives"),ft=(0,b.join)(v,"logs"),bt=(0,b.join)(v,"trash"),Ot=(0,b.join)(v,"backups"),Rt=(0,b.join)(v,"modes"),Nt=(0,b.join)(v,"settings.json"),Le=(0,b.join)(v,"claude-mem.db"),At=(0,b.join)(v,"vector-db"),It=(0,b.join)(pe,"settings.json"),Lt=(0,b.join)(pe,"commands"),Ct=(0,b.join)(pe,"CLAUDE.md");function Ce(d){(0,Ae.mkdirSync)(d,{recursive:!0})}function Me(){return(0,b.join)(Qe,"..")}var Z=class{db;constructor(e=Le){e!==":memory:"&&Ce(v),this.db=new ve.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable()}initializeSchema(){try{this.db.run(`
    ...
    `).all(...l)}getUserPromptsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:i}=t,a=s==="date_asc"?"ASC":"DESC",c=n?`LIMIT ${n}`:"",p=e.map(()=>"?").join(","),l=[...e],u=i?"AND s.project = ?":"";return i&&l.push(i),this.db.prepare(`
      ...
      ${c}
    `).all(...l)}getTimelineAroundTimestamp(e,t=10,s=10,n){return this.getTimelineAroundObservation(null,e,t,s,n)}getTimelineAroundObservation(e,t,s=10,n=10,i){let a=i?"AND project = ?":"",c=i?[i]:[],p,l;if(e!==null){let E=`
...
`).filter(n=>n.trim()),s="";for(let n=t.length-1;n>=0;n--)try{let i=t[n];if(!i.includes('"type":"assistant"'))continue;let a=JSON.parse(i);if(a.type==="assistant"&&a.message?.content&&Array.isArray(a.message.content)){let c="";for(let p of a.message.content)p.type==="text"&&(c+=p.text);if(c=c.replace(/<system-reminder>[\s\S]*?<\/system-reminder>/g,"").trim(),c){s=c;break}}}catch{continue}return{userMessage:"",assistantMessage:s}}catch(e){return S.failure("WORKER","Failed to extract prior messages from transcript",{transcriptPath:d},e),{userMessage:"",assistantMessage:""}}}async function it(d,e=!1){let t=tt(),s=d?.cwd??process.cwd(),n=xe(s),i=null;try{i=new Z}catch(I){if(I.code==="ERR_DLOPEN_FAILED"){try{(0,W.unlinkSync)(et)}catch{}return console.error("Native module rebuild needed - restart Claude Code to auto-fix"),""}throw I}let a=Array.from(t.observationTypes),c=a.map(()=>"?").join(","),p=Array.from(t.observationConcepts),l=p.map(()=>"?").join(","),u=i.db.prepare(`
  ...
  `).all(n,...a,...p,t.totalObservationCount),f=i.db.prepare(`
    ...
    ORDER BY created_at_epoch DESC
    LIMIT ?
  `).all(n,t.sessionCount+st),O="",E="";if(t.showLastMessage&&u.length>0){let I=d?.session_id,R=u.find(y=>y.sdk_session_id!==I);if(R){let y=R.sdk_session_id,U=rt(s),N=se.default.join((0,re.homedir)(),".claude","projects",U,`${y}.jsonl`),D=nt(N);O=D.userMessage,E=D.assistantMessage}}if(u.length===0&&f.length===0)return i?.close(),e?`
...
No previous sessions found for this project yet.`;let A=f.slice(0,t.sessionCount),g=u,r=[];if(e?(r.push(""),r.push(`${o.bright}${o.cyan}[${n}] recent context${o.reset}`),r.push(`${o.gray}${"\u2500".repeat(60)}${o.reset}`),r.push("")):(r.push(`# [${n}] recent context`),r.push("")),g.length>0){let R=P.getInstance().getActiveMode().observation_types.map(_=>`${_.emoji} ${_.id}`).join(" | ");e?r.push(`${o.dim}Legend: \u{1F3AF} session-request | ${R}${o.reset}`):r.push(`**Legend:** \u{1F3AF} session-request | ${R}`),r.push(""),e?(r.push(`${o.bright}\u{1F4A1} Column Key${o.reset}`),r.push(`${o.dim}  Read: Tokens to read this observation (cost to learn it now)${o.reset}`),r.push(`${o.dim}  Work: Tokens spent on work that produced this record (\u{1F50D} research, \u{1F6E0}\uFE0F building, \u2696\uFE0F  deciding)${o.reset}`)):(r.push("\u{1F4A1} **Column Key**:"),r.push("- **Read**: Tokens to read this observation (cost to learn it now)"),r.push("- **Work**: Tokens spent on work that produced this record (\u{1F50D} research, \u{1F6E0}\uFE0F building, \u2696\uFE0F  deciding)")),r.push(""),e?(r.push(`${o.dim}\u{1F4A1} Context Index: This semantic index (titles, types, files, tokens) is usually sufficient to understand past work.${o.reset}`),r.push(""),r.push(`${o.dim}When you need implementation details, rationale, or debugging context:${o.reset}`),r.push(`${o.dim}  - Use the mem-search skill to fetch full observations on-demand${o.reset}`),r.push(`${o.dim}  - Critical types (\u{1F534} bugfix, \u2696\uFE0F decision) often need detailed fetching${o.reset}`),r.push(`${o.dim}  - Trust this index over re-reading code for past decisions and learnings${o.reset}`)):(r.push("\u{1F4A1} **Context Index:** This semantic index (titles, types, files, tokens) is usually sufficient to understand past work."),r.push(""),r.push("When you need implementation details, rationale, or debugging context:"),r.push("- Use the mem-search skill to fetch full observations on-demand"),r.push("- Critical types (\u{1F534} bugfix, \u2696\uFE0F decision) often need detailed fetching"),r.push("- Trust this index over re-reading code for past decisions and learnings")),r.push("");let y=u.length,U=u.reduce((_,T)=>{let h=(T.title?.length||0)+(T.subtitle?.length||0)+(T.narrative?.length||0)+JSON.stringify(T.facts||[]).length;return _+Math.ceil(h/we)},0),N=u.reduce((_,T)=>_+(T.discovery_tokens||0),0),D=N-U,Y=N>0?Math.round(D/N*100):0,ue=t.showReadTokens||t.showWorkTokens||t.showSavingsAmount||t.showSavingsPercent;if(ue)if(e){if(r.push(`${o.bright}${o.cyan}\u{1F4CA} Context Economics${o.reset}`),r.push(`${o.dim}  Loading: ${y} observations (${U.toLocaleString()} tokens to read)${o.reset}`),r.push(`${o.dim}  Work investment: ${N.toLocaleString()} tokens spent on research, building, and decisions${o.reset}`),N>0&&(t.showSavingsAmount||t.showSavingsPercent)){let _="  Your savings: ";t.showSavingsAmount&&t.showSavingsPercent?_+=`${D.toLocaleString()} tokens (${Y}% reduction from reuse)`:t.showSavingsAmount?_+=`${D.toLocaleString()} tokens`:_+=`${Y}% reduction from reuse`,r.push(`${o.green}${_}${o.reset}`)}r.push("")}else{if(r.push("\u{1F4CA} **Context Economics**:"),r.push(`- Loading: ${y} observations (${U.toLocaleString()} tokens to read)`),r.push(`- Work investment: ${N.toLocaleString()} tokens spent on research, building, and decisions`),N>0&&(t.showSavingsAmount||t.showSavingsPercent)){let _="- Your savings: ";t.showSavingsAmount&&t.showSavingsPercent?_+=`${D.toLocaleString()} tokens (${Y}% reduction from reuse)`:t.showSavingsAmount?_+=`${D.toLocaleString()} tokens`:_+=`${Y}% reduction from reuse`,r.push(_)}r.push("")}let Fe=f[0]?.id,Pe=A.map((_,T)=>{let h=T===0?null:f[T+1];return{..._,displayEpoch:h?h.created_at_epoch:_.created_at_epoch,displayTime:h?h.created_at:_.created_at,shouldShowLink:_.id!==Fe}}),Xe=new Set(u.slice(0,t.fullObservationCount).map(_=>_.id)),Ee=[...g.map(_=>({type:"observation",data:_})),...Pe.map(_=>({type:"summary",data:_}))];Ee.sort((_,T)=>{let h=_.type==="observation"?_.data.created_at_epoch:_.data.displayEpoch,k=T.type==="observation"?T.data.created_at_epoch:T.data.displayEpoch;return h-k});let V=new Map;for(let _ of Ee){let T=_.type==="observation"?_.data.created_at:_.data.displayTime,h=ke(T);V.has(h)||V.set(h,[]),V.get(h).push(_)}let je=Array.from(V.entries()).sort((_,T)=>{let h=new Date(_[0]).getTime(),k=new Date(T[0]).getTime();return h-k});for(let[_,T]of je){e?(r.push(`${o.bright}${o.cyan}${_}${o.reset}`),r.push("")):(r.push(`### ${_}`),r.push(""));let h=null,k="",x=!1;for(let ne of T)if(ne.type==="summary"){x&&(r.push(""),x=!1,h=null,k="");let m=ne.data,w=`${m.request||"Session started"} (${De(m.displayTime)})`;e?r.push(`\u{1F3AF} ${o.yellow}#S${m.id}${o.reset} ${w}`):r.push(`**\u{1F3AF} #S${m.id}** ${w}`),r.push("")}else{let m=ne.data,w=$e(m.files_modified,s);w!==h&&(x&&r.push(""),e?r.push(`${o.dim}${w}${o.reset}`):r.push(`**${w}**`),e||(r.push("| ID | Time | T | Title | Read | Work |"),r.push("|----|------|---|-------|------|------|")),h=w,x=!0,k="");let F=ye(m.created_at),K=m.title||"Untitled",q=P.getInstance().getTypeIcon(m.type),Ge=(m.title?.length||0)+(m.subtitle?.length||0)+(m.narrative?.length||0)+JSON.stringify(m.facts||[]).length,X=Math.ceil(Ge/we),j=m.discovery_tokens||0,ie=P.getInstance().getWorkEmoji(m.type),ge=j>0?`${ie} ${j.toLocaleString()}`:"-",oe=F!==k,Te=oe?F:"";if(k=F,Xe.has(m.id)){let $=t.fullObservationField==="narrative"?m.narrative:m.facts?le(m.facts).join(`
`):null;if(e){let C=oe?`${o.dim}${F}${o.reset}`:" ".repeat(F.length),J=t.showReadTokens&&X>0?`${o.dim}(~${X}t)${o.reset}`:"",he=t.showWorkTokens&&j>0?`${o.dim}(${ie} ${j.toLocaleString()}t)${o.reset}`:"";r.push(`  ${o.dim}#${m.id}${o.reset}  ${C}  ${q}  ${o.bright}${K}${o.reset}`),$&&r.push(`    ${o.dim}${$}${o.reset}`),(J||he)&&r.push(`    ${J} ${he}`),r.push("")}else{x&&(r.push(""),x=!1),r.push(`**#${m.id}** ${Te||"\u2033"} ${q} **${K}**`),$&&(r.push(""),r.push($),r.push(""));let C=[];t.showReadTokens&&C.push(`Read: ~${X}`),t.showWorkTokens&&C.push(`Work: ${ge}`),C.length>0&&r.push(C.join(", ")),r.push(""),h=null}}else if(e){let $=oe?`${o.dim}${F}${o.reset}`:" ".repeat(F.length),C=t.showReadTokens&&X>0?`${o.dim}(~${X}t)${o.reset}`:"",J=t.showWorkTokens&&j>0?`${o.dim}(${ie} ${j.toLocaleString()}t)${o.reset}`:"";r.push(`  ${o.dim}#${m.id}${o.reset}  ${$}  ${q}  ${K} ${C} ${J}`)}else{let $=t.showReadTokens?`~${X}`:"",C=t.showWorkTokens?ge:"";r.push(`| #${m.id} | ${Te||"\u2033"} | ${q} | ${K} | ${$} | ${C} |`)}}x&&r.push("")}let L=f[0],me=u[0];if(t.showLastSummary&&L&&(L.investigated||L.learned||L.completed||L.next_steps)&&(!me||L.created_at_epoch>me.created_at_epoch)&&(r.push(...te("Investigated",L.investigated,o.blue,e)),r.push(...te("Learned",L.learned,o.yellow,e)),r.push(...te("Completed",L.completed,o.green,e)),r.push(...te("Next Steps",L.next_steps,o.magenta,e))),E&&(r.push(""),r.push("---"),r.push(""),e?(r.push(`${o.bright}${o.magenta}\u{1F4CB} Previously${o.reset}`),r.push(""),r.push(`${o.dim}A: ${E}${o.reset}`)):(r.push("**\u{1F4CB} Previously**"),r.push(""),r.push(`A: ${E}`)),r.push("")),ue&&N>0&&D>0){let _=Math.round(N/1e3);r.push(""),e?r.push(`${o.dim}\u{1F4B0} Access ${_}k tokens of past research & decisions for just ${U.toLocaleString()}t. Use the mem-search skill to access memories by ID instead of re-reading files.${o.reset}`):r.push(`\u{1F4B0} Access ${_}k tokens of past research & decisions for just ${U.toLocaleString()}t. Use the mem-search skill to access memories by ID instead of re-reading files.`)}}return i?.close(),r.join(`
