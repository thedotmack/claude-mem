...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),u=l?`${l[1].split("/").pop()}:${l[2]}`:"unknown",f={...s,location:u};return this.warn(e,`[HAPPY-PATH] ${t}`,f,n),i}},S=new ae;var $=class{static DEFAULTS={CLAUDE_MEM_MODEL:"claude-sonnet-4-5",CLAUDE_MEM_CONTEXT_OBSERVATIONS:"50",CLAUDE_MEM_WORKER_PORT:"37777",CLAUDE_MEM_WORKER_HOST:"127.0.0.1",CLAUDE_MEM_SKIP_TOOLS:"ListMcpResourcesTool,SlashCommand,Skill,TodoWrite,AskUserQuestion",CLAUDE_MEM_PROVIDER:"claude",CLAUDE_MEM_GEMINI_API_KEY:"",CLAUDE_MEM_GEMINI_MODEL:"gemini-2.5-flash-lite",CLAUDE_MEM_GEMINI_RATE_LIMITING_ENABLED:"true",CLAUDE_MEM_OPENROUTER_API_KEY:"",CLAUDE_MEM_OPENROUTER_MODEL:"xiaomi/mimo-v2-flash:free",CLAUDE_MEM_OPENROUTER_SITE_URL:"",CLAUDE_MEM_OPENROUTER_APP_NAME:"claude-mem",CLAUDE_MEM_OPENROUTER_MAX_CONTEXT_MESSAGES:"20",CLAUDE_MEM_OPENROUTER_MAX_TOKENS:"100000",CLAUDE_MEM_DATA_DIR:(0,fe.join)((0,be.homedir)(),".claude-mem"),CLAUDE_MEM_LOG_LEVEL:"INFO",CLAUDE_MEM_PYTHON_VERSION:"3.13",CLAUDE_CODE_PATH:"",CLAUDE_MEM_MODE:"code",CLAUDE_MEM_CONTEXT_SHOW_READ_TOKENS:"true",CLAUDE_MEM_CONTEXT_SHOW_WORK_TOKENS:"true",CLAUDE_MEM_CONTEXT_SHOW_SAVINGS_AMOUNT:"true",CLAUDE_MEM_CONTEXT_SHOW_SAVINGS_PERCENT:"true",CLAUDE_MEM_CONTEXT_OBSERVATION_TYPES:he,CLAUDE_MEM_CONTEXT_OBSERVATION_CONCEPTS:Se,CLAUDE_MEM_CONTEXT_FULL_COUNT:"5",CLAUDE_MEM_CONTEXT_FULL_FIELD:"narrative",CLAUDE_MEM_CONTEXT_SESSION_COUNT:"10",CLAUDE_MEM_CONTEXT_SHOW_LAST_SUMMARY:"true",CLAUDE_MEM_CONTEXT_SHOW_LAST_MESSAGE:"false"};static getAllDefaults(){return{...this.DEFAULTS}}static get(e){return this.DEFAULTS[e]}static getInt(e){let t=this.get(e);return parseInt(t,10)}static getBool(e){return this.get(e)==="true"}static loadFromFile(e){try{if(!(0,G.existsSync)(e))return this.getAllDefaults();let t=(0,G.readFileSync)(e,"utf-8"),s=JSON.parse(t),n=s;if(s.env&&typeof s.env=="object"){n=s.env;try{(0,G.writeFileSync)(e,JSON.stringify(n,null,2),"utf-8"),S.info("SETTINGS","Migrated settings file from nested to flat schema",{settingsPath:e})}catch(a){S.warn("SETTINGS","Failed to auto-migrate settings file",{settingsPath:e},a)}}let i={...this.DEFAULTS};for(let a of Object.keys(this.DEFAULTS))n[a]!==void 0&&(i[a]=n[a]);return i}catch(t){return S.warn("SETTINGS","Failed to load settings, using defaults",{settingsPath:e},t),this.getAllDefaults()}}};var Je={};function Ke(){return typeof __dirname<"u"?__dirname:(0,b.dirname)((0,Ne.fileURLToPath)(Je.url))}var qe=Ke(),M=$.get("CLAUDE_MEM_DATA_DIR"),de=process.env.CLAUDE_CONFIG_DIR||(0,b.join)((0,Oe.homedir)(),".claude"),gt=(0,b.join)(M,"archives"),ht=(0,b.join)(M,"logs"),St=(0,b.join)(M,"trash"),ft=(0,b.join)(M,"backups"),bt=(0,b.join)(M,"modes"),Ot=(0,b.join)(M,"settings.json"),Ae=(0,b.join)(M,"claude-mem.db"),Rt=(0,b.join)(M,"vector-db"),Nt=(0,b.join)(de,"settings.json"),At=(0,b.join)(de,"commands"),It=(0,b.join)(de,"CLAUDE.md");function Ie(d){(0,Re.mkdirSync)(d,{recursive:!0})}function Le(){return(0,b.join)(qe,"..")}var Q=class{db;constructor(e=Ae){e!==":memory:"&&Ie(M),this.db=new Ce.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable()}initializeSchema(){try{this.db.run(`
    ...
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,i,p,c);return{id:Number(u.lastInsertRowid),createdAtEpoch:c}}getSessionSummariesByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:i}=t,a=s==="date_asc"?"ASC":"DESC",c=n?`LIMIT ${n}`:"",p=e.map(()=>"?").join(","),l=[...e],u=i?`WHERE id IN (${p}) AND project = ?`:`WHERE id IN (${p})`;return i&&l.push(i),this.db.prepare(`
      ...
      ${c}
    `).all(...l)}getUserPromptsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:i}=t,a=s==="date_asc"?"ASC":"DESC",c=n?`LIMIT ${n}`:"",p=e.map(()=>"?").join(","),l=[...e],u=i?"AND s.project = ?":"";return i&&l.push(i),this.db.prepare(`
      SELECT
        up.*,
        s.project,
        s.sdk_session_id
    ...
    `).all(...l)}getTimelineAroundTimestamp(e,t=10,s=10,n){return this.getTimelineAroundObservation(null,e,t,s,n)}getTimelineAroundObservation(e,t,s=10,n=10,i){let a=i?"AND project = ?":"",c=i?[i]:[],p,l;if(e!==null){let m=`
      ...
      `,A=`
        ...
        ORDER BY created_at_epoch ASC
        LIMIT ?
      `;try{let T=this.db.prepare(m).all(t,...c,s),r=this.db.prepare(A).all(t,...c,n+1);if(T.length===0&&r.length===0)return{observations:[],sessions:[],prompts:[]};p=T.length>0?T[T.length-1].created_at_epoch:t,l=r.length>0?r[r.length-1].created_at_epoch:t}catch(T){return console.error("[SessionStore] Error getting boundary timestamps:",T.message,i?`(project: ${i})`:"(all projects)"),{observations:[],sessions:[],prompts:[]}}}let u=`
      SELECT *
      FROM observations
...
`).filter(n=>n.trim()),s="";for(let n=t.length-1;n>=0;n--)try{let i=t[n];if(!i.includes('"type":"assistant"'))continue;let a=JSON.parse(i);if(a.type==="assistant"&&a.message?.content&&Array.isArray(a.message.content)){let c="";for(let p of a.message.content)p.type==="text"&&(c+=p.text);if(c=c.replace(/<system-reminder>[\s\S]*?<\/system-reminder>/g,"").trim(),c){s=c;break}}}catch{continue}return{userMessage:"",assistantMessage:s}}catch(e){return S.failure("WORKER","Failed to extract prior messages from transcript",{transcriptPath:d},e),{userMessage:"",assistantMessage:""}}}async function rt(d,e=!1){let t=Ze(),s=d?.cwd??process.cwd(),n=$e(s),i=null;try{i=new Q}catch(I){if(I.code==="ERR_DLOPEN_FAILED"){try{(0,B.unlinkSync)(ze)}catch{}return console.error("Native module rebuild needed - restart Claude Code to auto-fix"),""}throw I}let a=Array.from(t.observationTypes),c=a.map(()=>"?").join(","),p=Array.from(t.observationConcepts),l=p.map(()=>"?").join(","),u=i.db.prepare(`
  ...
  `).all(n,...a,...p,t.totalObservationCount),f=i.db.prepare(`
    ...
    ORDER BY created_at_epoch DESC
    LIMIT ?
  `).all(n,t.sessionCount+et),O="",m="";if(t.showLastMessage&&u.length>0){let I=d?.session_id,R=u.find(D=>D.sdk_session_id!==I);if(R){let D=R.sdk_session_id,U=tt(s),N=ee.default.join((0,te.homedir)(),".claude","projects",U,`${D}.jsonl`),v=st(N);O=v.userMessage,m=v.assistantMessage}}if(u.length===0&&f.length===0)return i?.close(),e?`
...
No previous sessions found for this project yet.`;let A=f.slice(0,t.sessionCount),T=u,r=[];if(e?(r.push(""),r.push(`${o.bright}${o.cyan}[${n}] recent context${o.reset}`),r.push(`${o.gray}${"\u2500".repeat(60)}${o.reset}`),r.push("")):(r.push(`# [${n}] recent context`),r.push("")),T.length>0){let R=P.getInstance().getActiveMode().observation_types.map(_=>`${_.emoji} ${_.id}`).join(" | ");e?r.push(`${o.dim}Legend: \u{1F3AF} session-request | ${R}${o.reset}`):r.push(`**Legend:** \u{1F3AF} session-request | ${R}`),r.push(""),e?(r.push(`${o.bright}\u{1F4A1} Column Key${o.reset}`),r.push(`${o.dim}  Read: Tokens to read this observation (cost to learn it now)${o.reset}`),r.push(`${o.dim}  Work: Tokens spent on work that produced this record (\u{1F50D} research, \u{1F6E0}\uFE0F building, \u2696\uFE0F  deciding)${o.reset}`)):(r.push("\u{1F4A1} **Column Key**:"),r.push("- **Read**: Tokens to read this observation (cost to learn it now)"),r.push("- **Work**: Tokens spent on work that produced this record (\u{1F50D} research, \u{1F6E0}\uFE0F building, \u2696\uFE0F  deciding)")),r.push(""),e?(r.push(`${o.dim}\u{1F4A1} Context Index: This semantic index (titles, types, files, tokens) is usually sufficient to understand past work.${o.reset}`),r.push(""),r.push(`${o.dim}When you need implementation details, rationale, or debugging context:${o.reset}`),r.push(`${o.dim}  - Use the mem-search skill to fetch full observations on-demand${o.reset}`),r.push(`${o.dim}  - Critical types (\u{1F534} bugfix, \u2696\uFE0F decision) often need detailed fetching${o.reset}`),r.push(`${o.dim}  - Trust this index over re-reading code for past decisions and learnings${o.reset}`)):(r.push("\u{1F4A1} **Context Index:** This semantic index (titles, types, files, tokens) is usually sufficient to understand past work."),r.push(""),r.push("When you need implementation details, rationale, or debugging context:"),r.push("- Use the mem-search skill to fetch full observations on-demand"),r.push("- Critical types (\u{1F534} bugfix, \u2696\uFE0F decision) often need detailed fetching"),r.push("- Trust this index over re-reading code for past decisions and learnings")),r.push("");let D=u.length,U=u.reduce((_,g)=>{let h=(g.title?.length||0)+(g.subtitle?.length||0)+(g.narrative?.length||0)+JSON.stringify(g.facts||[]).length;return _+Math.ceil(h/Ue)},0),N=u.reduce((_,g)=>_+(g.discovery_tokens||0),0),v=N-U,H=N>0?Math.round(v/N*100):0,_e=t.showReadTokens||t.showWorkTokens||t.showSavingsAmount||t.showSavingsPercent;if(_e)if(e){if(r.push(`${o.bright}${o.cyan}\u{1F4CA} Context Economics${o.reset}`),r.push(`${o.dim}  Loading: ${D} observations (${U.toLocaleString()} tokens to read)${o.reset}`),r.push(`${o.dim}  Work investment: ${N.toLocaleString()} tokens spent on research, building, and decisions${o.reset}`),N>0&&(t.showSavingsAmount||t.showSavingsPercent)){let _="  Your savings: ";t.showSavingsAmount&&t.showSavingsPercent?_+=`${v.toLocaleString()} tokens (${H}% reduction from reuse)`:t.showSavingsAmount?_+=`${v.toLocaleString()} tokens`:_+=`${H}% reduction from reuse`,r.push(`${o.green}${_}${o.reset}`)}r.push("")}else{if(r.push("\u{1F4CA} **Context Economics**:"),r.push(`- Loading: ${D} observations (${U.toLocaleString()} tokens to read)`),r.push(`- Work investment: ${N.toLocaleString()} tokens spent on research, building, and decisions`),N>0&&(t.showSavingsAmount||t.showSavingsPercent)){let _="- Your savings: ";t.showSavingsAmount&&t.showSavingsPercent?_+=`${v.toLocaleString()} tokens (${H}% reduction from reuse)`:t.showSavingsAmount?_+=`${v.toLocaleString()} tokens`:_+=`${H}% reduction from reuse`,r.push(_)}r.push("")}let xe=f[0]?.id,we=A.map((_,g)=>{let h=g===0?null:f[g+1];return{..._,displayEpoch:h?h.created_at_epoch:_.created_at_epoch,displayTime:h?h.created_at:_.created_at,shouldShowLink:_.id!==xe}}),Fe=new Set(u.slice(0,t.fullObservationCount).map(_=>_.id)),le=[...T.map(_=>({type:"observation",data:_})),...we.map(_=>({type:"summary",data:_}))];le.sort((_,g)=>{let h=_.type==="observation"?_.data.created_at_epoch:_.data.displayEpoch,y=g.type==="observation"?g.data.created_at_epoch:g.data.displayEpoch;return h-y});let Y=new Map;for(let _ of le){let g=_.type==="observation"?_.data.created_at:_.data.displayTime,h=De(g);Y.has(h)||Y.set(h,[]),Y.get(h).push(_)}let Pe=Array.from(Y.entries()).sort((_,g)=>{let h=new Date(_[0]).getTime(),y=new Date(g[0]).getTime();return h-y});for(let[_,g]of Pe){e?(r.push(`${o.bright}${o.cyan}${_}${o.reset}`),r.push("")):(r.push(`### ${_}`),r.push(""));let h=null,y="",x=!1;for(let se of g)if(se.type==="summary"){x&&(r.push(""),x=!1,h=null,y="");let E=se.data,w=`${E.request||"Session started"} (${Me(E.displayTime)})`;e?r.push(`\u{1F3AF} ${o.yellow}#S${E.id}${o.reset} ${w}`):r.push(`**\u{1F3AF} #S${E.id}** ${w}`),r.push("")}else{let E=se.data,w=ye(E.files_modified,s);w!==h&&(x&&r.push(""),e?r.push(`${o.dim}${w}${o.reset}`):r.push(`**${w}**`),e||(r.push("| ID | Time | T | Title | Read | Work |"),r.push("|----|------|---|-------|------|------|")),h=w,x=!0,y="");let F=ve(E.created_at),V=E.title||"Untitled",K=P.getInstance().getTypeIcon(E.type),Xe=(E.title?.length||0)+(E.subtitle?.length||0)+(E.narrative?.length||0)+JSON.stringify(E.facts||[]).length,X=Math.ceil(Xe/Ue),j=E.discovery_tokens||0,re=P.getInstance().getWorkEmoji(E.type),Ee=j>0?`${re} ${j.toLocaleString()}`:"-",ne=F!==y,me=ne?F:"";if(y=F,Fe.has(E.id)){let k=t.fullObservationField==="narrative"?E.narrative:E.facts?pe(E.facts).join(`
`):null;if(e){let C=ne?`${o.dim}${F}${o.reset}`:" ".repeat(F.length),q=t.showReadTokens&&X>0?`${o.dim}(~${X}t)${o.reset}`:"",Te=t.showWorkTokens&&j>0?`${o.dim}(${re} ${j.toLocaleString()}t)${o.reset}`:"";r.push(`  ${o.dim}#${E.id}${o.reset}  ${C}  ${K}  ${o.bright}${V}${o.reset}`),k&&r.push(`    ${o.dim}${k}${o.reset}`),(q||Te)&&r.push(`    ${q} ${Te}`),r.push("")}else{x&&(r.push(""),x=!1),r.push(`**#${E.id}** ${me||"\u2033"} ${K} **${V}**`),k&&(r.push(""),r.push(k),r.push(""));let C=[];t.showReadTokens&&C.push(`Read: ~${X}`),t.showWorkTokens&&C.push(`Work: ${Ee}`),C.length>0&&r.push(C.join(", ")),r.push(""),h=null}}else if(e){let k=ne?`${o.dim}${F}${o.reset}`:" ".repeat(F.length),C=t.showReadTokens&&X>0?`${o.dim}(~${X}t)${o.reset}`:"",q=t.showWorkTokens&&j>0?`${o.dim}(${re} ${j.toLocaleString()}t)${o.reset}`:"";r.push(`  ${o.dim}#${E.id}${o.reset}  ${k}  ${K}  ${V} ${C} ${q}`)}else{let k=t.showReadTokens?`~${X}`:"",C=t.showWorkTokens?Ee:"";r.push(`| #${E.id} | ${me||"\u2033"} | ${K} | ${V} | ${k} | ${C} |`)}}x&&r.push("")}let L=f[0],ue=u[0];if(t.showLastSummary&&L&&(L.investigated||L.learned||L.completed||L.next_steps)&&(!ue||L.created_at_epoch>ue.created_at_epoch)&&(r.push(...Z("Investigated",L.investigated,o.blue,e)),r.push(...Z("Learned",L.learned,o.yellow,e)),r.push(...Z("Completed",L.completed,o.green,e)),r.push(...Z("Next Steps",L.next_steps,o.magenta,e))),m&&(r.push(""),r.push("---"),r.push(""),e?(r.push(`${o.bright}${o.magenta}\u{1F4CB} Previously${o.reset}`),r.push(""),r.push(`${o.dim}A: ${m}${o.reset}`)):(r.push("**\u{1F4CB} Previously**"),r.push(""),r.push(`A: ${m}`)),r.push("")),_e&&N>0&&v>0){let _=Math.round(N/1e3);r.push(""),e?r.push(`${o.dim}\u{1F4B0} Access ${_}k tokens of past research & decisions for just ${U.toLocaleString()}t. Use the mem-search skill to access memories by ID instead of re-reading files.${o.reset}`):r.push(`\u{1F4B0} Access ${_}k tokens of past research & decisions for just ${U.toLocaleString()}t. Use the mem-search skill to access memories by ID instead of re-reading files.`)}}return i?.close(),r.join(`
