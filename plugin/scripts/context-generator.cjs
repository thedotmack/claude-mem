...
`+JSON.stringify(o,null,2):l=" "+this.formatData(o));let _="";if(n){let{sessionId:E,memorySessionId:g,correlationId:h,...m}=n;Object.keys(m).length>0&&(_=` {${Object.entries(m).map(([f,O])=>`${f}=${O}`).join(", ")}}`)}let T=`[${i}] [${a}] [${d}] ${c}${s}${_}${l}`;if(this.logFilePath)try{(0,C.appendFileSync)(this.logFilePath,T+`
...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),l=c?`${c[1].split("/").pop()}:${c[2]}`:"unknown",_={...s,location:l};return this.warn(e,`[HAPPY-PATH] ${t}`,_,n),o}},u=new W;var Ot={};function ht(){return typeof __dirname<"u"?__dirname:(0,S.dirname)((0,ce.fileURLToPath)(Ot.url))}var Nt=ht(),R=L.get("CLAUDE_MEM_DATA_DIR"),H=process.env.CLAUDE_CONFIG_DIR||(0,S.join)((0,de.homedir)(),".claude"),Wt=(0,S.join)(R,"archives"),Ht=(0,S.join)(R,"logs"),Yt=(0,S.join)(R,"trash"),Vt=(0,S.join)(R,"backups"),Kt=(0,S.join)(R,"modes"),qt=(0,S.join)(R,"settings.json"),ue=(0,S.join)(R,"claude-mem.db"),Jt=(0,S.join)(R,"vector-db"),zt=(0,S.join)(R,"observer-sessions"),Qt=(0,S.join)(H,"settings.json"),Zt=(0,S.join)(H,"commands"),es=(0,S.join)(H,"CLAUDE.md");function le(r){(0,pe.mkdirSync)(r,{recursive:!0})}function _e(){return(0,S.join)(Nt,"..")}var $=class{db;constructor(e=ue){e!==":memory:"&&le(R),this.db=new me.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addSupersessionTrainingTables(),this.addHandoffObservationType()}initializeSchema(){this.db.run(`
    ...
    `),this.db.run("CREATE INDEX IF NOT EXISTS idx_learned_model_weights_trained ON learned_model_weights(trained_at_epoch DESC)"),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(21,new Date().toISOString()),u.debug("DB","Successfully added Supersession Training tables")}addHandoffObservationType(){if(this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(22))return;let t=this.db.query("PRAGMA table_info(observations)").all();if(this.db.query("SELECT sql FROM sqlite_master WHERE type='table' AND name='observations'").get()?.sql?.includes("'handoff'")){this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(22,new Date().toISOString());return}u.debug("DB","Adding handoff observation type for PreCompact continuity"),this.db.run("BEGIN TRANSACTION");try{let n=t.map(i=>i.name).filter(i=>i!=="id");this.db.run(`
        CREATE TABLE observations_new (
          id INTEGER PRIMARY KEY AUTOINCREMENT,
          memory_session_id TEXT NOT NULL,
          project TEXT NOT NULL,
      ...
      `),this.db.run(`
        CREATE TRIGGER observations_au AFTER UPDATE ON observations BEGIN
          ...
          VALUES (NEW.id, NEW.title, NEW.subtitle, NEW.narrative, NEW.text, NEW.facts, NEW.concepts);
        END
      `),this.db.run("COMMIT"),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(22,new Date().toISOString()),u.debug("DB","Successfully added handoff observation type")}catch(n){throw this.db.run("ROLLBACK"),n}}updateMemorySessionId(e,t){this.db.prepare(`
      UPDATE sdk_sessions
    ...
    `).all(e)}getObservationById(e){return this.db.prepare(`
      ...
      FROM observations
      WHERE id = ?
    `).get(e)||null}getObservationsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o,type:i,concepts:a,files:d}=t,c=s==="date_asc"?"ASC":"DESC",l=n?`LIMIT ${n}`:"",_=e.map(()=>"?").join(","),T=[...e],E=[];if(o&&(E.push("project = ?"),T.push(o)),i)if(Array.isArray(i)){let m=i.map(()=>"?").join(",");E.push(`type IN (${m})`),T.push(...i)}else E.push("type = ?"),T.push(i);if(a){let m=Array.isArray(a)?a:[a],b=m.map(()=>"EXISTS (SELECT 1 FROM json_each(concepts) WHERE value = ?)");T.push(...m),E.push(`(${b.join(" OR ")})`)}if(d){let m=Array.isArray(d)?d:[d],b=m.map(()=>"(EXISTS (SELECT 1 FROM json_each(files_read) WHERE value LIKE ?) OR EXISTS (SELECT 1 FROM json_each(files_modified) WHERE value LIKE ?))");m.forEach(f=>{T.push(`%${f}%`,`%${f}%`)}),E.push(`(${b.join(" OR ")})`)}let g=E.length>0?`WHERE id IN (${_}) AND ${E.join(" AND ")}`:`WHERE id IN (${_})`;return this.db.prepare(`
      SELECT *
      FROM observations
      ${g}
    ...
    `).run(e,t,s.type,s.title,s.subtitle,JSON.stringify(s.facts),s.narrative,JSON.stringify(s.concepts),JSON.stringify(s.files_read),JSON.stringify(s.files_modified),n||null,o,d,a);return{id:Number(l.lastInsertRowid),createdAtEpoch:a}}storeSummary(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString(),l=this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,d,a);return{id:Number(l.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a){let d=a??Date.now(),c=new Date(d).toISOString();return this.db.transaction(()=>{let _=[],T=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let g of s){let h=T.run(e,t,g.type,g.title,g.subtitle,JSON.stringify(g.facts),g.narrative,JSON.stringify(g.concepts),JSON.stringify(g.files_read),JSON.stringify(g.files_modified),o||null,i,c,d);_.push(Number(h.lastInsertRowid))}let E=null;if(n){let h=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,o||null,i,c,d);E=Number(h.lastInsertRowid)}return{observationIds:_,summaryId:E,createdAtEpoch:d}})()}storeObservationsAndMarkComplete(e,t,s,n,o,i,a,d=0,c){let l=c??Date.now(),_=new Date(l).toISOString();return this.db.transaction(()=>{let E=[],g=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let b of s){let f=g.run(e,t,b.type,b.title,b.subtitle,JSON.stringify(b.facts),b.narrative,JSON.stringify(b.concepts),JSON.stringify(b.files_read),JSON.stringify(b.files_modified),a||null,d,_,l);E.push(Number(f.lastInsertRowid))}let h;if(n){let f=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,a||null,d,_,l);h=Number(f.lastInsertRowid)}return this.db.prepare(`
