...
`+JSON.stringify(o,null,2):_=" "+this.formatData(o));let l="";if(n){let{sessionId:g,memorySessionId:S,correlationId:b,...p}=n;Object.keys(p).length>0&&(l=` {${Object.entries(p).map(([T,f])=>`${T}=${f}`).join(", ")}}`)}let E=`[${i}] [${a}] [${d}] ${c}${s}${l}${_}`;if(this.logFilePath)try{(0,L.appendFileSync)(this.logFilePath,E+`
...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),_=c?`${c[1].split("/").pop()}:${c[2]}`:"unknown",l={...s,location:_};return this.warn(e,`[HAPPY-PATH] ${t}`,l,n),o}},m=new ee;var Ft={};function xt(){return typeof __dirname<"u"?__dirname:(0,h.dirname)((0,he.fileURLToPath)(Ft.url))}var kt=xt();function $t(){if(process.env.CLAUDE_MEM_DATA_DIR)return process.env.CLAUDE_MEM_DATA_DIR;let r=(0,h.join)((0,te.homedir)(),".claude-mem"),e=(0,h.join)(r,"settings.json");try{if((0,H.existsSync)(e)){let{readFileSync:t}=require("fs"),s=JSON.parse(t(e,"utf-8")),n=s.env??s;if(n.CLAUDE_MEM_DATA_DIR)return n.CLAUDE_MEM_DATA_DIR}}catch{}return r}var C=$t(),D=process.env.CLAUDE_CONFIG_DIR||(0,h.join)((0,te.homedir)(),".claude"),ns=(0,h.join)(D,"plugins","marketplaces","thedotmack"),os=(0,h.join)(C,"archives"),is=(0,h.join)(C,"logs"),as=(0,h.join)(C,"trash"),ds=(0,h.join)(C,"backups"),us=(0,h.join)(C,"modes"),cs=(0,h.join)(C,"settings.json"),be=(0,h.join)(C,"claude-mem.db"),_s=(0,h.join)(C,"vector-db"),ms=(0,h.join)(C,"observer-sessions"),ls=(0,h.join)(D,"settings.json"),ps=(0,h.join)(D,"commands"),Es=(0,h.join)(D,"CLAUDE.md");function Ae(r){(0,H.mkdirSync)(r,{recursive:!0})}function Oe(){return(0,h.join)(kt,"..")}var Re=require("crypto");var wt=3e4;function j(r,e,t){return(0,Re.createHash)("sha256").update([r||"",e||"",t||""].join("\0")).digest("hex").slice(0,16)}function G(r,e,t,s){let n=t-wt;return r.prepare("SELECT id, created_at_epoch FROM observations WHERE content_hash = ? AND created_at_epoch > ? AND node IS ?").get(e,n,s??null)}function se(r){if(!r)return[];try{let e=JSON.parse(r);return Array.isArray(e)?e:[String(e)]}catch{return[r]}}var A="claude";function Pt(r){return r.trim().toLowerCase().replace(/\s+/g,"-")}function v(r){if(!r)return A;let e=Pt(r);return e?e==="transcript"||e.includes("codex")?"codex":e.includes("cursor")?"cursor":e.includes("claude")?"claude":e:A}function Ne(r){let e=["claude","codex","cursor"];return[...r].sort((t,s)=>{let n=e.indexOf(t),o=e.indexOf(s);return n!==-1||o!==-1?n===-1?1:o===-1?-1:n-o:t.localeCompare(s)})}function Xt(r,e){return{customTitle:r,platformSource:e?v(e):void 0}}var B=class{db;_currentNode=null;_currentPlatform=null;_currentInstance=null;_currentLlmSource=null;setLocalProvenance(e,t,s,n){this._currentNode=e,this._currentPlatform=t,this._currentInstance=s,this._currentLlmSource=n??null}constructor(e=be){e!==":memory:"&&Ae(C),this.db=new Ce.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys(),this.addObservationContentHashColumn(),this.addSessionCustomTitleColumn(),this.addSessionPlatformSourceColumn(),this.addObservationModelColumns(),this.addObservationProvenanceColumns(),this.addSummaryProvenanceColumns()}initializeSchema(){this.db.run(`
    ...
    `),this.db.run("CREATE INDEX IF NOT EXISTS idx_pending_messages_session ON pending_messages(session_db_id)"),this.db.run("CREATE INDEX IF NOT EXISTS idx_pending_messages_status ON pending_messages(status)"),this.db.run("CREATE INDEX IF NOT EXISTS idx_pending_messages_claude_session ON pending_messages(content_session_id)"),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(16,new Date().toISOString()),m.debug("DB","pending_messages table created successfully")}renameSessionIdColumns(){if(this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(17))return;m.debug("DB","Checking session ID columns for semantic clarity rename");let t=0,s=(n,o,i)=>{let a=this.db.query(`PRAGMA table_info(${n})`).all(),d=a.some(_=>_.name===o);return a.some(_=>_.name===i)?!1:d?(this.db.run(`ALTER TABLE ${n} RENAME COLUMN ${o} TO ${i}`),m.debug("DB",`Renamed ${n}.${o} to ${i}`),!0):(m.warn("DB",`Column ${o} not found in ${n}, skipping rename`),!1)};s("sdk_sessions","claude_session_id","content_session_id")&&t++,s("sdk_sessions","sdk_session_id","memory_session_id")&&t++,s("pending_messages","claude_session_id","content_session_id")&&t++,s("observations","sdk_session_id","memory_session_id")&&t++,s("session_summaries","sdk_session_id","memory_session_id")&&t++,s("user_prompts","claude_session_id","content_session_id")&&t++,this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(17,new Date().toISOString()),t>0?m.debug("DB",`Successfully renamed ${t} session ID columns`):m.debug("DB","No session ID column renames needed (already up to date)")}repairSessionIdColumnRename(){this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(19)||this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(19,new Date().toISOString())}addFailedAtEpochColumn(){if(this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(20))return;this.db.query("PRAGMA table_info(pending_messages)").all().some(n=>n.name==="failed_at_epoch")||(this.db.run("ALTER TABLE pending_messages ADD COLUMN failed_at_epoch INTEGER"),m.debug("DB","Added failed_at_epoch column to pending_messages table")),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(20,new Date().toISOString())}addOnUpdateCascadeToForeignKeys(){if(!this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(21)){m.debug("DB","Adding ON UPDATE CASCADE to FK constraints on observations and session_summaries"),this.db.run("PRAGMA foreign_keys = OFF"),this.db.run("BEGIN TRANSACTION");try{this.db.run("DROP TRIGGER IF EXISTS observations_ai"),this.db.run("DROP TRIGGER IF EXISTS observations_ad"),this.db.run("DROP TRIGGER IF EXISTS observations_au"),this.db.run("DROP TABLE IF EXISTS observations_new");let t=new Set(this.db.prepare("PRAGMA table_info(observations)").all().map(p=>p.name)),s=[],n=[];t.has("content_hash")&&(s.push("content_hash TEXT"),n.push("content_hash")),t.has("node")&&(s.push("node TEXT","platform TEXT","instance TEXT"),n.push("node","platform","instance")),t.has("llm_source")&&(s.push("llm_source TEXT"),n.push("llm_source")),t.has("generated_by_model")&&(s.push("generated_by_model TEXT"),n.push("generated_by_model")),t.has("relevance_count")&&(s.push("relevance_count INTEGER DEFAULT 0"),n.push("relevance_count"));let o=s.length>0?`,
      ...
      `),this.db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='observations_fts'").all().length>0&&this.db.run(`
          ...
          CREATE TRIGGER IF NOT EXISTS observations_au AFTER UPDATE ON observations BEGIN
            ...
            VALUES (new.id, new.title, new.subtitle, new.narrative, new.text, new.facts, new.concepts);
          END;
        `),this.db.run("DROP TABLE IF EXISTS session_summaries_new");let c=new Set(this.db.prepare("PRAGMA table_info(session_summaries)").all().map(p=>p.name)),_=[],l=[];c.has("node")&&(_.push("node TEXT","platform TEXT","instance TEXT"),l.push("node","platform","instance")),c.has("llm_source")&&(_.push("llm_source TEXT"),l.push("llm_source"));let E=_.length>0?`,
          ${`,
          `.join(_)}`:"",g="id, memory_session_id, project, request, investigated, learned, completed, next_steps, files_read, files_edited, notes, prompt_number, discovery_tokens, created_at, created_at_epoch",S=l.length>0?`${g}, ${l.join(", ")}`:g;this.db.run(`
        CREATE TABLE session_summaries_new (
          id INTEGER PRIMARY KEY AUTOINCREMENT,
          memory_session_id TEXT NOT NULL,
          project TEXT NOT NULL,
    ...
    `).get(e);if(_){if(t&&this.db.prepare(`
      ...
      VALUES (?, NULL, ?, ?, ?, ?, ?, ?, 'active')
    `).run(e,t,c,s,d.customTitle||null,i.toISOString(),a),this.db.prepare("SELECT id FROM sdk_sessions WHERE content_session_id = ?").get(e).id}saveUserPrompt(e,t,s,n,o,i,a){let d=new Date,c=d.getTime();return this.db.prepare(`
      INSERT INTO user_prompts
      (content_session_id, prompt_number, prompt_text, created_at, created_at_epoch, node, platform, instance, llm_source)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s,d.toISOString(),c,n||(this._currentNode??null),o||(this._currentPlatform??null),i||(this._currentInstance??null),a||(this._currentLlmSource??null)).lastInsertRowid}getUserPrompt(e,t){return this.db.prepare(`
      SELECT prompt_text
      FROM user_prompts
      WHERE content_session_id = ? AND prompt_number = ?
      LIMIT 1
    `).get(e,t)?.prompt_text??null}storeObservation(e,t,s,n,o=0,i,a,d,c,_,l){let E=i??Date.now(),g=new Date(E).toISOString(),S=j(e,s.title,s.narrative),b=G(this.db,S,E,d);if(b)return{id:b.id,createdAtEpoch:b.created_at_epoch};let R=this.db.prepare(`
      INSERT INTO observations
      (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
       files_read, files_modified, prompt_number, discovery_tokens, content_hash, created_at, created_at_epoch,
       generated_by_model, node, platform, instance, llm_source)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.type,s.title,s.subtitle,JSON.stringify(s.facts),s.narrative,JSON.stringify(s.concepts),JSON.stringify(s.files_read),JSON.stringify(s.files_modified),n||null,o,S,g,E,a||null,d||null,c||null,_||null,l||null);return{id:Number(R.lastInsertRowid),createdAtEpoch:E}}storeSummary(e,t,s,n,o=0,i,a,d,c,_){let l=i??Date.now(),E=new Date(l).toISOString(),S=this.db.prepare(`
      INSERT INTO session_summaries
      (memory_session_id, project, request, investigated, learned, completed,
       next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch,
       node, platform, instance, llm_source)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,E,l,a??null,d??null,c??null,_??null);return{id:Number(S.lastInsertRowid),createdAtEpoch:l}}storeObservations(e,t,s,n,o,i=0,a,d,c,_,l,E){let g=a??Date.now(),S=new Date(g).toISOString();return this.db.transaction(()=>{let p=[],R=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, content_hash, created_at, created_at_epoch,
         generated_by_model, node, platform, instance, llm_source)
      ...
      `);for(let f of s){let M=j(e,f.title,f.narrative),Q=G(this.db,M,g,c);if(Q){p.push(Q.id);continue}let N=R.run(e,t,f.type,f.title,f.subtitle,JSON.stringify(f.facts),f.narrative,JSON.stringify(f.concepts),JSON.stringify(f.files_read),JSON.stringify(f.files_modified),o||null,i,M,S,g,d||null,c??null,_??null,l??null,E??null);p.push(Number(N.lastInsertRowid))}let T=null;if(n){let M=this.db.prepare(`
          ...
          (memory_session_id, project, request, investigated, learned, completed,
           ...
           node, platform, instance, llm_source)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,o||null,i,S,g,c??null,_??null,l??null,E??null);T=Number(M.lastInsertRowid)}return{observationIds:p,summaryId:T,createdAtEpoch:g}})()}storeObservationsAndMarkComplete(e,t,s,n,o,i,a,d=0,c,_,l,E,g,S){let b=c??Date.now(),p=new Date(b).toISOString();return this.db.transaction(()=>{let T=[],f=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, content_hash, created_at, created_at_epoch,
         generated_by_model, node, platform, instance, llm_source)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let N of s){let P=j(e,N.title,N.narrative),ge=G(this.db,P,b,l);if(ge){T.push(ge.id);continue}let Ct=f.run(e,t,N.type,N.title,N.subtitle,JSON.stringify(N.facts),N.narrative,JSON.stringify(N.concepts),JSON.stringify(N.files_read),JSON.stringify(N.files_modified),a||null,d,P,p,b,_||null,l??null,E??null,g??null,S??null);T.push(Number(Ct.lastInsertRowid))}let M;if(n){let P=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch,
           node, platform, instance, llm_source)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,a||null,d,p,b,l??null,E??null,g??null,S??null);M=Number(P.lastInsertRowid)}return this.db.prepare(`
    ...
    `).all(...c)}getTimelineAroundTimestamp(e,t=10,s=10,n){return this.getTimelineAroundObservation(null,e,t,s,n)}getTimelineAroundObservation(e,t,s=10,n=10,o){let i=o?"AND project = ?":"",a=o?[o]:[],d,c;if(e!==null){let p=`
      ...
      `,R=`
        ...
        ORDER BY id ASC
        LIMIT ?
      `;try{let T=this.db.prepare(p).all(e,...a,s+1),f=this.db.prepare(R).all(e,...a,n+1);if(T.length===0&&f.length===0)return{observations:[],sessions:[],prompts:[]};d=T.length>0?T[T.length-1].created_at_epoch:t,c=f.length>0?f[f.length-1].created_at_epoch:t}catch(T){return m.error("DB","Error getting boundary observations",void 0,{error:T,project:o}),{observations:[],sessions:[],prompts:[]}}}else{let p=`
      ...
      `,R=`
        ...
        ORDER BY created_at_epoch ASC
        LIMIT ?
      `;try{let T=this.db.prepare(p).all(t,...a,s),f=this.db.prepare(R).all(t,...a,n+1);if(T.length===0&&f.length===0)return{observations:[],sessions:[],prompts:[]};d=T.length>0?T[T.length-1].created_at_epoch:t,c=f.length>0?f[f.length-1].created_at_epoch:t}catch(T){return m.error("DB","Error getting boundary timestamps",void 0,{error:T,project:o}),{observations:[],sessions:[],prompts:[]}}}let _=`
      SELECT *
      FROM observations
