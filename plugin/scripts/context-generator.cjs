...
`+JSON.stringify(o,null,2):u=" "+this.formatData(o));let l="";if(n){let{sessionId:E,memorySessionId:T,correlationId:h,...m}=n;Object.keys(m).length>0&&(l=` {${Object.entries(m).map(([S,I])=>`${S}=${String(I)}`).join(", ")}}`)}let g=`[${i}] [${a}] [${d}] ${p}${s}${l}${u}`;if(this.logFilePath)try{(0,M.appendFileSync)(this.logFilePath,g+`
...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),u=p?`${String(p[1].split("/").pop())}:${p[2]}`:"unknown",l={...s,location:u};return this.warn(e,`[HAPPY-PATH] ${t}`,l,n),o}},_=new K;var Nt={};function Mt(){return typeof __dirname<"u"?__dirname:(0,b.dirname)((0,ge.fileURLToPath)(Nt.url))}var Rt=Mt(),O=N.get("MAGIC_CLAUDE_MEM_DATA_DIR"),J=process.env.CLAUDE_CONFIG_DIR||(0,b.join)((0,me.homedir)(),".claude"),qt=(0,b.join)(O,"archives"),Kt=(0,b.join)(O,"logs"),Jt=(0,b.join)(O,"trash"),Qt=(0,b.join)(O,"backups"),zt=(0,b.join)(O,"modes"),Zt=(0,b.join)(O,"settings.json"),Te=(0,b.join)(O,"magic-claude-mem.db"),es=(0,b.join)(O,"vector-db"),ts=(0,b.join)(O,"observer-sessions"),ss=(0,b.join)(J,"settings.json"),rs=(0,b.join)(J,"commands"),ns=(0,b.join)(J,"CLAUDE.md");function fe(r){(0,Ee.mkdirSync)(r,{recursive:!0})}function Se(){return(0,b.join)(Rt,"..")}function P(r){return!r.request.trim()&&!r.investigated.trim()&&!r.learned.trim()&&!r.completed.trim()&&!r.next_steps.trim()}var Q=R(require("path"),1);function G(r){if(!r)return[];try{let e=JSON.parse(r);return Array.isArray(e)?e:[]}catch(e){return _.debug("PARSER","Failed to parse JSON array, using empty fallback",{preview:r.substring(0,50)},e),[]}}function he(r){return new Date(r).toLocaleString("en-US",{month:"short",day:"numeric",hour:"numeric",minute:"2-digit",hour12:!0})}function Ce(r){return new Date(r).toLocaleString("en-US",{hour:"numeric",minute:"2-digit",hour12:!0})}function Ie(r){return new Date(r).toLocaleString("en-US",{month:"short",day:"numeric",year:"numeric"})}function be(r,e){return Q.default.isAbsolute(r)?Q.default.relative(e,r):r}function Ae(r,e,t){let s=G(r);if(s.length>0)return be(s[0],e);if(t){let n=G(t);if(n.length>0)return be(n[0],e)}return"General"}function v(r){return r?Math.ceil(r.length/4):0}function F(r){return v(r.narrative??null)+v(r.title??null)+v(r.facts??null)+v(r.concepts??null)+v(r.text??null)}var j=class{db;constructor(e=Te){e!==":memory:"&&fe(O),this.db=new w(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addCompositeIndexes(),this.ensureReadTokensColumn(),this.createContextInjectionsTable(),this.ensureSubprocessPidColumn()}initializeSchema(){this.db.run(`
    ...
    `).run(new Date(t).toISOString(),t,e)}getRecentSummaries(e,t=10){return this.db.prepare(`
      SELECT
    ...
    `).all(e)}getObservationById(e){return this.db.prepare(`
      ...
      FROM observations
      WHERE id = ?
    `).get(e)||null}getObservationsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o,type:i,concepts:a,files:d}=t,p=s==="date_asc"?"ASC":"DESC",u=n?`LIMIT ${String(n)}`:"",l=e.map(()=>"?").join(","),g=[...e],E=[];if(o&&(E.push("project = ?"),g.push(o)),i)if(Array.isArray(i)){let m=i.map(()=>"?").join(",");E.push(`type IN (${m})`),g.push(...i)}else E.push("type = ?"),g.push(i);if(a){let m=Array.isArray(a)?a:[a],f=m.map(()=>"EXISTS (SELECT 1 FROM json_each(concepts) WHERE value = ?)");g.push(...m),E.push(`(${f.join(" OR ")})`)}if(d){let m=Array.isArray(d)?d:[d],f=m.map(()=>"(EXISTS (SELECT 1 FROM json_each(files_read) WHERE value LIKE ?) OR EXISTS (SELECT 1 FROM json_each(files_modified) WHERE value LIKE ?))");m.forEach(S=>{g.push(`%${S}%`,`%${S}%`)}),E.push(`(${f.join(" OR ")})`)}let T=E.length>0?`WHERE id IN (${l}) AND ${E.join(" AND ")}`:`WHERE id IN (${l})`;return this.db.prepare(`
      SELECT *
      FROM observations
      ${T}
    ...
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,d,a);return{id:Number(u.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a){let d=a??Date.now(),p=new Date(d).toISOString();return this.db.transaction(()=>{let l=[],g=this.db.prepare(`
      ...
      `);for(let T of s){let h=F({narrative:T.narrative,title:T.title,facts:JSON.stringify(T.facts),concepts:JSON.stringify(T.concepts),text:null}),m=g.run(e,t,T.type,T.title,T.subtitle,JSON.stringify(T.facts),T.narrative,JSON.stringify(T.concepts),JSON.stringify(T.files_read),JSON.stringify(T.files_modified),o||null,i,h,p,d);l.push(Number(m.lastInsertRowid))}let E=null;if(n)if(P(n))_.warn("DB","Skipping empty summary insert",{memorySessionId:e});else{let h=this.db.prepare(`
        ...
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, read_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let f of s){let S=F({narrative:f.narrative,title:f.title,facts:JSON.stringify(f.facts),concepts:JSON.stringify(f.concepts),text:null}),I=T.run(e,t,f.type,f.title,f.subtitle,JSON.stringify(f.facts),f.narrative,JSON.stringify(f.concepts),JSON.stringify(f.files_read),JSON.stringify(f.files_modified),a||null,d,S,l,u);E.push(Number(I.lastInsertRowid))}let h;if(n)if(P(n))_.warn("DB","Skipping empty summary insert",{memorySessionId:e});else{let S=this.db.prepare(`
            INSERT INTO session_summaries
            (memory_session_id, project, request, investigated, learned, completed,
             next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    `).all(...p)}getTimelineAroundTimestamp(e,t=10,s=10,n){return this.getTimelineAroundObservation(null,e,t,s,n)}getTimelineAroundObservation(e,t,s=10,n=10,o){let i=o?"AND project = ?":"",a=o?[o]:[],d,p;if(e!==null){let m=`
      ...
      `,f=`
        ...
        ORDER BY id ASC
        LIMIT ?
      `;try{let S=this.db.prepare(m).all(e,...a,s+1),I=this.db.prepare(f).all(e,...a,n+1);if(S.length===0&&I.length===0)return{observations:[],sessions:[],prompts:[]};d=S.length>0?S[S.length-1].created_at_epoch:t,p=I.length>0?I[I.length-1].created_at_epoch:t}catch(S){return _.error("DB","Error getting boundary observations",void 0,{error:S,project:o}),{observations:[],sessions:[],prompts:[]}}}else{let m=`
      ...
      `,f=`
        ...
        ORDER BY created_at_epoch ASC
        LIMIT ?
      `;try{let S=this.db.prepare(m).all(t,...a,s),I=this.db.prepare(f).all(t,...a,n+1);if(S.length===0&&I.length===0)return{observations:[],sessions:[],prompts:[]};d=S.length>0?S[S.length-1].created_at_epoch:t,p=I.length>0?I[I.length-1].created_at_epoch:t}catch(S){return _.error("DB","Error getting boundary timestamps",void 0,{error:S,project:o}),{observations:[],sessions:[],prompts:[]}}}let u=`
      SELECT *
      FROM observations
  ...
  `).all(e,...s,...o,t.totalObservationCount)}function re(r,e,t){return r.db.prepare(`
    ...
    ORDER BY created_at_epoch DESC
    LIMIT ?
  `).all(e,t.sessionCount+Z)}function De(r,e,t){let s=Array.from(t.observationTypes),n=s.map(()=>"?").join(","),o=Array.from(t.observationConcepts),i=o.map(()=>"?").join(","),a=e.map(()=>"?").join(",");return r.db.prepare(`
    SELECT
      id, memory_session_id, type, title, subtitle, narrative,
      facts, concepts, files_read, files_modified, discovery_tokens,
...
`}function ct(r,e,t,s){let n=[];return s?n.push(...Qe(r)):n.push(...$e(r)),s?n.push(...ze()):n.push(...Pe()),s?n.push(...Ze()):n.push(...Ge()),s?n.push(...et()):n.push(...Fe()),B(t)&&(s?n.push(...tt(e,t)):n.push(...je(e,t))),n}function Dt(r){let e=new Map;for(let s of r){let n=s.type==="observation"?s.data.created_at:s.data.displayTime,o=Ie(n);e.has(o)||e.set(o,[]),e.get(o)?.push(s)}let t=Array.from(e.entries()).sort((s,n)=>{let o=new Date(s[0]).getTime(),i=new Date(n[0]).getTime();return o-i});return new Map(t)}function xt(r,e){return e.fullObservationField==="narrative"?r.narrative:r.facts?G(r.facts).join(`
`):null}function kt(r,e,t,s,n,o){let i=[];o?i.push(...st(r)):i.push(...Xe(r));let a=null,d="",p=!1;for(let u of e)if(u.type==="summary"){p&&(i.push(""),p=!1,a=null,d="");let l=u.data,g=he(l.displayTime);o?i.push(...it(l,g)):i.push(...Ve(l,g))}else{let l=u.data,g=Ae(l.files_modified,n,l.files_read),E=Ce(l.created_at),T=E!==d,h=T?E:"";d=E;let m=t.has(l.id);if(g!==a&&(p&&i.push(""),o?i.push(...rt(g)):i.push(...Be(g)),a=g,p=!0),m){let f=xt(l,s);o?i.push(...ot(l,E,T,f,s)):(p&&(i.push(""),p=!1),i.push(...We(l,h,f,s)),a=null)}else o?i.push(nt(l,E,T,s)):i.push(He(l,h,s))}return p&&i.push(""),i}function ut(r,e,t,s,n){let o=[],i=Dt(r);for(let[a,d]of i)o.push(...kt(a,d,e,t,s,n));return o}function _t(r,e,t){return!(!r.showLastSummary||!e||!!!(e.investigated||e.learned||e.completed||e.next_steps)||t&&e.created_at_epoch<=t.created_at_epoch)}function lt(r,e){let t=[];return e?(t.push(...k("Investigated",r.investigated,c.blue)),t.push(...k("Learned",r.learned,c.yellow)),t.push(...k("Completed",r.completed,c.green)),t.push(...k("Next Steps",r.next_steps,c.magenta))):(t.push(...x("Investigated",r.investigated)),t.push(...x("Learned",r.learned)),t.push(...x("Completed",r.completed)),t.push(...x("Next Steps",r.next_steps))),t}function mt(r,e){return e?at(r):Ye(r)}function Et(r,e,t){return!B(e)||r.totalDiscoveryTokens<=0||r.savings<=0?[]:t?dt(r.totalDiscoveryTokens,r.totalReadTokens):qe(r.totalDiscoveryTokens,r.totalReadTokens)}var Ut=gt.default.join((0,Tt.homedir)(),".claude","plugins","marketplaces","magic-claude-mem","plugin",".install-version");function wt(){try{return new j}catch(r){if(r instanceof Error&&"code"in r&&r.code==="ERR_DLOPEN_FAILED"){try{(0,ft.unlinkSync)(Ut)}catch(e){_.debug("SYSTEM","Marker file cleanup failed (may not exist)",{},e)}return _.error("SYSTEM","Native module rebuild needed - restart Claude Code to auto-fix"),null}throw r}}function $t(r,e){return e?pt(r):Ke(r)}function Pt(r,e,t,s,n,o,i,a){let d=[];d.push(...ct(r,a,s,i));let p=t.slice(0,s.sessionCount),u=ke(p,t),l=oe(e,u),g=Ue(e,s.fullObservationCount);d.push(...ut(l,g,s,n,i));let E=t[0],T=e[0];_t(s,E,T)&&d.push(...lt(E,i));let h=ne(e,s,o,n);return d.push(...mt(h,i)),d.push(...Et(a,s,i)),d.join(`
`).trimEnd()}function ie(r,e=!1){return Y(r,e).text}function Y(r,e=!1){let t=z(),s=r?.cwd??process.cwd(),n=Me(s),o=r?.projects||[n],i=wt();if(!i)return{text:"",observationIds:[],totalReadTokens:0};try{let a=o.length>1?De(i,o,t):se(i,n,t),d=o.length>1?xe(i,o,t):re(i,n,t);if(a.length===0&&d.length===0)return{text:$t(n,e),observationIds:[],totalReadTokens:0};let p=a.map(l=>l.id),u=te(a);return{text:Pt(n,a,d,t,s,r?.session_id,e,u),observationIds:p,totalReadTokens:u.totalReadTokens}}finally{i.close()}}0&&(module.exports={generateContext,generateContextWithMeta});
