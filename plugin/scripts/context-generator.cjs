...
${e.stack}`:e.message;if(Array.isArray(e))return`[${e.length} items]`;let t=Object.keys(e);return t.length===0?"{}":t.length<=3?JSON.stringify(e):`{${t.length} keys: ${t.slice(0,3).join(", ")}...}`}return String(e)}formatTool(e,t){if(!t)return e;let s=t;if(typeof t=="string")try{s=JSON.parse(t)}catch{s=t}if(e==="Bash"&&s.command)return`${e}(${s.command})`;if(s.file_path)return`${e}(${s.file_path})`;if(s.notebook_path)return`${e}(${s.notebook_path})`;if(e==="Glob"&&s.pattern)return`${e}(${s.pattern})`;if(e==="Grep"&&s.pattern)return`${e}(${s.pattern})`;if(s.url)return`${e}(${s.url})`;if(s.query)return`${e}(${s.query})`;if(e==="Task"){if(s.subagent_type)return`${e}(${s.subagent_type})`;if(s.description)return`${e}(${s.description})`}return e==="Skill"&&s.skill?`${e}(${s.skill})`:e==="LSP"&&s.operation?`${e}(${s.operation})`:e}formatTimestamp(e){let t=e.getFullYear(),s=String(e.getMonth()+1).padStart(2,"0"),n=String(e.getDate()).padStart(2,"0"),o=String(e.getHours()).padStart(2,"0"),i=String(e.getMinutes()).padStart(2,"0"),a=String(e.getSeconds()).padStart(2,"0"),d=String(e.getMilliseconds()).padStart(3,"0");return`${t}-${s}-${n} ${o}:${i}:${a}.${d}`}log(e,t,s,n,o){if(e<this.getLevel())return;this.ensureLogFileInitialized();let i=this.formatTimestamp(new Date),a=K[e].padEnd(5),d=t.padEnd(6),_="";n?.correlationId?_=`[${n.correlationId}] `:n?.sessionId&&(_=`[session-${n.sessionId}] `);let m="";o!=null&&(o instanceof Error?m=this.getLevel()===0?`
${o.message}
${o.stack}`:` ${o.message}`:this.getLevel()===0&&typeof o=="object"?m=`
`+JSON.stringify(o,null,2):m=" "+this.formatData(o));let l="";if(n){let{sessionId:T,memorySessionId:S,correlationId:I,...u}=n;Object.keys(u).length>0&&(l=` {${Object.entries(u).map(([g,h])=>`${g}=${h}`).join(", ")}}`)}let E=`[${i}] [${a}] [${d}] ${_}${s}${l}${m}`;if(this.logFilePath)try{(0,A.appendFileSync)(this.logFilePath,E+`
...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),m=_?`${_[1].split("/").pop()}:${_[2]}`:"unknown",l={...s,location:m};return this.warn(e,`[HAPPY-PATH] ${t}`,l,n),o}},p=new J;var Lt={};function At(){return typeof __dirname<"u"?__dirname:(0,b.dirname)((0,le.fileURLToPath)(Lt.url))}var It=At();function Mt(){if(process.env.CLAUDE_MEM_DATA_DIR)return process.env.CLAUDE_MEM_DATA_DIR;let r=(0,b.join)((0,Q.homedir)(),".claude-mem"),e=(0,b.join)(r,"settings.json");try{if((0,$.existsSync)(e)){let{readFileSync:t}=require("fs"),s=JSON.parse(t(e,"utf-8")),n=s.env??s;if(n.CLAUDE_MEM_DATA_DIR)return n.CLAUDE_MEM_DATA_DIR}}catch{}return r}var N=Mt(),M=process.env.CLAUDE_CONFIG_DIR||(0,b.join)((0,Q.homedir)(),".claude"),Vt=(0,b.join)(M,"plugins","marketplaces","thedotmack"),qt=(0,b.join)(N,"archives"),Kt=(0,b.join)(N,"logs"),Jt=(0,b.join)(N,"trash"),Qt=(0,b.join)(N,"backups"),zt=(0,b.join)(N,"modes"),Zt=(0,b.join)(N,"settings.json"),Ee=(0,b.join)(N,"claude-mem.db"),es=(0,b.join)(N,"vector-db"),ts=(0,b.join)(N,"observer-sessions"),ss=(0,b.join)(M,"settings.json"),rs=(0,b.join)(M,"commands"),ns=(0,b.join)(M,"CLAUDE.md");function Te(r){(0,$.mkdirSync)(r,{recursive:!0})}function ge(){return(0,b.join)(It,"..")}var fe=require("crypto");var vt=3e4;function F(r,e,t){return(0,fe.createHash)("sha256").update((r||"")+(e||"")+(t||"")).digest("hex").slice(0,16)}function P(r,e,t){let s=t-vt;return r.prepare("SELECT id, created_at_epoch FROM observations WHERE content_hash = ? AND created_at_epoch > ?").get(e,s)}var X=class{db;constructor(e=Ee){e!==":memory:"&&Te(N),this.db=new be.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys(),this.addObservationContentHashColumn(),this.addSessionCustomTitleColumn(),this.addObservationProvenanceColumns()}initializeSchema(){this.db.run(`
    ...
    `).run(e,t,s,n.toISOString(),o).lastInsertRowid}getUserPrompt(e,t){return this.db.prepare(`
      ...
      LIMIT 1
    `).get(e,t)?.prompt_text??null}storeObservation(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString(),_=F(e,s.title,s.narrative),m=P(this.db,_,a);if(m)return{id:m.id,createdAtEpoch:m.created_at_epoch};let E=this.db.prepare(`
      INSERT INTO observations
      (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
       files_read, files_modified, prompt_number, discovery_tokens, content_hash, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.type,s.title,s.subtitle,JSON.stringify(s.facts),s.narrative,JSON.stringify(s.concepts),JSON.stringify(s.files_read),JSON.stringify(s.files_modified),n||null,o,_,d,a);return{id:Number(E.lastInsertRowid),createdAtEpoch:a}}storeSummary(e,t,s,n,o=0,i,a,d,_){let m=i??Date.now(),l=new Date(m).toISOString(),T=this.db.prepare(`
      INSERT INTO session_summaries
      (memory_session_id, project, request, investigated, learned, completed,
       next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch,
       node, platform, instance)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,l,m,a??null,d??null,_??null);return{id:Number(T.lastInsertRowid),createdAtEpoch:m}}storeObservations(e,t,s,n,o,i=0,a,d,_,m){let l=a??Date.now(),E=new Date(l).toISOString();return this.db.transaction(()=>{let S=[],I=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, content_hash, created_at, created_at_epoch,
         node, platform, instance)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let f of s){let g=F(e,f.title,f.narrative),h=P(this.db,g,l);if(h){S.push(h.id);continue}let R=I.run(e,t,f.type,f.title,f.subtitle,JSON.stringify(f.facts),f.narrative,JSON.stringify(f.concepts),JSON.stringify(f.files_read),JSON.stringify(f.files_modified),o||null,i,g,E,l,d??null,_??null,m??null);S.push(Number(R.lastInsertRowid))}let u=null;if(n){let g=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch,
           node, platform, instance)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,o||null,i,E,l,d??null,_??null,m??null);u=Number(g.lastInsertRowid)}return{observationIds:S,summaryId:u,createdAtEpoch:l}})()}storeObservationsAndMarkComplete(e,t,s,n,o,i,a,d=0,_,m,l,E){let T=_??Date.now(),S=new Date(T).toISOString();return this.db.transaction(()=>{let u=[],f=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, content_hash, created_at, created_at_epoch,
         node, platform, instance)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let R of s){let k=F(e,R.title,R.narrative),_e=P(this.db,k,T);if(_e){u.push(_e.id);continue}let ft=f.run(e,t,R.type,R.title,R.subtitle,JSON.stringify(R.facts),R.narrative,JSON.stringify(R.concepts),JSON.stringify(R.files_read),JSON.stringify(R.files_modified),a||null,d,k,S,T,m??null,l??null,E??null);u.push(Number(ft.lastInsertRowid))}let g;if(n){let k=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch,
           node, platform, instance)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,a||null,d,S,T,m??null,l??null,E??null);g=Number(k.lastInsertRowid)}return this.db.prepare(`
      ...
      `).run(T,o),{observationIds:u,summaryId:g,createdAtEpoch:T}})()}getSessionSummariesByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o}=t,i=s==="date_asc"?"ASC":"DESC",a=n?`LIMIT ${n}`:"",d=e.map(()=>"?").join(","),_=[...e],m=o?`WHERE id IN (${d}) AND project = ?`:`WHERE id IN (${d})`;return o&&_.push(o),this.db.prepare(`
    ...
    `).all(..._)}getTimelineAroundTimestamp(e,t=10,s=10,n){return this.getTimelineAroundObservation(null,e,t,s,n)}getTimelineAroundObservation(e,t,s=10,n=10,o){let i=o?"AND project = ?":"",a=o?[o]:[],d,_;if(e!==null){let u=`
      ...
      `,f=`
        ...
        ORDER BY id ASC
        LIMIT ?
      `;try{let g=this.db.prepare(u).all(e,...a,s+1),h=this.db.prepare(f).all(e,...a,n+1);if(g.length===0&&h.length===0)return{observations:[],sessions:[],prompts:[]};d=g.length>0?g[g.length-1].created_at_epoch:t,_=h.length>0?h[h.length-1].created_at_epoch:t}catch(g){return p.error("DB","Error getting boundary observations",void 0,{error:g,project:o}),{observations:[],sessions:[],prompts:[]}}}else{let u=`
      ...
      `,f=`
        ...
        ORDER BY created_at_epoch ASC
        LIMIT ?
      `;try{let g=this.db.prepare(u).all(t,...a,s),h=this.db.prepare(f).all(t,...a,n+1);if(g.length===0&&h.length===0)return{observations:[],sessions:[],prompts:[]};d=g.length>0?g[g.length-1].created_at_epoch:t,_=h.length>0?h[h.length-1].created_at_epoch:t}catch(g){return p.error("DB","Error getting boundary timestamps",void 0,{error:g,project:o}),{observations:[],sessions:[],prompts:[]}}}let m=`
      SELECT *
      FROM observations
...
`):null}function kt(r,e,t,s){let n=[];n.push(...$e(r));let o="";for(let i of e)if(i.type==="summary"){o="";let a=i.data,d=ae(a.displayTime);n.push(...je(a,d))}else{let a=i.data,d=de(a.created_at),m=d!==o?d:"";if(o=d,t.has(a.id)){let E=ct(a,s);n.push(...Xe(a,m,E,s))}else n.push(Pe(a,m,s))}return n}function wt(r,e,t,s,n){let o=[];o.push(...Qe(r));let i=null,a="";for(let d of e)if(d.type==="summary"){i=null,a="";let _=d.data,m=ae(_.displayTime);o.push(...tt(_,m))}else{let _=d.data,m=dt(_.files_modified,n,_.files_read),l=de(_.created_at),E=l!==a;a=l;let T=t.has(_.id);if(m!==i&&(o.push(...ze(m)),i=m),T){let S=ct(_,s);o.push(...et(_,l,E,S,s))}else o.push(Ze(_,l,E,s))}return o.push(""),o}function $t(r,e,t,s,n,o){return o?wt(r,e,t,s,n):kt(r,e,t,s)}function _t(r,e,t,s,n){let o=[],i=xt(r);for(let[a,d]of i)o.push(...$t(a,d,e,t,s,n));return o}function pt(r,e,t){return!(!r.showLastSummary||!e||!!!(e.investigated||e.learned||e.completed||e.next_steps)||t&&e.created_at_epoch<=t.created_at_epoch)}function mt(r,e){let t=[];return e?(t.push(...x("Investigated",r.investigated,c.blue)),t.push(...x("Learned",r.learned,c.yellow)),t.push(...x("Completed",r.completed,c.green)),t.push(...x("Next Steps",r.next_steps,c.magenta))):(t.push(...U("Investigated",r.investigated)),t.push(...U("Learned",r.learned)),t.push(...U("Completed",r.completed)),t.push(...U("Next Steps",r.next_steps))),t}function ut(r,e){return e?st(r):Ge(r)}function lt(r,e,t){return!B(e)||r.totalDiscoveryTokens<=0||r.savings<=0?[]:t?rt(r.totalDiscoveryTokens,r.totalReadTokens):He(r.totalDiscoveryTokens,r.totalReadTokens)}var Ft=Et.default.join((0,Tt.homedir)(),".claude","plugins","marketplaces","thedotmack","plugin",".install-version");function Pt(){try{return new X}catch(r){if(r.code==="ERR_DLOPEN_FAILED"){try{(0,gt.unlinkSync)(Ft)}catch(e){p.debug("SYSTEM","Marker file cleanup failed (may not exist)",{},e)}return p.error("SYSTEM","Native module rebuild needed - restart Claude Code to auto-fix"),null}throw r}}function Xt(r,e){return e?nt(r):Be(r)}function jt(r,e,t,s,n,o,i){let a=[],d=te(e);a.push(...ot(r,d,s,i));let _=t.slice(0,s.sessionCount),m=Le(_,t),l=oe(e,m),E=ve(e,s.fullObservationCount);a.push(..._t(l,E,s,n,i));let T=t[0],S=e[0];pt(s,T,S)&&a.push(...mt(T,i));let I=ne(e,s,o,n);return a.push(...ut(I,i)),a.push(...lt(d,s,i)),a.join(`
