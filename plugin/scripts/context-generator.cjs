...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),u=p?`${p[1].split("/").pop()}:${p[2]}`:"unknown",m={...s,location:u};return this.warn(e,`[HAPPY-PATH] ${t}`,m,n),o}},c=new W;var Rt={};function ht(){return typeof __dirname<"u"?__dirname:(0,S.dirname)((0,de.fileURLToPath)(Rt.url))}var Ot=ht(),N=C.get("CLAUDE_MEM_DATA_DIR"),L=process.env.CLAUDE_CONFIG_DIR||(0,S.join)((0,ie.homedir)(),".claude"),Ht=(0,S.join)(L,"plugins","marketplaces","thedotmack"),Bt=(0,S.join)(N,"archives"),Wt=(0,S.join)(N,"logs"),Yt=(0,S.join)(N,"trash"),Vt=(0,S.join)(N,"backups"),qt=(0,S.join)(N,"modes"),Kt=(0,S.join)(N,"settings.json"),_e=(0,S.join)(N,"claude-mem.db"),Jt=(0,S.join)(N,"vector-db"),Qt=(0,S.join)(N,"observer-sessions"),zt=(0,S.join)(L,"settings.json"),Zt=(0,S.join)(L,"commands"),es=(0,S.join)(L,"CLAUDE.md");function pe(r){(0,ae.mkdirSync)(r,{recursive:!0})}function ce(){return(0,S.join)(Ot,"..")}var ue=require("crypto");var Nt=3e4;function me(r,e,t){return(0,ue.createHash)("sha256").update((r||"")+(e||"")+(t||"")).digest("hex").slice(0,16)}function le(r,e,t){let s=t-Nt;return r.prepare("SELECT id, created_at_epoch FROM observations WHERE content_hash = ? AND created_at_epoch > ?").get(e,s)}var $=class{db;constructor(e=_e){e!==":memory:"&&pe(N),this.db=new Ee.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys(),this.addObservationContentHashColumn(),this.addSessionCustomTitleColumn(),this.addSupersessionTrainingTables(),this.addHandoffObservationType()}initializeSchema(){this.db.run(`
    ...
    `).all(e)}getObservationById(e){return this.db.prepare(`
      ...
      FROM observations
      WHERE id = ?
    `).get(e)||null}getObservationsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o,type:i,concepts:a,files:_}=t,p=s==="date_asc"?"ASC":"DESC",u=n?`LIMIT ${n}`:"",m=e.map(()=>"?").join(","),T=[...e],E=[];if(o&&(E.push("project = ?"),T.push(o)),i)if(Array.isArray(i)){let l=i.map(()=>"?").join(",");E.push(`type IN (${l})`),T.push(...i)}else E.push("type = ?"),T.push(i);if(a){let l=Array.isArray(a)?a:[a],f=l.map(()=>"EXISTS (SELECT 1 FROM json_each(concepts) WHERE value = ?)");T.push(...l),E.push(`(${f.join(" OR ")})`)}if(_){let l=Array.isArray(_)?_:[_],f=l.map(()=>"(EXISTS (SELECT 1 FROM json_each(files_read) WHERE value LIKE ?) OR EXISTS (SELECT 1 FROM json_each(files_modified) WHERE value LIKE ?))");l.forEach(b=>{T.push(`%${b}%`,`%${b}%`)}),E.push(`(${f.join(" OR ")})`)}let g=E.length>0?`WHERE id IN (${m}) AND ${E.join(" AND ")}`:`WHERE id IN (${m})`;return this.db.prepare(`
      SELECT *
      FROM observations
      ${g}
    ...
    `).run(e,t,s,n.toISOString(),o).lastInsertRowid}getUserPrompt(e,t){return this.db.prepare(`
      ...
      LIMIT 1
    `).get(e,t)?.prompt_text??null}storeObservation(e,t,s,n,o=0,i){let a=i??Date.now(),_=new Date(a).toISOString(),p=me(e,s.title,s.narrative),u=le(this.db,p,a);if(u)return{id:u.id,createdAtEpoch:u.created_at_epoch};let T=this.db.prepare(`
      INSERT INTO observations
      (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
       files_read, files_modified, prompt_number, discovery_tokens, content_hash, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.type,s.title,s.subtitle,JSON.stringify(s.facts),s.narrative,JSON.stringify(s.concepts),JSON.stringify(s.files_read),JSON.stringify(s.files_modified),n||null,o,p,_,a);return{id:Number(T.lastInsertRowid),createdAtEpoch:a}}storeSummary(e,t,s,n,o=0,i){let a=i??Date.now(),_=new Date(a).toISOString(),u=this.db.prepare(`
      INSERT INTO session_summaries
      (memory_session_id, project, request, investigated, learned, completed,
       next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,_,a);return{id:Number(u.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a){let _=a??Date.now(),p=new Date(_).toISOString();return this.db.transaction(()=>{let m=[],T=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let g of s){let h=T.run(e,t,g.type,g.title,g.subtitle,JSON.stringify(g.facts),g.narrative,JSON.stringify(g.concepts),JSON.stringify(g.files_read),JSON.stringify(g.files_modified),o||null,i,p,_);m.push(Number(h.lastInsertRowid))}let E=null;if(n){let h=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,o||null,i,p,_);E=Number(h.lastInsertRowid)}return{observationIds:m,summaryId:E,createdAtEpoch:_}})()}storeObservationsAndMarkComplete(e,t,s,n,o,i,a,_=0,p){let u=p??Date.now(),m=new Date(u).toISOString();return this.db.transaction(()=>{let E=[],g=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let f of s){let b=g.run(e,t,f.type,f.title,f.subtitle,JSON.stringify(f.facts),f.narrative,JSON.stringify(f.concepts),JSON.stringify(f.files_read),JSON.stringify(f.files_modified),a||null,_,m,u);E.push(Number(b.lastInsertRowid))}let h;if(n){let b=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,a||null,_,m,u);h=Number(b.lastInsertRowid)}return this.db.prepare(`
      ...
      `).run(u,o),{observationIds:E,summaryId:h,createdAtEpoch:u}})()}getSessionSummariesByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o}=t,i=s==="date_asc"?"ASC":"DESC",a=n?`LIMIT ${n}`:"",_=e.map(()=>"?").join(","),p=[...e],u=o?`WHERE id IN (${_}) AND project = ?`:`WHERE id IN (${_})`;return o&&p.push(o),this.db.prepare(`
      SELECT * FROM session_summaries
      ${u}
      ORDER BY created_at_epoch ${i}
      ${a}
    `).all(...p)}getUserPromptsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o}=t,i=s==="date_asc"?"ASC":"DESC",a=n?`LIMIT ${n}`:"",_=e.map(()=>"?").join(","),p=[...e],u=o?"AND s.project = ?":"";return o&&p.push(o),this.db.prepare(`
      SELECT
        up.*,
        s.project,
        s.memory_session_id
    ...
    `).all(...p)}getTimelineAroundTimestamp(e,t=10,s=10,n){return this.getTimelineAroundObservation(null,e,t,s,n)}getTimelineAroundObservation(e,t,s=10,n=10,o){let i=o?"AND project = ?":"",a=o?[o]:[],_,p;if(e!==null){let l=`
      ...
      `,f=`
        ...
        ORDER BY id ASC
        LIMIT ?
      `;try{let b=this.db.prepare(l).all(e,...a,s+1),R=this.db.prepare(f).all(e,...a,n+1);if(b.length===0&&R.length===0)return{observations:[],sessions:[],prompts:[]};_=b.length>0?b[b.length-1].created_at_epoch:t,p=R.length>0?R[R.length-1].created_at_epoch:t}catch(b){return c.error("DB","Error getting boundary observations",void 0,{error:b,project:o}),{observations:[],sessions:[],prompts:[]}}}else{let l=`
      ...
      `,f=`
        ...
        ORDER BY created_at_epoch ASC
        LIMIT ?
      `;try{let b=this.db.prepare(l).all(t,...a,s),R=this.db.prepare(f).all(t,...a,n+1);if(b.length===0&&R.length===0)return{observations:[],sessions:[],prompts:[]};_=b.length>0?b[b.length-1].created_at_epoch:t,p=R.length>0?R[R.length-1].created_at_epoch:t}catch(b){return c.error("DB","Error getting boundary timestamps",void 0,{error:b,project:o}),{observations:[],sessions:[],prompts:[]}}}let u=`
      SELECT *
      FROM observations
...
`}function et(r,e,t,s){let n=[];return s?n.push(...Ge(r)):n.push(...Ce(r)),s?n.push(...je()):n.push(...Le()),s?n.push(...He()):n.push(...ve()),s?n.push(...Be()):n.push(...ye()),P(t)&&(s?n.push(...We(e,t)):n.push(...Me(e,t))),n}var ee=y(require("path"),1);function H(r){if(!r)return[];try{let e=JSON.parse(r);return Array.isArray(e)?e:[]}catch(e){return c.debug("PARSER","Failed to parse JSON array, using empty fallback",{preview:r?.substring(0,50)},e),[]}}function st(r){return new Date(r).toLocaleString("en-US",{month:"short",day:"numeric",hour:"numeric",minute:"2-digit",hour12:!0})}function rt(r){return new Date(r).toLocaleString("en-US",{hour:"numeric",minute:"2-digit",hour12:!0})}function nt(r){return new Date(r).toLocaleString("en-US",{month:"short",day:"numeric",year:"numeric"})}function tt(r,e){return ee.default.isAbsolute(r)?ee.default.relative(e,r):r}function ot(r,e,t){let s=H(r);if(s.length>0)return tt(s[0],e);if(t){let n=H(t);if(n.length>0)return tt(n[0],e)}return"General"}function Lt(r){let e=new Map;for(let s of r){let n=s.type==="observation"?s.data.created_at:s.data.displayTime,o=nt(n);e.has(o)||e.set(o,[]),e.get(o).push(s)}let t=Array.from(e.entries()).sort((s,n)=>{let o=new Date(s[0]).getTime(),i=new Date(n[0]).getTime();return o-i});return new Map(t)}function vt(r,e){return e.fullObservationField==="narrative"?r.narrative:r.facts?H(r.facts).join(`
`):null}function yt(r,e,t,s,n,o){let i=[];o?i.push(...Ye(r)):i.push(...De(r));let a=null,_="",p=!1;for(let u of e)if(u.type==="summary"){p&&(i.push(""),p=!1,a=null,_="");let m=u.data,T=st(m.displayTime);o?i.push(...Je(m,T)):i.push(...we(m,T))}else{let m=u.data,T=ot(m.files_modified,n,m.files_read),E=rt(m.created_at),g=E!==_,h=g?E:"";_=E;let l=t.has(m.id);if(T!==a&&(p&&i.push(""),o?i.push(...Ve(T)):i.push(...Ue(T)),a=T,p=!0),l){let f=vt(m,s);o?i.push(...Ke(m,E,g,f,s)):(p&&!o&&(i.push(""),p=!1),i.push(...ke(m,h,f,s)),a=null)}else o?i.push(qe(m,E,g,s)):i.push(xe(m,h,s))}return p&&i.push(""),i}function it(r,e,t,s,n){let o=[],i=Lt(r);for(let[a,_]of i)o.push(...yt(a,_,e,t,s,n));return o}function at(r,e,t){return!(!r.showLastSummary||!e||!!!(e.investigated||e.learned||e.completed||e.next_steps)||t&&e.created_at_epoch<=t.created_at_epoch)}function dt(r,e){let t=[];return e?(t.push(...x("Investigated",r.investigated,d.blue)),t.push(...x("Learned",r.learned,d.yellow)),t.push(...x("Completed",r.completed,d.green)),t.push(...x("Next Steps",r.next_steps,d.magenta))):(t.push(...U("Investigated",r.investigated)),t.push(...U("Learned",r.learned)),t.push(...U("Completed",r.completed)),t.push(...U("Next Steps",r.next_steps))),t}function _t(r,e){return e?Qe(r):$e(r)}function pt(r,e,t){return!P(e)||r.totalDiscoveryTokens<=0||r.savings<=0?[]:t?ze(r.totalDiscoveryTokens,r.totalReadTokens):Fe(r.totalDiscoveryTokens,r.totalReadTokens)}var Mt=ct.default.join((0,ut.homedir)(),".claude","plugins","marketplaces","thedotmack","plugin",".install-version");function Dt(){try{return new $}catch(r){if(r.code==="ERR_DLOPEN_FAILED"){try{(0,mt.unlinkSync)(Mt)}catch(e){c.debug("SYSTEM","Marker file cleanup failed (may not exist)",{},e)}return c.error("SYSTEM","Native module rebuild needed - restart Claude Code to auto-fix"),null}throw r}}function Ut(r,e){return e?Ze(r):Pe(r)}function xt(r,e,t,s,n,o,i){let a=[],_=K(e);a.push(...et(r,_,s,i));let p=t.slice(0,s.sessionCount),u=Ne(p,t),m=Z(e,u),T=Ae(e,s.fullObservationCount);a.push(...it(m,T,s,n,i));let E=t[0],g=e[0];at(s,E,g)&&a.push(...dt(E,i));let h=z(e,s,o,n);return a.push(..._t(h,i)),a.push(...pt(_,s,i)),a.join(`
`).trimEnd()}async function te(r,e=!1){let t=Y(),s=r?.cwd??process.cwd(),n=ge(s),o=r?.projects||[n],i=Dt();if(!i)return"";try{let a=o.length>1?Oe(i,o,t):J(i,n,t),_=o.length>1?Re(i,o,t):Q(i,n,t);return a.length===0&&_.length===0?Ut(n,e):xt(n,a,_,t,s,r?.session_id,e)}finally{i.close()}}0&&(module.exports={generateContext});
