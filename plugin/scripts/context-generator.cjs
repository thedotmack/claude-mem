...
${e.stack}`:e.message;if(Array.isArray(e))return`[${e.length} items]`;let s=Object.keys(e);return s.length===0?"{}":s.length<=3?JSON.stringify(e):`{${s.length} keys: ${s.slice(0,3).join(", ")}...}`}return String(e)}formatTool(e,s){if(!s)return e;try{let t=typeof s=="string"?JSON.parse(s):s;if(e==="Bash"&&t.command){let n=t.command.length>50?t.command.substring(0,50)+"...":t.command;return`${e}(${n})`}if(t.file_path){let n=t.file_path.split("/").pop()||t.file_path;return`${e}(${n})`}if(t.notebook_path){let n=t.notebook_path.split("/").pop()||t.notebook_path;return`${e}(${n})`}if(e==="Glob"&&t.pattern)return`${e}(${t.pattern})`;if(e==="Grep"&&t.pattern){let n=t.pattern.length>30?t.pattern.substring(0,30)+"...":t.pattern;return`${e}(${n})`}if(t.url){let n=t.url.length>40?t.url.substring(0,40)+"...":t.url;return`${e}(${n})`}if(t.query){let n=t.query.length>40?t.query.substring(0,40)+"...":t.query;return`${e}(${n})`}if(e==="Task"){if(t.subagent_type)return`${e}(${t.subagent_type})`;if(t.description){let n=t.description.length>30?t.description.substring(0,30)+"...":t.description;return`${e}(${n})`}}return e==="Skill"&&t.skill?`${e}(${t.skill})`:e==="LSP"&&t.operation?`${e}(${t.operation})`:e}catch{return e}}formatTimestamp(e){let s=e.getFullYear(),t=String(e.getMonth()+1).padStart(2,"0"),n=String(e.getDate()).padStart(2,"0"),i=String(e.getHours()).padStart(2,"0"),d=String(e.getMinutes()).padStart(2,"0"),c=String(e.getSeconds()).padStart(2,"0"),_=String(e.getMilliseconds()).padStart(3,"0");return`${s}-${t}-${n} ${i}:${d}:${c}.${_}`}log(e,s,t,n,i){if(e<this.getLevel())return;let d=this.formatTimestamp(new Date),c=ne[e].padEnd(5),_=s.padEnd(6),u="";n?.correlationId?u=`[${n.correlationId}] `:n?.sessionId&&(u=`[session-${n.sessionId}] `);let l="";i!=null&&(this.getLevel()===0&&typeof i=="object"?l=`
