...
`+JSON.stringify(o,null,2):l=" "+this.formatData(o));let u="";if(n){let{sessionId:E,memorySessionId:g,correlationId:h,...m}=n;Object.keys(m).length>0&&(u=` {${Object.entries(m).map(([f,R])=>`${f}=${R}`).join(", ")}}`)}let T=`[${i}] [${a}] [${c}] ${p}${s}${u}${l}`;if(this.logFilePath)try{(0,A.appendFileSync)(this.logFilePath,T+`
...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),l=p?`${p[1].split("/").pop()}:${p[2]}`:"unknown",u={...s,location:l};return this.warn(e,`[HAPPY-PATH] ${t}`,u,n),o}},_=new W;var Ct={};function Rt(){return typeof __dirname<"u"?__dirname:(0,S.dirname)((0,pe.fileURLToPath)(Ct.url))}var Nt=Rt(),N=I.get("CLAUDE_MEM_DATA_DIR"),y=process.env.CLAUDE_CONFIG_DIR||(0,S.join)((0,de.homedir)(),".claude"),Vt=(0,S.join)(y,"plugins","marketplaces","thedotmack"),qt=(0,S.join)(N,"archives"),Kt=(0,S.join)(N,"logs"),Jt=(0,S.join)(N,"trash"),zt=(0,S.join)(N,"backups"),Qt=(0,S.join)(N,"modes"),Zt=(0,S.join)(N,"settings.json"),_e=(0,S.join)(N,"claude-mem.db"),es=(0,S.join)(N,"vector-db"),ts=(0,S.join)(N,"observer-sessions"),ss=(0,S.join)(y,"settings.json"),rs=(0,S.join)(y,"commands"),ns=(0,S.join)(y,"CLAUDE.md");function le(r){(0,ce.mkdirSync)(r,{recursive:!0})}function ue(){return(0,S.join)(Nt,"..")}var me=require("crypto");var At=3e4;function Ee(r,e,t){return(0,me.createHash)("sha256").update((r||"")+(e||"")+(t||"")).digest("hex").slice(0,16)}function Te(r,e,t){let s=t-At;return r.prepare("SELECT id, created_at_epoch FROM observations WHERE content_hash = ? AND created_at_epoch > ?").get(e,s)}var $=class{db;constructor(e=_e){e!==":memory:"&&le(N),this.db=new ge.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys(),this.addObservationContentHashColumn(),this.addSessionCustomTitleColumn()}initializeSchema(){this.db.run(`
    ...
    `).all(e)}getObservationById(e){return this.db.prepare(`
      ...
      FROM observations
      WHERE id = ?
    `).get(e)||null}getObservationsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o,type:i,concepts:a,files:c}=t,p=s==="date_asc"?"ASC":"DESC",l=n?`LIMIT ${n}`:"",u=e.map(()=>"?").join(","),T=[...e],E=[];if(o&&(E.push("project = ?"),T.push(o)),i)if(Array.isArray(i)){let m=i.map(()=>"?").join(",");E.push(`type IN (${m})`),T.push(...i)}else E.push("type = ?"),T.push(i);if(a){let m=Array.isArray(a)?a:[a],b=m.map(()=>"EXISTS (SELECT 1 FROM json_each(concepts) WHERE value = ?)");T.push(...m),E.push(`(${b.join(" OR ")})`)}if(c){let m=Array.isArray(c)?c:[c],b=m.map(()=>"(EXISTS (SELECT 1 FROM json_each(files_read) WHERE value LIKE ?) OR EXISTS (SELECT 1 FROM json_each(files_modified) WHERE value LIKE ?))");m.forEach(f=>{T.push(`%${f}%`,`%${f}%`)}),E.push(`(${b.join(" OR ")})`)}let g=E.length>0?`WHERE id IN (${u}) AND ${E.join(" AND ")}`:`WHERE id IN (${u})`;return this.db.prepare(`
      SELECT *
      FROM observations
      ${g}
    ...
    `).run(e,t,s,n.toISOString(),o).lastInsertRowid}getUserPrompt(e,t){return this.db.prepare(`
      ...
      LIMIT 1
    `).get(e,t)?.prompt_text??null}storeObservation(e,t,s,n,o=0,i){let a=i??Date.now(),c=new Date(a).toISOString(),p=Ee(e,s.title,s.narrative),l=Te(this.db,p,a);if(l)return{id:l.id,createdAtEpoch:l.created_at_epoch};let T=this.db.prepare(`
    ...
    `).run(e,t,s.type,s.title,s.subtitle,JSON.stringify(s.facts),s.narrative,JSON.stringify(s.concepts),JSON.stringify(s.files_read),JSON.stringify(s.files_modified),n||null,o,p,c,a);return{id:Number(T.lastInsertRowid),createdAtEpoch:a}}storeSummary(e,t,s,n,o=0,i){let a=i??Date.now(),c=new Date(a).toISOString(),l=this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,c,a);return{id:Number(l.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a){let c=a??Date.now(),p=new Date(c).toISOString();return this.db.transaction(()=>{let u=[],T=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
      ...
      `);for(let g of s){let h=T.run(e,t,g.type,g.title,g.subtitle,JSON.stringify(g.facts),g.narrative,JSON.stringify(g.concepts),JSON.stringify(g.files_read),JSON.stringify(g.files_modified),o||null,i,p,c);u.push(Number(h.lastInsertRowid))}let E=null;if(n){let h=this.db.prepare(`
          ...
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,o||null,i,p,c);E=Number(h.lastInsertRowid)}return{observationIds:u,summaryId:E,createdAtEpoch:c}})()}storeObservationsAndMarkComplete(e,t,s,n,o,i,a,c=0,p){let l=p??Date.now(),u=new Date(l).toISOString();return this.db.transaction(()=>{let E=[],g=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let b of s){let f=g.run(e,t,b.type,b.title,b.subtitle,JSON.stringify(b.facts),b.narrative,JSON.stringify(b.concepts),JSON.stringify(b.files_read),JSON.stringify(b.files_modified),a||null,c,u,l);E.push(Number(f.lastInsertRowid))}let h;if(n){let f=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,a||null,c,u,l);h=Number(f.lastInsertRowid)}return this.db.prepare(`
    ...
    `).all(...p)}getUserPromptsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o}=t,i=s==="date_asc"?"ASC":"DESC",a=n?`LIMIT ${n}`:"",c=e.map(()=>"?").join(","),p=[...e],l=o?"AND s.project = ?":"";return o&&p.push(o),this.db.prepare(`
      ...
      ${a}
    `).all(...p)}getTimelineAroundTimestamp(e,t=10,s=10,n){return this.getTimelineAroundObservation(null,e,t,s,n)}getTimelineAroundObservation(e,t,s=10,n=10,o){let i=o?"AND project = ?":"",a=o?[o]:[],c,p;if(e!==null){let m=`
...
`}function st(r,e,t,s){let n=[];return s?n.push(...He(r)):n.push(...ve(r)),s?n.push(...Be()):n.push(...Le()),s?n.push(...We()):n.push(...Me()),s?n.push(...Ye()):n.push(...De()),P(t)&&(s?n.push(...Ve(e,t)):n.push(...xe(e,t))),n}var ee=L(require("path"),1);function H(r){if(!r)return[];try{let e=JSON.parse(r);return Array.isArray(e)?e:[]}catch(e){return _.debug("PARSER","Failed to parse JSON array, using empty fallback",{preview:r?.substring(0,50)},e),[]}}function nt(r){return new Date(r).toLocaleString("en-US",{month:"short",day:"numeric",hour:"numeric",minute:"2-digit",hour12:!0})}function ot(r){return new Date(r).toLocaleString("en-US",{hour:"numeric",minute:"2-digit",hour12:!0})}function it(r){return new Date(r).toLocaleString("en-US",{month:"short",day:"numeric",year:"numeric"})}function rt(r,e){return ee.default.isAbsolute(r)?ee.default.relative(e,r):r}function at(r,e,t){let s=H(r);if(s.length>0)return rt(s[0],e);if(t){let n=H(t);if(n.length>0)return rt(n[0],e)}return"General"}function Lt(r){let e=new Map;for(let s of r){let n=s.type==="observation"?s.data.created_at:s.data.displayTime,o=it(n);e.has(o)||e.set(o,[]),e.get(o).push(s)}let t=Array.from(e.entries()).sort((s,n)=>{let o=new Date(s[0]).getTime(),i=new Date(n[0]).getTime();return o-i});return new Map(t)}function Mt(r,e){return e.fullObservationField==="narrative"?r.narrative:r.facts?H(r.facts).join(`
`):null}function Dt(r,e,t,s,n,o){let i=[];o?i.push(...qe(r)):i.push(...Ue(r));let a=null,c="",p=!1;for(let l of e)if(l.type==="summary"){p&&(i.push(""),p=!1,a=null,c="");let u=l.data,T=nt(u.displayTime);o?i.push(...Qe(u,T)):i.push(...Fe(u,T))}else{let u=l.data,T=at(u.files_modified,n,u.files_read),E=ot(u.created_at),g=E!==c,h=g?E:"";c=E;let m=t.has(u.id);if(T!==a&&(p&&i.push(""),o?i.push(...Ke(T)):i.push(...ke(T)),a=T,p=!0),m){let b=Mt(u,s);o?i.push(...ze(u,E,g,b,s)):(p&&!o&&(i.push(""),p=!1),i.push(...$e(u,h,b,s)),a=null)}else o?i.push(Je(u,E,g,s)):i.push(we(u,h,s))}return p&&i.push(""),i}function dt(r,e,t,s,n){let o=[],i=Lt(r);for(let[a,c]of i)o.push(...Dt(a,c,e,t,s,n));return o}function ct(r,e,t){return!(!r.showLastSummary||!e||!!!(e.investigated||e.learned||e.completed||e.next_steps)||t&&e.created_at_epoch<=t.created_at_epoch)}function pt(r,e){let t=[];return e?(t.push(...U("Investigated",r.investigated,d.blue)),t.push(...U("Learned",r.learned,d.yellow)),t.push(...U("Completed",r.completed,d.green)),t.push(...U("Next Steps",r.next_steps,d.magenta))):(t.push(...x("Investigated",r.investigated)),t.push(...x("Learned",r.learned)),t.push(...x("Completed",r.completed)),t.push(...x("Next Steps",r.next_steps))),t}function _t(r,e){return e?Ze(r):Pe(r)}function lt(r,e,t){return!P(e)||r.totalDiscoveryTokens<=0||r.savings<=0?[]:t?et(r.totalDiscoveryTokens,r.totalReadTokens):je(r.totalDiscoveryTokens,r.totalReadTokens)}var xt=ut.default.join((0,mt.homedir)(),".claude","plugins","marketplaces","thedotmack","plugin",".install-version");function Ut(){try{return new $}catch(r){if(r.code==="ERR_DLOPEN_FAILED"){try{(0,Et.unlinkSync)(xt)}catch(e){_.debug("SYSTEM","Marker file cleanup failed (may not exist)",{},e)}return _.error("SYSTEM","Native module rebuild needed - restart Claude Code to auto-fix"),null}throw r}}function kt(r,e){return e?tt(r):Xe(r)}function wt(r,e,t,s,n,o,i){let a=[],c=K(e);a.push(...st(r,c,s,i));let p=t.slice(0,s.sessionCount),l=Ae(p,t),u=Z(e,l),T=Ie(e,s.fullObservationCount);a.push(...dt(u,T,s,n,i));let E=t[0],g=e[0];ct(s,E,g)&&a.push(...pt(E,i));let h=Q(e,s,o,n);return a.push(..._t(h,i)),a.push(...lt(c,s,i)),a.join(`
`).trimEnd()}async function te(r,e=!1){let t=Y(),s=r?.cwd??process.cwd(),n=be(s),o=r?.projects||[n],i=Ut();if(!i)return"";try{let a=o.length>1?Ne(i,o,t):J(i,n,t),c=o.length>1?Ce(i,o,t):z(i,n,t);return a.length===0&&c.length===0?kt(n,e):wt(n,a,c,t,s,r?.session_id,e)}finally{i.close()}}0&&(module.exports={generateContext});
