...
`+JSON.stringify(o,null,2):l=" "+this.formatData(o));let m="";if(n){let{sessionId:E,memorySessionId:T,correlationId:h,..._}=n;Object.keys(_).length>0&&(m=` {${Object.entries(_).map(([f,C])=>`${f}=${String(C)}`).join(", ")}}`)}let g=`[${i}] [${a}] [${d}] ${u}${s}${m}${l}`;if(this.logFilePath)try{(0,N.appendFileSync)(this.logFilePath,g+`
...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),l=u?`${String(u[1].split("/").pop())}:${u[2]}`:"unknown",m={...s,location:l};return this.warn(e,`[HAPPY-PATH] ${t}`,m,n),o}},c=new Y;var At={};function Ct(){return typeof __dirname<"u"?__dirname:(0,b.dirname)((0,me.fileURLToPath)(At.url))}var Rt=Ct(),A=y.get("CLAUDE_MEM_DATA_DIR"),V=process.env.CLAUDE_CONFIG_DIR||(0,b.join)((0,ce.homedir)(),".claude"),Wt=(0,b.join)(A,"archives"),Yt=(0,b.join)(A,"logs"),Vt=(0,b.join)(A,"trash"),qt=(0,b.join)(A,"backups"),Kt=(0,b.join)(A,"modes"),Jt=(0,b.join)(A,"settings.json"),_e=(0,b.join)(A,"claude-mem.db"),Qt=(0,b.join)(A,"vector-db"),zt=(0,b.join)(A,"observer-sessions"),Zt=(0,b.join)(V,"settings.json"),es=(0,b.join)(V,"commands"),ts=(0,b.join)(V,"CLAUDE.md");function Ee(r){(0,le.mkdirSync)(r,{recursive:!0})}function ge(){return(0,b.join)(Rt,"..")}function $(r){return!r.request.trim()&&!r.investigated.trim()&&!r.learned.trim()&&!r.completed.trim()&&!r.next_steps.trim()}var P=class{db;constructor(e=_e){e!==":memory:"&&Ee(A),this.db=new U(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn()}initializeSchema(){this.db.run(`
    ...
    `).get(e,t)?.prompt_text??null}storeObservation(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString(),l=this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.type,s.title,s.subtitle,JSON.stringify(s.facts),s.narrative,JSON.stringify(s.concepts),JSON.stringify(s.files_read),JSON.stringify(s.files_modified),n||null,o,d,a);return{id:Number(l.lastInsertRowid),createdAtEpoch:a}}storeSummary(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString();if($(s))return c.warn("DB","Skipping empty summary insert",{memorySessionId:e}),{id:0,createdAtEpoch:a};let l=this.db.prepare(`
    ...
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,d,a);return{id:Number(l.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a){let d=a??Date.now(),u=new Date(d).toISOString();return this.db.transaction(()=>{let m=[],g=this.db.prepare(`
      ...
      `);for(let T of s){let h=g.run(e,t,T.type,T.title,T.subtitle,JSON.stringify(T.facts),T.narrative,JSON.stringify(T.concepts),JSON.stringify(T.files_read),JSON.stringify(T.files_modified),o||null,i,u,d);m.push(Number(h.lastInsertRowid))}let E=null;if(n)if($(n))c.warn("DB","Skipping empty summary insert",{memorySessionId:e});else{let h=this.db.prepare(`
            ...
            (memory_session_id, project, request, investigated, learned, completed,
             next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
          `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,o||null,i,u,d);E=Number(h.lastInsertRowid)}return{observationIds:m,summaryId:E,createdAtEpoch:d}})()}storeObservationsAndMarkComplete(e,t,s,n,o,i,a,d=0,u){let l=u??Date.now(),m=new Date(l).toISOString();return this.db.transaction(()=>{let E=[],T=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      ...
      `).run(l,o),{observationIds:E,summaryId:h,createdAtEpoch:l}})()}getSessionSummariesByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o}=t,i=s==="date_asc"?"ASC":"DESC",a=n?`LIMIT ${String(n)}`:"",d=e.map(()=>"?").join(","),u=[...e],l=o?`WHERE id IN (${d}) AND project = ?`:`WHERE id IN (${d})`;return o&&u.push(o),this.db.prepare(`
      SELECT * FROM session_summaries
      ${l}
      ORDER BY created_at_epoch ${i}
      ${a}
    `).all(...u)}getUserPromptsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o}=t,i=s==="date_asc"?"ASC":"DESC",a=n?`LIMIT ${String(n)}`:"",d=e.map(()=>"?").join(","),u=[...e],l=o?"AND s.project = ?":"";return o&&u.push(o),this.db.prepare(`
      SELECT
        up.*,
        s.project,
        s.memory_session_id
...
`):null}function vt(r,e,t,s,n,o){let i=[];o?i.push(...qe(r)):i.push(...ke(r));let a=null,d="",u=!1;for(let l of e)if(l.type==="summary"){u&&(i.push(""),u=!1,a=null,d="");let m=l.data,g=nt(m.displayTime);o?i.push(...ze(m,g)):i.push(...Pe(m,g))}else{let m=l.data,g=at(m.files_modified,n,m.files_read),E=ot(m.created_at),T=E!==d,h=T?E:"";d=E;let _=t.has(m.id);if(g!==a&&(u&&i.push(""),o?i.push(...Ke(g)):i.push(...Ue(g)),a=g,u=!0),_){let S=Lt(m,s);o?i.push(...Qe(m,E,T,S,s)):(u&&(i.push(""),u=!1),i.push(...$e(m,h,S,s)),a=null)}else o?i.push(Je(m,E,T,s)):i.push(we(m,h,s))}return u&&i.push(""),i}function dt(r,e,t,s,n){let o=[],i=Mt(r);for(let[a,d]of i)o.push(...vt(a,d,e,t,s,n));return o}function pt(r,e,t){return!(!r.showLastSummary||!e||!!!(e.investigated||e.learned||e.completed||e.next_steps)||t&&e.created_at_epoch<=t.created_at_epoch)}function ut(r,e){let t=[];return e?(t.push(...x("Investigated",r.investigated,p.blue)),t.push(...x("Learned",r.learned,p.yellow)),t.push(...x("Completed",r.completed,p.green)),t.push(...x("Next Steps",r.next_steps,p.magenta))):(t.push(...D("Investigated",r.investigated)),t.push(...D("Learned",r.learned)),t.push(...D("Completed",r.completed)),t.push(...D("Next Steps",r.next_steps))),t}function ct(r,e){return e?Ze(r):Fe(r)}function lt(r,e,t){return!j(e)||r.totalDiscoveryTokens<=0||r.savings<=0?[]:t?et(r.totalDiscoveryTokens,r.totalReadTokens):je(r.totalDiscoveryTokens,r.totalReadTokens)}var Dt=mt.default.join((0,_t.homedir)(),".claude","plugins","marketplaces","doublefx","plugin",".install-version");function xt(){try{return new P}catch(r){if(r instanceof Error&&"code"in r&&r.code==="ERR_DLOPEN_FAILED"){try{(0,Et.unlinkSync)(Dt)}catch(e){c.debug("SYSTEM","Marker file cleanup failed (may not exist)",{},e)}return c.error("SYSTEM","Native module rebuild needed - restart Claude Code to auto-fix"),null}throw r}}function kt(r,e){return e?tt(r):Xe(r)}function Ut(r,e,t,s,n,o,i){let a=[],d=Q(e);a.push(...st(r,d,s,i));let u=t.slice(0,s.sessionCount),l=Ne(u,t),m=te(e,l),g=Ie(e,s.fullObservationCount);a.push(...dt(m,g,s,n,i));let E=t[0],T=e[0];pt(s,E,T)&&a.push(...ut(E,i));let h=ee(e,s,o,n);return a.push(...ct(h,i)),a.push(...lt(d,s,i)),a.join(`
`).trimEnd()}function re(r,e=!1){let t=q(),s=r?.cwd??process.cwd(),n=fe(s),o=r?.projects||[n],i=xt();if(!i)return"";try{let a=o.length>1?Re(i,o,t):z(i,n,t),d=o.length>1?Ae(i,o,t):Z(i,n,t);return a.length===0&&d.length===0?kt(n,e):Ut(n,a,d,t,s,r?.session_id,e)}finally{i.close()}}0&&(module.exports={generateContext});
