...
`+JSON.stringify(i,null,2):l=" "+this.formatData(i));let g="";if(n){let{sessionId:m,sdkSessionId:I,correlationId:T,...t}=n;Object.keys(t).length>0&&(g=` {${Object.entries(t).map(([R,b])=>`${R}=${b}`).join(", ")}}`)}let f=`[${a}] [${d}] [${_}] ${E}${r}${g}${l}`;e===3?console.error(f):console.log(f)}debug(e,s,r,n){this.log(0,e,s,r,n)}info(e,s,r,n){this.log(1,e,s,r,n)}warn(e,s,r,n){this.log(2,e,s,r,n)}error(e,s,r,n){this.log(3,e,s,r,n)}dataIn(e,s,r,n){this.info(e,`\u2192 ${s}`,r,n)}dataOut(e,s,r,n){this.info(e,`\u2190 ${s}`,r,n)}success(e,s,r,n){this.info(e,`\u2713 ${s}`,r,n)}failure(e,s,r,n){this.error(e,`\u2717 ${s}`,r,n)}timing(e,s,r,n){this.info(e,`\u23F1 ${s}`,n,{duration:`${r}ms`})}},$=new oe;var U=class{static DEFAULTS={CLAUDE_MEM_MODEL:"claude-haiku-4-5",CLAUDE_MEM_CONTEXT_OBSERVATIONS:"50",CLAUDE_MEM_WORKER_PORT:"37777",CLAUDE_MEM_SKIP_TOOLS:"ListMcpResourcesTool,SlashCommand,Skill,TodoWrite,AskUserQuestion",CLAUDE_MEM_DATA_DIR:(0,he.join)((0,Se.homedir)(),".claude-mem"),CLAUDE_MEM_LOG_LEVEL:"INFO",CLAUDE_MEM_PYTHON_VERSION:"3.13",CLAUDE_CODE_PATH:"",CLAUDE_MEM_CONTEXT_SHOW_READ_TOKENS:"true",CLAUDE_MEM_CONTEXT_SHOW_WORK_TOKENS:"true",CLAUDE_MEM_CONTEXT_SHOW_SAVINGS_AMOUNT:"true",CLAUDE_MEM_CONTEXT_SHOW_SAVINGS_PERCENT:"true",CLAUDE_MEM_CONTEXT_OBSERVATION_TYPES:Te,CLAUDE_MEM_CONTEXT_OBSERVATION_CONCEPTS:ge,CLAUDE_MEM_CONTEXT_FULL_COUNT:"5",CLAUDE_MEM_CONTEXT_FULL_FIELD:"narrative",CLAUDE_MEM_CONTEXT_SESSION_COUNT:"10",CLAUDE_MEM_CONTEXT_SHOW_LAST_SUMMARY:"true",CLAUDE_MEM_CONTEXT_SHOW_LAST_MESSAGE:"false"};static getAllDefaults(){return{...this.DEFAULTS}}static get(e){return this.DEFAULTS[e]}static getInt(e){let s=this.get(e);return parseInt(s,10)}static getBool(e){return this.get(e)==="true"}static loadFromFile(e){if(!(0,j.existsSync)(e))return this.getAllDefaults();let s=(0,j.readFileSync)(e,"utf-8"),r=JSON.parse(s),n=r;if(r.env&&typeof r.env=="object"){n=r.env;try{(0,j.writeFileSync)(e,JSON.stringify(n,null,2),"utf-8"),$.info("SETTINGS","Migrated settings file from nested to flat schema",{settingsPath:e})}catch(a){$.warn("SETTINGS","Failed to auto-migrate settings file",{settingsPath:e},a)}}let i={...this.DEFAULTS};for(let a of Object.keys(this.DEFAULTS))n[a]!==void 0&&(i[a]=n[a]);return i}};var We={};function Pe(){return typeof __dirname<"u"?__dirname:(0,O.dirname)((0,Oe.fileURLToPath)(We.url))}var _s=Pe(),y=U.get("CLAUDE_MEM_DATA_DIR"),ie=process.env.CLAUDE_CONFIG_DIR||(0,O.join)((0,fe.homedir)(),".claude"),us=(0,O.join)(y,"archives"),ls=(0,O.join)(y,"logs"),Es=(0,O.join)(y,"trash"),ms=(0,O.join)(y,"backups"),Ts=(0,O.join)(y,"settings.json"),Re=(0,O.join)(y,"claude-mem.db"),gs=(0,O.join)(y,"vector-db"),hs=(0,O.join)(ie,"settings.json"),Ss=(0,O.join)(ie,"commands"),fs=(0,O.join)(ie,"CLAUDE.md");function Ne(c){(0,be.mkdirSync)(c,{recursive:!0})}var J=class{db;constructor(){Ne(y),this.db=new Ae.default(Re),this.db.pragma("journal_mode = WAL"),this.db.pragma("synchronous = NORMAL"),this.db.pragma("foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn()}initializeSchema(){try{this.db.exec(`
    ...
    `).get(e)||null}getFilesForSession(e){let r=this.db.prepare(`
      ...
      FROM observations
      WHERE sdk_session_id = ?
    `).all(e),n=new Set,i=new Set;for(let a of r){if(a.files_read)try{let d=JSON.parse(a.files_read);Array.isArray(d)&&d.forEach(_=>n.add(_))}catch{}if(a.files_modified)try{let d=JSON.parse(a.files_modified);Array.isArray(d)&&d.forEach(_=>i.add(_))}catch{}}return{filesRead:Array.from(n),filesModified:Array.from(i)}}getSessionById(e){return this.db.prepare(`
    ...
    `).get(e)||(this.db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, 'active')
      `).run(e,e,s,a.toISOString(),d),console.error(`[SessionStore] Auto-created session record for session_id: ${e}`));let g=this.db.prepare(`
      INSERT INTO observations
      (sdk_session_id, project, type, title, subtitle, facts, narrative, concepts,
       files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,s,r.type,r.title,r.subtitle,JSON.stringify(r.facts),r.narrative,JSON.stringify(r.concepts),JSON.stringify(r.files_read),JSON.stringify(r.files_modified),n||null,i,a.toISOString(),d);return{id:Number(g.lastInsertRowid),createdAtEpoch:d}}storeSummary(e,s,r,n,i=0){let a=new Date,d=a.getTime();this.db.prepare(`
      SELECT id FROM sdk_sessions WHERE sdk_session_id = ?
    `).get(e)||(this.db.prepare(`
        INSERT INTO sdk_sessions
        (claude_session_id, sdk_session_id, project, started_at, started_at_epoch, status)
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,s,r.request,r.investigated,r.learned,r.completed,r.next_steps,r.notes,n||null,i,a.toISOString(),d);return{id:Number(g.lastInsertRowid),createdAtEpoch:d}}markSessionCompleted(e){let s=new Date,r=s.getTime();this.db.prepare(`
      UPDATE sdk_sessions
    ...
    `).run(s.toISOString(),r,e)}markSessionFailed(e){let s=new Date,r=s.getTime();this.db.prepare(`
      ...
      WHERE id = ?
    `).run(s.toISOString(),r,e)}getSessionSummariesByIds(e,s={}){if(e.length===0)return[];let{orderBy:r="date_desc",limit:n,project:i}=s,a=r==="date_asc"?"ASC":"DESC",d=n?`LIMIT ${n}`:"",_=e.map(()=>"?").join(","),E=[...e],l=i?`WHERE id IN (${_}) AND project = ?`:`WHERE id IN (${_})`;return i&&E.push(i),this.db.prepare(`
      SELECT * FROM session_summaries
      ${l}
      ORDER BY created_at_epoch ${a}
      ${d}
    `).all(...E)}getUserPromptsByIds(e,s={}){if(e.length===0)return[];let{orderBy:r="date_desc",limit:n,project:i}=s,a=r==="date_asc"?"ASC":"DESC",d=n?`LIMIT ${n}`:"",_=e.map(()=>"?").join(","),E=[...e],l=i?"AND s.project = ?":"";return i&&E.push(i),this.db.prepare(`
      SELECT
        up.*,
        s.project,
        s.sdk_session_id
      ...
      ${d}
    `).all(...E)}getTimelineAroundTimestamp(e,s=10,r=10,n){return this.getTimelineAroundObservation(null,e,s,r,n)}getTimelineAroundObservation(e,s,r=10,n=10,i){let a=i?"AND project = ?":"",d=i?[i]:[],_,E;if(e!==null){let m=`
      ...
      `,I=`
        ...
        ORDER BY id ASC
        LIMIT ?
      `;try{let T=this.db.prepare(m).all(e,...d,r+1),t=this.db.prepare(I).all(e,...d,n+1);if(T.length===0&&t.length===0)return{observations:[],sessions:[],prompts:[]};_=T.length>0?T[T.length-1].created_at_epoch:s,E=t.length>0?t[t.length-1].created_at_epoch:s}catch(T){return console.error("[SessionStore] Error getting boundary observations:",T.message),{observations:[],sessions:[],prompts:[]}}}else{let m=`
      ...
      `,I=`
        ...
        ORDER BY created_at_epoch ASC
        LIMIT ?
      `;try{let T=this.db.prepare(m).all(s,...d,r),t=this.db.prepare(I).all(s,...d,n+1);if(T.length===0&&t.length===0)return{observations:[],sessions:[],prompts:[]};_=T.length>0?T[T.length-1].created_at_epoch:s,E=t.length>0?t[t.length-1].created_at_epoch:s}catch(T){return console.error("[SessionStore] Error getting boundary timestamps:",T.message),{observations:[],sessions:[],prompts:[]}}}let l=`
      SELECT *
      FROM observations
...
`).filter(n=>n.trim()),r="";for(let n=s.length-1;n>=0;n--)try{let i=s[n];if(!i.includes('"type":"assistant"'))continue;let a=JSON.parse(i);if(a.type==="assistant"&&a.message?.content&&Array.isArray(a.message.content)){let d="";for(let _ of a.message.content)_.type==="text"&&(d+=_.text);if(d=d.replace(/<system-reminder>[\s\S]*?<\/system-reminder>/g,"").trim(),d){r=d;break}}}catch{continue}return{userMessage:"",assistantMessage:r}}catch(e){return $.failure("WORKER","Failed to extract prior messages from transcript",{transcriptPath:c},e),{userMessage:"",assistantMessage:""}}}async function Qe(c,e=!1){let s=Be(),r=c?.cwd??process.cwd(),n=r?w.default.basename(r):"unknown-project",i=null;try{i=new J}catch(N){if(N.code==="ERR_DLOPEN_FAILED"){try{(0,B.unlinkSync)(je)}catch{}return console.error("Native module rebuild needed - restart Claude Code to auto-fix"),""}throw N}let a=Array.from(s.observationTypes),d=a.map(()=>"?").join(","),_=Array.from(s.observationConcepts),E=_.map(()=>"?").join(","),l=i.db.prepare(`
    ...
    ORDER BY created_at_epoch DESC
    LIMIT ?
  `).all(n,...a,..._,s.totalObservationCount),g=i.db.prepare(`
    SELECT id, sdk_session_id, request, investigated, learned, completed, next_steps, created_at, created_at_epoch
    FROM session_summaries
    WHERE project = ?
    ORDER BY created_at_epoch DESC
    LIMIT ?
  `).all(n,s.sessionCount+He),f="",m="";if(s.showLastMessage&&l.length>0)try{let N=c?.session_id,R=l.find(b=>b.sdk_session_id!==N);if(R){let b=R.sdk_session_id,D=qe(r),F=w.default.join((0,z.homedir)(),".claude","projects",D,`${b}.jsonl`),H=Je(F);f=H.userMessage,m=H.assistantMessage}}catch{}if(l.length===0&&g.length===0)return i?.close(),e?`
