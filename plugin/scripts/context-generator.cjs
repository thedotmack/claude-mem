...
`+JSON.stringify(o,null,2):m=" "+this.formatData(o));let u="";if(n){let{sessionId:E,memorySessionId:T,correlationId:h,..._}=n;Object.keys(_).length>0&&(u=` {${Object.entries(_).map(([S,C])=>`${S}=${C}`).join(", ")}}`)}let g=`[${i}] [${a}] [${d}] ${c}${s}${u}${m}`;if(this.logFilePath)try{(0,I.appendFileSync)(this.logFilePath,g+`
...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),m=c?`${c[1].split("/").pop()}:${c[2]}`:"unknown",u={...s,location:m};return this.warn(e,`[HAPPY-PATH] ${t}`,u,n),o}},l=new G;var At={};function Ot(){return typeof __dirname<"u"?__dirname:(0,b.dirname)((0,le.fileURLToPath)(At.url))}var Ct=Ot(),R=N.get("CLAUDE_MEM_DATA_DIR"),Y=process.env.CLAUDE_CONFIG_DIR||(0,b.join)((0,pe.homedir)(),".claude"),Gt=(0,b.join)(R,"archives"),Yt=(0,b.join)(R,"logs"),Vt=(0,b.join)(R,"trash"),qt=(0,b.join)(R,"backups"),Kt=(0,b.join)(R,"modes"),Jt=(0,b.join)(R,"settings.json"),ue=(0,b.join)(R,"claude-mem.db"),zt=(0,b.join)(R,"vector-db"),Qt=(0,b.join)(R,"observer-sessions"),Zt=(0,b.join)(Y,"settings.json"),es=(0,b.join)(Y,"commands"),ts=(0,b.join)(Y,"CLAUDE.md");function me(r){(0,ce.mkdirSync)(r,{recursive:!0})}function _e(){return(0,b.join)(Ct,"..")}function $(r){return!r.request?.trim()&&!r.investigated?.trim()&&!r.learned?.trim()&&!r.completed?.trim()&&!r.next_steps?.trim()}var w=class{db;constructor(e=ue){e!==":memory:"&&me(R),this.db=new Ee.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn()}initializeSchema(){this.db.run(`
    ...
    `).get(e,t)?.prompt_text??null}storeObservation(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString(),m=this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.type,s.title,s.subtitle,JSON.stringify(s.facts),s.narrative,JSON.stringify(s.concepts),JSON.stringify(s.files_read),JSON.stringify(s.files_modified),n||null,o,d,a);return{id:Number(m.lastInsertRowid),createdAtEpoch:a}}storeSummary(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString(),c=this.db.prepare("SELECT id FROM session_summaries WHERE memory_session_id = ? LIMIT 1").get(e);if(c)return $(s)?(l.warn("DB","Skipping summary update: new summary has all empty fields, preserving existing",{memorySessionId:e,existingId:c.id}),{id:c.id,createdAtEpoch:a}):(this.db.prepare(`
        UPDATE session_summaries
        SET project=?, request=?, investigated=?, learned=?, completed=?,
            next_steps=?, notes=?, prompt_number=?, discovery_tokens=?,
            created_at=?, created_at_epoch=?
        WHERE id = ?
      `).run(t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,d,a,c.id),{id:c.id,createdAtEpoch:a});let u=this.db.prepare(`
      INSERT INTO session_summaries
      (memory_session_id, project, request, investigated, learned, completed,
       next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,d,a);return{id:Number(u.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a){let d=a??Date.now(),c=new Date(d).toISOString();return this.db.transaction(()=>{let u=[],g=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let T of s){let h=g.run(e,t,T.type,T.title,T.subtitle,JSON.stringify(T.facts),T.narrative,JSON.stringify(T.concepts),JSON.stringify(T.files_read),JSON.stringify(T.files_modified),o||null,i,c,d);u.push(Number(h.lastInsertRowid))}let E=null;if(n){let T=this.db.prepare("SELECT id FROM session_summaries WHERE memory_session_id = ? LIMIT 1").get(e);if(T)$(n)?(l.warn("DB","Skipping summary update: new summary has all empty fields, preserving existing",{memorySessionId:e,existingId:T.id}),E=T.id):(this.db.prepare(`
              UPDATE session_summaries
              SET project=?, request=?, investigated=?, learned=?, completed=?,
                  next_steps=?, notes=?, prompt_number=?, discovery_tokens=?,
                  created_at=?, created_at_epoch=?
              WHERE id = ?
            `).run(t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,o||null,i,c,d,T.id),E=T.id);else{let _=this.db.prepare(`
            INSERT INTO session_summaries
            (memory_session_id, project, request, investigated, learned, completed,
             next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
          `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,o||null,i,c,d);E=Number(_.lastInsertRowid)}}return{observationIds:u,summaryId:E,createdAtEpoch:d}})()}storeObservationsAndMarkComplete(e,t,s,n,o,i,a,d=0,c){let m=c??Date.now(),u=new Date(m).toISOString();return this.db.transaction(()=>{let E=[],T=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let f of s){let S=T.run(e,t,f.type,f.title,f.subtitle,JSON.stringify(f.facts),f.narrative,JSON.stringify(f.concepts),JSON.stringify(f.files_read),JSON.stringify(f.files_modified),a||null,d,u,m);E.push(Number(S.lastInsertRowid))}let h;if(n){let f=this.db.prepare("SELECT id FROM session_summaries WHERE memory_session_id = ? LIMIT 1").get(e);if(f)$(n)?(l.warn("DB","Skipping summary update: new summary has all empty fields, preserving existing",{memorySessionId:e,existingId:f.id}),h=f.id):(this.db.prepare(`
              UPDATE session_summaries
              SET project=?, request=?, investigated=?, learned=?, completed=?,
                  next_steps=?, notes=?, prompt_number=?, discovery_tokens=?,
                  created_at=?, created_at_epoch=?
...
`):null}function Lt(r,e,t,s,n,o){let i=[];o?i.push(...Ve(r)):i.push(...xe(r));let a=null,d="",c=!1;for(let m of e)if(m.type==="summary"){c&&(i.push(""),c=!1,a=null,d="");let u=m.data,g=rt(u.displayTime);o?i.push(...ze(u,g)):i.push(...we(u,g))}else{let u=m.data,g=it(u.files_modified,n,u.files_read),E=nt(u.created_at),T=E!==d,h=T?E:"";d=E;let _=t.has(u.id);if(g!==a&&(c&&i.push(""),o?i.push(...qe(g)):i.push(...ke(g)),a=g,c=!0),_){let f=yt(u,s);o?i.push(...Je(u,E,T,f,s)):(c&&!o&&(i.push(""),c=!1),i.push(...$e(u,h,f,s)),a=null)}else o?i.push(Ke(u,E,T,s)):i.push(Ue(u,h,s))}return c&&i.push(""),i}function at(r,e,t,s,n){let o=[],i=Mt(r);for(let[a,d]of i)o.push(...Lt(a,d,e,t,s,n));return o}function dt(r,e,t){return!(!r.showLastSummary||!e||!!!(e.investigated||e.learned||e.completed||e.next_steps)||t&&e.created_at_epoch<=t.created_at_epoch)}function pt(r,e){let t=[];return e?(t.push(...x("Investigated",r.investigated,p.blue)),t.push(...x("Learned",r.learned,p.yellow)),t.push(...x("Completed",r.completed,p.green)),t.push(...x("Next Steps",r.next_steps,p.magenta))):(t.push(...D("Investigated",r.investigated)),t.push(...D("Learned",r.learned)),t.push(...D("Completed",r.completed)),t.push(...D("Next Steps",r.next_steps))),t}function ct(r,e){return e?Qe(r):Pe(r)}function lt(r,e,t){return!F(e)||r.totalDiscoveryTokens<=0||r.savings<=0?[]:t?Ze(r.totalDiscoveryTokens,r.totalReadTokens):Fe(r.totalDiscoveryTokens,r.totalReadTokens)}var vt=ut.default.join((0,mt.homedir)(),".claude","plugins","marketplaces","thedotmack","plugin",".install-version");function Dt(){try{return new w}catch(r){if(r.code==="ERR_DLOPEN_FAILED"){try{(0,_t.unlinkSync)(vt)}catch(e){l.debug("SYSTEM","Marker file cleanup failed (may not exist)",{},e)}return l.error("SYSTEM","Native module rebuild needed - restart Claude Code to auto-fix"),null}throw r}}function xt(r,e){return e?et(r):je(r)}function kt(r,e,t,s,n,o,i){let a=[],d=J(e);a.push(...tt(r,d,s,i));let c=t.slice(0,s.sessionCount),m=Re(c,t),u=ee(e,m),g=Ie(e,s.fullObservationCount);a.push(...at(u,g,s,n,i));let E=t[0],T=e[0];dt(s,E,T)&&a.push(...pt(E,i));let h=Z(e,s,o,n);return a.push(...ct(h,i)),a.push(...lt(d,s,i)),a.join(`
