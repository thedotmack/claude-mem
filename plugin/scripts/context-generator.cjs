...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),l=u?`${u[1].split("/").pop()}:${u[2]}`:"unknown",m={...r,location:l};return this.warn(e,`[HAPPY-PATH] ${s}`,m,n),i}},M=new ne;var k=class{static DEFAULTS={CLAUDE_MEM_MODEL:"claude-sonnet-4-5",CLAUDE_MEM_CONTEXT_OBSERVATIONS:"50",CLAUDE_MEM_WORKER_PORT:"37777",CLAUDE_MEM_WORKER_HOST:"127.0.0.1",CLAUDE_MEM_SKIP_TOOLS:"ListMcpResourcesTool,SlashCommand,Skill,TodoWrite,AskUserQuestion",CLAUDE_MEM_DATA_DIR:(0,Se.join)((0,be.homedir)(),".claude-mem"),CLAUDE_MEM_LOG_LEVEL:"INFO",CLAUDE_MEM_PYTHON_VERSION:"3.13",CLAUDE_CODE_PATH:"",CLAUDE_MEM_CONTEXT_SHOW_READ_TOKENS:"true",CLAUDE_MEM_CONTEXT_SHOW_WORK_TOKENS:"true",CLAUDE_MEM_CONTEXT_SHOW_SAVINGS_AMOUNT:"true",CLAUDE_MEM_CONTEXT_SHOW_SAVINGS_PERCENT:"true",CLAUDE_MEM_CONTEXT_OBSERVATION_TYPES:ge,CLAUDE_MEM_CONTEXT_OBSERVATION_CONCEPTS:he,CLAUDE_MEM_CONTEXT_FULL_COUNT:"5",CLAUDE_MEM_CONTEXT_FULL_FIELD:"narrative",CLAUDE_MEM_CONTEXT_SESSION_COUNT:"10",CLAUDE_MEM_CONTEXT_SHOW_LAST_SUMMARY:"true",CLAUDE_MEM_CONTEXT_SHOW_LAST_MESSAGE:"false"};static getAllDefaults(){return{...this.DEFAULTS}}static get(e){return this.DEFAULTS[e]}static getInt(e){let s=this.get(e);return parseInt(s,10)}static getBool(e){return this.get(e)==="true"}static loadFromFile(e){if(!(0,P.existsSync)(e))return this.getAllDefaults();let s=(0,P.readFileSync)(e,"utf-8"),r=JSON.parse(s),n=r;if(r.env&&typeof r.env=="object"){n=r.env;try{(0,P.writeFileSync)(e,JSON.stringify(n,null,2),"utf-8"),M.info("SETTINGS","Migrated settings file from nested to flat schema",{settingsPath:e})}catch(a){M.warn("SETTINGS","Failed to auto-migrate settings file",{settingsPath:e},a)}}let i={...this.DEFAULTS};for(let a of Object.keys(this.DEFAULTS))n[a]!==void 0&&(i[a]=n[a]);return i}};var Ge={};function Be(){return typeof __dirname<"u"?__dirname:(0,O.dirname)((0,Re.fileURLToPath)(Ge.url))}var us=Be(),C=k.get("CLAUDE_MEM_DATA_DIR"),oe=process.env.CLAUDE_CONFIG_DIR||(0,O.join)((0,fe.homedir)(),".claude"),ls=(0,O.join)(C,"archives"),Es=(0,O.join)(C,"logs"),ms=(0,O.join)(C,"trash"),Ts=(0,O.join)(C,"backups"),gs=(0,O.join)(C,"settings.json"),Ne=(0,O.join)(C,"claude-mem.db"),hs=(0,O.join)(C,"vector-db"),Ss=(0,O.join)(oe,"settings.json"),bs=(0,O.join)(oe,"commands"),fs=(0,O.join)(oe,"CLAUDE.md");function Ae(c){(0,Oe.mkdirSync)(c,{recursive:!0})}var q=class{db;constructor(){Ae(C),this.db=new Ie.Database(Ne),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable()}initializeSchema(){try{this.db.run(`
    ...
    `).all(e)}getObservationById(e){return this.db.prepare(`
      ...
      FROM observations
      WHERE id = ?
    `).get(e)||null}getObservationsByIds(e,s={}){if(e.length===0)return[];let{orderBy:r="date_desc",limit:n,project:i,type:a,concepts:d,files:_}=s,u=r==="date_asc"?"ASC":"DESC",l=n?`LIMIT ${n}`:"",m=e.map(()=>"?").join(","),b=[...e],T=[];if(i&&(T.push("project = ?"),b.push(i)),a)if(Array.isArray(a)){let t=a.map(()=>"?").join(",");T.push(`type IN (${t})`),b.push(...a)}else T.push("type = ?"),b.push(a);if(d){let t=Array.isArray(d)?d:[d],N=t.map(()=>"EXISTS (SELECT 1 FROM json_each(concepts) WHERE value = ?)");b.push(...t),T.push(`(${N.join(" OR ")})`)}if(_){let t=Array.isArray(_)?_:[_],N=t.map(()=>"(EXISTS (SELECT 1 FROM json_each(files_read) WHERE value LIKE ?) OR EXISTS (SELECT 1 FROM json_each(files_modified) WHERE value LIKE ?))");t.forEach(R=>{b.push(`%${R}%`,`%${R}%`)}),T.push(`(${N.join(" OR ")})`)}let A=T.length>0?`WHERE id IN (${m}) AND ${T.join(" AND ")}`:`WHERE id IN (${m})`;return this.db.prepare(`
      SELECT *
      FROM observations
      ${A}
    ...
    `).run(s.toISOString(),r,e)}markSessionFailed(e){let s=new Date,r=s.getTime();this.db.prepare(`
      ...
      WHERE id = ?
    `).run(s.toISOString(),r,e)}getSessionSummariesByIds(e,s={}){if(e.length===0)return[];let{orderBy:r="date_desc",limit:n,project:i}=s,a=r==="date_asc"?"ASC":"DESC",d=n?`LIMIT ${n}`:"",_=e.map(()=>"?").join(","),u=[...e],l=i?`WHERE id IN (${_}) AND project = ?`:`WHERE id IN (${_})`;return i&&u.push(i),this.db.prepare(`
      SELECT * FROM session_summaries
      ${l}
      ORDER BY created_at_epoch ${a}
      ${d}
    `).all(...u)}getUserPromptsByIds(e,s={}){if(e.length===0)return[];let{orderBy:r="date_desc",limit:n,project:i}=s,a=r==="date_asc"?"ASC":"DESC",d=n?`LIMIT ${n}`:"",_=e.map(()=>"?").join(","),u=[...e],l=i?"AND s.project = ?":"";return i&&u.push(i),this.db.prepare(`
      SELECT
        up.*,
        s.project,
        s.sdk_session_id
      ...
      ${d}
    `).all(...u)}getTimelineAroundTimestamp(e,s=10,r=10,n){return this.getTimelineAroundObservation(null,e,s,r,n)}getTimelineAroundObservation(e,s,r=10,n=10,i){let a=i?"AND project = ?":"",d=i?[i]:[],_,u;if(e!==null){let T=`
      ...
      `,A=`
        ...
        ORDER BY created_at_epoch ASC
        LIMIT ?
      `;try{let g=this.db.prepare(T).all(s,...d,r),t=this.db.prepare(A).all(s,...d,n+1);if(g.length===0&&t.length===0)return{observations:[],sessions:[],prompts:[]};_=g.length>0?g[g.length-1].created_at_epoch:s,u=t.length>0?t[t.length-1].created_at_epoch:s}catch(g){return console.error("[SessionStore] Error getting boundary timestamps:",g.message,i?`(project: ${i})`:"(all projects)"),{observations:[],sessions:[],prompts:[]}}}let l=`
      SELECT *
      FROM observations
...
`).filter(n=>n.trim()),r="";for(let n=s.length-1;n>=0;n--)try{let i=s[n];if(!i.includes('"type":"assistant"'))continue;let a=JSON.parse(i);if(a.type==="assistant"&&a.message?.content&&Array.isArray(a.message.content)){let d="";for(let _ of a.message.content)_.type==="text"&&(d+=_.text);if(d=d.replace(/<system-reminder>[\s\S]*?<\/system-reminder>/g,"").trim(),d){r=d;break}}}catch{continue}return{userMessage:"",assistantMessage:r}}catch(e){return M.failure("WORKER","Failed to extract prior messages from transcript",{transcriptPath:c},e),{userMessage:"",assistantMessage:""}}}async function ze(c,e=!1){let s=Ke(),r=c?.cwd??process.cwd(),n=r?H.default.basename(r):"unknown-project",i=null;try{i=new q}catch(N){if(N.code==="ERR_DLOPEN_FAILED"){try{(0,W.unlinkSync)(Ve)}catch{}return console.error("Native module rebuild needed - restart Claude Code to auto-fix"),""}throw N}let a=Array.from(s.observationTypes),d=a.map(()=>"?").join(","),_=Array.from(s.observationConcepts),u=_.map(()=>"?").join(","),l=i.db.prepare(`
    ...
    ORDER BY created_at_epoch DESC
    LIMIT ?
  `).all(n,...a,..._,s.totalObservationCount),m=i.db.prepare(`
    SELECT id, sdk_session_id, request, investigated, learned, completed, next_steps, created_at, created_at_epoch
    FROM session_summaries
    WHERE project = ?
    ORDER BY created_at_epoch DESC
    LIMIT ?
  `).all(n,s.sessionCount+qe),b="",T="";if(s.showLastMessage&&l.length>0)try{let N=c?.session_id,R=l.find(f=>f.sdk_session_id!==N);if(R){let f=R.sdk_session_id,y=Je(r),w=H.default.join((0,Q.homedir)(),".claude","projects",y,`${f}.jsonl`),j=Qe(w);b=j.userMessage,T=j.assistantMessage}}catch{}if(l.length===0&&m.length===0)return i?.close(),e?`
