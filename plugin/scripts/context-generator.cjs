...
`+JSON.stringify(o,null,2):m=" "+this.formatData(o));let u="";if(n){let{sessionId:E,memorySessionId:T,correlationId:b,..._}=n;Object.keys(_).length>0&&(u=` {${Object.entries(_).map(([f,R])=>`${f}=${R}`).join(", ")}}`)}let g=`[${i}] [${a}] [${d}] ${p}${s}${u}${m}`;if(this.logFilePath)try{(0,y.appendFileSync)(this.logFilePath,g+`
