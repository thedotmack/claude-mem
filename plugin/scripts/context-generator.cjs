...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),u=c?`${c[1].split("/").pop()}:${c[2]}`:"unknown",_={...s,location:u};return this.warn(e,`[HAPPY-PATH] ${t}`,_,n),o}},m=new B;var R=class{static DEFAULTS={CLAUDE_MEM_MODEL:"claude-sonnet-4-5",CLAUDE_MEM_CONTEXT_OBSERVATIONS:"50",CLAUDE_MEM_WORKER_PORT:"37777",CLAUDE_MEM_WORKER_HOST:"127.0.0.1",CLAUDE_MEM_SKIP_TOOLS:"ListMcpResourcesTool,SlashCommand,Skill,TodoWrite,AskUserQuestion",CLAUDE_MEM_PROVIDER:"claude",CLAUDE_MEM_GEMINI_API_KEY:"",CLAUDE_MEM_GEMINI_MODEL:"gemini-2.5-flash-lite",CLAUDE_MEM_GEMINI_RATE_LIMITING_ENABLED:"true",CLAUDE_MEM_OPENROUTER_API_KEY:"",CLAUDE_MEM_OPENROUTER_MODEL:"xiaomi/mimo-v2-flash:free",CLAUDE_MEM_OPENROUTER_SITE_URL:"",CLAUDE_MEM_OPENROUTER_APP_NAME:"claude-mem",CLAUDE_MEM_OPENROUTER_MAX_CONTEXT_MESSAGES:"20",CLAUDE_MEM_OPENROUTER_MAX_TOKENS:"100000",CLAUDE_MEM_DATA_DIR:(0,re.join)((0,ne.homedir)(),".claude-mem"),CLAUDE_MEM_LOG_LEVEL:"INFO",CLAUDE_MEM_PYTHON_VERSION:"3.13",CLAUDE_CODE_PATH:"",CLAUDE_MEM_MODE:"code",CLAUDE_MEM_CONTEXT_SHOW_READ_TOKENS:"true",CLAUDE_MEM_CONTEXT_SHOW_WORK_TOKENS:"true",CLAUDE_MEM_CONTEXT_SHOW_SAVINGS_AMOUNT:"true",CLAUDE_MEM_CONTEXT_SHOW_SAVINGS_PERCENT:"true",CLAUDE_MEM_CONTEXT_OBSERVATION_TYPES:te,CLAUDE_MEM_CONTEXT_OBSERVATION_CONCEPTS:se,CLAUDE_MEM_CONTEXT_FULL_COUNT:"5",CLAUDE_MEM_CONTEXT_FULL_FIELD:"narrative",CLAUDE_MEM_CONTEXT_SESSION_COUNT:"10",CLAUDE_MEM_CONTEXT_SHOW_LAST_SUMMARY:"true",CLAUDE_MEM_CONTEXT_SHOW_LAST_MESSAGE:"false"};static getAllDefaults(){return{...this.DEFAULTS}}static get(e){return this.DEFAULTS[e]}static getInt(e){let t=this.get(e);return parseInt(t,10)}static getBool(e){return this.get(e)==="true"}static loadFromFile(e){try{if(!(0,y.existsSync)(e))return this.getAllDefaults();let t=(0,y.readFileSync)(e,"utf-8"),s=JSON.parse(t),n=s;if(s.env&&typeof s.env=="object"){n=s.env;try{(0,y.writeFileSync)(e,JSON.stringify(n,null,2),"utf-8"),m.info("SETTINGS","Migrated settings file from nested to flat schema",{settingsPath:e})}catch(i){m.warn("SETTINGS","Failed to auto-migrate settings file",{settingsPath:e},i)}}let o={...this.DEFAULTS};for(let i of Object.keys(this.DEFAULTS))n[i]!==void 0&&(o[i]=n[i]);return o}catch(t){return m.warn("SETTINGS","Failed to load settings, using defaults",{settingsPath:e},t),this.getAllDefaults()}}};var gt={};function _t(){return typeof __dirname<"u"?__dirname:(0,S.dirname)((0,ae.fileURLToPath)(gt.url))}var Et=_t(),I=R.get("CLAUDE_MEM_DATA_DIR"),G=process.env.CLAUDE_CONFIG_DIR||(0,S.join)((0,oe.homedir)(),".claude"),jt=(0,S.join)(I,"archives"),Xt=(0,S.join)(I,"logs"),Bt=(0,S.join)(I,"trash"),Gt=(0,S.join)(I,"backups"),Ht=(0,S.join)(I,"modes"),Wt=(0,S.join)(I,"settings.json"),de=(0,S.join)(I,"claude-mem.db"),Yt=(0,S.join)(I,"vector-db"),Vt=(0,S.join)(G,"settings.json"),Kt=(0,S.join)(G,"commands"),qt=(0,S.join)(G,"CLAUDE.md");function pe(r){(0,ie.mkdirSync)(r,{recursive:!0})}function me(){return(0,S.join)(Et,"..")}var $=class{db;constructor(e=de){e!==":memory:"&&pe(I),this.db=new ce.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn()}initializeSchema(){this.db.run(`
    ...
    `);let e=this.db.prepare("SELECT version FROM schema_versions ORDER BY version").all();(e.length>0?Math.max(...e.map(s=>s.version)):0)===0&&(m.info("DB","Initializing fresh database with migration004"),this.db.run(`
      ...
      `),this.db.prepare("INSERT INTO schema_versions (version, applied_at) VALUES (?, ?)").run(4,new Date().toISOString()),m.info("DB","Migration004 applied successfully"))}ensureWorkerPortColumn(){if(this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(5))return;this.db.query("PRAGMA table_info(sdk_sessions)").all().some(n=>n.name==="worker_port")||(this.db.run("ALTER TABLE sdk_sessions ADD COLUMN worker_port INTEGER"),m.info("DB","Added worker_port column to sdk_sessions table")),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(5,new Date().toISOString())}ensurePromptTrackingColumns(){if(this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(6))return;this.db.query("PRAGMA table_info(sdk_sessions)").all().some(p=>p.name==="prompt_counter")||(this.db.run("ALTER TABLE sdk_sessions ADD COLUMN prompt_counter INTEGER DEFAULT 0"),m.info("DB","Added prompt_counter column to sdk_sessions table")),this.db.query("PRAGMA table_info(observations)").all().some(p=>p.name==="prompt_number")||(this.db.run("ALTER TABLE observations ADD COLUMN prompt_number INTEGER"),m.info("DB","Added prompt_number column to observations table")),this.db.query("PRAGMA table_info(session_summaries)").all().some(p=>p.name==="prompt_number")||(this.db.run("ALTER TABLE session_summaries ADD COLUMN prompt_number INTEGER"),m.info("DB","Added prompt_number column to session_summaries table")),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(6,new Date().toISOString())}removeSessionSummariesUniqueConstraint(){if(this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(7))return;if(!this.db.query("PRAGMA index_list(session_summaries)").all().some(n=>n.unique===1)){this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(7,new Date().toISOString());return}m.info("DB","Removing UNIQUE constraint from session_summaries.memory_session_id"),this.db.run("BEGIN TRANSACTION"),this.db.run(`
    ...
    `).all(e)}getObservationById(e){return this.db.prepare(`
      ...
      FROM observations
      WHERE id = ?
    `).get(e)||null}getObservationsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o,type:i,concepts:a,files:p}=t,c=s==="date_asc"?"ASC":"DESC",u=n?`LIMIT ${n}`:"",_=e.map(()=>"?").join(","),g=[...e],E=[];if(o&&(E.push("project = ?"),g.push(o)),i)if(Array.isArray(i)){let l=i.map(()=>"?").join(",");E.push(`type IN (${l})`),g.push(...i)}else E.push("type = ?"),g.push(i);if(a){let l=Array.isArray(a)?a:[a],f=l.map(()=>"EXISTS (SELECT 1 FROM json_each(concepts) WHERE value = ?)");g.push(...l),E.push(`(${f.join(" OR ")})`)}if(p){let l=Array.isArray(p)?p:[p],f=l.map(()=>"(EXISTS (SELECT 1 FROM json_each(files_read) WHERE value LIKE ?) OR EXISTS (SELECT 1 FROM json_each(files_modified) WHERE value LIKE ?))");l.forEach(T=>{g.push(`%${T}%`,`%${T}%`)}),E.push(`(${f.join(" OR ")})`)}let b=E.length>0?`WHERE id IN (${_}) AND ${E.join(" AND ")}`:`WHERE id IN (${_})`;return this.db.prepare(`
      SELECT *
      FROM observations
      ${b}
    ...
    `).run(e,t,s,n.toISOString(),o).lastInsertRowid}getUserPrompt(e,t){return this.db.prepare(`
      ...
      LIMIT 1
    `).get(e,t)?.prompt_text??null}storeObservation(e,t,s,n,o=0,i){let a=i??Date.now(),p=new Date(a).toISOString(),u=this.db.prepare(`
      INSERT INTO observations
      (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
       files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.type,s.title,s.subtitle,JSON.stringify(s.facts),s.narrative,JSON.stringify(s.concepts),JSON.stringify(s.files_read),JSON.stringify(s.files_modified),n||null,o,p,a);return{id:Number(u.lastInsertRowid),createdAtEpoch:a}}storeSummary(e,t,s,n,o=0,i){let a=i??Date.now(),p=new Date(a).toISOString(),u=this.db.prepare(`
      INSERT INTO session_summaries
      (memory_session_id, project, request, investigated, learned, completed,
       next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,p,a);return{id:Number(u.lastInsertRowid),createdAtEpoch:a}}storeObservationsAndMarkComplete(e,t,s,n,o,i,a,p=0,c){let u=c??Date.now(),_=new Date(u).toISOString();return this.db.transaction(()=>{let E=[],b=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let f of s){let T=b.run(e,t,f.type,f.title,f.subtitle,JSON.stringify(f.facts),f.narrative,JSON.stringify(f.concepts),JSON.stringify(f.files_read),JSON.stringify(f.files_modified),a||null,p,_,u);E.push(Number(T.lastInsertRowid))}let O;if(n){let T=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,a||null,p,_,u);O=Number(T.lastInsertRowid)}return this.db.prepare(`
      ...
      `).run(u,o),{observationIds:E,summaryId:O,createdAtEpoch:u}})()}getSessionSummariesByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o}=t,i=s==="date_asc"?"ASC":"DESC",a=n?`LIMIT ${n}`:"",p=e.map(()=>"?").join(","),c=[...e],u=o?`WHERE id IN (${p}) AND project = ?`:`WHERE id IN (${p})`;return o&&c.push(o),this.db.prepare(`
      SELECT * FROM session_summaries
      ${u}
      ORDER BY created_at_epoch ${i}
      ${a}
    `).all(...c)}getUserPromptsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o}=t,i=s==="date_asc"?"ASC":"DESC",a=n?`LIMIT ${n}`:"",p=e.map(()=>"?").join(","),c=[...e],u=o?"AND s.project = ?":"";return o&&c.push(o),this.db.prepare(`
      SELECT
        up.*,
        s.project,
        s.memory_session_id
    ...
    `).all(...c)}getTimelineAroundTimestamp(e,t=10,s=10,n){return this.getTimelineAroundObservation(null,e,t,s,n)}getTimelineAroundObservation(e,t,s=10,n=10,o){let i=o?"AND project = ?":"",a=o?[o]:[],p,c;if(e!==null){let l=`
      ...
      `,f=`
        ...
        ORDER BY id ASC
        LIMIT ?
      `;try{let T=this.db.prepare(l).all(e,...a,s+1),C=this.db.prepare(f).all(e,...a,n+1);if(T.length===0&&C.length===0)return{observations:[],sessions:[],prompts:[]};p=T.length>0?T[T.length-1].created_at_epoch:t,c=C.length>0?C[C.length-1].created_at_epoch:t}catch(T){return m.error("DB","Error getting boundary observations",void 0,{error:T,project:o}),{observations:[],sessions:[],prompts:[]}}}else{let l=`
      ...
      `,f=`
        ...
        ORDER BY created_at_epoch ASC
        LIMIT ?
      `;try{let T=this.db.prepare(l).all(t,...a,s),C=this.db.prepare(f).all(t,...a,n+1);if(T.length===0&&C.length===0)return{observations:[],sessions:[],prompts:[]};p=T.length>0?T[T.length-1].created_at_epoch:t,c=C.length>0?C[C.length-1].created_at_epoch:t}catch(T){return m.error("DB","Error getting boundary timestamps",void 0,{error:T,project:o}),{observations:[],sessions:[],prompts:[]}}}let u=`
      SELECT *
      FROM observations
...
`}function Ke(r,e,t,s){let n=[];return s?n.push(...$e(r)):n.push(...Oe(r)),s?n.push(...Ue()):n.push(...Ce()),s?n.push(...we()):n.push(...Re()),s?n.push(...Fe()):n.push(...Ie()),w(t)&&(s?n.push(...Pe(e,t)):n.push(...Ne(e,t))),n}var Q=M(require("path"),1);function z(r){if(!r)return[];try{let e=JSON.parse(r);return Array.isArray(e)?e:[]}catch(e){return m.debug("PARSER","Failed to parse JSON array, using empty fallback",{preview:r?.substring(0,50)},e),[]}}function qe(r){return new Date(r).toLocaleString("en-US",{month:"short",day:"numeric",hour:"numeric",minute:"2-digit",hour12:!0})}function Je(r){return new Date(r).toLocaleString("en-US",{hour:"numeric",minute:"2-digit",hour12:!0})}function Qe(r){return new Date(r).toLocaleString("en-US",{month:"short",day:"numeric",year:"numeric"})}function ht(r,e){return Q.default.isAbsolute(r)?Q.default.relative(e,r):r}function ze(r,e){let t=z(r);return t.length>0?ht(t[0],e):"General"}function bt(r){let e=new Map;for(let s of r){let n=s.type==="observation"?s.data.created_at:s.data.displayTime,o=Qe(n);e.has(o)||e.set(o,[]),e.get(o).push(s)}let t=Array.from(e.entries()).sort((s,n)=>{let o=new Date(s[0]).getTime(),i=new Date(n[0]).getTime();return o-i});return new Map(t)}function Ot(r,e){return e.fullObservationField==="narrative"?r.narrative:r.facts?z(r.facts).join(`
`):null}function Ct(r,e,t,s,n,o){let i=[];o?i.push(...je(r)):i.push(...ye(r));let a=null,p="",c=!1;for(let u of e)if(u.type==="summary"){c&&(i.push(""),c=!1,a=null,p="");let _=u.data,g=qe(_.displayTime);o?i.push(...He(_,g)):i.push(...Le(_,g))}else{let _=u.data,g=ze(_.files_modified,n),E=Je(_.created_at),b=E!==p,O=b?E:"";p=E;let l=t.has(_.id);if(g!==a&&(c&&i.push(""),o?i.push(...Xe(g)):i.push(...Ae(g)),a=g,c=!0),l){let f=Ot(_,s);o?i.push(...Ge(_,E,b,f,s)):(c&&!o&&(i.push(""),c=!1),i.push(...ve(_,O,f,s)),a=null)}else o?i.push(Be(_,E,b,s)):i.push(Me(_,O,s))}return c&&i.push(""),i}function Ze(r,e,t,s,n){let o=[],i=bt(r);for(let[a,p]of i)o.push(...Ct(a,p,e,t,s,n));return o}function et(r,e,t){return!(!r.showLastSummary||!e||!!!(e.investigated||e.learned||e.completed||e.next_steps)||t&&e.created_at_epoch<=t.created_at_epoch)}function tt(r,e){let t=[];return e?(t.push(...D("Investigated",r.investigated,d.blue)),t.push(...D("Learned",r.learned,d.yellow)),t.push(...D("Completed",r.completed,d.green)),t.push(...D("Next Steps",r.next_steps,d.magenta))):(t.push(...L("Investigated",r.investigated)),t.push(...L("Learned",r.learned)),t.push(...L("Completed",r.completed)),t.push(...L("Next Steps",r.next_steps))),t}function st(r,e){return e?We(r):De(r)}function rt(r,e,t){return!w(e)||r.totalDiscoveryTokens<=0||r.savings<=0?[]:t?Ye(r.totalDiscoveryTokens,r.totalReadTokens):xe(r.totalDiscoveryTokens,r.totalReadTokens)}var Rt=nt.default.join((0,ot.homedir)(),".claude","plugins","marketplaces","thedotmack","plugin",".install-version");function It(){try{return new $}catch(r){if(r.code==="ERR_DLOPEN_FAILED"){try{(0,it.unlinkSync)(Rt)}catch(e){m.debug("SYSTEM","Marker file cleanup failed (may not exist)",{},e)}return m.error("SYSTEM","Native module rebuild needed - restart Claude Code to auto-fix"),null}throw r}}function Nt(r,e){return e?Ve(r):ke(r)}function yt(r,e,t,s,n,o,i){let a=[],p=Y(e);a.push(...Ke(r,p,s,i));let c=t.slice(0,s.sessionCount),u=he(c,t),_=J(e,u),g=be(e,s.fullObservationCount);a.push(...Ze(_,g,s,n,i));let E=t[0],b=e[0];et(s,E,b)&&a.push(...tt(E,i));let O=q(e,s,o,n);return a.push(...st(O,i)),a.push(...rt(p,s,i)),a.join(`
`).trimEnd()}async function Z(r,e=!1){let t=H(),s=r?.cwd??process.cwd(),n=le(s),o=It();if(!o)return"";try{let i=V(o,n,t),a=K(o,n,t);return i.length===0&&a.length===0?Nt(n,e):yt(n,i,a,t,s,r?.session_id,e)}finally{o.close()}}0&&(module.exports={generateContext});
