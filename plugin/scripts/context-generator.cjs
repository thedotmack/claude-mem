...
`+JSON.stringify(o,null,2):m=" "+this.formatData(o));let u="";if(n){let{sessionId:E,memorySessionId:T,correlationId:b,..._}=n;Object.keys(_).length>0&&(u=` {${Object.entries(_).map(([S,R])=>`${S}=${R}`).join(", ")}}`)}let g=`[${i}] [${a}] [${d}] ${c}${s}${u}${m}`;if(this.logFilePath)try{(0,I.appendFileSync)(this.logFilePath,g+`
...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),m=c?`${c[1].split("/").pop()}:${c[2]}`:"unknown",u={...s,location:m};return this.warn(e,`[HAPPY-PATH] ${t}`,u,n),o}},l=new W;var Rt={};function bt(){return typeof __dirname<"u"?__dirname:(0,h.dirname)((0,ce.fileURLToPath)(Rt.url))}var Ot=bt(),C=y.get("CLAUDE_MEM_DATA_DIR"),G=process.env.CLAUDE_CONFIG_DIR||(0,h.join)((0,de.homedir)(),".claude"),Wt=(0,h.join)(C,"archives"),Gt=(0,h.join)(C,"logs"),Yt=(0,h.join)(C,"trash"),Vt=(0,h.join)(C,"backups"),qt=(0,h.join)(C,"modes"),Kt=(0,h.join)(C,"settings.json"),le=(0,h.join)(C,"claude-mem.db"),Jt=(0,h.join)(C,"vector-db"),zt=(0,h.join)(C,"observer-sessions"),Qt=(0,h.join)(G,"settings.json"),Zt=(0,h.join)(G,"commands"),es=(0,h.join)(G,"CLAUDE.md");function ue(r){(0,pe.mkdirSync)(r,{recursive:!0})}function me(){return(0,h.join)(Ot,"..")}var $=class{db;constructor(e=le){e!==":memory:"&&ue(C),this.db=new _e.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn()}initializeSchema(){this.db.run(`
    ...
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,d,a);return{id:Number(u.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a){let d=a??Date.now(),c=new Date(d).toISOString();return this.db.transaction(()=>{let u=[],g=this.db.prepare(`
        ...
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let T of s){let b=g.run(e,t,T.type,T.title,T.subtitle,JSON.stringify(T.facts),T.narrative,JSON.stringify(T.concepts),JSON.stringify(T.files_read),JSON.stringify(T.files_modified),o||null,i,c,d);u.push(Number(b.lastInsertRowid))}let E=null;if(n){let T=this.db.prepare("SELECT id FROM session_summaries WHERE memory_session_id = ? LIMIT 1").get(e);if(T)this.db.prepare(`
            UPDATE session_summaries
            SET project=?, request=?, investigated=?, learned=?, completed=?,
                next_steps=?, notes=?, prompt_number=?, discovery_tokens=?,
                created_at=?, created_at_epoch=?
        ...
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let f of s){let S=T.run(e,t,f.type,f.title,f.subtitle,JSON.stringify(f.facts),f.narrative,JSON.stringify(f.concepts),JSON.stringify(f.files_read),JSON.stringify(f.files_modified),a||null,d,u,m);E.push(Number(S.lastInsertRowid))}let b;if(n){let f=this.db.prepare("SELECT id FROM session_summaries WHERE memory_session_id = ? LIMIT 1").get(e);if(f)this.db.prepare(`
            UPDATE session_summaries
            SET project=?, request=?, investigated=?, learned=?, completed=?,
                next_steps=?, notes=?, prompt_number=?, discovery_tokens=?,
                created_at=?, created_at_epoch=?
            WHERE id = ?
          `).run(t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,a||null,d,u,m,f.id),b=f.id;else{let R=this.db.prepare(`
            INSERT INTO session_summaries
            (memory_session_id, project, request, investigated, learned, completed,
             next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
          `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,a||null,d,u,m);b=Number(R.lastInsertRowid)}}return this.db.prepare(`
      ...
      `).run(m,o),{observationIds:E,summaryId:b,createdAtEpoch:m}})()}getSessionSummariesByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o}=t,i=s==="date_asc"?"ASC":"DESC",a=n?`LIMIT ${n}`:"",d=e.map(()=>"?").join(","),c=[...e],m=o?`WHERE id IN (${d}) AND project = ?`:`WHERE id IN (${d})`;return o&&c.push(o),this.db.prepare(`
