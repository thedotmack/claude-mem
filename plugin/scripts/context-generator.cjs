...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),m=_?`${_[1].split("/").pop()}:${_[2]}`:"unknown",u={...s,location:m};return this.warn(e,`[HAPPY-PATH] ${t}`,u,n),o}},c=new W;var Ct={};function Rt(){return typeof __dirname<"u"?__dirname:(0,b.dirname)((0,ce.fileURLToPath)(Ct.url))}var Nt=Rt(),N=I.get("CLAUDE_MEM_DATA_DIR"),L=process.env.CLAUDE_CONFIG_DIR||(0,b.join)((0,de.homedir)(),".claude"),Yt=(0,b.join)(L,"plugins","marketplaces","thedotmack"),qt=(0,b.join)(N,"archives"),Kt=(0,b.join)(N,"logs"),Jt=(0,b.join)(N,"trash"),Qt=(0,b.join)(N,"backups"),zt=(0,b.join)(N,"modes"),Zt=(0,b.join)(N,"settings.json"),_e=(0,b.join)(N,"claude-mem.db"),es=(0,b.join)(N,"vector-db"),ts=(0,b.join)(N,"observer-sessions"),ss=(0,b.join)(L,"settings.json"),rs=(0,b.join)(L,"commands"),ns=(0,b.join)(L,"CLAUDE.md");function me(r){(0,pe.mkdirSync)(r,{recursive:!0})}function ue(){return(0,b.join)(Nt,"..")}var le=require("crypto");var At=3e4;function Ee(r,e,t){return(0,le.createHash)("sha256").update((r||"")+(e||"")+(t||"")).digest("hex").slice(0,16)}function Te(r,e,t){let s=t-At;return r.prepare("SELECT id, created_at_epoch FROM observations WHERE content_hash = ? AND created_at_epoch > ?").get(e,s)}var $=class{db;constructor(e=_e){e!==":memory:"&&me(N),this.db=new ge.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys(),this.addObservationContentHashColumn(),this.addSessionCustomTitleColumn(),this.ensureReadCountColumns()}initializeSchema(){this.db.run(`
    ...
    `).get(e);if(a)return t&&this.db.prepare(`
      ...
      VALUES (?, NULL, ?, ?, ?, ?, ?, 'active')
    `).run(e,t,s,n||null,o.toISOString(),i);let d=this.db.prepare("SELECT id, project FROM sdk_sessions WHERE content_session_id = ?").get(e);return t&&t!==d.project&&(this.db.prepare("UPDATE sdk_sessions SET project = ? WHERE id = ?").run(t,d.id),c.info("SESSION","Updated project for existing session",{sessionId:d.id,oldProject:d.project||"(empty)",newProject:t})),d.id}saveUserPrompt(e,t,s){let n=new Date,o=n.getTime();return this.db.prepare(`
    ...
    `).run(e,t,s,n.toISOString(),o).lastInsertRowid}getUserPrompt(e,t){return this.db.prepare(`
      ...
      LIMIT 1
    `).get(e,t)?.prompt_text??null}storeObservation(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString(),_=Ee(e,s.title,s.narrative),m=Te(this.db,_,a);if(m)return{id:m.id,createdAtEpoch:m.created_at_epoch};let T=this.db.prepare(`
  ...
  `).all(e,...s,...o,t.totalObservationCount)}function Q(r,e,t){return r.db.prepare(`
    ...
    ORDER BY created_at_epoch DESC
    LIMIT ?
  `).all(e,t.sessionCount+Y)}function Ne(r,e,t){let s=Array.from(t.observationTypes),n=s.map(()=>"?").join(","),o=Array.from(t.observationConcepts),i=o.map(()=>"?").join(","),a=e.map(()=>"?").join(",");return r.db.prepare(`
    SELECT
      id, memory_session_id, type, title, subtitle, narrative,
      facts, concepts, files_read, files_modified, discovery_tokens,
