...
`+JSON.stringify(o,null,2):m=" "+this.formatData(o));let u="";if(n){let{sessionId:E,memorySessionId:g,correlationId:h,...l}=n;Object.keys(l).length>0&&(u=` {${Object.entries(l).map(([S,N])=>`${S}=${N}`).join(", ")}}`)}let T=`[${i}] [${a}] [${_}] ${p}${s}${u}${m}`;if(this.logFilePath)try{(0,I.appendFileSync)(this.logFilePath,T+`
...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),m=p?`${p[1].split("/").pop()}:${p[2]}`:"unknown",u={...s,location:m};return this.warn(e,`[HAPPY-PATH] ${t}`,u,n),o}},c=new W;var At={};function Nt(){return typeof __dirname<"u"?__dirname:(0,f.dirname)((0,pe.fileURLToPath)(At.url))}var Rt=Nt(),R=C.get("CLAUDE_MEM_DATA_DIR"),L=process.env.CLAUDE_CONFIG_DIR||(0,f.join)((0,de.homedir)(),".claude"),Vt=(0,f.join)(L,"plugins","marketplaces","thedotmack"),qt=(0,f.join)(R,"archives"),Kt=(0,f.join)(R,"logs"),Jt=(0,f.join)(R,"trash"),Qt=(0,f.join)(R,"backups"),zt=(0,f.join)(R,"modes"),Zt=(0,f.join)(R,"settings.json"),ce=(0,f.join)(R,"claude-mem.db"),es=(0,f.join)(R,"vector-db"),ts=(0,f.join)(R,"observer-sessions"),ss=(0,f.join)(L,"settings.json"),rs=(0,f.join)(L,"commands"),ns=(0,f.join)(L,"CLAUDE.md");function me(r){(0,_e.mkdirSync)(r,{recursive:!0})}function ue(){return(0,f.join)(Rt,"..")}var le=require("crypto");var It=3e4;function Ee(r,e,t){return(0,le.createHash)("sha256").update((r||"")+(e||"")+(t||"")).digest("hex").slice(0,16)}function Te(r,e,t){let s=t-It;return r.prepare("SELECT id, created_at_epoch FROM observations WHERE content_hash = ? AND created_at_epoch > ?").get(e,s)}var $=class{db;constructor(e=ce){e!==":memory:"&&me(R),this.db=new ge.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys(),this.addObservationContentHashColumn(),this.addSessionCustomTitleColumn(),this.addSupersessionTrainingTables(),this.addHandoffObservationType()}initializeSchema(){this.db.run(`
    ...
    `).all(e)}getObservationById(e){return this.db.prepare(`
      ...
      FROM observations
      WHERE id = ?
    `).get(e)||null}getObservationsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o,type:i,concepts:a,files:_}=t,p=s==="date_asc"?"ASC":"DESC",m=n?`LIMIT ${n}`:"",u=e.map(()=>"?").join(","),T=[...e],E=[];if(o&&(E.push("project = ?"),T.push(o)),i)if(Array.isArray(i)){let l=i.map(()=>"?").join(",");E.push(`type IN (${l})`),T.push(...i)}else E.push("type = ?"),T.push(i);if(a){let l=Array.isArray(a)?a:[a],b=l.map(()=>"EXISTS (SELECT 1 FROM json_each(concepts) WHERE value = ?)");T.push(...l),E.push(`(${b.join(" OR ")})`)}if(_){let l=Array.isArray(_)?_:[_],b=l.map(()=>"(EXISTS (SELECT 1 FROM json_each(files_read) WHERE value LIKE ?) OR EXISTS (SELECT 1 FROM json_each(files_modified) WHERE value LIKE ?))");l.forEach(S=>{T.push(`%${S}%`,`%${S}%`)}),E.push(`(${b.join(" OR ")})`)}let g=E.length>0?`WHERE id IN (${u}) AND ${E.join(" AND ")}`:`WHERE id IN (${u})`;return this.db.prepare(`
      SELECT *
      FROM observations
      ${g}
    ...
    `).run(e,t,s,n.toISOString(),o).lastInsertRowid}getUserPrompt(e,t){return this.db.prepare(`
      ...
      LIMIT 1
    `).get(e,t)?.prompt_text??null}storeObservation(e,t,s,n,o=0,i){let a=i??Date.now(),_=new Date(a).toISOString(),p=Ee(e,s.title,s.narrative),m=Te(this.db,p,a);if(m)return{id:m.id,createdAtEpoch:m.created_at_epoch};let T=this.db.prepare(`
    ...
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,_,a);return{id:Number(m.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a){let _=a??Date.now(),p=new Date(_).toISOString();return this.db.transaction(()=>{let u=[],T=this.db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let g of s){let h=T.run(e,t,g.type,g.title,g.subtitle,JSON.stringify(g.facts),g.narrative,JSON.stringify(g.concepts),JSON.stringify(g.files_read),JSON.stringify(g.files_modified),o||null,i,p,_);u.push(Number(h.lastInsertRowid))}let E=null;if(n){let h=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,o||null,i,p,_);E=Number(h.lastInsertRowid)}return{observationIds:u,summaryId:E,createdAtEpoch:_}})()}storeObservationsAndMarkComplete(e,t,s,n,o,i,a,_=0,p){let m=p??Date.now(),u=new Date(m).toISOString();return this.db.transaction(()=>{let E=[],g=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let b of s){let S=g.run(e,t,b.type,b.title,b.subtitle,JSON.stringify(b.facts),b.narrative,JSON.stringify(b.concepts),JSON.stringify(b.files_read),JSON.stringify(b.files_modified),a||null,_,u,m);E.push(Number(S.lastInsertRowid))}let h;if(n){let S=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      ...
      `).run(m,o),{observationIds:E,summaryId:h,createdAtEpoch:m}})()}getSessionSummariesByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o}=t,i=s==="date_asc"?"ASC":"DESC",a=n?`LIMIT ${n}`:"",_=e.map(()=>"?").join(","),p=[...e],m=o?`WHERE id IN (${_}) AND project = ?`:`WHERE id IN (${_})`;return o&&p.push(o),this.db.prepare(`
    ...
    `).all(...p)}getUserPromptsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o}=t,i=s==="date_asc"?"ASC":"DESC",a=n?`LIMIT ${n}`:"",_=e.map(()=>"?").join(","),p=[...e],m=o?"AND s.project = ?":"";return o&&p.push(o),this.db.prepare(`
      ...
      ${a}
    `).all(...p)}getTimelineAroundTimestamp(e,t=10,s=10,n){return this.getTimelineAroundObservation(null,e,t,s,n)}getTimelineAroundObservation(e,t,s=10,n=10,o){let i=o?"AND project = ?":"",a=o?[o]:[],_,p;if(e!==null){let l=`
      ...
      `,b=`
        ...
        ORDER BY id ASC
        LIMIT ?
      `;try{let S=this.db.prepare(l).all(e,...a,s+1),N=this.db.prepare(b).all(e,...a,n+1);if(S.length===0&&N.length===0)return{observations:[],sessions:[],prompts:[]};_=S.length>0?S[S.length-1].created_at_epoch:t,p=N.length>0?N[N.length-1].created_at_epoch:t}catch(S){return c.error("DB","Error getting boundary observations",void 0,{error:S,project:o}),{observations:[],sessions:[],prompts:[]}}}else{let l=`
      ...
      `,b=`
        ...
        ORDER BY created_at_epoch ASC
        LIMIT ?
      `;try{let S=this.db.prepare(l).all(t,...a,s),N=this.db.prepare(b).all(t,...a,n+1);if(S.length===0&&N.length===0)return{observations:[],sessions:[],prompts:[]};_=S.length>0?S[S.length-1].created_at_epoch:t,p=N.length>0?N[N.length-1].created_at_epoch:t}catch(S){return c.error("DB","Error getting boundary timestamps",void 0,{error:S,project:o}),{observations:[],sessions:[],prompts:[]}}}let m=`
      SELECT *
      FROM observations
...
`}function st(r,e,t,s){let n=[];return s?n.push(...Be(r)):n.push(...ve(r)),s?n.push(...He()):n.push(...Me()),s?n.push(...We()):n.push(...De()),s?n.push(...Ye()):n.push(...ye()),P(t)&&(s?n.push(...Ve(e,t)):n.push(...Ue(e,t))),n}var ee=M(require("path"),1);function B(r){if(!r)return[];try{let e=JSON.parse(r);return Array.isArray(e)?e:[]}catch(e){return c.debug("PARSER","Failed to parse JSON array, using empty fallback",{preview:r?.substring(0,50)},e),[]}}function nt(r){return new Date(r).toLocaleString("en-US",{month:"short",day:"numeric",hour:"numeric",minute:"2-digit",hour12:!0})}function ot(r){return new Date(r).toLocaleString("en-US",{hour:"numeric",minute:"2-digit",hour12:!0})}function it(r){return new Date(r).toLocaleString("en-US",{month:"short",day:"numeric",year:"numeric"})}function rt(r,e){return ee.default.isAbsolute(r)?ee.default.relative(e,r):r}function at(r,e,t){let s=B(r);if(s.length>0)return rt(s[0],e);if(t){let n=B(t);if(n.length>0)return rt(n[0],e)}return"General"}function Mt(r){let e=new Map;for(let s of r){let n=s.type==="observation"?s.data.created_at:s.data.displayTime,o=it(n);e.has(o)||e.set(o,[]),e.get(o).push(s)}let t=Array.from(e.entries()).sort((s,n)=>{let o=new Date(s[0]).getTime(),i=new Date(n[0]).getTime();return o-i});return new Map(t)}function Dt(r,e){return e.fullObservationField==="narrative"?r.narrative:r.facts?B(r.facts).join(`
`):null}function yt(r,e,t,s,n,o){let i=[];o?i.push(...qe(r)):i.push(...xe(r));let a=null,_="",p=!1;for(let m of e)if(m.type==="summary"){p&&(i.push(""),p=!1,a=null,_="");let u=m.data,T=nt(u.displayTime);o?i.push(...ze(u,T)):i.push(...Fe(u,T))}else{let u=m.data,T=at(u.files_modified,n,u.files_read),E=ot(u.created_at),g=E!==_,h=g?E:"";_=E;let l=t.has(u.id);if(T!==a&&(p&&i.push(""),o?i.push(...Ke(T)):i.push(...ke(T)),a=T,p=!0),l){let b=Dt(u,s);o?i.push(...Qe(u,E,g,b,s)):(p&&!o&&(i.push(""),p=!1),i.push(...$e(u,h,b,s)),a=null)}else o?i.push(Je(u,E,g,s)):i.push(we(u,h,s))}return p&&i.push(""),i}function dt(r,e,t,s,n){let o=[],i=Mt(r);for(let[a,_]of i)o.push(...yt(a,_,e,t,s,n));return o}function _t(r,e,t){return!(!r.showLastSummary||!e||!!!(e.investigated||e.learned||e.completed||e.next_steps)||t&&e.created_at_epoch<=t.created_at_epoch)}function pt(r,e){let t=[];return e?(t.push(...x("Investigated",r.investigated,d.blue)),t.push(...x("Learned",r.learned,d.yellow)),t.push(...x("Completed",r.completed,d.green)),t.push(...x("Next Steps",r.next_steps,d.magenta))):(t.push(...U("Investigated",r.investigated)),t.push(...U("Learned",r.learned)),t.push(...U("Completed",r.completed)),t.push(...U("Next Steps",r.next_steps))),t}function ct(r,e){return e?Ze(r):Pe(r)}function mt(r,e,t){return!P(e)||r.totalDiscoveryTokens<=0||r.savings<=0?[]:t?et(r.totalDiscoveryTokens,r.totalReadTokens):Xe(r.totalDiscoveryTokens,r.totalReadTokens)}var Ut=ut.default.join((0,lt.homedir)(),".claude","plugins","marketplaces","thedotmack","plugin",".install-version");function xt(){try{return new $}catch(r){if(r.code==="ERR_DLOPEN_FAILED"){try{(0,Et.unlinkSync)(Ut)}catch(e){c.debug("SYSTEM","Marker file cleanup failed (may not exist)",{},e)}return c.error("SYSTEM","Native module rebuild needed - restart Claude Code to auto-fix"),null}throw r}}function kt(r,e){return e?tt(r):Ge(r)}function wt(r,e,t,s,n,o,i){let a=[],_=K(e);a.push(...st(r,_,s,i));let p=t.slice(0,s.sessionCount),m=Ie(p,t),u=Z(e,m),T=Ce(e,s.fullObservationCount);a.push(...dt(u,T,s,n,i));let E=t[0],g=e[0];_t(s,E,g)&&a.push(...pt(E,i));let h=z(e,s,o,n);return a.push(...ct(h,i)),a.push(...mt(_,s,i)),a.join(`
