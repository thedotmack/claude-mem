...
`+JSON.stringify(o,null,2):u=" "+this.formatData(o));let _="";if(n){let{sessionId:E,memorySessionId:T,correlationId:b,...m}=n;Object.keys(m).length>0&&(_=` {${Object.entries(m).map(([f,O])=>`${f}=${String(O)}`).join(", ")}}`)}let g=`[${i}] [${a}] [${d}] ${p}${s}${_}${u}`;if(this.logFilePath)try{(0,R.appendFileSync)(this.logFilePath,g+`
...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),u=p?`${String(p[1].split("/").pop())}:${p[2]}`:"unknown",_={...s,location:u};return this.warn(e,`[HAPPY-PATH] ${t}`,_,n),o}},l=new K;var vt={};function Lt(){return typeof __dirname<"u"?__dirname:(0,C.dirname)((0,ge.fileURLToPath)(vt.url))}var yt=Lt(),I=M.get("MAGIC_CLAUDE_MEM_DATA_DIR"),J=process.env.CLAUDE_CONFIG_DIR||(0,C.join)((0,me.homedir)(),".claude"),zt=(0,C.join)(I,"archives"),Qt=(0,C.join)(I,"logs"),Zt=(0,C.join)(I,"trash"),es=(0,C.join)(I,"backups"),ts=(0,C.join)(I,"modes"),ss=(0,C.join)(I,"settings.json"),Te=(0,C.join)(I,"magic-claude-mem.db"),rs=(0,C.join)(I,"vector-db"),ns=(0,C.join)(I,"observer-sessions"),os=(0,C.join)(J,"settings.json"),is=(0,C.join)(J,"commands"),as=(0,C.join)(J,"CLAUDE.md");function Se(r){(0,Ee.mkdirSync)(r,{recursive:!0})}function fe(){return(0,C.join)(yt,"..")}function Ce(r){return r.prepare(`
    SELECT id, content_session_id, project, user_prompt, started_at_epoch
    FROM sdk_sessions
    WHERE status = 'active'
    ORDER BY started_at_epoch DESC
  `).all()}function be(r,e){let t=new Date;return r.prepare(`
    UPDATE sdk_sessions
    SET
      status = 'completed',
      completed_at = ?,
  ...
  `).run(t.toISOString(),t.getTime(),e).changes>0}function Ae(r,e){let t=new Date;return r.prepare(`
    UPDATE sdk_sessions
    SET
      status = 'completed',
      completed_at = ?,
  ...
  `).run(t.toISOString(),t.getTime(),e).changes}function G(r){return!r.request.trim()&&!r.investigated.trim()&&!r.learned.trim()&&!r.completed.trim()&&!r.next_steps.trim()}var z=N(require("path"),1);function P(r){if(!r)return[];try{let e=JSON.parse(r);return Array.isArray(e)?e:[]}catch(e){return l.debug("PARSER","Failed to parse JSON array, using empty fallback",{preview:r.substring(0,50)},e),[]}}function he(r){return new Date(r).toLocaleString("en-US",{month:"short",day:"numeric",hour:"numeric",minute:"2-digit",hour12:!0})}function Ie(r){return new Date(r).toLocaleString("en-US",{hour:"numeric",minute:"2-digit",hour12:!0})}function Re(r){return new Date(r).toLocaleString("en-US",{month:"short",day:"numeric",year:"numeric"})}function Oe(r,e){return z.default.isAbsolute(r)?z.default.relative(e,r):r}function Ne(r,e,t){let s=P(r);if(s.length>0)return Oe(s[0],e);if(t){let n=P(t);if(n.length>0)return Oe(n[0],e)}return"General"}function v(r){return r?Math.ceil(r.length/4):0}function F(r){return v(r.narrative??null)+v(r.title??null)+v(r.facts??null)+v(r.concepts??null)+v(r.text??null)}var j=class{db;constructor(e=Te){e!==":memory:"&&Se(I),this.db=new w(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addCompositeIndexes(),this.ensureReadTokensColumn(),this.createContextInjectionsTable(),this.ensureSubprocessPidColumn(),this.recreateFTSTablesWithUnicode61()}initializeSchema(){this.db.run(`
    ...
    `).run(t,e)}completeSession(e){let t=Date.now();this.db.prepare(`
      ...
      WHERE id = ? AND status = 'active'
    `).run(new Date(t).toISOString(),t,e)}getActiveSessions(){return Ce(this.db)}closeActiveSessionById(e){return be(this.db,e)}closeStaleSessionsOlderThan(e){return Ae(this.db,e)}getRecentSummaries(e,t=10){return this.db.prepare(`
      SELECT
    ...
    `).run(e,t,s,n.toISOString(),o).lastInsertRowid}getUserPrompt(e,t){return this.db.prepare(`
      ...
      LIMIT 1
    `).get(e,t)?.prompt_text??null}storeObservation(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString(),p=F({narrative:s.narrative,title:s.title,facts:JSON.stringify(s.facts),concepts:JSON.stringify(s.concepts),text:null}),_=this.db.prepare(`
      INSERT INTO observations
      (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
       files_read, files_modified, prompt_number, discovery_tokens, read_tokens, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.type,s.title,s.subtitle,JSON.stringify(s.facts),s.narrative,JSON.stringify(s.concepts),JSON.stringify(s.files_read),JSON.stringify(s.files_modified),n||null,o,p,d,a);return{id:Number(_.lastInsertRowid),createdAtEpoch:a}}storeSummary(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString();if(G(s))return l.warn("DB","Skipping empty summary insert",{memorySessionId:e}),{id:0,createdAtEpoch:a};let u=this.db.prepare(`
      INSERT INTO session_summaries
      (memory_session_id, project, request, investigated, learned, completed,
       next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,d,a);return{id:Number(u.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a){let d=a??Date.now(),p=new Date(d).toISOString();return this.db.transaction(()=>{let _=[],g=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, read_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let T of s){let b=F({narrative:T.narrative,title:T.title,facts:JSON.stringify(T.facts),concepts:JSON.stringify(T.concepts),text:null}),m=g.run(e,t,T.type,T.title,T.subtitle,JSON.stringify(T.facts),T.narrative,JSON.stringify(T.concepts),JSON.stringify(T.files_read),JSON.stringify(T.files_modified),o||null,i,b,p,d);_.push(Number(m.lastInsertRowid))}let E=null;if(n)if(G(n))l.warn("DB","Skipping empty summary insert",{memorySessionId:e});else{let b=this.db.prepare(`
            INSERT INTO session_summaries
            (memory_session_id, project, request, investigated, learned, completed,
             next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
          `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,o||null,i,p,d);E=Number(b.lastInsertRowid)}return{observationIds:_,summaryId:E,createdAtEpoch:d}})()}storeObservationsAndMarkComplete(e,t,s,n,o,i,a,d=0,p){let u=p??Date.now(),_=new Date(u).toISOString();return this.db.transaction(()=>{let E=[],T=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, read_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let S of s){let f=F({narrative:S.narrative,title:S.title,facts:JSON.stringify(S.facts),concepts:JSON.stringify(S.concepts),text:null}),O=T.run(e,t,S.type,S.title,S.subtitle,JSON.stringify(S.facts),S.narrative,JSON.stringify(S.concepts),JSON.stringify(S.files_read),JSON.stringify(S.files_modified),a||null,d,f,_,u);E.push(Number(O.lastInsertRowid))}let b;if(n)if(G(n))l.warn("DB","Skipping empty summary insert",{memorySessionId:e});else{let f=this.db.prepare(`
            INSERT INTO session_summaries
            (memory_session_id, project, request, investigated, learned, completed,
             next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      ...
      `).run(u,o),{observationIds:E,summaryId:b,createdAtEpoch:u}})()}getSessionSummariesByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o}=t,i=s==="date_asc"?"ASC":"DESC",a=n?`LIMIT ${String(n)}`:"",d=e.map(()=>"?").join(","),p=[...e],u=o?`WHERE id IN (${d}) AND project = ?`:`WHERE id IN (${d})`;return o&&p.push(o),this.db.prepare(`
...
`}function _t(r,e,t,s){let n=[];return s?n.push(...et(r)):n.push(...Fe(r)),s?n.push(...tt()):n.push(...je()),s?n.push(...st()):n.push(...Xe()),s?n.push(...rt()):n.push(...Be()),B(t)&&(s?n.push(...nt(e,t)):n.push(...He(e,t))),n}function Ut(r){let e=new Map;for(let s of r){let n=s.type==="observation"?s.data.created_at:s.data.displayTime,o=Re(n);e.has(o)||e.set(o,[]),e.get(o)?.push(s)}let t=Array.from(e.entries()).sort((s,n)=>{let o=new Date(s[0]).getTime(),i=new Date(n[0]).getTime();return o-i});return new Map(t)}function wt(r,e){return e.fullObservationField==="narrative"?r.narrative:r.facts?P(r.facts).join(`
`):null}function $t(r,e,t,s,n,o){let i=[];o?i.push(...ot(r)):i.push(...We(r));let a=null,d="",p=!1;for(let u of e)if(u.type==="summary"){p&&(i.push(""),p=!1,a=null,d="");let _=u.data,g=he(_.displayTime);o?i.push(...pt(_,g)):i.push(...Ke(_,g))}else{let _=u.data,g=Ne(_.files_modified,n,_.files_read),E=Ie(_.created_at),T=E!==d,b=T?E:"";d=E;let m=t.has(_.id);if(g!==a&&(p&&i.push(""),o?i.push(...it(g)):i.push(...Ve(g)),a=g,p=!0),m){let S=wt(_,s);o?i.push(...dt(_,E,T,S,s)):(p&&(i.push(""),p=!1),i.push(...Ye(_,b,S,s)),a=null)}else o?i.push(at(_,E,T,s)):i.push(qe(_,b,s))}return p&&i.push(""),i}function mt(r,e,t,s,n){let o=[],i=Ut(r);for(let[a,d]of i)o.push(...$t(a,d,e,t,s,n));return o}function Et(r,e,t){return!(!r.showLastSummary||!e||!!!(e.investigated||e.learned||e.completed||e.next_steps)||t&&e.created_at_epoch<=t.created_at_epoch)}function gt(r,e){let t=[];return e?(t.push(...k("Investigated",r.investigated,c.blue)),t.push(...k("Learned",r.learned,c.yellow)),t.push(...k("Completed",r.completed,c.green)),t.push(...k("Next Steps",r.next_steps,c.magenta))):(t.push(...x("Investigated",r.investigated)),t.push(...x("Learned",r.learned)),t.push(...x("Completed",r.completed)),t.push(...x("Next Steps",r.next_steps))),t}function Tt(r,e){return e?ct(r):Je(r)}function St(r,e,t){return!B(e)||r.totalDiscoveryTokens<=0||r.savings<=0?[]:t?ut(r.totalDiscoveryTokens,r.totalReadTokens):ze(r.totalDiscoveryTokens,r.totalReadTokens)}var Gt=ft.default.join((0,Ct.homedir)(),".claude","plugins","marketplaces","magic-claude-mem","plugin",".install-version");function Pt(){try{return new j}catch(r){if(r instanceof Error&&"code"in r&&r.code==="ERR_DLOPEN_FAILED"){try{(0,bt.unlinkSync)(Gt)}catch(e){l.debug("SYSTEM","Marker file cleanup failed (may not exist)",{},e)}return l.error("SYSTEM","Native module rebuild needed - restart Claude Code to auto-fix"),null}throw r}}function Ft(r,e){return e?lt(r):Qe(r)}function jt(r,e,t,s,n,o,i,a){let d=[];d.push(..._t(r,a,s,i));let p=t.slice(0,s.sessionCount),u=$e(p,t),_=oe(e,u),g=Ge(e,s.fullObservationCount);d.push(...mt(_,g,s,n,i));let E=t[0],T=e[0];Et(s,E,T)&&d.push(...gt(E,i));let b=ne(e,s,o,n);return d.push(...Tt(b,i)),d.push(...St(a,s,i)),d.join(`
`).trimEnd()}function ie(r,e=!1){return q(r,e).text}function q(r,e=!1){let t=Q(),s=r?.cwd??process.cwd(),n=Le(s),o=r?.projects||[n],i=Pt();if(!i)return{text:"",observationIds:[],totalReadTokens:0};try{let a=o.length>1?Ue(i,o,t):se(i,n,t),d=o.length>1?we(i,o,t):re(i,n,t);if(a.length===0&&d.length===0)return{text:Ft(n,e),observationIds:[],totalReadTokens:0};let p=a.map(_=>_.id),u=te(a);return{text:jt(n,a,d,t,s,r?.session_id,e,u),observationIds:p,totalReadTokens:u.totalReadTokens}}finally{i.close()}}0&&(module.exports={generateContext,generateContextWithMeta});
