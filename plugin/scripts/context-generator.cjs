...
`)}debug(e,t,s,n){this.log(0,e,t,s,n)}info(e,t,s,n){this.log(1,e,t,s,n)}warn(e,t,s,n){this.log(2,e,t,s,n)}setErrorSink(e){ae=e}error(e,t,s,n){this.log(3,e,t,s,n),this.routeErrorToSink(t,s,n)}routeErrorToSink(e,t,s){try{if(!ae||!(s instanceof Error))return;ae(s)}catch{}}dataIn(e,t,s,n){this.info(e,`\u2192 ${t}`,s,n)}dataOut(e,t,s,n){this.info(e,`\u2190 ${t}`,s,n)}success(e,t,s,n){this.info(e,`\u2713 ${t}`,s,n)}failure(e,t,s,n){this.error(e,`\u2717 ${t}`,s,n)}},E=new _e;var Le=require("crypto");function Me(r,e,t){return(0,Le.createHash)("sha256").update([r||"",e||"",t||""].join("\0")).digest("hex").slice(0,16)}var p="claude";function ds(r){return r.trim().toLowerCase().replace(/\s+/g,"-")}function I(r){if(!r)return p;let e=ds(r);return e?e==="transcript"||e.includes("codex")?"codex":e.includes("cursor")?"cursor":e.includes("claude")?"claude":e:p}function De(r){let e=["claude","codex","cursor"];return[...r].sort((t,s)=>{let n=e.indexOf(t),o=e.indexOf(s);return n!==-1||o!==-1?n===-1?1:o===-1?-1:n-o:t.localeCompare(s)})}function ve(r,e,t,s,n){let o=Date.now()-s,i=n!==void 0?"up.session_db_id = ?":"up.content_session_id = ?",a=n??e;return r.prepare(`
    ...
    LIMIT 1
  `).get(a,t,o)??void 0}var xe=["private","claude-mem-context","system_instruction","system-instruction","persisted-output","system-reminder"],ye=new RegExp(`<(${xe.join("|")})\\b[^>]*>[\\s\\S]*?</\\1>`,"g"),ke=/<system-reminder>[\s\S]*?<\/system-reminder>/g,Ue=100;function _s(r){let e=Object.fromEntries(xe.map(n=>[n,0]));ye.lastIndex=0;let t=0,s=r.replace(ye,(n,o)=>(e[o]=(e[o]??0)+1,t+=1,""));return t>Ue&&E.warn("SYSTEM","tag count exceeds limit",void 0,{tagCount:t,maxAllowed:Ue,contentLength:r.length}),{stripped:s.trim(),counts:e}}function Fe(r){return _s(r).stripped}var Es=["task-notification"],rr=new RegExp(`^\\s*<(${Es.join("|")})\\b[^>]*>(?:(?!<\\1\\b|</\\1\\b)[\\s\\S])*</\\1>\\s*$`),nr=256*1024;var Ee=4e3;function Y(r){let e=r.trim(),s=Fe(r).trim()||e;return s.length<=Ee?s:(E.debug("DB","Truncated stored prompt text to the configured cap",{originalLength:s.length,storedLength:Ee}),`${s.slice(0,Ee-1)}\u2026`)}var Pe=require("bun:sqlite"),we=require("node:path");var us=5e3;function ms(r){r!==":memory:"&&Ne((0,we.dirname)(r))}function ue(r){r.run(`PRAGMA busy_timeout = ${us}`)}function cs(r){let{tableCount:e}=r.query("SELECT COUNT(*) AS tableCount FROM sqlite_master WHERE type = 'table'").get(),{page_count:t}=r.query("PRAGMA page_count").get();return e>0||t>1?!1:(r.run("PRAGMA auto_vacuum = INCREMENTAL"),!0)}function ps(r){ue(r),r.run("PRAGMA journal_mode = WAL"),r.run("PRAGMA synchronous = NORMAL"),r.run("PRAGMA foreign_keys = ON"),r.run("PRAGMA journal_size_limit = 4194304")}function $e(r){ms(r);let e=new Pe.Database(r);return cs(e),ps(e),e}var q=class{db;constructor(e=he){e instanceof Xe.Database?(this.db=e,ue(this.db)):this.db=$e(e),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys(),this.addObservationContentHashColumn(),this.addSessionCustomTitleColumn(),this.addSessionPlatformSourceColumn(),this.addObservationModelColumns(),this.ensureMergedIntoProjectColumns(),this.addObservationSubagentColumns(),this.addObservationsUniqueContentHashIndex(),this.addObservationsMetadataColumn(),this.dropDeadPendingMessagesColumns(),this.ensurePendingMessagesToolUseIdColumn(),this.dropWorkerPidColumn(),this.ensureSDKSessionsPlatformContentIdentity(),this.ensureUserPromptsSessionDbId(),this.ensurePendingMessagesSessionToolUniqueIndex()}getIndexColumns(e){return this.db.query(`PRAGMA index_info(${JSON.stringify(e)})`).all().map(t=>t.name)}hasUniqueIndexOnColumns(e,t){return this.db.query(`PRAGMA index_list(${e})`).all().some(n=>{if(n.unique!==1)return!1;let o=this.getIndexColumns(n.name);return o.length===t.length&&o.every((i,a)=>i===t[a])})}resolvePromptSessionDbId(e,t,s){if(t!==void 0)return t;let n=s?I(s):void 0;return n?this.db.prepare(`
        SELECT id
        FROM sdk_sessions
    ...
    `).all(ie),t=[],s=new Set,n={};for(let i of e){let a=I(i.platform_source);n[a]||(n[a]=[]),n[a].includes(i.project)||n[a].push(i.project),s.has(i.project)||(s.add(i.project),t.push(i.project))}let o=De(Object.keys(n));return{projects:t,sources:o,projectsBySource:Object.fromEntries(o.map(i=>[i,n[i]||[]]))}}getLatestUserPrompt(e,t){let s=this.resolvePromptSessionDbId(e,t),n=s!==null?"up.session_db_id = ?":"up.content_session_id = ?",o=s!==null?s:e;return this.db.prepare(`
      ...
      ORDER BY up.created_at_epoch DESC
      LIMIT 1
    `).get(o)}findRecentDuplicateUserPrompt(e,t,s,n){return ve(this.db,e,Y(t),s,this.resolvePromptSessionDbId(e,n)??void 0)}getRecentSessionsWithStatus(e,t=3,s){let n=[e],o="";return s&&(o=`AND COALESCE(NULLIF(s.platform_source, ''), '${p}') = ?`,n.push(I(s))),n.push(t),this.db.prepare(`
      SELECT * FROM (
        SELECT
          s.memory_session_id,
          s.status,
    ...
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,d,a);return{id:Number(m.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a,d){let _=a??Date.now(),m=new Date(_).toISOString();return this.db.transaction(()=>{let l=[],T=this.db.prepare(`
        ...
        RETURNING id
      `),b=this.db.prepare("SELECT id FROM observations WHERE memory_session_id = ? AND content_hash = ?");for(let g of s){let S=Me(e,g.title,g.narrative),C=T.get(e,t,g.type,g.title,g.subtitle,JSON.stringify(g.facts),g.narrative,JSON.stringify(g.concepts),JSON.stringify(g.files_read),JSON.stringify(g.files_modified),o||null,i,g.agent_type??null,g.agent_id??null,S,m,_,d||null,g.metadata??null);if(C){l.push(C.id);continue}let U=b.get(e,S);if(!U)throw new Error(`storeObservations: ON CONFLICT without existing row for content_hash=${S}`);l.push(U.id)}let A=null;if(n){let S=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,o||null,i,m,_);A=Number(S.lastInsertRowid)}return{observationIds:l,summaryId:A,createdAtEpoch:_}})()}getSessionSummariesByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o,platformSource:i}=t,a=s==="relevance",d=a?"":`ORDER BY ss.created_at_epoch ${s==="date_asc"?"ASC":"DESC"}`,_=n&&!a?`LIMIT ${n}`:"",m=e.map(()=>"?").join(","),c=[...e],l=[];o&&(l.push("ss.project = ?"),c.push(o)),i&&(l.push(`COALESCE(NULLIF(s.platform_source, ''), '${p}') = ?`),c.push(I(i)));let T=l.length>0?`AND ${l.join(" AND ")}`:"",A=this.db.prepare(`
      SELECT ss.*
      ...
      ${d}
      ${_}
    `).all(...c);if(!a)return A;let g=new Map(A.map(C=>[C.id,C])),S=e.map(C=>g.get(C)).filter(C=>!!C);return n?S.slice(0,n):S}getUserPromptsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o,platformSource:i}=t,a=s==="relevance",d=a?"":`ORDER BY up.created_at_epoch ${s==="date_asc"?"ASC":"DESC"}`,_=n?`LIMIT ${n}`:"",m=e.map(()=>"?").join(","),c=[...e],l=[];o&&(l.push("s.project = ?"),c.push(o)),i&&(l.push(`COALESCE(NULLIF(s.platform_source, ''), '${p}') = ?`),c.push(I(i)));let T=l.length>0?`AND ${l.join(" AND ")}`:"",A=this.db.prepare(`
      SELECT
        up.*,
        s.project,
        s.memory_session_id,
      ...
      ${d}
      ${_}
    `).all(...c);if(!a)return A;let g=new Map(A.map(S=>[S.id,S]));return e.map(S=>g.get(S)).filter(S=>!!S)}getTimelineAroundTimestamp(e,t=10,s=10,n,o){return this.getTimelineAroundObservation(null,e,t,s,n,o)}getTimelineAroundObservation(e,t,s=10,n=10,o,i){let a=i?I(i):void 0,d=(O,h)=>{let R=[],L=[];return o&&(R.push(`${O}.project = ?`),L.push(o)),a&&(R.push(`COALESCE(NULLIF(${h}.platform_source, ''), '${p}') = ?`),L.push(a)),{clause:R.length>0?`AND ${R.join(" AND ")}`:"",params:L}},_=d("o","src"),m=d("ss","src"),c=d("s","s"),l,T;if(e!==null){let O=`
