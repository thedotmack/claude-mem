...
`+JSON.stringify(o,null,2):u=" "+this.formatData(o));let _="";if(n){let{sessionId:E,memorySessionId:T,correlationId:C,...m}=n;Object.keys(m).length>0&&(_=` {${Object.entries(m).map(([f,O])=>`${f}=${String(O)}`).join(", ")}}`)}let g=`[${i}] [${a}] [${d}] ${c}${s}${_}${u}`;if(this.logFilePath)try{(0,R.appendFileSync)(this.logFilePath,g+`
    ...
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,d,a);return{id:Number(u.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a){let d=a??Date.now(),c=new Date(d).toISOString();return this.db.transaction(()=>{let _=[],g=this.db.prepare(`
      ...
      `);for(let T of s){let C=F({narrative:T.narrative,title:T.title,facts:JSON.stringify(T.facts),concepts:JSON.stringify(T.concepts),text:null}),m=g.run(e,t,T.type,T.title,T.subtitle,JSON.stringify(T.facts),T.narrative,JSON.stringify(T.concepts),JSON.stringify(T.files_read),JSON.stringify(T.files_modified),o||null,i,C,c,d);_.push(Number(m.lastInsertRowid))}let E=null;if(n)if(P(n))l.warn("DB","Skipping empty summary insert",{memorySessionId:e});else{let C=this.db.prepare(`
        ...
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, read_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let S of s){let f=F({narrative:S.narrative,title:S.title,facts:JSON.stringify(S.facts),concepts:JSON.stringify(S.concepts),text:null}),O=T.run(e,t,S.type,S.title,S.subtitle,JSON.stringify(S.facts),S.narrative,JSON.stringify(S.concepts),JSON.stringify(S.files_read),JSON.stringify(S.files_modified),a||null,d,f,_,u);E.push(Number(O.lastInsertRowid))}let C;if(n)if(P(n))l.warn("DB","Skipping empty summary insert",{memorySessionId:e});else{let f=this.db.prepare(`
            INSERT INTO session_summaries
            (memory_session_id, project, request, investigated, learned, completed,
             next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
