...
`+JSON.stringify(o,null,2):u=" "+this.formatData(o));let m="";if(n){let{sessionId:E,memorySessionId:T,correlationId:h,..._}=n;Object.keys(_).length>0&&(m=` {${Object.entries(_).map(([f,C])=>`${f}=${C}`).join(", ")}}`)}let g=`[${i}] [${a}] [${d}] ${c}${s}${m}${u}`;if(this.logFilePath)try{(0,N.appendFileSync)(this.logFilePath,g+`
...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),u=c?`${c[1].split("/").pop()}:${c[2]}`:"unknown",m={...s,location:u};return this.warn(e,`[HAPPY-PATH] ${t}`,m,n),o}},l=new W;var At={};function Ot(){return typeof __dirname<"u"?__dirname:(0,b.dirname)((0,le.fileURLToPath)(At.url))}var Ct=Ot(),I=R.get("CLAUDE_MEM_DATA_DIR"),Y=process.env.CLAUDE_CONFIG_DIR||(0,b.join)((0,pe.homedir)(),".claude"),Wt=(0,b.join)(I,"archives"),Yt=(0,b.join)(I,"logs"),Vt=(0,b.join)(I,"trash"),qt=(0,b.join)(I,"backups"),Kt=(0,b.join)(I,"modes"),Jt=(0,b.join)(I,"settings.json"),ue=(0,b.join)(I,"claude-mem.db"),zt=(0,b.join)(I,"vector-db"),Qt=(0,b.join)(I,"observer-sessions"),Zt=(0,b.join)(Y,"settings.json"),es=(0,b.join)(Y,"commands"),ts=(0,b.join)(Y,"CLAUDE.md");function me(r){(0,ce.mkdirSync)(r,{recursive:!0})}function _e(){return(0,b.join)(Ct,"..")}function $(r){return!r.request?.trim()&&!r.investigated?.trim()&&!r.learned?.trim()&&!r.completed?.trim()&&!r.next_steps?.trim()}var w=class{db;constructor(e=ue){e!==":memory:"&&me(I),this.db=new Ee.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn()}initializeSchema(){this.db.run(`
    ...
    `).get(e,t)?.prompt_text??null}storeObservation(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString(),u=this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.type,s.title,s.subtitle,JSON.stringify(s.facts),s.narrative,JSON.stringify(s.concepts),JSON.stringify(s.files_read),JSON.stringify(s.files_modified),n||null,o,d,a);return{id:Number(u.lastInsertRowid),createdAtEpoch:a}}storeSummary(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString();if($(s))return l.warn("DB","Skipping empty summary insert",{memorySessionId:e}),{id:0,createdAtEpoch:a};let u=this.db.prepare(`
    ...
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,d,a);return{id:Number(u.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a){let d=a??Date.now(),c=new Date(d).toISOString();return this.db.transaction(()=>{let m=[],g=this.db.prepare(`
        ...
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let T of s){let h=g.run(e,t,T.type,T.title,T.subtitle,JSON.stringify(T.facts),T.narrative,JSON.stringify(T.concepts),JSON.stringify(T.files_read),JSON.stringify(T.files_modified),o||null,i,c,d);m.push(Number(h.lastInsertRowid))}let E=null;if(n)if($(n))l.warn("DB","Skipping empty summary insert",{memorySessionId:e});else{let h=this.db.prepare(`
            INSERT INTO session_summaries
            (memory_session_id, project, request, investigated, learned, completed,
             next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
          `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,o||null,i,c,d);E=Number(h.lastInsertRowid)}return{observationIds:m,summaryId:E,createdAtEpoch:d}})()}storeObservationsAndMarkComplete(e,t,s,n,o,i,a,d=0,c){let u=c??Date.now(),m=new Date(u).toISOString();return this.db.transaction(()=>{let E=[],T=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let S of s){let f=T.run(e,t,S.type,S.title,S.subtitle,JSON.stringify(S.facts),S.narrative,JSON.stringify(S.concepts),JSON.stringify(S.files_read),JSON.stringify(S.files_modified),a||null,d,m,u);E.push(Number(f.lastInsertRowid))}let h;if(n)if($(n))l.warn("DB","Skipping empty summary insert",{memorySessionId:e});else{let f=this.db.prepare(`
            INSERT INTO session_summaries
            (memory_session_id, project, request, investigated, learned, completed,
             next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
`}function tt(r,e,t,s){let n=[];return s?n.push(...Be(r)):n.push(...ye(r)),s?n.push(...He()):n.push(...Me()),s?n.push(...Ge()):n.push(...Le()),s?n.push(...We()):n.push(...ve()),F(t)&&(s?n.push(...Ye(e,t)):n.push(...De(e,t))),n}var te=M(require("path"),1);function H(r){if(!r)return[];try{let e=JSON.parse(r);return Array.isArray(e)?e:[]}catch(e){return l.debug("PARSER","Failed to parse JSON array, using empty fallback",{preview:r?.substring(0,50)},e),[]}}function rt(r){return new Date(r).toLocaleString("en-US",{month:"short",day:"numeric",hour:"numeric",minute:"2-digit",hour12:!0})}function nt(r){return new Date(r).toLocaleString("en-US",{hour:"numeric",minute:"2-digit",hour12:!0})}function ot(r){return new Date(r).toLocaleString("en-US",{month:"short",day:"numeric",year:"numeric"})}function st(r,e){return te.default.isAbsolute(r)?te.default.relative(e,r):r}function it(r,e,t){let s=H(r);if(s.length>0)return st(s[0],e);if(t){let n=H(t);if(n.length>0)return st(n[0],e)}return"General"}function yt(r){let e=new Map;for(let s of r){let n=s.type==="observation"?s.data.created_at:s.data.displayTime,o=ot(n);e.has(o)||e.set(o,[]),e.get(o).push(s)}let t=Array.from(e.entries()).sort((s,n)=>{let o=new Date(s[0]).getTime(),i=new Date(n[0]).getTime();return o-i});return new Map(t)}function Mt(r,e){return e.fullObservationField==="narrative"?r.narrative:r.facts?H(r.facts).join(`
`):null}function Lt(r,e,t,s,n,o){let i=[];o?i.push(...Ve(r)):i.push(...xe(r));let a=null,d="",c=!1;for(let u of e)if(u.type==="summary"){c&&(i.push(""),c=!1,a=null,d="");let m=u.data,g=rt(m.displayTime);o?i.push(...ze(m,g)):i.push(...we(m,g))}else{let m=u.data,g=it(m.files_modified,n,m.files_read),E=nt(m.created_at),T=E!==d,h=T?E:"";d=E;let _=t.has(m.id);if(g!==a&&(c&&i.push(""),o?i.push(...qe(g)):i.push(...ke(g)),a=g,c=!0),_){let S=Mt(m,s);o?i.push(...Je(m,E,T,S,s)):(c&&!o&&(i.push(""),c=!1),i.push(...$e(m,h,S,s)),a=null)}else o?i.push(Ke(m,E,T,s)):i.push(Ue(m,h,s))}return c&&i.push(""),i}function at(r,e,t,s,n){let o=[],i=yt(r);for(let[a,d]of i)o.push(...Lt(a,d,e,t,s,n));return o}function dt(r,e,t){return!(!r.showLastSummary||!e||!!!(e.investigated||e.learned||e.completed||e.next_steps)||t&&e.created_at_epoch<=t.created_at_epoch)}function pt(r,e){let t=[];return e?(t.push(...x("Investigated",r.investigated,p.blue)),t.push(...x("Learned",r.learned,p.yellow)),t.push(...x("Completed",r.completed,p.green)),t.push(...x("Next Steps",r.next_steps,p.magenta))):(t.push(...D("Investigated",r.investigated)),t.push(...D("Learned",r.learned)),t.push(...D("Completed",r.completed)),t.push(...D("Next Steps",r.next_steps))),t}function ct(r,e){return e?Qe(r):Pe(r)}function lt(r,e,t){return!F(e)||r.totalDiscoveryTokens<=0||r.savings<=0?[]:t?Ze(r.totalDiscoveryTokens,r.totalReadTokens):Fe(r.totalDiscoveryTokens,r.totalReadTokens)}var vt=ut.default.join((0,mt.homedir)(),".claude","plugins","marketplaces","thedotmack","plugin",".install-version");function Dt(){try{return new w}catch(r){if(r.code==="ERR_DLOPEN_FAILED"){try{(0,_t.unlinkSync)(vt)}catch(e){l.debug("SYSTEM","Marker file cleanup failed (may not exist)",{},e)}return l.error("SYSTEM","Native module rebuild needed - restart Claude Code to auto-fix"),null}throw r}}function xt(r,e){return e?et(r):je(r)}function kt(r,e,t,s,n,o,i){let a=[],d=J(e);a.push(...tt(r,d,s,i));let c=t.slice(0,s.sessionCount),u=Ie(c,t),m=ee(e,u),g=Ne(e,s.fullObservationCount);a.push(...at(m,g,s,n,i));let E=t[0],T=e[0];dt(s,E,T)&&a.push(...pt(E,i));let h=Z(e,s,o,n);return a.push(...ct(h,i)),a.push(...lt(d,s,i)),a.join(`
