#!/usr/bin/env node
import x from"path";import{homedir as Ee}from"os";import{existsSync as Te,readFileSync as he,unlinkSync as ge}from"fs";import{stdin as P}from"process";import{fileURLToPath as be}from"url";import{dirname as Se}from"path";import{createRequire as me}from"node:module";import{join as f,dirname as de,basename as Ue}from"path";import{homedir as K}from"os";import{existsSync as Xe,mkdirSync as ce}from"fs";import{fileURLToPath as pe}from"url";function ue(){return typeof __dirname<"u"?__dirname:de(pe(import.meta.url))}var je=ue(),L=process.env.CLAUDE_MEM_DATA_DIR||f(K(),".claude-mem"),B=process.env.CLAUDE_CONFIG_DIR||f(K(),".claude"),He=f(L,"archives"),Pe=f(L,"logs"),We=f(L,"trash"),Ge=f(L,"backups"),Ye=f(L,"settings.json"),V=f(L,"claude-mem.db"),qe=f(L,"vector-db"),Ke=f(B,"settings.json"),Ve=f(B,"commands"),Je=f(B,"CLAUDE.md");function J(c){ce(c,{recursive:!0})}var j=(i=>(i[i.DEBUG=0]="DEBUG",i[i.INFO=1]="INFO",i[i.WARN=2]="WARN",i[i.ERROR=3]="ERROR",i[i.SILENT=4]="SILENT",i))(j||{}),H=class{level;useColor;constructor(){let e=process.env.CLAUDE_MEM_LOG_LEVEL?.toUpperCase()||"INFO";this.level=j[e]??1,this.useColor=process.stdout.isTTY??!1}correlationId(e,s){return`obs-${e}-${s}`}sessionId(e){return`session-${e}`}formatData(e){if(e==null)return"";if(typeof e=="string")return e;if(typeof e=="number"||typeof e=="boolean")return e.toString();if(typeof e=="object"){if(e instanceof Error)return this.level===0?`${e.message}
...
`+JSON.stringify(i,null,2):n=" "+this.formatData(i));let b="";if(r){let{sessionId:T,sdkSessionId:R,correlationId:l,...p}=r;Object.keys(p).length>0&&(b=` {${Object.entries(p).map(([$,A])=>`${$}=${A}`).join(", ")}}`)}let S=`[${a}] [${d}] [${m}] ${g}${t}${b}${n}`;e===3?console.error(S):console.log(S)}debug(e,s,t,r){this.log(0,e,s,t,r)}info(e,s,t,r){this.log(1,e,s,t,r)}warn(e,s,t,r){this.log(2,e,s,t,r)}error(e,s,t,r){this.log(3,e,s,t,r)}dataIn(e,s,t,r){this.info(e,`\u2192 ${s}`,t,r)}dataOut(e,s,t,r){this.info(e,`\u2190 ${s}`,t,r)}success(e,s,t,r){this.info(e,`\u2713 ${s}`,t,r)}failure(e,s,t,r){this.error(e,`\u2717 ${s}`,t,r)}timing(e,s,t,r){this.info(e,`\u23F1 ${s}`,r,{duration:`${t}ms`})}},Q=new H;var le=me(import.meta.url),_e=le("better-sqlite3"),M=class{db;constructor(){J(L),this.db=new _e(V),this.db.pragma("journal_mode = WAL"),this.db.pragma("synchronous = NORMAL"),this.db.pragma("foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn()}initializeSchema(){try{this.db.exec(`
    ...
    `;try{let T=this.db.prepare(n).all(m,g,...d),R=this.db.prepare(b).all(m,g,...d),l=this.db.prepare(S).all(m,g,...d);return{observations:T,sessions:R.map(p=>({id:p.id,sdk_session_id:p.sdk_session_id,project:p.project,request:p.request,completed:p.completed,next_steps:p.next_steps,created_at:p.created_at,created_at_epoch:p.created_at_epoch})),prompts:l.map(p=>({id:p.id,claude_session_id:p.claude_session_id,project:p.project,prompt:p.prompt_text,created_at:p.created_at,created_at_epoch:p.created_at_epoch}))}}catch(T){return console.error("[SessionStore] Error querying timeline records:",T.message),{observations:[],sessions:[],prompts:[]}}}close(){this.db.close()}};var fe=be(import.meta.url),Re=Se(fe),Ne=x.join(Re,"../../.install-version");function Ie(){try{let c=x.join(Ee(),".claude","settings.json");if(Te(c)){let e=JSON.parse(he(c,"utf-8"));if(e.env?.CLAUDE_MEM_CONTEXT_OBSERVATIONS){let s=parseInt(e.env.CLAUDE_MEM_CONTEXT_OBSERVATIONS,10);if(!isNaN(s)&&s>0)return s}}}catch{}return parseInt(process.env.CLAUDE_MEM_CONTEXT_OBSERVATIONS||"50",10)}var Oe=Ie(),z=10,Z=4,ye=1,o={reset:"\x1B[0m",bright:"\x1B[1m",dim:"\x1B[2m",cyan:"\x1B[36m",green:"\x1B[32m",yellow:"\x1B[33m",blue:"\x1B[34m",magenta:"\x1B[35m",gray:"\x1B[90m",red:"\x1B[31m"};function Le(c){if(!c)return[];try{let e=JSON.parse(c);return Array.isArray(e)?e:[]}catch{return[]}}function ve(c){return new Date(c).toLocaleString("en-US",{month:"short",day:"numeric",hour:"numeric",minute:"2-digit",hour12:!0})}function Ae(c){return new Date(c).toLocaleString("en-US",{hour:"numeric",minute:"2-digit",hour12:!0})}function De(c){return new Date(c).toLocaleString("en-US",{month:"short",day:"numeric",year:"numeric"})}function Ce(c,e){return x.isAbsolute(c)?x.relative(e,c):c}function w(c,e,s,t){return e?t?[`${s}${c}:${o.reset} ${e}`,""]:[`**${c}**: ${e}`,""]:[]}async function ee(c,e=!1){let s=c?.cwd??process.cwd(),t=s?x.basename(s):"unknown-project",r;try{r=new M}catch(b){if(b.code==="ERR_DLOPEN_FAILED"){try{ge(Ne)}catch{}console.error("\u26A0\uFE0F  Native module rebuild needed - restart Claude Code to auto-fix"),console.error("   (This happens after Node.js version upgrades)"),process.exit(0)}throw b}let i=r.db.prepare(`
    SELECT
    ...
    ORDER BY created_at_epoch DESC
    LIMIT ?
  `).all(t,Oe),a=r.db.prepare(`
    SELECT id, sdk_session_id, request, investigated, learned, completed, next_steps, created_at, created_at_epoch
    FROM session_summaries
    WHERE project = ?
    ORDER BY created_at_epoch DESC
    LIMIT ?
  `).all(t,z+ye);if(i.length===0&&a.length===0)return r.close(),e?`
