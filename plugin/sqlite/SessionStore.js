...
"use strict";var x=Object.defineProperty;var ne=Object.getOwnPropertyDescriptor;var oe=Object.getOwnPropertyNames;var ie=Object.prototype.hasOwnProperty;var ae=(d,e)=>{for(var s in e)x(d,s,{get:e[s],enumerable:!0})},de=(d,e,s,t)=>{if(e&&typeof e=="object"||typeof e=="function")for(let r of oe(e))!ie.call(d,r)&&r!==s&&x(d,r,{get:()=>e[r],enumerable:!(t=ne(e,r))||t.enumerable});return d};var _e=d=>de(x({},"__esModule",{value:!0}),d);var fe={};ae(fe,{SessionStore:()=>j});module.exports=_e(fe);var H=require("bun:sqlite");var R=require("path"),X=require("os"),D=require("fs"),W=require("url");var Ee=null;function pe(d){return(Ee??process.stderr.write.bind(process.stderr))(d)}function U(d){pe(d)}var ve=process.platform==="win32";function ue(d){return d.replace(/^\uFEFF/,"")}function M(d){return JSON.parse(ue(d))}function me(){return typeof __dirname<"u"?__dirname:(0,R.dirname)((0,W.fileURLToPath)(__IMPORT_META_URL__))}var Me=me();function le(){if(process.env.CLAUDE_MEM_DATA_DIR)return process.env.CLAUDE_MEM_DATA_DIR;let d=(0,R.join)((0,X.homedir)(),".claude-mem"),e=(0,R.join)(d,"settings.json");try{if((0,D.existsSync)(e)){let s=M((0,D.readFileSync)(e,"utf-8")),t=s.env??s;if(t.CLAUDE_MEM_DATA_DIR)return t.CLAUDE_MEM_DATA_DIR}}catch{}return d}var g=le(),ce=process.env.CLAUDE_CONFIG_DIR||(0,R.join)((0,X.homedir)(),".claude"),Fe=(0,R.join)(ce,"plugins","marketplaces","thedotmack"),Te=(0,R.join)(g,"logs"),we=(0,R.join)(g,"settings.json"),q=(0,R.join)(g,"claude-mem.db"),be=(0,R.join)(g,"observer-sessions"),F=(0,R.basename)(be);function V(d){(0,D.mkdirSync)(d,{recursive:!0})}var k={dataDir:()=>g,workerPid:()=>(0,R.join)(g,"worker.pid"),serverPid:()=>(0,R.join)(g,".server-beta.pid"),serverPort:()=>(0,R.join)(g,".server-beta.port"),serverRuntime:()=>(0,R.join)(g,".server-beta.runtime.json"),settings:()=>(0,R.join)(g,"settings.json"),database:()=>(0,R.join)(g,"claude-mem.db"),chroma:()=>(0,R.join)(g,"chroma"),combinedCerts:()=>(0,R.join)(g,"combined_certs.pem"),transcriptsConfig:()=>(0,R.join)(g,"transcript-watch.json"),transcriptsState:()=>(0,R.join)(g,"transcript-watch-state.json"),corpora:()=>(0,R.join)(g,"corpora"),supervisorRegistry:()=>(0,R.join)(g,"supervisor.json"),envFile:()=>(0,R.join)(g,".env"),logsDir:()=>Te};var C=require("fs"),Y=require("path");var G=(n=>(n[n.DEBUG=0]="DEBUG",n[n.INFO=1]="INFO",n[n.WARN=2]="WARN",n[n.ERROR=3]="ERROR",n[n.SILENT=4]="SILENT",n))(G||{}),P=null,B=class{level=null;useColor;logFilePath=null;logFileInitialized=!1;constructor(){this.useColor=process.stdout.isTTY??!1}ensureLogFileInitialized(){if(!this.logFileInitialized){this.logFileInitialized=!0;try{let e=k.logsDir();(0,C.existsSync)(e)||(0,C.mkdirSync)(e,{recursive:!0});let s=new Date().toISOString().split("T")[0];this.logFilePath=(0,Y.join)(e,`claude-mem-${s}.log`)}catch(e){console.error("[LOGGER] Failed to initialize log file:",e instanceof Error?e.message:String(e)),this.logFilePath=null}}}getLevel(){if(this.level===null)try{let e=k.settings();if((0,C.existsSync)(e)){let s=(0,C.readFileSync)(e,"utf-8"),r=(M(s).CLAUDE_MEM_LOG_LEVEL||"INFO").toUpperCase();this.level=G[r]??1}else this.level=1}catch(e){console.error("[LOGGER] Failed to load log level from settings:",e instanceof Error?e.message:String(e)),this.level=1}return this.level}formatData(e){if(e==null)return"";if(typeof e=="string")return e;if(typeof e=="number"||typeof e=="boolean")return e.toString();if(typeof e=="object"){if(e instanceof Error)return this.getLevel()===0?`${e.message}
${e.stack}`:e.message;if(Array.isArray(e))return`[${e.length} items]`;let s=Object.keys(e);return s.length===0?"{}":s.length<=3?JSON.stringify(e):`{${s.length} keys: ${s.slice(0,3).join(", ")}...}`}return String(e)}formatTool(e,s){if(!s)return e;let t=s;if(typeof s=="string")try{t=JSON.parse(s)}catch{t=s}if(e==="Bash"&&t.command)return`${e}(${t.command})`;if(t.file_path)return`${e}(${t.file_path})`;if(t.notebook_path)return`${e}(${t.notebook_path})`;if(e==="Glob"&&t.pattern)return`${e}(${t.pattern})`;if(e==="Grep"&&t.pattern)return`${e}(${t.pattern})`;if(t.url)return`${e}(${t.url})`;if(t.query)return`${e}(${t.query})`;if(e==="Task"){if(t.subagent_type)return`${e}(${t.subagent_type})`;if(t.description)return`${e}(${t.description})`}return e==="Skill"&&t.skill?`${e}(${t.skill})`:e==="LSP"&&t.operation?`${e}(${t.operation})`:e}formatTimestamp(e){let s=e.getFullYear(),t=String(e.getMonth()+1).padStart(2,"0"),r=String(e.getDate()).padStart(2,"0"),n=String(e.getHours()).padStart(2,"0"),o=String(e.getMinutes()).padStart(2,"0"),i=String(e.getSeconds()).padStart(2,"0"),E=String(e.getMilliseconds()).padStart(3,"0");return`${s}-${t}-${r} ${n}:${o}:${i}.${E}`}log(e,s,t,r,n){if(e<this.getLevel())return;this.ensureLogFileInitialized();let o=this.formatTimestamp(new Date),i=G[e].padEnd(5),E=s.padEnd(6),_="";r?.correlationId?_=`[${r.correlationId}] `:r?.sessionId&&(_=`[session-${r.sessionId}] `);let u="";if(n!=null)if(n instanceof Error)u=this.getLevel()===0?`
${n.message}
${n.stack}`:` ${n.message}`;else if(this.getLevel()===0&&typeof n=="object")try{u=`
`+JSON.stringify(n,null,2)}catch{u=" "+this.formatData(n)}else u=" "+this.formatData(n);let I="";if(r){let{sessionId:N,memorySessionId:O,correlationId:S,...m}=r;Object.keys(m).length>0&&(I=` {${Object.entries(m).map(([f,v])=>`${f}=${v}`).join(", ")}}`)}let c=`[${o}] [${i}] [${E}] ${_}${t}${I}${u}`;if(this.logFilePath)try{(0,C.appendFileSync)(this.logFilePath,c+`
`,"utf8")}catch(N){let O=N instanceof Error?N:new Error(String(N));U(`[LOGGER] Failed to write to log file: ${O.message}
${O.stack??""}
`)}else U(c+`
`)}debug(e,s,t,r){this.log(0,e,s,t,r)}info(e,s,t,r){this.log(1,e,s,t,r)}warn(e,s,t,r){this.log(2,e,s,t,r)}setErrorSink(e){P=e}error(e,s,t,r){this.log(3,e,s,t,r),this.routeErrorToSink(s,t,r)}routeErrorToSink(e,s,t){try{if(!P||!(t instanceof Error))return;P(t)}catch{}}dataIn(e,s,t,r){this.info(e,`\u2192 ${s}`,t,r)}dataOut(e,s,t,r){this.info(e,`\u2190 ${s}`,t,r)}success(e,s,t,r){this.info(e,`\u2713 ${s}`,t,r)}failure(e,s,t,r){this.error(e,`\u2717 ${s}`,t,r)}},a=new B;var K=require("crypto");function Q(d,e,s){return(0,K.createHash)("sha256").update([d||"",e||"",s||""].join("\0")).digest("hex").slice(0,16)}var p="claude";function Re(d){return d.trim().toLowerCase().replace(/\s+/g,"-")}function L(d){if(!d)return p;let e=Re(d);return e?e==="transcript"||e.includes("codex")?"codex":e.includes("cursor")?"cursor":e.includes("claude")?"claude":e:p}function J(d){let e=["claude","codex","cursor"];return[...d].sort((s,t)=>{let r=e.indexOf(s),n=e.indexOf(t);return r!==-1||n!==-1?r===-1?1:n===-1?-1:r-n:s.localeCompare(t)})}function z(d,e,s,t,r){let n=Date.now()-t,o=r!==void 0?"up.session_db_id = ?":"up.content_session_id = ?",i=r??e;return d.prepare(`
    SELECT
      up.*,
      s.memory_session_id,
      s.project,
    ...
    LIMIT 1
  `).get(i,s,n)??void 0}var se=["private","claude-mem-context","system_instruction","system-instruction","persisted-output","system-reminder"],Z=new RegExp(`<(${se.join("|")})\\b[^>]*>[\\s\\S]*?</\\1>`,"g");var ee=100;function Se(d){let e=Object.fromEntries(se.map(r=>[r,0]));Z.lastIndex=0;let s=0,t=d.replace(Z,(r,n)=>(e[n]=(e[n]??0)+1,s+=1,""));return s>ee&&a.warn("SYSTEM","tag count exceeds limit",void 0,{tagCount:s,maxAllowed:ee,contentLength:d.length}),{stripped:t.trim(),counts:e}}function te(d){return Se(d).stripped}var Ne=["task-notification"],qe=new RegExp(`^\\s*<(${Ne.join("|")})\\b[^>]*>(?:(?!<\\1\\b|</\\1\\b)[\\s\\S])*</\\1>\\s*$`),Ve=256*1024;var $=4e3;function w(d){let e=d.trim(),t=te(d).trim()||e;return t.length<=$?t:(a.debug("DB","Truncated stored prompt text to the configured cap",{originalLength:t.length,storedLength:$}),`${t.slice(0,$-1)}\u2026`)}var Ie=require("bun:sqlite");var he=5e3,ge=4194304;function Oe(d){return d.prepare(`
    SELECT name
    FROM sqlite_master
    WHERE type = 'table'
      AND name NOT LIKE 'sqlite_%'
    LIMIT 1
  `).get()!=null}function y(d,e,s){try{d.run(e)}catch(t){let r=t instanceof Error?t:new Error(String(t));throw a.warn("DB",`Failed to apply SQLite pragma ${s}`,{sql:e},r),t}}function re(d,e={}){let{enableWal:s=!0,enableIncrementalAutoVacuum:t=!0}=e;y(d,`PRAGMA busy_timeout = ${he}`,"busy_timeout"),y(d,"PRAGMA foreign_keys = ON","foreign_keys"),y(d,"PRAGMA synchronous = NORMAL","synchronous"),y(d,`PRAGMA journal_size_limit = ${ge}`,"journal_size_limit"),t&&!Oe(d)&&y(d,"PRAGMA auto_vacuum = INCREMENTAL","auto_vacuum"),s&&y(d,"PRAGMA journal_mode = WAL","journal_mode")}var j=class{db;constructor(e=q){e instanceof H.Database?this.db=e:(e!==":memory:"&&V(g),this.db=new H.Database(e)),re(this.db),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys(),this.addObservationContentHashColumn(),this.addSessionCustomTitleColumn(),this.addSessionPlatformSourceColumn(),this.addObservationModelColumns(),this.ensureMergedIntoProjectColumns(),this.addObservationSubagentColumns(),this.addObservationsUniqueContentHashIndex(),this.addObservationsMetadataColumn(),this.dropDeadPendingMessagesColumns(),this.ensurePendingMessagesToolUseIdColumn(),this.dropWorkerPidColumn(),this.ensureSDKSessionsPlatformContentIdentity(),this.ensureUserPromptsSessionDbId(),this.ensurePendingMessagesSessionToolUniqueIndex(),this.createAdvisorCallsTable()}getIndexColumns(e){return this.db.query(`PRAGMA index_info(${JSON.stringify(e)})`).all().map(s=>s.name)}hasUniqueIndexOnColumns(e,s){return this.db.query(`PRAGMA index_list(${e})`).all().some(r=>{if(r.unique!==1)return!1;let n=this.getIndexColumns(r.name);return n.length===s.length&&n.every((o,i)=>o===s[i])})}resolvePromptSessionDbId(e,s,t){if(s!==void 0)return s;let r=t?L(t):void 0;return r?this.db.prepare(`
        SELECT id
        FROM sdk_sessions
    ...
    `),this.db.run(`
      ...
      SELECT
        ...
        status, worker_port, prompt_counter, custom_title
      FROM sdk_sessions
    `),this.db.run("DROP TABLE sdk_sessions"),this.db.run("ALTER TABLE sdk_sessions_new RENAME TO sdk_sessions"),this.db.run("CREATE INDEX IF NOT EXISTS idx_sdk_sessions_claude_id ON sdk_sessions(content_session_id)"),this.db.run("CREATE INDEX IF NOT EXISTS idx_sdk_sessions_sdk_id ON sdk_sessions(memory_session_id)"),this.db.run("CREATE INDEX IF NOT EXISTS idx_sdk_sessions_project ON sdk_sessions(project)"),this.db.run("CREATE INDEX IF NOT EXISTS idx_sdk_sessions_status ON sdk_sessions(status)"),this.db.run("CREATE INDEX IF NOT EXISTS idx_sdk_sessions_started ON sdk_sessions(started_at_epoch DESC)"),this.db.run("CREATE INDEX IF NOT EXISTS idx_sdk_sessions_platform_source ON sdk_sessions(platform_source)"),this.db.run("CREATE UNIQUE INDEX IF NOT EXISTS ux_sdk_sessions_platform_content ON sdk_sessions(platform_source, content_session_id)"),e||this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(33,new Date().toISOString())}ensureUserPromptsSessionDbId(){let e=this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(34);if(this.db.query("SELECT name FROM sqlite_master WHERE type='table' AND name='user_prompts'").all().length===0){this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(34,new Date().toISOString());return}let r=this.db.query("PRAGMA table_info(user_prompts)").all().some(_=>_.name==="session_db_id"),o=this.db.query("PRAGMA foreign_key_list(user_prompts)").all().some(_=>_.table==="sdk_sessions"&&_.from==="content_session_id");if(e&&r&&!o)return;let i=this.db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='user_prompts_fts'").all().length>0,E=r?`COALESCE(up.session_db_id, (
    ...
    `),this.db.run("CREATE INDEX IF NOT EXISTS idx_pending_messages_session ON pending_messages(session_db_id)"),this.db.run("CREATE INDEX IF NOT EXISTS idx_pending_messages_status ON pending_messages(status)"),this.db.run("CREATE INDEX IF NOT EXISTS idx_pending_messages_claude_session ON pending_messages(content_session_id)"),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(16,new Date().toISOString()),a.debug("DB","pending_messages table created successfully")}createAdvisorCallsTable(){if(this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(36))return;if(this.db.query("SELECT name FROM sqlite_master WHERE type='table' AND name='advisor_calls'").all().length>0){this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(36,new Date().toISOString());return}a.debug("DB","Creating advisor_calls table"),this.db.run(`
    ...
    `),this.db.run("CREATE INDEX IF NOT EXISTS idx_advisor_calls_session ON advisor_calls(session_db_id)"),this.db.run("CREATE INDEX IF NOT EXISTS idx_advisor_calls_project ON advisor_calls(project)"),this.db.run("CREATE INDEX IF NOT EXISTS idx_advisor_calls_created ON advisor_calls(created_at_epoch DESC)"),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(36,new Date().toISOString()),a.debug("DB","advisor_calls table created successfully")}recordAdvisorCall(e){let s=new Date,t=this.db.prepare(`
    ...
    `).get(e);if(!r)throw new Error(`Session ${e} not found in sdk_sessions`);r.memory_session_id!==s&&(this.db.prepare(`
      ...
      WHERE project IS NOT NULL AND project != ''
        AND project != ?
    `,r=[F];return s&&(t+=" AND COALESCE(platform_source, ?) = ?",r.push(p,s)),t+=" ORDER BY project ASC",this.db.prepare(t).all(...r).map(o=>o.project)}getProjectCatalog(){let e=this.db.prepare(`
      SELECT
    ...
    `).all(F),s=[],t=new Set,r={};for(let o of e){let i=L(o.platform_source);r[i]||(r[i]=[]),r[i].includes(o.project)||r[i].push(o.project),t.has(o.project)||(t.add(o.project),s.push(o.project))}let n=J(Object.keys(r));return{projects:s,sources:n,projectsBySource:Object.fromEntries(n.map(o=>[o,r[o]||[]]))}}getLatestUserPrompt(e,s){let t=this.resolvePromptSessionDbId(e,s),r=t!==null?"up.session_db_id = ?":"up.content_session_id = ?",n=t!==null?t:e;return this.db.prepare(`
      ...
      ORDER BY up.created_at_epoch DESC
      LIMIT 1
    `).get(n)}findRecentDuplicateUserPrompt(e,s,t,r){return z(this.db,e,w(s),t,this.resolvePromptSessionDbId(e,r)??void 0)}getRecentSessionsWithStatus(e,s=3,t){let r=[e],n="";return t&&(n=`AND COALESCE(NULLIF(s.platform_source, ''), '${p}') = ?`,r.push(L(t))),r.push(s),this.db.prepare(`
      SELECT * FROM (
        SELECT
          s.memory_session_id,
          s.status,
    ...
    `).get(e,L(s))||null:this.db.prepare(`
        ...
        FROM observations
        WHERE id = ?
      `).get(e)||null}getObservationsByIds(e,s={}){if(e.length===0)return[];let{orderBy:t="date_desc",limit:r,project:n,platformSource:o,type:i,concepts:E,files:_}=s,u=t==="relevance",I=u?"":`ORDER BY o.created_at_epoch ${t==="date_asc"?"ASC":"DESC"}`,c=r&&!u?`LIMIT ${r}`:"",N=e.map(()=>"?").join(","),O=[...e],S=[];if(n&&(S.push("o.project = ?"),O.push(n)),o&&(S.push(`COALESCE(NULLIF(s.platform_source, ''), '${p}') = ?`),O.push(L(o))),i)if(Array.isArray(i)){let h=i.map(()=>"?").join(",");S.push(`o.type IN (${h})`),O.push(...i)}else S.push("o.type = ?"),O.push(i);if(E){let h=Array.isArray(E)?E:[E],b=h.map(()=>"EXISTS (SELECT 1 FROM json_each(o.concepts) WHERE value = ?)");O.push(...h),S.push(`(${b.join(" OR ")})`)}if(_){let h=Array.isArray(_)?_:[_],b=h.map(()=>"(EXISTS (SELECT 1 FROM json_each(o.files_read) WHERE value LIKE ?) OR EXISTS (SELECT 1 FROM json_each(o.files_modified) WHERE value LIKE ?))");h.forEach(A=>{O.push(`%${A}%`,`%${A}%`)}),S.push(`(${b.join(" OR ")})`)}let m=S.length>0?`WHERE o.id IN (${N}) AND ${S.join(" AND ")}`:`WHERE o.id IN (${N})`,f=this.db.prepare(`
      SELECT o.*
      FROM observations o
      LEFT JOIN sdk_sessions s ON s.memory_session_id = o.memory_session_id
      ${m}
      ${I}
      ${c}
    `).all(...O);if(!u)return f;let v=new Map(f.map(h=>[h.id,h])),T=e.map(h=>v.get(h)).filter(h=>!!h);return r?T.slice(0,r):T}getSummaryForSession(e,s){let t=[e],r="";return s&&(r=`
        AND EXISTS (
          SELECT 1
    ...
    `).get(e).count}createSDKSession(e,s,t,r,n){let o=new Date,i=o.getTime(),E=n?L(n):p,_=w(t),u=this.db.prepare(`
      SELECT id, platform_source
      FROM sdk_sessions
      WHERE COALESCE(NULLIF(platform_source, ''), ?) = ?
        AND content_session_id = ?
    `).get(p,E,e);if(u)return s&&this.db.prepare(`
      ...
      VALUES (?, NULL, ?, ?, ?, ?, ?, ?, 'active')
    `).run(e,s,E,_,r||null,o.toISOString(),i);return Number(I.lastInsertRowid)}saveUserPrompt(e,s,t,r){let n=new Date,o=n.getTime(),i=w(t),E=this.resolvePromptSessionDbId(e,r);return this.db.prepare(`
      INSERT INTO user_prompts
      (session_db_id, content_session_id, prompt_number, prompt_text, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?)
    `).run(E,e,s,i,n.toISOString(),o).lastInsertRowid}getUserPrompt(e,s,t){let r=this.resolvePromptSessionDbId(e,t);return r!==null?this.db.prepare(`
        SELECT prompt_text
        FROM user_prompts
      ...
      LIMIT 1
    `).get(e,s)?.prompt_text??null}storeObservation(e,s,t,r,n=0,o,i){let E=this.storeObservations(e,s,[t],null,r,n,o,i);return{id:E.observationIds[0],createdAtEpoch:E.createdAtEpoch}}storeSummary(e,s,t,r,n=0,o){let i=o??Date.now(),E=new Date(i).toISOString(),u=this.db.prepare(`
      INSERT INTO session_summaries
      (memory_session_id, project, request, investigated, learned, completed,
       next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,s,t.request,t.investigated,t.learned,t.completed,t.next_steps,t.notes,r||null,n,E,i);return{id:Number(u.lastInsertRowid),createdAtEpoch:i}}storeObservations(e,s,t,r,n,o=0,i,E){let _=i??Date.now(),u=new Date(_).toISOString();return this.db.transaction(()=>{let c=[],N=this.db.prepare(`
        ...
        RETURNING id
      `),O=this.db.prepare("SELECT id FROM observations WHERE memory_session_id = ? AND content_hash = ?");for(let m of t){let l=Q(e,m.title,m.narrative),f=N.get(e,s,m.type,m.title,m.subtitle,JSON.stringify(m.facts),m.narrative,JSON.stringify(m.concepts),JSON.stringify(m.files_read),JSON.stringify(m.files_modified),n||null,o,m.agent_type??null,m.agent_id??null,l,u,_,E||null,m.metadata??null);if(f){c.push(f.id);continue}let v=O.get(e,l);if(!v)throw new Error(`storeObservations: ON CONFLICT without existing row for content_hash=${l}`);c.push(v.id)}let S=null;if(r){let l=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,s,r.request,r.investigated,r.learned,r.completed,r.next_steps,r.notes,n||null,o,u,_);S=Number(l.lastInsertRowid)}return{observationIds:c,summaryId:S,createdAtEpoch:_}})()}getSessionSummariesByIds(e,s={}){if(e.length===0)return[];let{orderBy:t="date_desc",limit:r,project:n,platformSource:o}=s,i=t==="relevance",E=i?"":`ORDER BY ss.created_at_epoch ${t==="date_asc"?"ASC":"DESC"}`,_=r&&!i?`LIMIT ${r}`:"",u=e.map(()=>"?").join(","),I=[...e],c=[];n&&(c.push("ss.project = ?"),I.push(n)),o&&(c.push(`COALESCE(NULLIF(s.platform_source, ''), '${p}') = ?`),I.push(L(o)));let N=c.length>0?`AND ${c.join(" AND ")}`:"",S=this.db.prepare(`
      SELECT ss.*
      ...
      ${E}
      ${_}
    `).all(...I);if(!i)return S;let m=new Map(S.map(f=>[f.id,f])),l=e.map(f=>m.get(f)).filter(f=>!!f);return r?l.slice(0,r):l}getUserPromptsByIds(e,s={}){if(e.length===0)return[];let{orderBy:t="date_desc",limit:r,project:n,platformSource:o}=s,i=t==="relevance",E=i?"":`ORDER BY up.created_at_epoch ${t==="date_asc"?"ASC":"DESC"}`,_=r?`LIMIT ${r}`:"",u=e.map(()=>"?").join(","),I=[...e],c=[];n&&(c.push("s.project = ?"),I.push(n)),o&&(c.push(`COALESCE(NULLIF(s.platform_source, ''), '${p}') = ?`),I.push(L(o)));let N=c.length>0?`AND ${c.join(" AND ")}`:"",S=this.db.prepare(`
      SELECT
        up.*,
        s.project,
        s.memory_session_id,
      ...
      ${E}
      ${_}
    `).all(...I);if(!i)return S;let m=new Map(S.map(l=>[l.id,l]));return e.map(l=>m.get(l)).filter(l=>!!l)}getTimelineAroundTimestamp(e,s=10,t=10,r,n){return this.getTimelineAroundObservation(null,e,s,t,r,n)}getTimelineAroundObservation(e,s,t=10,r=10,n,o){let i=o?L(o):void 0,E=(T,h)=>{let b=[],A=[];return n&&(b.push(`${T}.project = ?`),A.push(n)),i&&(b.push(`COALESCE(NULLIF(${h}.platform_source, ''), '${p}') = ?`),A.push(i)),{clause:b.length>0?`AND ${b.join(" AND ")}`:"",params:A}},_=E("o","src"),u=E("ss","src"),I=E("s","s"),c,N;if(e!==null){let T=`
