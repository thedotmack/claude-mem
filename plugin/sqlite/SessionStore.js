  ...
  `).get(i,s,n)??void 0}var Z=["private","claude-mem-context","system_instruction","system-instruction","persisted-output","system-reminder"],J=new RegExp(`<(${Z.join("|")})\\b[^>]*>[\\s\\S]*?</\\1>`,"g");var z=100;function ce(E){let e=Object.fromEntries(Z.map(r=>[r,0]));J.lastIndex=0;let s=0,t=E.replace(J,(r,n)=>(e[n]=(e[n]??0)+1,s+=1,""));return s>z&&a.warn("SYSTEM","tag count exceeds limit",void 0,{tagCount:s,maxAllowed:z,contentLength:E.length}),{stripped:t.trim(),counts:e}}function ee(E){return ce(E).stripped}var Te=["task-notification"],Fe=new RegExp(`^\\s*<(${Te.join("|")})\\b[^>]*>(?:(?!<\\1\\b|</\\1\\b)[\\s\\S])*</\\1>\\s*$`),we=256*1024;var P=4e3;function y(E){let e=E.trim(),t=ee(E).trim()||e;return t.length<=P?t:(a.debug("DB","Truncated stored prompt text to the configured cap",{originalLength:t.length,storedLength:P}),`${t.slice(0,P-1)}\u2026`)}var be=5e3,H=class{db;constructor(e=j){e instanceof B.Database?this.db=e:(e!==":memory:"&&W(O),this.db=new B.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.db.run("PRAGMA journal_size_limit = 4194304")),this.db.run(`PRAGMA busy_timeout = ${be}`),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys(),this.addObservationContentHashColumn(),this.addSessionCustomTitleColumn(),this.addSessionPlatformSourceColumn(),this.addObservationModelColumns(),this.ensureMergedIntoProjectColumns(),this.addObservationSubagentColumns(),this.addObservationsUniqueContentHashIndex(),this.addObservationsMetadataColumn(),this.dropDeadPendingMessagesColumns(),this.ensurePendingMessagesToolUseIdColumn(),this.dropWorkerPidColumn(),this.ensureSDKSessionsPlatformContentIdentity(),this.ensureUserPromptsSessionDbId(),this.ensurePendingMessagesSessionToolUniqueIndex()}getIndexColumns(e){return this.db.query(`PRAGMA index_info(${JSON.stringify(e)})`).all().map(s=>s.name)}hasUniqueIndexOnColumns(e,s){return this.db.query(`PRAGMA index_list(${e})`).all().some(r=>{if(r.unique!==1)return!1;let n=this.getIndexColumns(r.name);return n.length===s.length&&n.every((o,i)=>o===s[i])})}resolvePromptSessionDbId(e,s,t){if(s!==void 0)return s;let r=t?L(t):void 0;return r?this.db.prepare(`
    ...
    `),this.db.run(`
      ...
      SELECT
        ...
        status, worker_port, prompt_counter, custom_title
      FROM sdk_sessions
    `),this.db.run("DROP TABLE sdk_sessions"),this.db.run("ALTER TABLE sdk_sessions_new RENAME TO sdk_sessions"),this.db.run("CREATE INDEX IF NOT EXISTS idx_sdk_sessions_claude_id ON sdk_sessions(content_session_id)"),this.db.run("CREATE INDEX IF NOT EXISTS idx_sdk_sessions_sdk_id ON sdk_sessions(memory_session_id)"),this.db.run("CREATE INDEX IF NOT EXISTS idx_sdk_sessions_project ON sdk_sessions(project)"),this.db.run("CREATE INDEX IF NOT EXISTS idx_sdk_sessions_status ON sdk_sessions(status)"),this.db.run("CREATE INDEX IF NOT EXISTS idx_sdk_sessions_started ON sdk_sessions(started_at_epoch DESC)"),this.db.run("CREATE INDEX IF NOT EXISTS idx_sdk_sessions_platform_source ON sdk_sessions(platform_source)"),this.db.run("CREATE UNIQUE INDEX IF NOT EXISTS ux_sdk_sessions_platform_content ON sdk_sessions(platform_source, content_session_id)"),e||this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(33,new Date().toISOString())}ensureUserPromptsSessionDbId(){let e=this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(34);if(this.db.query("SELECT name FROM sqlite_master WHERE type='table' AND name='user_prompts'").all().length===0){this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(34,new Date().toISOString());return}let r=this.db.query("PRAGMA table_info(user_prompts)").all().some(d=>d.name==="session_db_id"),o=this.db.query("PRAGMA foreign_key_list(user_prompts)").all().some(d=>d.table==="sdk_sessions"&&d.from==="content_session_id");if(e&&r&&!o)return;let i=this.db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='user_prompts_fts'").all().length>0,_=r?`COALESCE(up.session_db_id, (
    ...
    `).all(F),s=[],t=new Set,r={};for(let o of e){let i=L(o.platform_source);r[i]||(r[i]=[]),r[i].includes(o.project)||r[i].push(o.project),t.has(o.project)||(t.add(o.project),s.push(o.project))}let n=K(Object.keys(r));return{projects:s,sources:n,projectsBySource:Object.fromEntries(n.map(o=>[o,r[o]||[]]))}}getLatestUserPrompt(e,s){let t=this.resolvePromptSessionDbId(e,s),r=t!==null?"up.session_db_id = ?":"up.content_session_id = ?",n=t!==null?t:e;return this.db.prepare(`
      ...
      ORDER BY up.created_at_epoch DESC
      LIMIT 1
    `).get(n)}findRecentDuplicateUserPrompt(e,s,t,r){return Q(this.db,e,y(s),t,this.resolvePromptSessionDbId(e,r)??void 0)}getRecentSessionsWithStatus(e,s=3,t){let r=[e],n="";return t&&(n=`AND COALESCE(NULLIF(s.platform_source, ''), '${p}') = ?`,r.push(L(t))),r.push(s),this.db.prepare(`
      SELECT * FROM (
        SELECT
          s.memory_session_id,
          s.status,
    ...
    `).get(e,L(s))||null:this.db.prepare(`
        ...
        FROM observations
        WHERE id = ?
      `).get(e)||null}getObservationsByIds(e,s={}){if(e.length===0)return[];let{orderBy:t="date_desc",limit:r,project:n,platformSource:o,type:i,concepts:_,files:d}=s,u=t==="relevance",I=u?"":`ORDER BY o.created_at_epoch ${t==="date_asc"?"ASC":"DESC"}`,c=r&&!u?`LIMIT ${r}`:"",S=e.map(()=>"?").join(","),g=[...e],N=[];if(n&&(N.push("o.project = ?"),g.push(n)),o&&(N.push(`COALESCE(NULLIF(s.platform_source, ''), '${p}') = ?`),g.push(L(o))),i)if(Array.isArray(i)){let h=i.map(()=>"?").join(",");N.push(`o.type IN (${h})`),g.push(...i)}else N.push("o.type = ?"),g.push(i);if(_){let h=Array.isArray(_)?_:[_],b=h.map(()=>"EXISTS (SELECT 1 FROM json_each(o.concepts) WHERE value = ?)");g.push(...h),N.push(`(${b.join(" OR ")})`)}if(d){let h=Array.isArray(d)?d:[d],b=h.map(()=>"(EXISTS (SELECT 1 FROM json_each(o.files_read) WHERE value LIKE ?) OR EXISTS (SELECT 1 FROM json_each(o.files_modified) WHERE value LIKE ?))");h.forEach(A=>{g.push(`%${A}%`,`%${A}%`)}),N.push(`(${b.join(" OR ")})`)}let m=N.length>0?`WHERE o.id IN (${S}) AND ${N.join(" AND ")}`:`WHERE o.id IN (${S})`,f=this.db.prepare(`
      SELECT o.*
      FROM observations o
      LEFT JOIN sdk_sessions s ON s.memory_session_id = o.memory_session_id
      ${m}
      ${I}
      ${c}
    `).all(...g);if(!u)return f;let v=new Map(f.map(h=>[h.id,h])),T=e.map(h=>v.get(h)).filter(h=>!!h);return r?T.slice(0,r):T}getSummaryForSession(e,s){let t=[e],r="";return s&&(r=`
        AND EXISTS (
          SELECT 1
    ...
    `).get(e).count}createSDKSession(e,s,t,r,n){let o=new Date,i=o.getTime(),_=n?L(n):p,d=y(t),u=this.db.prepare(`
    ...
    `).get(p,_,e);if(u)return s&&this.db.prepare(`
      ...
      VALUES (?, NULL, ?, ?, ?, ?, ?, ?, 'active')
    `).run(e,s,_,d,r||null,o.toISOString(),i);return Number(I.lastInsertRowid)}saveUserPrompt(e,s,t,r){let n=new Date,o=n.getTime(),i=y(t),_=this.resolvePromptSessionDbId(e,r);return this.db.prepare(`
      INSERT INTO user_prompts
      (session_db_id, content_session_id, prompt_number, prompt_text, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?)
    `).run(_,e,s,i,n.toISOString(),o).lastInsertRowid}getUserPrompt(e,s,t){let r=this.resolvePromptSessionDbId(e,t);return r!==null?this.db.prepare(`
        SELECT prompt_text
        FROM user_prompts
      ...
      LIMIT 1
    `).get(e,s)?.prompt_text??null}storeObservation(e,s,t,r,n=0,o,i){let _=this.storeObservations(e,s,[t],null,r,n,o,i);return{id:_.observationIds[0],createdAtEpoch:_.createdAtEpoch}}storeSummary(e,s,t,r,n=0,o){let i=o??Date.now(),_=new Date(i).toISOString(),u=this.db.prepare(`
      INSERT INTO session_summaries
      (memory_session_id, project, request, investigated, learned, completed,
       next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,s,t.request,t.investigated,t.learned,t.completed,t.next_steps,t.notes,r||null,n,_,i);return{id:Number(u.lastInsertRowid),createdAtEpoch:i}}storeObservations(e,s,t,r,n,o=0,i,_){let d=i??Date.now(),u=new Date(d).toISOString();return this.db.transaction(()=>{let c=[],S=this.db.prepare(`
        ...
        RETURNING id
      `),g=this.db.prepare("SELECT id FROM observations WHERE memory_session_id = ? AND content_hash = ?");for(let m of t){let l=Y(e,m.title,m.narrative),f=S.get(e,s,m.type,m.title,m.subtitle,JSON.stringify(m.facts),m.narrative,JSON.stringify(m.concepts),JSON.stringify(m.files_read),JSON.stringify(m.files_modified),n||null,o,m.agent_type??null,m.agent_id??null,l,u,d,_||null,m.metadata??null);if(f){c.push(f.id);continue}let v=g.get(e,l);if(!v)throw new Error(`storeObservations: ON CONFLICT without existing row for content_hash=${l}`);c.push(v.id)}let N=null;if(r){let l=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,s,r.request,r.investigated,r.learned,r.completed,r.next_steps,r.notes,n||null,o,u,d);N=Number(l.lastInsertRowid)}return{observationIds:c,summaryId:N,createdAtEpoch:d}})()}getSessionSummariesByIds(e,s={}){if(e.length===0)return[];let{orderBy:t="date_desc",limit:r,project:n,platformSource:o}=s,i=t==="relevance",_=i?"":`ORDER BY ss.created_at_epoch ${t==="date_asc"?"ASC":"DESC"}`,d=r&&!i?`LIMIT ${r}`:"",u=e.map(()=>"?").join(","),I=[...e],c=[];n&&(c.push("ss.project = ?"),I.push(n)),o&&(c.push(`COALESCE(NULLIF(s.platform_source, ''), '${p}') = ?`),I.push(L(o)));let S=c.length>0?`AND ${c.join(" AND ")}`:"",N=this.db.prepare(`
      SELECT ss.*
      ...
      ${_}
      ${d}
    `).all(...I);if(!i)return N;let m=new Map(N.map(f=>[f.id,f])),l=e.map(f=>m.get(f)).filter(f=>!!f);return r?l.slice(0,r):l}getUserPromptsByIds(e,s={}){if(e.length===0)return[];let{orderBy:t="date_desc",limit:r,project:n,platformSource:o}=s,i=t==="relevance",_=i?"":`ORDER BY up.created_at_epoch ${t==="date_asc"?"ASC":"DESC"}`,d=r?`LIMIT ${r}`:"",u=e.map(()=>"?").join(","),I=[...e],c=[];n&&(c.push("s.project = ?"),I.push(n)),o&&(c.push(`COALESCE(NULLIF(s.platform_source, ''), '${p}') = ?`),I.push(L(o)));let S=c.length>0?`AND ${c.join(" AND ")}`:"",N=this.db.prepare(`
      SELECT
        up.*,
        s.project,
        s.memory_session_id,
      ...
      ${_}
      ${d}
    `).all(...I);if(!i)return N;let m=new Map(N.map(l=>[l.id,l]));return e.map(l=>m.get(l)).filter(l=>!!l)}getTimelineAroundTimestamp(e,s=10,t=10,r,n){return this.getTimelineAroundObservation(null,e,s,t,r,n)}getTimelineAroundObservation(e,s,t=10,r=10,n,o){let i=o?L(o):void 0,_=(T,h)=>{let b=[],A=[];return n&&(b.push(`${T}.project = ?`),A.push(n)),i&&(b.push(`COALESCE(NULLIF(${h}.platform_source, ''), '${p}') = ?`),A.push(i)),{clause:b.length>0?`AND ${b.join(" AND ")}`:"",params:A}},d=_("o","src"),u=_("ss","src"),I=_("s","s"),c,S;if(e!==null){let T=`
      ...
      `,h=`
        ...
        LIMIT ?
      `;try{let b=this.db.prepare(T).all(s,...d.params,t),A=this.db.prepare(h).all(s,...d.params,r+1);if(b.length===0&&A.length===0)return{observations:[],sessions:[],prompts:[]};c=b.length>0?b[b.length-1].created_at_epoch:s,S=A.length>0?A[A.length-1].created_at_epoch:s}catch(b){return b instanceof Error?a.error("DB","Error getting boundary timestamps",{project:n},b):a.error("DB","Error getting boundary timestamps with non-Error",{},new Error(String(b))),{observations:[],sessions:[],prompts:[]}}}let g=`
      SELECT o.*
      FROM observations o
      LEFT JOIN sdk_sessions src ON src.memory_session_id = o.memory_session_id
