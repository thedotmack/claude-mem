# claude-mem vendored fork of chroma-mcp 0.2.6

Source: chroma-mcp 0.2.6 sdist from PyPI (upstream:
https://github.com/chroma-core/chroma-mcp). Vendored for the
multilingual-e5 embedding migration — see
`plans/2026-07-29-e5-embedding-migration.md`.

Fork deltas vs upstream 0.2.6 (all marked "claude-mem fork" in code):

1. `src/chroma_mcp/server.py` — new registry entry `e5-multilingual` →
   `SentenceTransformerEmbeddingFunction(model_name="intfloat/multilingual-e5-small",
   normalize_embeddings=True)`. Registry-safe: chromadb persists it as the
   known EF name `sentence_transformer`, so collections survive restarts and
   can even be served by a STOCK chroma-mcp via `build_from_config` (given
   `sentence-transformers` in its env).
2. `pyproject.toml` — `sentence-transformers>=4.1.0` promoted from an
   optional extra to a required dependency (pulls torch + transformers).
3. `chroma_add_documents` hardening — `get_or_create_collection` replaced by
   `get_collection` + explicit error, so a collection can never be auto-born
   with the default EF bypassing the chosen `embedding_function_name`.
4. `chroma_create_collection` docstring fix — upstream listed `ollama`,
   which is not in the registry dict (would KeyError).

## HuggingFace download gotcha: HF_HUB_DISABLE_XET

The first use of `e5-multilingual` downloads ~476 MB
(`intfloat/multilingual-e5-small`) from HuggingFace. `huggingface_hub`
defaults to the hf-xet transfer protocol, which **hangs at 0 bytes behind
some proxies**. Workaround: set `HF_HUB_DISABLE_XET=1` in the environment of
the spawned server to fall back to plain HTTPS downloads. claude-mem sets
this automatically for its spawned subprocess (see
`ChromaMcpManager.getSpawnEnv`); set it manually if you run this fork
standalone.
