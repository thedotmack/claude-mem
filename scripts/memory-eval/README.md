...
## Кросс-платформенные сравнения: канал доставки — контролируемая переменная

Платформы различаются каналом инъекции: у Kimi контекст доходит до модели
**только** через `UserPromptSubmit` (SessionStart-результаты отбрасываются,
поддержки additionalContext/systemMessage нет), у Claude Code каналов
несколько. Поэтому сравнение «качества памяти» между платформами без контроля
канала измеряет не память, а разницу в доставке (confound). Правило: либо на
всех платформах гонять инъекцию через один и тот же канал (UserPromptSubmit),
либо вводить канал отдельным фактором в дизайн эксперимента и отчётах.
