...
const DB_PATH = path.join(os.homedir(), '.magic-claude-mem', 'magic-claude-mem.db');
const SETTINGS_PATH = path.join(os.homedir(), '.magic-claude-mem', 'settings.json');
const settings = SettingsDefaultsManager.loadFromFile(SETTINGS_PATH);
const OBSERVATION_LIMIT = parseInt(settings.MAGIC_CLAUDE_MEM_CONTEXT_OBSERVATIONS, 10) || 50;
...
function walkDirectoriesWithIgnore(dir: string, folders: Set<string>, depth: number = 0): void {
  ...
  const ignorePatterns = [
    ...
    '.magic-claude-mem', '.open-next', '.turbo'
  ];
 ...
 *
 * For each file with <magic-claude-mem-context> tags:
...
function cleanupAutoGeneratedFiles(workingDir: string, dryRun: boolean): void {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          try {
            ...
            if (content.includes('<magic-claude-mem-context>')) {
  ...
  for (const file of filesToProcess) {
    ...
    try {
      ...
      // Strip the magic-claude-mem-context tagged section
      const stripped = content.replace(/<magic-claude-mem-context>[\s\S]*?<\/magic-claude-mem-context>/g, '').trim();
