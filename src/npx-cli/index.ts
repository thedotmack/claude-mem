...
${pc.bold('Install Commands')} (no Bun required):
  ...
  ${pc.cyan('npx claude-mem install --provider claude|gemini|openrouter|openai-codex')}   Set LLM provider non-interactively
...
function parseInstallOptions(argv: string[]): InstallOptions {
  ...
  if (provider !== undefined && provider !== 'claude' && provider !== 'gemini' && provider !== 'openrouter' && provider !== 'openai-codex') {
    console.error(`Unknown --provider: ${provider}. Allowed: claude, gemini, openrouter, openai-codex`);
    process.exit(1);
  }
