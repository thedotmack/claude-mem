...
${styleText('bold', 'Install Commands')} (no Bun required):
  ...
  ${styleText('cyan', 'npx claude-mem install --provider claude|gemini|openrouter|orcarouter')}   Set LLM provider non-interactively
...
function parseInstallOptions(argv: string[]): InstallOptions {
  ...
  if (provider !== undefined && provider !== 'claude' && provider !== 'gemini' && provider !== 'openrouter' && provider !== 'orcarouter') {
    console.error(`Unknown --provider: ${provider}. Allowed: claude, gemini, openrouter, orcarouter`);
    process.exit(1);
  }
