...
${pc.bold('Install Commands')} (no Bun required):
  ...
  ${pc.cyan('npx claude-mem install --provider claude|codex|gemini|openrouter')}   Set LLM provider non-interactively
  ${pc.cyan('npx claude-mem install --model <id>')}   Set provider model (Claude/Codex)
...
function parseInstallOptions(argv: string[]): InstallOptions {
  ...
  if (provider !== undefined && provider !== 'claude' && provider !== 'codex' && provider !== 'gemini' && provider !== 'openrouter') {
    console.error(`Unknown --provider: ${provider}. Allowed: claude, codex, gemini, openrouter`);
    process.exit(1);
  }
