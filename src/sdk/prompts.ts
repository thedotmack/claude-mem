...
function buildObservationOutputInstruction(options?: ObservationPromptOptions): string {
  if (!options?.maxObservations && !options?.requireNarrative && !options?.extraOutputRules?.length) {
    return 'Return either one or more <observation>...</observation> blocks, or <skip_summary reason="noise" /> if this tool use should be skipped.';
  }

  const lines: string[] = [];
  if (options.maxObservations && options.maxObservations > 0) {
    lines.push(`Return either <skip_summary reason="noise" /> or at most ${options.maxObservations} <observation>...</observation> blocks.`);
  } else {
    lines.push('Return either one or more <observation>...</observation> blocks, or <skip_summary reason="noise" /> if this tool use should be skipped.');
  }
