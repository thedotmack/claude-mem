...
export function configureCursorMcp(target: CursorInstallTarget): number {
  ...
  if (!mcpServerPath) {
    ...
    console.error('   Expected at: ~/.claude/plugins/marketplaces/magic-claude-mem/plugin/scripts/mcp-server.cjs');
    return 1;
  }
...
export async function installCursorHooks(_sourceDir: string, target: CursorInstallTarget): Promise<number> {
  ...
  if (!workerServicePath) {
    ...
    console.error('   Expected at: ~/.claude/plugins/marketplaces/magic-claude-mem/plugin/scripts/worker-service.cjs');
    return 1;
  }
  ...
  switch (subcommand) {
    case 'install': {
      ...
      if (!cursorHooksDir) {
        ...
        console.error('   Expected at: ~/.claude/plugins/marketplaces/magic-claude-mem/cursor-hooks/');
        return 1;
      }
