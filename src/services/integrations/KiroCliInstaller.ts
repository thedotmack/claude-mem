 ...
 * into model context, so the worker warm-up hook MUST redirect its output;
 * only `hook kiro context` may print. `stop` output is parsed by Kiro for a
 * `{"decision":"block"}` override — the kiro adapter guarantees empty stdout
 * on non-context events (see src/cli/adapters/kiro.ts).
 */
