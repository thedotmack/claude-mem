/**
 * Session creation and update functions (MySQL async)
 */
...
/**
 * Create a new SDK session (idempotent)
 */
export async function createSDKSession(
  ...
  const row = await db.prepare('SELECT id FROM sdk_sessions WHERE content_session_id = ?')
    .get(contentSessionId) as { id: number };
  return row.id;
}

/**
 * Update the memory session ID
 */
