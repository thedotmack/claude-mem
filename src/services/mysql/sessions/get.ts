/**
 * Session retrieval functions (MySQL async)
 */
...
export async function getSessionById(db: MySQLDatabase, id: number): Promise<SessionBasic | null> {
  return await db.prepare(`
    SELECT id, content_session_id, memory_session_id, project,
           COALESCE(platform_source, 'claude') as platform_source,
           user_prompt, custom_title
    FROM sdk_sessions WHERE id = ? LIMIT 1
  `).get(id) as SessionBasic || null;
}
...
export async function getSdkSessionsBySessionIds(
  ...
  return await db.prepare(`
    ...
    FROM sdk_sessions WHERE memory_session_id IN (${placeholders})
    ORDER BY started_at_epoch DESC
  `).all(...memorySessionIds) as SessionFull[];
}
...
export async function getRecentSessionsWithStatus(
  ...
  return await db.prepare(`
    SELECT * FROM (
      SELECT s.memory_session_id, s.status, s.started_at, s.started_at_epoch,
             s.user_prompt,
  ...
  `).all(project, limit) as SessionWithStatus[];
}
...
export async function getSessionSummaryById(
  ...
  return await db.prepare(`
    SELECT id, memory_session_id, content_session_id, project, user_prompt,
           request_summary, learned_summary, status, created_at, created_at_epoch
    FROM sdk_sessions WHERE id = ? LIMIT 1
  `).get(id) as SessionSummaryDetail || null;
}
