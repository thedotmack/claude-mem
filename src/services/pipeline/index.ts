...
export class ObservationPipeline {
  ...
  async execute(input: AcquireInput): Promise<PipelineExecution> {
    ...
    try {
      // Stage 1: Acquire
      const acquireResult = await this.executeStage('acquire', input);
      ...
      const prepareResult = await this.executeStage('prepare', prepareInput);
      ...
      const processResult = await this.executeStage('process', processInput);
      ...
      const parseResult = await this.executeStage('parse', parseInput);
      ...
      const renderResult = await this.executeStage('render', renderInput);
  ...
  async retryFrom(execution: PipelineExecution, stage: PipelineStage): Promise<PipelineExecution> {
    ...
    // This requires having the previous stage's output available
    const stageIndex = ['acquire', 'prepare', 'process', 'parse', 'render'].indexOf(stage);
    ...
    try {
      ...
      switch (stage) {
        case 'acquire':
          ...
          data = await this.acquireStage.execute(input as AcquireInput) as T;
          break;
        case 'prepare':
          if (!this.prepareStage) throw new Error('Prepare stage not initialized');
          data = await this.prepareStage.execute(input as PrepareInput) as T;
          break;
        case 'process':
          if (!this.processStage) throw new Error('Process stage not initialized');
          data = await this.processStage.execute(input as ProcessInput) as T;
          break;
        case 'parse':
          if (!this.parseStage) throw new Error('Parse stage not initialized');
          data = await this.parseStage.execute(input as ParseInput) as T;
          break;
        case 'render':
          if (!this.renderStage) throw new Error('Render stage not initialized');
          data = await this.renderStage.execute(input as RenderInput) as T;
          break;
      }
