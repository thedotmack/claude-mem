...
export class WorkerService {
  ...
  private async initializeBackground(): Promise<void> {
    try {
      ...
      // Recover sessions stuck in 'active' from a previous crash
      const recoveredCount = this.dbManager.getSessionStore().recoverStaleSessions(10);
      if (recoveredCount > 0) {
        logger.info('SYSTEM', `Recovered ${recoveredCount} stale sessions from previous crash`);
      }
