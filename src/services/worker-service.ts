...
export class WorkerService {
  ...
  private registerRoutes(): void {
    ...
    this.server.registerRoutes(new SessionRoutes(this.sessionManager, this.dbManager, this.sdkAgent, this.geminiAgent, this.openRouterAgent, this.miniMaxAgent, this.sessionEventBroadcaster, this));
  ...
  private async runFallbackForTerminatedSession(
    ...
    if (isMiniMaxAvailable()) {
      try {
        await this.miniMaxAgent.startSession(session, this);
        return;
      } catch (e) {
        logger.warn('SDK', 'Fallback MiniMax failed, trying OpenRouter', {
          sessionId: sessionDbId,
          error: e instanceof Error ? e.message : String(e)
        });
      }
