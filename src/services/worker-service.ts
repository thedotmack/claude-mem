...
export class WorkerService {
  ...
  private async initializeBackground(): Promise<void> {
    try {
      ...
      // Kill stale subprocess PIDs from crashed sessions, validating PID identity first
      ...
      if (stalePids.length > 0) {
        ...
        for (const { sessionDbId, pid } of stalePids) {
          ...
          try {
            // Retrieve command line to verify PID identity before kill
            ...
            if (isWindows) {
              ...
              cmdline = execSync(`ps -p ${String(pid)} -o args= 2>/dev/null`, { encoding: 'utf-8' }).trim() || null;
            }
            ...
            if (cmdline !== null && cmdline.includes('claude')) {
              ...
              logger.info('SYSTEM', `Stale PID ${String(pid)} no longer exists`);
            }
          } catch {
            // Process already dead (ESRCH) or ps/PowerShell failed - expected after crash
          }
          this.dbManager.getSessionStore().clearSubprocessPid(sessionDbId);
        }
