...
// Default OpenRouter API endpoint (can be overridden via CLAUDE_MEM_OPENROUTER_BASE_URL setting)
...
export class OpenRouterAgent {
  ...
  private async queryOpenRouterMultiTurn(
    ...
    // Build the full API URL from base URL, stripping trailing slash
    const effectiveBaseUrl = (baseUrl || DEFAULT_OPENROUTER_BASE_URL).replace(/\/+$/, '');
  ...
  private getOpenRouterConfig(): { apiKey: string; baseUrl: string; model: string; siteUrl?: string; appName?: string } {
    ...
    // Base URL: configurable for local OpenAI-compatible servers (llama.cpp, Ollama, LM Studio)
