...
export class OpenRouterProvider extends OpenAICompatibleProvider<OpenRouterConfig> {
  ...
  protected async query(history: ConversationMessage[], config: OpenRouterConfig, abortSignal?: AbortSignal): Promise<ProviderQueryResult> {
    return this.queryOpenRouterMultiTurn(history, config.apiKey, config.model, config.apiUrl, config.siteUrl, config.appName, abortSignal);
  }
  ...
  private async queryOpenRouterMultiTurn(
    ...
    const data = await withRetry<OpenRouterResponse>(async (attemptSignal) => {
      ...
      return responseData;
    }, { label: `OpenRouter ${model}`, abortSignal });
