...
import { OrcaRouterProvider, isOrcaRouterSelected, isOrcaRouterAvailable } from '../../OrcaRouterProvider.js';
...
export class SessionRoutes extends BaseRouteHandler {
  ...
  private getSelectedProvider(): 'claude' | 'gemini' | 'openrouter' | 'orcarouter' {
    if (isOrcaRouterSelected() && isOrcaRouterAvailable()) {
      return 'orcarouter';
    }
  ...
  private async startGeneratorWithProvider(
    ...
    const agent = provider === 'orcarouter' ? this.orcaRouterAgent
      : (provider === 'openrouter' ? this.openRouterAgent
        : (provider === 'gemini' ? this.geminiAgent : this.sdkAgent));
    const agentName = provider === 'orcarouter' ? 'OrcaRouter'
