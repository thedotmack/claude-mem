...
import { DockerModelRunnerAgent, isDockerModelRunnerSelected, isDockerModelRunnerAvailable } from '../../DockerModelRunnerAgent.js';
...
export class SessionRoutes extends BaseRouteHandler {
  ...
  private getActiveAgent(): SDKAgent | GeminiAgent | OpenRouterAgent | DockerModelRunnerAgent {
    if (isDockerModelRunnerSelected()) {
      logger.debug('SESSION', 'Using Docker Model Runner agent');
   ...
   */
  private getSelectedProvider(): 'claude' | 'gemini' | 'openrouter' | 'docker-model-runner' {
  ...
  private startGeneratorWithProvider(
    ...
    provider: 'claude' | 'gemini' | 'openrouter' | 'docker-model-runner',
    source: string
  ): void {
    if (!session) return;
    ...
    const agent = provider === 'docker-model-runner' ? this.dockerModelRunnerAgent : (provider === 'openrouter' ? this.openRouterAgent : (provider === 'gemini' ? this.geminiAgent : this.sdkAgent));
    const agentName = provider === 'docker-model-runner' ? 'Docker Model Runner' : (provider === 'openrouter' ? 'OpenRouter' : (provider === 'gemini' ? 'Gemini' : 'Claude SDK'));
