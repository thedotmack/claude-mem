...
export class SettingsRoutes extends BaseRouteHandler {
  ...
  private handleUpdateSettings = this.wrapHandler((req: Request, res: Response): void => {
    ...
    const target = settings.env && typeof settings.env === 'object' && !Array.isArray(settings.env)
      ? settings.env
      : settings;
  ...
  private validateSettings(settings: any): { valid: boolean; error?: string } {
    ...
    if (settings.CLAUDE_MEM_CODEX_REASONING_EFFORT) {
      ...
      if (!validEfforts.includes(String(settings.CLAUDE_MEM_CODEX_REASONING_EFFORT).toLowerCase())) {
        return { valid: false, error: 'CLAUDE_MEM_CODEX_REASONING_EFFORT must be one of: none, minimal, low, medium, high, xhigh' };
      }
    }
