...
export class SettingsRoutes extends BaseRouteHandler {
  ...
  private validateSettings(settings: any): { valid: boolean; error?: string } {
    ...
    // Validate CLAUDE_MEM_OPENROUTER_API_URL if provided
    if (settings.CLAUDE_MEM_OPENROUTER_API_URL) {
      try {
        new URL(settings.CLAUDE_MEM_OPENROUTER_API_URL);
      } catch (error) {
        logger.debug('SETTINGS', 'Invalid URL format', { url: settings.CLAUDE_MEM_OPENROUTER_API_URL, error: error instanceof Error ? error.message : String(error) });
        return { valid: false, error: 'CLAUDE_MEM_OPENROUTER_API_URL must be a valid URL' };
      }
    }
