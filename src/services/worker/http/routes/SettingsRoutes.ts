...
export class SettingsRoutes extends BaseRouteHandler {
  ...
  private validateSettings(settings: any): { valid: boolean; error?: string } {
    ...
    if (settings.CLAUDE_MEM_CLAUDE_MAX_TOKENS !== undefined) {
      const rawTokens = settings.CLAUDE_MEM_CLAUDE_MAX_TOKENS;
      const tokens = typeof rawTokens === 'string' && /^\d+$/.test(rawTokens)
        ? Number(rawTokens)
        : Number.NaN;
      if (!Number.isSafeInteger(tokens) || tokens < 1000 || tokens > 1000000) {
        return { valid: false, error: 'CLAUDE_MEM_CLAUDE_MAX_TOKENS must be an integer between 1000 and 1000000' };
      }
    }
