...
export interface SettingsDefaults {
  ...
  CLAUDE_MEM_TIER_FAST_MODEL: string;        // #2289 â€?resolved by $TIER:fast in CLAUDE_MEM_MODEL
  CLAUDE_MEM_TIER_SMART_MODEL: string;       // #2289 â€?resolved by $TIER:smart in CLAUDE_MEM_MODEL
...
export class SettingsDefaultsManager {
  private static readonly DEFAULTS: SettingsDefaults = {
    ...
    CLAUDE_MEM_OPENROUTER_BASE_URL: '',  // #2382/#2590/#2622/#2393 â€?optional OpenAI-compatible base URL (e.g. https://api.deepseek.com, http://localhost:1234/v1). Empty = default OpenRouter endpoint.
    ...
    CLAUDE_MEM_HOOK_FAIL_LOUD_THRESHOLD: '3',  // Plan 05 Phase 8 â€?escalate to exit code 2 after N consecutive worker-unreachable hook invocations
    CLAUDE_MEM_EXCLUDED_PROJECTS: '',  // Comma-separated glob patterns for excluded project paths
    CLAUDE_MEM_FOLDER_MD_EXCLUDE: '[]',  // JSON array of folder paths to exclude from CLAUDE.md generation
    CLAUDE_MEM_FOLDER_MD_SKELETON_DENYLIST: '[]',  // #2400 â€?JSON array of glob patterns; when a folder matches AND its generated CLAUDE.md would be empty/skeleton, skip injection (avoids polluting non-content dirs with empty skeletons). Default [] preserves existing behavior.
    ...
    CLAUDE_MEM_TIER_SIMPLE_MODEL: 'haiku', // Portable tier alias â€?works across Direct API, Bedrock, Vertex, Azure (see #1463)
    CLAUDE_MEM_TIER_SUMMARY_MODEL: '',                // Empty = use default model for summaries
    CLAUDE_MEM_TIER_FAST_MODEL: 'haiku',              // #2289 â€?$TIER:fast resolves here (portable alias)
    CLAUDE_MEM_TIER_SMART_MODEL: 'sonnet',            // #2289 â€?$TIER:smart resolves here (portable alias)
    ...
    CLAUDE_MEM_SERVER_BETA_URL: `http://127.0.0.1:${process.env.CLAUDE_MEM_SERVER_PORT ?? String(37877 + ((process.getuid?.() ?? 77) % 100))}`,  // Default server-beta runtime URL â€?UID-derived for multi-account isolation
  ...
  static loadFromFile(settingsPath: string, applyEnvOverrides = true): SettingsDefaults {
    try {
      ...
      // Strip UTF-8 BOM if present â€?Windows tools (editors, formatters, CLI
      ...
      if (settings.env && typeof settings.env === 'object') {
        ...
        try {
          ...
          // stderr, never stdout â€?same JSON-on-stdout contract as above.
