...
describe('#2292 — blocking diagnostics are no longer swallowed', () => {
  it('emitBlockingError surfaces blocking messages through the buffered window', () => {
    // Simulate hookCommand: install the stderr buffer that previously swallowed
    // EVERYTHING (the #2292 no-op). Blocking errors must bypass the buffer and
    // reach real stderr.
    ...
    try {
      ...
      // The blocking path:
      emitBlockingError('unrecoverable hook failure', { skipExit: true });
      const surfaced = real.chunks.join('');
      expect(surfaced).toContain('unrecoverable hook failure');
  ...
  it('worker-utils recordWorkerUnreachable logs without blocking hook output (source contract)', () => {
    const src = readFileSync(join(REPO_ROOT, 'src', 'shared', 'worker-utils.ts'), 'utf-8');
    // Worker-unreachable fallback must not fail hooks; agent hosts may re-enter failed Stop hooks.
    expect(src).toContain('continuing without memory hook worker');
