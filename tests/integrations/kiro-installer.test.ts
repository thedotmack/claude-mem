...
describe('buildKiroHooksBlock', () => {
  it('wires the four Kiro events with correct injection hygiene', () => {
    ...
    // agentSpawn stdout is injected into model context: worker warm-up MUST be
    // silenced; only `hook kiro context` may print.
    expect(hooks.agentSpawn).toHaveLength(2);
    expect(hooks.agentSpawn[0].command).toContain('" start >/dev/null');
    expect(hooks.agentSpawn[1].command).toContain('hook kiro context');
    expect(hooks.agentSpawn[1].command).not.toContain('/dev/null');
...
describe('isClaudeMemHookEntry', () => {
  it('claims worker-service commands, leaves user hooks alone', () => {
...
describe('installKiroCliIntegration / uninstallKiroCliIntegration', () => {
  ...
  it('patches existing agents, registers MCP, copies skills, and uninstalls exactly', async () => {
    ...
    expect(patched.hooks.agentSpawn).toHaveLength(2);
  ...
  it('creates a fallback agent when none exist and removes it on uninstall', async () => {
    ...
    expect(fallback.hooks.agentSpawn).toHaveLength(2);
