...
// boots the worker's HTTP surface in-process and proves the GA-gating facts
// from the plan-07 test matrix that DON'T require a real pg/redis container:
//   1. a mode is loaded (the #2443 boot guard succeeds)
//   2. an API key can be created (the operator key-gen path works)
//   3. the viewer responds (the #2552 static handler is mounted)
//
...
import { createServerApiKey, verifyServerApiKey, DEFAULT_LOCAL_API_KEY_SCOPES } from '../../src/server/auth/sqlite-api-key-service.js';
...
describe('server runtime in-process smoke (#2550)', () => {
  ...
  beforeEach(async () => {
    ...
    // Mount the viewer static handler (#2552) the way the real runtime does.
  ...
  it('creates an API key via the operator key-gen path and verifies it', () => {
    const created = createServerApiKey(db, { name: 'smoke-key' });
    // The default key gets working read+write scopes.
    ...
    expect(verifyServerApiKey(db, created.rawKey, ['memories:write'])?.record.id).toBe(created.record.id);
  });
