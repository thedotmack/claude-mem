...
/** Get the port assigned by the OS after listen(0). */
function getAssignedPort(server: Server): number {
  const addr = server.getHttpServer()!.address() as net.AddressInfo;
  return addr.port;
}
...
describe('Server', () => {
  ...
  describe('listen', () => {
    ...
    it('should reject if port is already in use', async () => {
      ...
      // Let OS assign a port, then try to bind the same one
      await server.listen(0, '127.0.0.1');
      const testPort = getAssignedPort(server);
