...
interface ParsedSettings {
  ...
  MAGIC_CLAUDE_MEM_PROVIDER?: string;
  MAGIC_CLAUDE_MEM_OPENAI_COMPAT_API_KEY?: string;
  MAGIC_CLAUDE_MEM_OPENAI_COMPAT_MODEL?: string;
  MAGIC_CLAUDE_MEM_OPENROUTER_API_KEY?: string;
  MAGIC_CLAUDE_MEM_OPENROUTER_MODEL?: string;
  [key: string]: unknown;
}
...
describe('SettingsDefaultsManager', () => {
  ...
  describe('loadFromFile', () => {
    describe('file does not exist', () => {
      ...
      it('should write pretty-printed JSON (2-space indent)', () => {
        ...
        expect(content).toContain('  "MAGIC_CLAUDE_MEM_MODEL"');
      });
    ...
    describe('file exists with valid content', () => {
      it('should return parsed content when file has valid JSON', () => {
        const customSettings = {
          MAGIC_CLAUDE_MEM_MODEL: 'custom-model',
          MAGIC_CLAUDE_MEM_WORKER_PORT: '12345',
        };
        writeFileSync(settingsPath, JSON.stringify(customSettings));

        const result = SettingsDefaultsManager.loadFromFile(settingsPath);

        expect(result.MAGIC_CLAUDE_MEM_MODEL).toBe('custom-model');
        expect(result.MAGIC_CLAUDE_MEM_WORKER_PORT).toBe('12345');
      });

      it('should merge file settings with defaults for missing keys', () => {
        // Only set one value, defaults should fill the rest
        const partialSettings = {
          MAGIC_CLAUDE_MEM_MODEL: 'partial-model',
        };
        ...
        expect(result.MAGIC_CLAUDE_MEM_MODEL).toBe('partial-model');
        // Other values should come from defaults
        expect(result.MAGIC_CLAUDE_MEM_WORKER_PORT).toBe(defaults.MAGIC_CLAUDE_MEM_WORKER_PORT);
        expect(result.MAGIC_CLAUDE_MEM_WORKER_HOST).toBe(defaults.MAGIC_CLAUDE_MEM_WORKER_HOST);
        expect(result.MAGIC_CLAUDE_MEM_LOG_LEVEL).toBe(defaults.MAGIC_CLAUDE_MEM_LOG_LEVEL);
      });

      it('should not modify existing file when loading', () => {
        const customSettings = {
          MAGIC_CLAUDE_MEM_MODEL: 'do-not-change',
      ...
      it('should handle all settings keys correctly', () => {
        ...
        fullSettings.MAGIC_CLAUDE_MEM_MODEL = 'all-keys-model';
        fullSettings.MAGIC_CLAUDE_MEM_PROVIDER = 'gemini';
        writeFileSync(settingsPath, JSON.stringify(fullSettings));

        const result = SettingsDefaultsManager.loadFromFile(settingsPath);

        expect(result.MAGIC_CLAUDE_MEM_MODEL).toBe('all-keys-model');
        expect(result.MAGIC_CLAUDE_MEM_PROVIDER).toBe('gemini');
      });
    });
    ...
    describe('nested schema migration', () => {
      it('should migrate old nested { env: {...} } schema to flat schema', () => {
        const nestedSettings = {
          env: {
            MAGIC_CLAUDE_MEM_MODEL: 'nested-model',
            MAGIC_CLAUDE_MEM_WORKER_PORT: '54321',
          },
        };
        ...
        expect(result.MAGIC_CLAUDE_MEM_MODEL).toBe('nested-model');
        expect(result.MAGIC_CLAUDE_MEM_WORKER_PORT).toBe('54321');
      });

      it('should auto-migrate file from nested to flat schema', () => {
        const nestedSettings = {
          env: {
            MAGIC_CLAUDE_MEM_MODEL: 'migrated-model',
          },
        };
        writeFileSync(settingsPath, JSON.stringify(nestedSettings));
        ...
        expect(parsed.MAGIC_CLAUDE_MEM_MODEL).toBe('migrated-model');
      });
    });

    describe('OpenRouter to OpenAI-compat migration', () => {
      it('should migrate MAGIC_CLAUDE_MEM_OPENROUTER_* keys to MAGIC_CLAUDE_MEM_OPENAI_COMPAT_*', () => {
        const oldSettings = {
          MAGIC_CLAUDE_MEM_PROVIDER: 'openrouter',
          MAGIC_CLAUDE_MEM_OPENROUTER_API_KEY: 'sk-or-test-key',
          MAGIC_CLAUDE_MEM_OPENROUTER_MODEL: 'xiaomi/mimo-v2-flash:free',
          MAGIC_CLAUDE_MEM_OPENROUTER_SITE_URL: 'https://example.com',
          MAGIC_CLAUDE_MEM_OPENROUTER_APP_NAME: 'test-app',
          MAGIC_CLAUDE_MEM_OPENROUTER_MAX_CONTEXT_MESSAGES: '30',
          MAGIC_CLAUDE_MEM_OPENROUTER_MAX_TOKENS: '200000',
        };
        ...
        expect(result.MAGIC_CLAUDE_MEM_PROVIDER).toBe('openai-compat');
        expect(result.MAGIC_CLAUDE_MEM_OPENAI_COMPAT_API_KEY).toBe('sk-or-test-key');
        expect(result.MAGIC_CLAUDE_MEM_OPENAI_COMPAT_MODEL).toBe('xiaomi/mimo-v2-flash:free');
        expect(result.MAGIC_CLAUDE_MEM_OPENAI_COMPAT_SITE_URL).toBe('https://example.com');
        expect(result.MAGIC_CLAUDE_MEM_OPENAI_COMPAT_APP_NAME).toBe('test-app');
        expect(result.MAGIC_CLAUDE_MEM_OPENAI_COMPAT_MAX_CONTEXT_MESSAGES).toBe('30');
        expect(result.MAGIC_CLAUDE_MEM_OPENAI_COMPAT_MAX_TOKENS).toBe('200000');
      });

      it('should auto-migrate file on disk from OpenRouter to OpenAI-compat keys', () => {
        const oldSettings = {
          MAGIC_CLAUDE_MEM_PROVIDER: 'openrouter',
          MAGIC_CLAUDE_MEM_OPENROUTER_API_KEY: 'sk-or-migrate-key',
          MAGIC_CLAUDE_MEM_OPENROUTER_MODEL: 'test-model',
        };
        writeFileSync(settingsPath, JSON.stringify(oldSettings));
        ...
        const content = readFileSync(settingsPath, 'utf-8');
        const parsed = JSON.parse(content) as ParsedSettings;
        expect(parsed.MAGIC_CLAUDE_MEM_PROVIDER).toBe('openai-compat');
        expect(parsed.MAGIC_CLAUDE_MEM_OPENAI_COMPAT_API_KEY).toBe('sk-or-migrate-key');
        expect(parsed.MAGIC_CLAUDE_MEM_OPENAI_COMPAT_MODEL).toBe('test-model');
        // Old keys should be removed
        expect(parsed.MAGIC_CLAUDE_MEM_OPENROUTER_API_KEY).toBeUndefined();
        expect(parsed.MAGIC_CLAUDE_MEM_OPENROUTER_MODEL).toBeUndefined();
      });

      it('should not migrate if new keys already exist', () => {
        const settings = {
          MAGIC_CLAUDE_MEM_PROVIDER: 'openai-compat',
          MAGIC_CLAUDE_MEM_OPENAI_COMPAT_API_KEY: 'new-key',
          MAGIC_CLAUDE_MEM_OPENROUTER_API_KEY: 'old-key',  // Should be ignored
        };
        ...
        expect(result.MAGIC_CLAUDE_MEM_OPENAI_COMPAT_API_KEY).toBe('new-key');
      ...
      it('should migrate MAGIC_CLAUDE_MEM_OPENROUTER_BASE_URL to MAGIC_CLAUDE_MEM_OPENAI_COMPAT_BASE_URL', () => {
        const oldSettings = {
          MAGIC_CLAUDE_MEM_PROVIDER: 'openrouter',
          MAGIC_CLAUDE_MEM_OPENROUTER_API_KEY: 'key',
          MAGIC_CLAUDE_MEM_OPENROUTER_BASE_URL: 'http://localhost:8317/v1/chat/completions',
        };
        writeFileSync(settingsPath, JSON.stringify(oldSettings));

        const result = SettingsDefaultsManager.loadFromFile(settingsPath);

        expect(result.MAGIC_CLAUDE_MEM_OPENAI_COMPAT_BASE_URL).toBe('http://localhost:8317/v1/chat/completions');
      });
    });
    ...
    describe('edge cases', () => {
      ...
      it('should ignore unknown keys in file', () => {
        const settingsWithUnknown = {
          MAGIC_CLAUDE_MEM_MODEL: 'known-model',
        ...
        expect(result.MAGIC_CLAUDE_MEM_MODEL).toBe('known-model');
        expect((result as Record<string, unknown>).UNKNOWN_KEY).toBeUndefined();
      });

      it('should handle file with BOM', () => {
        const bom = '\uFEFF';
        const settings = { MAGIC_CLAUDE_MEM_MODEL: 'bom-model' };
  ...
  describe('getAllDefaults', () => {
    ...
    it('should include all expected keys', () => {
      ...
      // Core settings
      expect(defaults.MAGIC_CLAUDE_MEM_MODEL).toBeDefined();
      expect(defaults.MAGIC_CLAUDE_MEM_WORKER_PORT).toBeDefined();
      expect(defaults.MAGIC_CLAUDE_MEM_WORKER_HOST).toBeDefined();

      // Provider settings
      expect(defaults.MAGIC_CLAUDE_MEM_PROVIDER).toBeDefined();
      expect(defaults.MAGIC_CLAUDE_MEM_GEMINI_API_KEY).toBeDefined();
      expect(defaults.MAGIC_CLAUDE_MEM_OPENAI_COMPAT_API_KEY).toBeDefined();

      // System settings
      expect(defaults.MAGIC_CLAUDE_MEM_DATA_DIR).toBeDefined();
      expect(defaults.MAGIC_CLAUDE_MEM_LOG_LEVEL).toBeDefined();
    });
  });
  ...
  describe('getInt', () => {
    it('should return integer value for numeric string', () => {
      expect(SettingsDefaultsManager.getInt('MAGIC_CLAUDE_MEM_WORKER_PORT')).toBe(37777);
      expect(SettingsDefaultsManager.getInt('MAGIC_CLAUDE_MEM_CONTEXT_OBSERVATIONS')).toBe(50);
    });
  });

  describe('getBool', () => {
    it('should return true for "true" string', () => {
      expect(SettingsDefaultsManager.getBool('MAGIC_CLAUDE_MEM_CONTEXT_SHOW_READ_TOKENS')).toBe(true);
    });

    it('should return false for non-"true" string', () => {
      expect(SettingsDefaultsManager.getBool('MAGIC_CLAUDE_MEM_CONTEXT_SHOW_LAST_MESSAGE')).toBe(false);
    });
  });
});
