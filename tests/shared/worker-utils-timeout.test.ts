...
// this import the dynamic `import('../../src/shared/worker-utils.js')` calls
// below can be the first to evaluate it — while the env var points at a
// soon-deleted temp dir — poisoning every later-loaded module in the same bun
// process (e.g. ProcessManager's PID_FILE, failing tests/infrastructure/ in
// combined runs). worker-utils itself is unaffected: it resolves its settings
// path at call time via SettingsDefaultsManager.get('CLAUDE_MEM_DATA_DIR').
