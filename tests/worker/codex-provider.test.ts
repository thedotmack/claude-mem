...
import type { ActiveSession, PendingMessage, ConversationMessage } from '../../src/services/worker-types.js';
...
describe('CodexProvider prompt formatting', () => {
  it('rejects the latest message when it alone exceeds the token budget', () => {
...
describe('CodexProvider observation response sanitation', () => {
  it('compresses a large field in bounded requests and then records the following queued observation', async () => {
    const { provider, storeObservations, confirmClaimedMessages } = createProviderFlowHarness([
      { type: 'observation', tool_name: 'Read', tool_input: { path: 'large.log' },
        tool_response: 'substantive log evidence\n'.repeat(14000), prompt_number: 2 },
      { type: 'observation', tool_name: 'Bash', tool_input: {}, tool_response: 'Later durable change', prompt_number: 2 },
    ], []);
    let observationTurns = 0;
    let compressionTurns = 0;
    const requestBytes: number[] = [];
    // Exercise real Codex formatting, quota admission, and the chunking callback,
    // replacing only the native transport so no subprocess/auth/network is used.
    (provider as any).query = (CodexProvider.prototype as any).query.bind(provider);
    (provider as any).appServer.runTurn = async (options: { prompt: string }) => {
      requestBytes.push(Buffer.byteLength(options.prompt, 'utf8'));
      if (requestBytes.length === 1) return { content: '' };
      if (options.prompt.includes('<payload>')) {
        compressionTurns++;
        return { content: 'Condensed durable log evidence for the observation.' };
      }
      observationTurns++;
      return { content: `<observation><type>discovery</type><title>Queued finding ${observationTurns}</title><narrative>Durable result ${observationTurns} remains available for future work.</narrative></observation>` };
    };
    const session = createProviderFlowSession();
    await provider.startSession(session);
    expect(compressionTurns).toBeGreaterThan(1);
    expect(requestBytes.every(bytes => bytes <= 100000)).toBe(true);
    expect(observationTurns).toBe(2);
    expect(storeObservations).toHaveBeenCalledTimes(2);
    expect(confirmClaimedMessages).toHaveBeenCalledTimes(2);
  });

  it('records eight durable observations in one prompt without a silent cutoff', async () => {
    const messages: PendingMessage[] = Array.from({ length: 8 }, (_, i) => ({
      type: 'observation', tool_name: 'Bash', tool_input: { cmd: `check-${i}` },
      tool_response: `durable finding ${i}`, prompt_number: 2,
    }));
    const responses = ['', ...messages.map((_, i) => `<observation><type>discovery</type><title>Finding ${i}</title><narrative>Durable change ${i} matters for future work.</narrative></observation>`)];
    const { provider, storeObservations } = createProviderFlowHarness(messages, responses, { existingObservationCount: 6 });
    const session = createProviderFlowSession();
    await provider.startSession(session);
    expect(provider.queryCalls).toHaveBeenCalledTimes(9);
    expect(storeObservations).toHaveBeenCalledTimes(8);
    expect(session.conversationHistory.filter(message => message.role === 'assistant')).toHaveLength(8);
  });

  it('retains the explicit noise marker exactly once in assistant history', async () => {
    const noise = '<skip_summary reason="noise" />';
    const { provider, confirmClaimedMessages } = createProviderFlowHarness({
      type: 'observation', tool_name: 'Bash', tool_input: {}, tool_response: 'no change', prompt_number: 2,
    }, ['', noise]);
    const session = createProviderFlowSession();
    await provider.startSession(session);
    expect(confirmClaimedMessages).toHaveBeenCalledWith(session.sessionDbId);
    expect(session.conversationHistory.filter(message => message.role === 'assistant')).toEqual([{ role: 'assistant', content: noise }]);
  });
...
<observation>
  ...
  it('continues Codex observation compression beyond six observations per prompt', async () => {
    ...
    expect(countObservationsForPrompt).not.toHaveBeenCalled();
    expect(provider.queryCalls).toHaveBeenCalledTimes(2);
