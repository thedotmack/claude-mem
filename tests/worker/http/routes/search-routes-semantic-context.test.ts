...
describe('SearchRoutes /api/context/semantic', () => {
  ...
  it('retains scoped results when the relevance-ordered retry adds nothing new', async () => {
    ...
    expect(searchMock.mock.calls[0][0]).not.toHaveProperty('semanticLimit');
    expect(searchMock.mock.calls[0][2]).toEqual({ semanticHydrationLimit: 100 });
    expect(searchMock.mock.calls[1][0]).toMatchObject({ query: baseReq.body.q, limit: '2', type: 'observations', format: 'json', orderBy: 'relevance' });
    expect(searchMock.mock.calls[1][0]).not.toHaveProperty('semanticLimit');
    expect(searchMock.mock.calls[1][2]).toEqual({ semanticHydrationLimit: 100 });
  ...
  it('falls back to one unscoped retry when scoped results are empty', async () => {
    ...
    expect(searchMock.mock.calls[1][0]).toMatchObject({ limit: '5', orderBy: 'relevance' });
    expect(searchMock.mock.calls[1][0]).not.toHaveProperty('semanticLimit');
    expect(searchMock.mock.calls[1][2]).toEqual({ semanticHydrationLimit: 100 });
  ...
  it('recovers matches where merged_into_project matches the requested project', async () => {
    ...
    expect(body.count).toBe(1);
    expect(searchMock.mock.calls[1][0]).toMatchObject({ limit: '3', type: 'observations', orderBy: 'relevance' });
    expect(searchMock.mock.calls[1][0]).not.toHaveProperty('semanticLimit');
    expect(searchMock.mock.calls[1][2]).toEqual({ semanticHydrationLimit: 100 });
  ...
  it('supplements partial scoped hits with merged project matches from the unscoped retry', async () => {
    ...
    const [body] = res.json.mock.calls[0] as any[];
    expect(searchMock).toHaveBeenCalledTimes(2);
    expect(searchMock.mock.calls[0][0]).toMatchObject({ project: 'request-project', limit: '5', orderBy: 'relevance' });
    expect(searchMock.mock.calls[0][0]).not.toHaveProperty('semanticLimit');
    expect(searchMock.mock.calls[0][2]).toEqual({ semanticHydrationLimit: 100 });
    expect(searchMock.mock.calls[1][0]).toMatchObject({ limit: '5', orderBy: 'relevance' });
    expect(searchMock.mock.calls[1][0]).not.toHaveProperty('semanticLimit');
